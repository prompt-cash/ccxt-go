...
func (this *Coinbasepro) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    details := this.SafeValue(currency , MkString("details") , MkMap(&VarMap{})); _ = details;
    precision := this.SafeNumber(currency , MkString("max_precision") ); _ = precision;
...
func (this *Coinbasepro) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetProductsIdBook"), this.Extend(request , params )); _ = response;
...
func (this *Coinbasepro) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.Parse8601(this.SafeValue(ticker , MkString("time") )); _ = timestamp;
  bid := this.SafeNumber(ticker , MkString("bid") ); _ = bid;
  ask := this.SafeNumber(ticker , MkString("ask") ); _ = ask;
  last := this.SafeNumber(ticker , MkString("price") ); _ = last;
  symbol := OpTrinary(OpEq2(market , MkUndefined() ), MkUndefined() , *(market ).At(MkString("symbol") )); _ = symbol;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Coinbasepro) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  method := this.SafeString(*this.At(MkString("options")) , MkString("fetchTickerMethod") , MkString("publicGetProductsIdTicker") ); _ = method;
...
func (this *Coinbasepro) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.Parse8601(this.SafeString2(trade , MkString("time") , MkString("created_at") )); _ = timestamp;
  marketId := this.SafeString(trade , MkString("product_id") ); _ = marketId;
  ...
  if IsTrue(OpNotEq2(market , MkUndefined() )) {
    feeCurrencyId := this.SafeStringLower(market , MkString("quoteId") ); _ = feeCurrencyId;
    costField := OpAdd(feeCurrencyId , MkString("_value") ); _ = costField;
  ...
  feeCost := this.SafeNumber2(trade , MkString("fill_fees") , MkString("fee") ); _ = feeCost;
  fee := MkMap(&VarMap{
        "cost":feeCost ,
  ...
  side := OpTrinary(OpEq2(*(trade ).At(MkString("side") ), MkString("buy") ), MkString("sell") , MkString("buy") ); _ = side;
...
func (this *Coinbasepro) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetProductsIdTrades"), this.Extend(request , params )); _ = response;
  return this.ParseTrades(response , market , since , limit );
}
...
func (this *Coinbasepro) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  granularity := *(*this.At(MkString("timeframes")) ).At(timeframe ); _ = granularity;
  request := MkMap(&VarMap{
  ...
  response := this.Call(MkString("publicGetProductsIdCandles"), this.Extend(request , params )); _ = response;
...
func (this *Coinbasepro) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  filled := this.SafeNumber(order , MkString("filled_size") ); _ = filled;
  amount := this.SafeNumber(order , MkString("size") , filled ); _ = amount;
  cost := this.SafeNumber(order , MkString("executed_value") ); _ = cost;
  feeCost := this.SafeNumber(order , MkString("fill_fees") ); _ = feeCost;
  ...
  timeInForce := this.SafeString(order , MkString("time_in_force") ); _ = timeInForce;
  postOnly := this.SafeValue(order , MkString("post_only") ); _ = postOnly;
  stopPrice := this.SafeNumber(order , MkString("stop_price") ); _ = stopPrice;
...
func (this *Coinbasepro) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  clientOrderId := this.SafeString2(params , MkString("clientOrderId") , MkString("client_oid") ); _ = clientOrderId;
...
func (this *Coinbasepro) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"status":MkString("done") }); _ = request;
  return this.FetchOpenOrders(symbol , since , limit , this.Extend(request , params ));
}
...
func (this *Coinbasepro) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  stopPrice := this.SafeNumber2(params , MkString("stopPrice") , MkString("stop_price") ); _ = stopPrice;
  ...
  timeInForce := this.SafeString2(params , MkString("timeInForce") , MkString("time_in_force") ); _ = timeInForce;
  ...
  if IsTrue(OpEq2(type_ , MkString("limit") )) {
    ...
    *(request ).At(MkString("size") )= this.AmountToPrecision(symbol , amount );
    ...
    if IsTrue(OpEq2(type_ , MkString("market") )) {
      cost := this.SafeNumber2(params , MkString("cost") , MkString("funds") ); _ = cost;
      if IsTrue(OpEq2(cost , MkUndefined() )) {
...
func (this *Coinbasepro) Deposit(goArgs ...*Variant) *Variant{
  ...
  method := MkString("privatePostDeposits") ; _ = method;
...
func (this *Coinbasepro) Withdraw(goArgs ...*Variant) *Variant{
  ...
  method := MkString("privatePostWithdrawals") ; _ = method;
  if IsTrue(OpHasMember(MkString("payment_method_id") , params )) {
    OpAddAssign(& method , MkString("PaymentMethod") );
  } else {
    if IsTrue(OpHasMember(MkString("coinbase_account_id") , params )) {
...
func (this *Coinbasepro) FetchTransactions(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(id , MkUndefined() )) {
    if IsTrue(OpNotEq2(code , MkUndefined() )) {
      ...
      accountsByCurrencyCode := this.IndexBy(*this.At(MkString("accounts")) , MkString("currency") ); _ = accountsByCurrencyCode;
      account := this.SafeValue(accountsByCurrencyCode , code ); _ = account;
  ...
  if IsTrue(OpEq2(id , MkUndefined() )) {
    ...
    for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
      account_id := this.SafeString(*(response ).At(i ), MkString("account_id") ); _ = account_id;
      account := this.SafeValue(*this.At(MkString("accountsById")) , account_id ); _ = account;
      code := this.SafeString(account , MkString("currency") ); _ = code;
...
func (this *Coinbasepro) ParseTransactionStatus(goArgs ...*Variant) *Variant{
  ...
  canceled := this.SafeValue(transaction , MkString("canceled_at") ); _ = canceled;
  if IsTrue(canceled ) {
    return MkString("canceled") ;
  }
  processed := this.SafeValue(transaction , MkString("processed_at") ); _ = processed;
  completed := this.SafeValue(transaction , MkString("completed_at") ); _ = completed;
  if IsTrue(completed ) {
    return MkString("ok") ;
  } else {
...
func (this *Coinbasepro) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  details := this.SafeValue(transaction , MkString("details") , MkMap(&VarMap{})); _ = details;
  id := this.SafeString(transaction , MkString("id") ); _ = id;
  txid := this.SafeString(details , MkString("crypto_transaction_hash") ); _ = txid;
  timestamp := this.Parse8601(this.SafeString(transaction , MkString("created_at") )); _ = timestamp;
  updated := this.Parse8601(this.SafeString(transaction , MkString("processed_at") )); _ = updated;
  ...
  status := this.ParseTransactionStatus(transaction ); _ = status;
  amount := this.SafeNumber(transaction , MkString("amount") ); _ = amount;
  type_ := this.SafeString(transaction , MkString("type") ); _ = type_;
  address := this.SafeString(details , MkString("crypto_address") ); _ = address;
  tag := this.SafeString(details , MkString("destination_tag") ); _ = tag;
  ...
  if IsTrue(OpEq2(type_ , MkString("withdraw") )) {
    ...
    feeCost := this.SafeNumber(details , MkString("fee") ); _ = feeCost;
    if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
...
func (this *Coinbasepro) CreateDepositAddress(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  currency := this.Currency(code ); _ = currency;
  accounts := this.SafeValue(*this.At(MkString("options")) , MkString("coinbaseAccounts") ); _ = accounts;
  ...
  account := this.SafeValue(*(*this.At(MkString("options")) ).At(MkString("coinbaseAccountsByCurrencyId") ), currencyId ); _ = account;
  if IsTrue(OpEq2(account , MkUndefined() )) {
  ...
  request := MkMap(&VarMap{"id":*(account ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostCoinbaseAccountsIdAddresses"), this.Extend(request , params )); _ = response;
  address := this.SafeString(response , MkString("address") ); _ = address;
  tag := this.SafeString(response , MkString("destination_tag") ); _ = tag;
  return MkMap(&VarMap{
        "currency":code ,
...
func (this *Coinbasepro) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    what := OpAdd(nonce , OpAdd(method , OpAdd(request , payload ))); _ = what;
    secret := this.Base64ToBinary(*this.At(MkString("secret")) ); _ = secret;
    signature := this.Hmac(this.Encode(what ), secret , MkString("sha256") , MkString("base64") ); _ = signature;
