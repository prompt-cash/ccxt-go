...
func (this *Lykke) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = market
	marketId := this.SafeString(trade, MkString("AssetPairId"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market)
	_ = symbol
	id := this.SafeString2(trade, MkString("id"), MkString("Id"))
	_ = id
	orderId := this.SafeString(trade, MkString("OrderId"))
	_ = orderId
	timestamp := this.Parse8601(this.SafeString2(trade, MkString("dateTime"), MkString("DateTime")))
	_ = timestamp
	priceString := this.SafeString2(trade, MkString("price"), MkString("Price"))
	_ = priceString
	amountString := this.SafeString2(trade, MkString("volume"), MkString("Amount"))
	_ = amountString
	side := this.SafeStringLower(trade, MkString("action"))
	_ = side
...
func (this *Lykke) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("mobileGetTradesAssetPairId"), this.Extend(request, params))
	_ = response
...
func (this *Lykke) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetWallets"), params)
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = balance
		currencyId := this.SafeString(balance, MkString("AssetId"))
		_ = currencyId
...
func (this *Lykke) CreateOrder(goArgs ...*Variant) *Variant {
	...
	method := OpAdd(MkString("privatePostOrdersV2"), this.Capitalize(type_))
	_ = method
	result := this.Call(method, this.Extend(query, params))
	_ = result
	id := this.SafeString(result, MkString("Id"))
	_ = id
...
func (this *Lykke) FetchMarkets(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	markets := this.Call(MkString("publicGetAssetPairs"))
	_ = markets
	...
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		...
		_ = symbol
		pricePrecision := this.SafeString(market, MkString("Accuracy"))
		_ = pricePrecision
...
func (this *Lykke) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = request
	ticker := this.Call(MkString("mobileGetMarketMarket"), this.Extend(request, params))
	_ = ticker
...
func (this *Lykke) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = status
	marketId := this.SafeString(order, MkString("AssetPairId"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market)
	_ = symbol
	lastTradeTimestamp := this.Parse8601(this.SafeString(order, MkString("LastMatchTime")))
	...
	remaining := Math.Abs(this.SafeNumber(order, MkString("RemainingVolume")))
	_ = remaining
...
func (this *Lykke) FetchOrders(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetOrders"), params)
	_ = response
	market := OpCopy(MkUndefined())
	_ = market
	if IsTrue(OpNotEq2(symbol, MkUndefined())) {
...
func (this *Lykke) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"status": MkString("InOrderBook")})
	_ = request
...
func (this *Lykke) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"status": MkString("Matched")})
	_ = request
	return this.FetchOrders(symbol, since, limit, this.Extend(request, params))
}
...
func (this *Lykke) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("publicGetOrderBooksAssetPairId"), this.Extend(MkMap(&VarMap{"AssetPairId": this.MarketId(symbol)}), params))
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		sideTimestamp := this.Parse8601(*(side).At(MkString("Timestamp")))
		_ = sideTimestamp
