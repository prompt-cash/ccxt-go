...
func (this *Btctradeua) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  bids := this.Call(MkString("publicGetTradesBuySymbol"), this.Extend(request , params )); _ = bids;
  asks := this.Call(MkString("publicGetTradesSellSymbol"), this.Extend(request , params )); _ = asks;
...
func (this *Btctradeua) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetJapanStatHighSymbol"), this.Extend(request , params )); _ = response;
  ticker := this.SafeValue(response , MkString("trades") ); _ = ticker;
  ...
  if IsTrue(OpGt(tickerLength , MkInteger(0) )) {
    ...
    for i := OpCopy(start ); IsTrue(OpLw(i , ticker.Length )); OpIncr(& i ){
      ...
      low := this.SafeNumber(candle , MkInteger(3) ); _ = low;
      ...
      baseVolume := this.SafeNumber(candle , MkInteger(5) ); _ = baseVolume;
    ...
    last := OpSub(tickerLength , MkInteger(1) ); _ = last;
...
func (this *Btctradeua) ConvertMonthNameToString(goArgs ...*Variant) *Variant{
  cyrillic := GoGetArg(goArgs, 0, MkUndefined()); _ = cyrillic;
  months := MkMap(&VarMap{
        "Jan":MkString("01") ,
        "Feb":MkString("02") ,
        "Mar":MkString("03") ,
...
func (this *Btctradeua) ParseExchangeSpecificDatetime(goArgs ...*Variant) *Variant{
  cyrillic := GoGetArg(goArgs, 0, MkUndefined()); _ = cyrillic;
  parts := cyrillic.Split(MkString(" ") ); _ = parts;
  month := *(parts ).At(MkInteger(0) ); _ = month;
  day := (*(parts ).At(MkInteger(1) )).Call(MkString("replace") , MkString(",") , MkString("") ); _ = day;
  if IsTrue(OpLw(day.Length , MkInteger(2) )) {
    day = OpAdd(MkString("0") , day );
  }
  year := (*(parts ).At(MkInteger(2) )).Call(MkString("replace") , MkString(",") , MkString("") ); _ = year;
  ...
  hms := *(parts ).At(MkInteger(3) ); _ = hms;
  hmsParts := hms.Split(MkString(":") ); _ = hmsParts;
  h := this.SafeString(hmsParts , MkInteger(0) ); _ = h;
  m := MkString("00") ; _ = m;
  ampm := this.SafeString(parts , MkInteger(4) ); _ = ampm;
  if IsTrue(OpEq2(h , MkString("noon") )) {
    h = MkString("12") ;
  } else {
    intH := parseInt(h ); _ = intH;
  ...
  ymd := (MkArray(&VarArray{
          ...
          month ,
          day ,
          })).Call(MkString("join") , MkString("-") ); _ = ymd;
  ymdhms := OpAdd(ymd , OpAdd(MkString("T") , OpAdd(h , OpAdd(MkString(":") , OpAdd(m , MkString(":00") ))))); _ = ymdhms;
  timestamp := this.Parse8601(ymdhms ); _ = timestamp;
  intM := parseInt(m ); _ = intM;
...
func (this *Btctradeua) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.ParseExchangeSpecificDatetime(this.SafeString(trade , MkString("pub_date") )); _ = timestamp;
  ...
  amountString := this.SafeString(trade , MkString("amnt_trade") ); _ = amountString;
...
func (this *Btctradeua) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetDealsSymbol"), this.Extend(request , params )); _ = response;
  trades := MkArray(&VarArray{}); _ = trades;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    id := this.SafeInteger(*(response ).At(i ), MkString("id") ); _ = id;
...
func (this *Btctradeua) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  method := OpAdd(MkString("privatePost") , OpAdd(this.Capitalize(side ), MkString("Id") )); _ = method;
...
func (this *Btctradeua) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostMyOrdersSymbol"), this.Extend(request , params )); _ = response;
  orders := this.SafeValue(response , MkString("your_open_orders") ); _ = orders;
...
func (this *Btctradeua) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    auth := OpAdd(body , *this.At(MkString("secret")) ); _ = auth;
