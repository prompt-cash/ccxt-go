...
func (this *Bw) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetExchangeConfigControllerWebsiteMarketcontrollerGetByWebId"), params ); _ = response;
  markets := this.SafeValue(response , MkString("datas") , MkArray(&VarArray{})); _ = markets;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    ...
    numericId := parseInt(id ); _ = numericId;
    name := this.SafeStringUpper(market , MkString("name") ); _ = name;
    base := MkUndefined(); _ = base;
    quote := MkUndefined(); _ = quote;
    ArrayUnpack(name.Split(MkString("_") ), &base, &quote);
    base = this.SafeCurrencyCode(base );
    quote = this.SafeCurrencyCode(quote );
    baseId := this.SafeString(market , MkString("sellerCurrencyId") ); _ = baseId;
    quoteId := this.SafeString(market , MkString("buyerCurrencyId") ); _ = quoteId;
    baseNumericId := parseInt(baseId ); _ = baseNumericId;
    quoteNumericId := parseInt(quoteId ); _ = quoteNumericId;
    symbol := OpAdd(base , OpAdd(MkString("/") , quote )); _ = symbol;
    state := this.SafeInteger(market , MkString("state") ); _ = state;
    active := OpEq2(state , MkInteger(1) ); _ = active;
    fee := this.SafeNumber(market , MkString("defaultFee") ); _ = fee;
...
func (this *Bw) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetExchangeConfigControllerWebsiteCurrencycontrollerGetCurrencyList"), params ); _ = response;
  currencies := this.SafeValue(response , MkString("datas") , MkArray(&VarArray{})); _ = currencies;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    ...
    id := this.SafeString(currency , MkString("currencyId") ); _ = id;
    code := this.SafeCurrencyCode(this.SafeStringUpper(currency , MkString("name") )); _ = code;
    state := this.SafeInteger(currency , MkString("state") ); _ = state;
    active := OpEq2(state , MkInteger(1) ); _ = active;
...
func (this *Bw) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(ticker , MkInteger(0) ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  timestamp := this.Milliseconds(); _ = timestamp;
  close := this.SafeNumber(ticker , MkInteger(1) ); _ = close;
...
func (this *Bw) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetApiDataV1Ticker"), this.Extend(request , params )); _ = response;
  ticker := this.SafeValue(response , MkString("datas") , MkArray(&VarArray{})); _ = ticker;
...
func (this *Bw) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicGetApiDataV1Tickers"), params ); _ = response;
  datas := this.SafeValue(response , MkString("datas") , MkArray(&VarArray{})); _ = datas;
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , datas.Length )); OpIncr(& i ){
    ticker := this.ParseTicker(*(datas ).At(i )); _ = ticker;
...
func (this *Bw) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetApiDataV1Entrusts"), this.Extend(request , params )); _ = response;
  orderbook := this.SafeValue(response , MkString("datas") , MkArray(&VarArray{})); _ = orderbook;
...
func (this *Bw) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeTimestamp(trade , MkInteger(2) ); _ = timestamp;
  priceString := this.SafeString(trade , MkInteger(5) ); _ = priceString;
  amountString := this.SafeString(trade , MkInteger(6) ); _ = amountString;
  ...
  marketId := this.SafeString(trade , MkInteger(1) ); _ = marketId;
  ...
  if IsTrue(OpNotEq2(marketId , MkUndefined() )) {
    if IsTrue(OpHasMember(marketId , *this.At(MkString("markets_by_id")) )) {
      ...
      marketName := this.SafeString(trade , MkInteger(3) ); _ = marketName;
  ...
  sideString := this.SafeString(trade , MkInteger(4) ); _ = sideString;
  side := OpTrinary(OpEq2(sideString , MkString("ask") ), MkString("sell") , MkString("buy") ); _ = side;
  return MkMap(&VarMap{
        "id":MkUndefined() ,
        "timestamp":timestamp ,
        "datetime":this.Iso8601(timestamp ),
...
func (this *Bw) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetApiDataV1Trades"), this.Extend(request , params )); _ = response;
  trades := this.SafeValue(response , MkString("datas") , MkArray(&VarArray{})); _ = trades;
...
func (this *Bw) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetApiDataV1Klines"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("datas") , MkArray(&VarArray{})); _ = data;
...
func (this *Bw) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostExchangeFundControllerWebsiteFundcontrollerFindbypage"), params ); _ = response;
  data := this.SafeValue(response , MkString("datas") , MkMap(&VarMap{})); _ = data;
  balances := this.SafeValue(data , MkString("list") , MkArray(&VarArray{})); _ = balances;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , balances.Length )); OpIncr(& i ){
    ...
    currencyId := this.SafeString(balance , MkString("currencyTypeId") ); _ = currencyId;
...
func (this *Bw) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostExchangeEntrustControllerWebsiteEntrustControllerAddEntrust"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("datas") ); _ = data;
  id := this.SafeString(data , MkString("entrustId") ); _ = id;
  return MkMap(&VarMap{
        "id":id ,
        "info":response ,
...
func (this *Bw) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  timestamp := this.SafeInteger(order , MkString("createTime") ); _ = timestamp;
  ...
  filled := this.SafeNumber(order , MkString("completeAmount") ); _ = filled;
  remaining := this.SafeNumber2(order , MkString("availabelAmount") , MkString("availableAmount") ); _ = remaining;
  cost := this.SafeNumber(order , MkString("totalMoney") ); _ = cost;
...
func (this *Bw) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetExchangeEntrustControllerWebsiteEntrustControllerGetEntrustById"), this.Extend(request , params )); _ = response;
  order := this.SafeValue(response , MkString("datas") , MkMap(&VarMap{})); _ = order;
...
func (this *Bw) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostExchangeEntrustControllerWebsiteEntrustControllerCancelEntrust"), this.Extend(request , params )); _ = response;
  return MkMap(&VarMap{
        "info":response ,
        "id":id ,
        });
...
func (this *Bw) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetExchangeEntrustControllerWebsiteEntrustControllerGetUserEntrustRecordFromCacheWithPage"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("datas") , MkMap(&VarMap{})); _ = data;
  orders := this.SafeValue(data , MkString("entrustList") , MkArray(&VarArray{})); _ = orders;
...
func (this *Bw) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetExchangeEntrustControllerWebsiteEntrustControllerGetUserEntrustList"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("datas") , MkMap(&VarMap{})); _ = data;
  orders := this.SafeValue(data , MkString("entrustList") , MkArray(&VarArray{})); _ = orders;
  return this.ParseOrders(orders , market , since , limit );
}
...
func (this *Bw) Sign(goArgs ...*Variant) *Variant{
  ...
  url := OpAdd(this.ImplodeHostname(*(*this.At(MkString("urls")) ).At(MkString("api") )), OpAdd(MkString("/") , path )); _ = url;
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ms := (this.Milliseconds()).Call(MkString("toString") ); _ = ms;
    content := MkString("") ; _ = content;
    if IsTrue(OpEq2(method , MkString("GET") )) {
      sortedParams := this.Keysort(params ); _ = sortedParams;
      keys := GoGetKeys(sortedParams ); _ = keys;
    ...
    signature := OpAdd(*this.At(MkString("apiKey")) , OpAdd(ms , OpAdd(content , *this.At(MkString("secret")) ))); _ = signature;
    hash := this.Hash(this.Encode(signature ), MkString("md5") ); _ = hash;
...
func (this *Bw) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"currencyTypeName":*(currency ).At(MkString("name") )}); _ = request;
  response := this.Call(MkString("privatePostExchangeFundControllerWebsiteFundcontrollerGetPayinAddress"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("datas") , MkMap(&VarMap{})); _ = data;
...
func (this *Bw) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString(transaction , MkString("depositId") , MkString("withdrawalId") ); _ = id;
  address := this.SafeString2(transaction , MkString("depositAddress") , MkString("withdrawalAddress") ); _ = address;
  currencyId := this.SafeString2(transaction , MkString("currencyId") , MkString("currencyTypeId") ); _ = currencyId;
  ...
  type_ := OpTrinary(OpHasMember(MkString("depositId") , transaction ), MkString("deposit") , MkString("withdrawal") ); _ = type_;
  amount := this.SafeNumber2(transaction , MkString("actuallyAmount") , MkString("amount") ); _ = amount;
  status := this.ParseTransactionStatus(this.SafeString2(transaction , MkString("verifyStatus") , MkString("state") )); _ = status;
  timestamp := this.SafeInteger(transaction , MkString("createTime") ); _ = timestamp;
...
func (this *Bw) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostExchangeFundControllerWebsiteFundcontrollerGetPayinCoinRecord"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("datas") , MkMap(&VarMap{})); _ = data;
  deposits := this.SafeValue(data , MkString("list") , MkArray(&VarArray{})); _ = deposits;
...
func (this *Bw) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetExchangeFundControllerWebsiteFundwebsitecontrollerGetpayoutcoinrecord"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("datas") , MkMap(&VarMap{})); _ = data;
  withdrawals := this.SafeValue(data , MkString("list") , MkArray(&VarArray{})); _ = withdrawals;
...
func (this *Bw) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  resMsg := this.SafeValue(response , MkString("resMsg") ); _ = resMsg;
  errorCode := this.SafeString(resMsg , MkString("code") ); _ = errorCode;
