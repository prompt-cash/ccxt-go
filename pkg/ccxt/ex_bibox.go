...
func (this *Bibox) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"cmd":MkString("pairList") }); _ = request;
  response := this.Call(MkString("publicGetMdata"), this.Extend(request , params )); _ = response;
...
func (this *Bibox) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(market , MkUndefined() )) {
    ...
    baseId := this.SafeString(ticker , MkString("coin_symbol") ); _ = baseId;
    quoteId := this.SafeString(ticker , MkString("currency_symbol") ); _ = quoteId;
    base := this.SafeCurrencyCode(baseId ); _ = base;
    quote := this.SafeCurrencyCode(quoteId ); _ = quote;
  ...
  baseVolume := this.SafeNumber2(ticker , MkString("vol") , MkString("vol24H") ); _ = baseVolume;
  ...
  percentage := this.SafeString(ticker , MkString("percent") ); _ = percentage;
...
func (this *Bibox) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"cmd":MkString("marketAll") }); _ = request;
  response := this.Call(MkString("publicGetMdata"), this.Extend(request , params )); _ = response;
  tickers := this.ParseTickers(*(response ).At(MkString("result") ), symbols ); _ = tickers;
  result := this.IndexBy(tickers , MkString("symbol") ); _ = result;
...
func (this *Bibox) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeInteger2(trade , MkString("time") , MkString("createdAt") ); _ = timestamp;
  side := this.SafeInteger2(trade , MkString("side") , MkString("order_side") ); _ = side;
  ...
  if IsTrue(OpEq2(market , MkUndefined() )) {
    ...
    if IsTrue(OpEq2(marketId , MkUndefined() )) {
      baseId := this.SafeString(trade , MkString("coin_symbol") ); _ = baseId;
      quoteId := this.SafeString(trade , MkString("currency_symbol") ); _ = quoteId;
  ...
  feeCurrency := this.SafeString(trade , MkString("fee_symbol") ); _ = feeCurrency;
  ...
  feeRate := OpCopy(MkUndefined() ); _ = feeRate;
...
func (this *Bibox) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  method := this.SafeString(*this.At(MkString("options")) , MkString("fetchCurrencies") , MkString("fetch_currencies_public") ); _ = method;
  return this.Call(method , params );
}

func (this *Bibox) FetchCurrenciesPublic(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"cmd":MkString("currencies") }); _ = request;
  response := this.Call(MkString("publicGetCdata"), this.Extend(request , params )); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    ...
    precision := this.SafeInteger(currency , MkString("valid_decimals") ); _ = precision;
    deposit := this.SafeValue(currency , MkString("enable_deposit") ); _ = deposit;
    withdraw := this.SafeValue(currency , MkString("enable_withdraw") ); _ = withdraw;
    active := OpAnd(deposit , withdraw ); _ = active;
...
func (this *Bibox) FetchCurrenciesPrivate(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostTransfer"), this.Extend(request , params )); _ = response;
  currencies := this.SafeValue(response , MkString("result") ); _ = currencies;
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    currency := *(currencies ).At(i ); _ = currency;
    id := this.SafeString(currency , MkString("symbol") ); _ = id;
    name := *(currency ).At(MkString("name") ); _ = name;
    code := this.SafeCurrencyCode(id ); _ = code;
...
func (this *Bibox) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  type_ := this.SafeString(params , MkString("type") , MkString("assets") ); _ = type_;
  ...
  response := this.Call(MkString("privatePostTransfer"), request ); _ = response;
  balances := this.SafeValue(response , MkString("result") ); _ = balances;
  result := MkMap(&VarMap{"info":balances }); _ = result;
  indexed := OpCopy(MkUndefined() ); _ = indexed;
  ...
  keys := GoGetKeys(indexed ); _ = keys;
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    id := *(keys ).At(i ); _ = id;
    code := id.ToUpperCase(); _ = code;
    ...
    balance := *(indexed ).At(id ); _ = balance;
...
func (this *Bibox) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"page":MkInteger(1) }); _ = request;
  ...
  deposits := this.SafeValue(*(response ).At(MkString("result") ), MkString("items") , MkArray(&VarArray{})); _ = deposits;
...
func (this *Bibox) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  withdrawals := this.SafeValue(*(response ).At(MkString("result") ), MkString("items") , MkArray(&VarArray{})); _ = withdrawals;
...
func (this *Bibox) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  address := this.SafeString(transaction , MkString("to_address") ); _ = address;
  currencyId := this.SafeString(transaction , MkString("coin_symbol") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  timestamp := this.SafeString(transaction , MkString("createdAt") ); _ = timestamp;
  tag := this.SafeString(transaction , MkString("addr_remark") ); _ = tag;
...
func (this *Bibox) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  orderType := OpTrinary(OpEq2(type_ , MkString("limit") ), MkInteger(2) , MkInteger(1) ); _ = orderType;
  ...
  response := this.Call(MkString("privatePostOrderpending"), request ); _ = response;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Bibox) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrderpending"), request ); _ = response;
  order := this.SafeValue(response , MkString("result") ); _ = order;
...
func (this *Bibox) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(market , MkUndefined() )) {
    ...
    baseId := this.SafeString(order , MkString("coin_symbol") ); _ = baseId;
    quoteId := this.SafeString(order , MkString("currency_symbol") ); _ = quoteId;
    if IsTrue(OpAnd(OpNotEq2(baseId , MkUndefined() ), OpNotEq2(quoteId , MkUndefined() ))) {
      marketId = OpAdd(baseId , OpAdd(MkString("_") , quoteId ));
    }
    if IsTrue(OpHasMember(marketId , *this.At(MkString("markets_by_id")) )) {
  ...
  rawType := this.SafeString(order , MkString("order_type") ); _ = rawType;
  type_ := OpTrinary(OpEq2(rawType , MkString("1") ), MkString("market") , MkString("limit") ); _ = type_;
  timestamp := this.SafeInteger(order , MkString("createdAt") ); _ = timestamp;
  price := this.SafeNumber(order , MkString("price") ); _ = price;
  average := this.SafeNumber(order , MkString("deal_price") ); _ = average;
  filled := this.SafeNumber(order , MkString("deal_amount") ); _ = filled;
  amount := this.SafeNumber(order , MkString("amount") ); _ = amount;
  cost := this.SafeNumber2(order , MkString("deal_money") , MkString("money") ); _ = cost;
  rawSide := this.SafeString(order , MkString("order_side") ); _ = rawSide;
  side := OpTrinary(OpEq2(rawSide , MkString("1") ), MkString("buy") , MkString("sell") ); _ = side;
...
func (this *Bibox) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  size := OpTrinary(limit , limit , MkInteger(200) ); _ = size;
  request := MkMap(&VarMap{
  ...
  response := this.Call(MkString("privatePostOrderpending"), request ); _ = response;
  orders := this.SafeValue(*(response ).At(MkString("result") ), MkString("items") , MkArray(&VarArray{})); _ = orders;
...
func (this *Bibox) FetchClosedOrders(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined() ); _ = symbol;
  since := GoGetArg(goArgs, 1, MkUndefined() ); _ = since;
  limit := GoGetArg(goArgs, 2, MkInteger(200) ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
...
func (this *Bibox) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrderpending"), request ); _ = response;
  trades := this.SafeValue(*(response ).At(MkString("result") ), MkString("items") , MkArray(&VarArray{})); _ = trades;
...
func (this *Bibox) Sign(goArgs ...*Variant) *Variant{
  ...
  url := OpAdd(this.ImplodeHostname(*(*this.At(MkString("urls")) ).At(MkString("api") )), OpAdd(MkString("/") , OpAdd(*this.At(MkString("version")) , OpAdd(MkString("/") , path )))); _ = url;
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    if IsTrue(OpEq2(api , MkString("v2private") )) {
      ...
      json_params := this.Json(params ); _ = json_params;
      body = MkMap(&VarMap{
          "body":json_params ,
...
func (this *Bibox) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("error") , response )) {
    if IsTrue(OpHasMember(MkString("code") , *(response ).At(MkString("error") ))) {
      code := this.SafeString(*(response ).At(MkString("error") ), MkString("code") ); _ = code;
