...
func (this *Bitz) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("marketGetSymbolList"), params)
	_ = response
	markets := this.SafeValue(response, MkString("data"))
	_ = markets
	ids := GoGetKeys(markets)
	_ = ids
	...
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		...
		_ = numericId
		baseId := this.SafeString(market, MkString("coinFrom"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("coinTo"))
		_ = quoteId
		...
		_ = symbol
		pricePrecisionString := this.SafeString(market, MkString("priceFloat"))
		_ = pricePrecisionString
		minPrice := this.ParsePrecision(pricePrecisionString)
		_ = minPrice
		precision := MkMap(&VarMap{
		...
		_ = precision
		minAmount := this.SafeString(market, MkString("minTrade"))
		_ = minAmount
...
func (this *Bitz) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("assetsPostGetUserAssets"), params)
	_ = response
	balances := this.SafeValue(*(response).At(MkString("data")), MkString("info"))
	_ = balances
	timestamp := this.ParseMicrotime(this.SafeString(response, MkString("microtime")))
	_ = timestamp
	result := MkMap(&VarMap{
		"info":      response,
		"timestamp": timestamp,
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = balance
		currencyId := this.SafeString(balance, MkString("name"))
		_ = currencyId
...
func (this *Bitz) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = symbol
	last := this.SafeNumber(ticker, MkString("now"))
	_ = last
...
func (this *Bitz) ParseMicrotime(goArgs ...*Variant) *Variant {
	microtime := GoGetArg(goArgs, 0, MkUndefined())
	_ = microtime
	if IsTrue(OpEq2(microtime, MkUndefined())) {
		return microtime
	}
	parts := microtime.Split(MkString(" "))
	_ = parts
	milliseconds := parseFloat(*(parts).At(MkInteger(0)))
	_ = milliseconds
	seconds := parseInt(*(parts).At(MkInteger(1)))
	_ = seconds
...
func (this *Bitz) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("marketGetTicker"), this.Extend(request, params))
	_ = response
	ticker := this.ParseTicker(*(response).At(MkString("data")), market)
	_ = ticker
	timestamp := this.ParseMicrotime(this.SafeString(response, MkString("microtime")))
	_ = timestamp
...
func (this *Bitz) FetchTickers(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("marketGetTickerall"), this.Extend(request, params))
	_ = response
...
func (this *Bitz) FetchTime(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("marketGetGetServerTime"), params)
	_ = response
...
func (this *Bitz) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = response
	orderbook := this.SafeValue(response, MkString("data"))
	_ = orderbook
	timestamp := this.ParseMicrotime(this.SafeString(response, MkString("microtime")))
	_ = timestamp
...
func (this *Bitz) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = id
	timestamp := this.SafeTimestamp(trade, MkString("T"))
	_ = timestamp
	...
	priceString := this.SafeString(trade, MkString("p"))
	_ = priceString
	amountString := this.SafeString(trade, MkString("n"))
	_ = amountString
	...
	_ = cost
	side := this.SafeString(trade, MkString("s"))
	_ = side
	return MkMap(&VarMap{
		"timestamp":    timestamp,
		"datetime":     this.Iso8601(timestamp),
...
func (this *Bitz) FetchTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"symbol": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("marketGetOrder"), this.Extend(request, params))
	_ = response
...
func (this *Bitz) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("marketGetKline"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	bars := this.SafeValue(data, MkString("bars"), MkArray(&VarArray{}))
	_ = bars
...
func (this *Bitz) ParseOrder(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(market, MkUndefined())) {
		baseId := this.SafeString(order, MkString("coinFrom"))
		_ = baseId
		quoteId := this.SafeString(order, MkString("coinTo"))
		_ = quoteId
	...
	side := this.SafeString(order, MkString("flag"))
	_ = side
	...
	_ = price
	amount := this.SafeNumber(order, MkString("number"))
	_ = amount
	remaining := this.SafeNumber(order, MkString("numberOver"))
	_ = remaining
	filled := this.SafeNumber(order, MkString("numberDeal"))
	_ = filled
	...
	cost := this.SafeNumber(order, MkString("orderTotalPrice"))
	_ = cost
...
func (this *Bitz) CreateOrder(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	orderType := OpTrinary(OpEq2(side, MkString("buy")), MkString("1"), MkString("2"))
	_ = orderType
	...
	_ = request
	response := this.Call(MkString("tradePostAddEntrustSheet"), this.Extend(request, params))
	_ = response
	timestamp := this.ParseMicrotime(this.SafeString(response, MkString("microtime")))
	_ = timestamp
	order := this.Extend(MkMap(&VarMap{"timestamp": timestamp}), *(response).At(MkString("data")))
	_ = order
...
func (this *Bitz) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"entrustSheetId": id})
	_ = request
	response := this.Call(MkString("tradePostCancelEntrustSheet"), this.Extend(request, params))
	_ = response
	return response
}
...
func (this *Bitz) FetchOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"entrustSheetId": id})
	_ = request
	response := this.Call(MkString("tradePostGetEntrustSheetInfo"), this.Extend(request, params))
	_ = response
...
func (this *Bitz) FetchOrdersWithMethod(goArgs ...*Variant) *Variant {
	...
	_ = response
	orders := this.SafeValue(*(response).At(MkString("data")), MkString("data"), MkArray(&VarArray{}))
	_ = orders
...
func (this *Bitz) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = currency
	timestamp := this.SafeInteger(transaction, MkString("updated"))
	_ = timestamp
	...
	_ = fee
	feeCost := this.SafeNumber(transaction, MkString("network_fee"))
	_ = feeCost
...
func (this *Bitz) FetchTransactionsForType(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("tradePostDepositOrWithdraw"), this.Extend(request, params))
	_ = response
	transactions := this.SafeValue(*(response).At(MkString("data")), MkString("data"), MkArray(&VarArray{}))
	_ = transactions
...
func (this *Bitz) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("tradePostCoinOut"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
...
func (this *Bitz) Sign(goArgs ...*Variant) *Variant {
	...
	_ = path
	api := GoGetArg(goArgs, 1, MkString("market"))
	_ = api
	...
	_ = baseUrl
	url := OpAdd(baseUrl, OpAdd(MkString("/"), OpAdd(this.Capitalize(api), OpAdd(MkString("/"), path))))
	_ = url
...
func (this *Bitz) HandleErrors(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpNotEq2(status, MkUndefined())) {
		...
		if IsTrue(OpEq2(status, MkString("200"))) {
			code := this.SafeInteger(response, MkString("data"))
			_ = code
