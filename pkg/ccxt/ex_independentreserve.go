...
func (this *Independentreserve) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	baseCurrencies := this.Call(MkString("publicGetGetValidPrimaryCurrencyCodes"), params)
	_ = baseCurrencies
	quoteCurrencies := this.Call(MkString("publicGetGetValidSecondaryCurrencyCodes"), params)
	_ = quoteCurrencies
	limits := this.Call(MkString("publicGetGetOrderMinimumVolumes"), params)
	_ = limits
	result := MkArray(&VarArray{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, baseCurrencies.Length)); OpIncr(&i) {
		baseId := *(baseCurrencies).At(i)
		_ = baseId
		base := this.SafeCurrencyCode(baseId)
		_ = base
		minAmount := this.SafeNumber(limits, baseId)
		_ = minAmount
		for j := MkInteger(0); IsTrue(OpLw(j, quoteCurrencies.Length)); OpIncr(&j) {
			quoteId := *(quoteCurrencies).At(j)
			_ = quoteId
...
func (this *Independentreserve) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	balances := this.Call(MkString("privatePostGetAccounts"), params)
	_ = balances
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = balance
		currencyId := this.SafeString(balance, MkString("CurrencyCode"))
		_ = currencyId
...
func (this *Independentreserve) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = response
	timestamp := this.Parse8601(this.SafeString(response, MkString("CreatedTimestampUtc")))
	_ = timestamp
...
func (this *Independentreserve) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	timestamp := this.Parse8601(this.SafeString(ticker, MkString("CreatedTimestampUtc")))
	_ = timestamp
...
func (this *Independentreserve) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetGetMarketSummary"), this.Extend(request, params))
	_ = response
...
func (this *Independentreserve) ParseOrder(goArgs ...*Variant) *Variant {
	...
	symbol := OpCopy(MkUndefined())
	_ = symbol
	baseId := this.SafeString(order, MkString("PrimaryCurrencyCode"))
	_ = baseId
	quoteId := this.SafeString(order, MkString("SecondaryCurrencyCode"))
	_ = quoteId
	...
	orderType := this.SafeString2(order, MkString("Type"), MkString("OrderType"))
	_ = orderType
	...
	timestamp := this.Parse8601(this.SafeString(order, MkString("CreatedTimestampUtc")))
	_ = timestamp
	amount := this.SafeNumber2(order, MkString("VolumeOrdered"), MkString("Volume"))
	_ = amount
	filled := this.SafeNumber(order, MkString("VolumeFilled"))
	_ = filled
	remaining := this.SafeNumber(order, MkString("Outstanding"))
	_ = remaining
	feeRate := this.SafeNumber(order, MkString("FeePercent"))
	_ = feeRate
	...
	_ = fee
	id := this.SafeString(order, MkString("OrderGuid"))
	_ = id
	status := this.ParseOrderStatus(this.SafeString(order, MkString("Status")))
	_ = status
	cost := this.SafeNumber(order, MkString("Value"))
	_ = cost
...
func (this *Independentreserve) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privatePostGetOrderDetails"), this.Extend(MkMap(&VarMap{"orderGuid": id}), params))
	_ = response
...
func (this *Independentreserve) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	*(request).At(MkString("pageSize")) = OpCopy(limit)
	response := this.Call(MkString("privatePostGetOpenOrders"), this.Extend(request, params))
	_ = response
...
func (this *Independentreserve) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	*(request).At(MkString("pageIndex")) = MkInteger(1)
	*(request).At(MkString("pageSize")) = OpCopy(limit)
	response := this.Call(MkString("privatePostGetClosedOrders"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("Data"), MkArray(&VarArray{}))
	_ = data
	return this.ParseOrders(data, market, since, limit)
...
func (this *Independentreserve) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	pageIndex := this.SafeInteger(params, MkString("pageIndex"), MkInteger(1))
	_ = pageIndex
	if IsTrue(OpEq2(limit, MkUndefined())) {
		limit = MkInteger(50)
	}
	...
	_ = request
	response := this.Call(MkString("privatePostGetTrades"), this.Extend(request, params))
	_ = response
	market := OpCopy(MkUndefined())
	_ = market
	if IsTrue(OpNotEq2(symbol, MkUndefined())) {
...
func (this *Independentreserve) ParseTrade(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.Parse8601(*(trade).At(MkString("TradeTimestampUtc")))
	_ = timestamp
	id := this.SafeString(trade, MkString("TradeGuid"))
	_ = id
	orderId := this.SafeString(trade, MkString("OrderGuid"))
	_ = orderId
	priceString := this.SafeString2(trade, MkString("Price"), MkString("SecondaryCurrencyTradePrice"))
	_ = priceString
	amountString := this.SafeString2(trade, MkString("VolumeTraded"), MkString("PrimaryCurrencyAmount"))
	_ = amountString
	...
	_ = cost
	baseId := this.SafeString(trade, MkString("PrimaryCurrencyCode"))
	_ = baseId
	quoteId := this.SafeString(trade, MkString("SecondaryCurrencyCode"))
	_ = quoteId
	...
	_ = symbol
	side := this.SafeString(trade, MkString("OrderType"))
	_ = side
...
func (this *Independentreserve) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetGetRecentTrades"), this.Extend(request, params))
	_ = response
...
func (this *Independentreserve) CreateOrder(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	capitalizedOrderType := this.Capitalize(type_)
	_ = capitalizedOrderType
	method := OpAdd(MkString("privatePostPlace"), OpAdd(capitalizedOrderType, MkString("Order")))
	_ = method
	orderType := OpCopy(capitalizedOrderType)
	_ = orderType
...
func (this *Independentreserve) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"orderGuid": id})
	_ = request
