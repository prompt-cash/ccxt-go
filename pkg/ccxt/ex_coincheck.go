...
func (this *Coincheck) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	balances := this.Call(MkString("privateGetAccountsBalance"), params)
	_ = balances
	...
	for i := MkInteger(0); IsTrue(OpLw(i, codes.Length)); OpIncr(&i) {
		...
		if IsTrue(OpHasMember(currencyId, balances)) {
			...
			_ = account
			reserved := OpAdd(currencyId, MkString("_reserved"))
			_ = reserved
...
func (this *Coincheck) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetExchangeOrdersOpens"), params)
	_ = response
	rawOrders := this.SafeValue(response, MkString("orders"), MkArray(&VarArray{}))
	_ = rawOrders
	parsedOrders := this.ParseOrders(rawOrders, market, since, limit)
	_ = parsedOrders
...
func (this *Coincheck) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = id
	side := this.SafeString(order, MkString("order_type"))
	_ = side
	timestamp := this.Parse8601(this.SafeString(order, MkString("created_at")))
	_ = timestamp
	amount := this.SafeNumber(order, MkString("pending_amount"))
	_ = amount
	remaining := this.SafeNumber(order, MkString("pending_amount"))
	_ = remaining
...
func (this *Coincheck) FetchTicker(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"pair": *(market).At(MkString("id"))})
	_ = request
	ticker := this.Call(MkString("publicGetTicker"), this.Extend(request, params))
	_ = ticker
...
func (this *Coincheck) ParseTrade(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpHasMember(MkString("liquidity"), trade)) {
		...
		funds := this.SafeValue(trade, MkString("funds"), MkMap(&VarMap{}))
		_ = funds
...
func (this *Coincheck) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	response := this.Call(MkString("privateGetExchangeOrdersTransactions"), this.Extend(MkMap(&VarMap{}), params))
	_ = response
...
func (this *Coincheck) CreateOrder(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(type_, MkString("market"))) {
		order_type := OpAdd(type_, OpAdd(MkString("_"), side))
		_ = order_type
		*(request).At(MkString("order_type")) = OpCopy(order_type)
		prefix := OpTrinary(OpEq2(side, MkString("buy")), OpAdd(order_type, MkString("_")), MkString(""))
		_ = prefix
	...
	response := this.Call(MkString("privatePostExchangeOrders"), this.Extend(request, params))
	_ = response
...
func (this *Coincheck) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		auth := OpAdd(nonce, OpAdd(url, queryString))
		_ = auth
