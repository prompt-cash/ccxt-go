...
func (this *Currencycom) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	markets := this.SafeValue(response, MkString("symbols"))
	_ = markets
	...
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		...
		_ = active
		type_ := this.SafeStringLower(market, MkString("marketType"))
		_ = type_
		...
		_ = entry
		exchangeFee := this.SafeNumber2(market, MkString("exchangeFee"), MkString("tradingFee"))
		_ = exchangeFee
		makerFee := this.SafeNumber(market, MkString("makerFee"), exchangeFee)
		_ = makerFee
		takerFee := this.SafeNumber(market, MkString("takerFee"), exchangeFee)
		_ = takerFee
...
func (this *Currencycom) FetchAccounts(goArgs ...*Variant) *Variant {
	...
	_ = response
	accounts := this.SafeValue(response, MkString("balances"), MkArray(&VarArray{}))
	_ = accounts
	result := MkArray(&VarArray{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, accounts.Length)); OpIncr(&i) {
		...
		_ = account
		accountId := this.SafeInteger(account, MkString("accountId"))
		_ = accountId
...
func (this *Currencycom) FetchTicker(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"symbol": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetTicker24hr"), this.Extend(request, params))
	_ = response
...
func (this *Currencycom) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("publicGetTicker24hr"), params)
	_ = response
...
func (this *Currencycom) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetKlines"), this.Extend(request, params))
	_ = response
...
func (this *Currencycom) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = cost
	id := this.SafeString2(trade, MkString("a"), MkString("id"))
	_ = id
...
func (this *Currencycom) FetchTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetAggTrades"), this.Extend(request, params))
	_ = response
...
func (this *Currencycom) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = uppercaseType
	newOrderRespType := this.SafeValue(*(*this.At(MkString("options"))).At(MkString("newOrderRespType")), type_, MkString("RESULT"))
	_ = newOrderRespType
	request := MkMap(&VarMap{
		"symbol":           *(market).At(MkString("id")),
...
func (this *Currencycom) CancelOrder(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	origClientOrderId := this.SafeValue(params, MkString("origClientOrderId"))
	_ = origClientOrderId
	request := MkMap(&VarMap{"symbol": *(market).At(MkString("id"))})
	_ = request
...
func (this *Currencycom) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetMyTrades"), this.Extend(request, params))
	_ = response
	return this.ParseTrades(response, market, since, limit)
}
