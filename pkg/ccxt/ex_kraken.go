...
func (this *Kraken) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  markets := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = markets;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    ...
    baseId := this.SafeString(market , MkString("base") ); _ = baseId;
    quoteId := this.SafeString(market , MkString("quote") ); _ = quoteId;
    base := this.SafeCurrencyCode(baseId ); _ = base;
    quote := this.SafeCurrencyCode(quoteId ); _ = quote;
    darkpool := OpGtEq(id.IndexOf(MkString(".d") ), MkInteger(0) ); _ = darkpool;
    altname := this.SafeString(market , MkString("altname") ); _ = altname;
    symbol := OpTrinary(darkpool , altname , OpAdd(base , OpAdd(MkString("/") , quote ))); _ = symbol;
    makerFees := this.SafeValue(market , MkString("fees_maker") , MkArray(&VarArray{})); _ = makerFees;
    firstMakerFee := this.SafeValue(makerFees , MkInteger(0) , MkArray(&VarArray{})); _ = firstMakerFee;
    firstMakerFeeRate := this.SafeNumber(firstMakerFee , MkInteger(1) ); _ = firstMakerFeeRate;
    maker := OpCopy(MkUndefined() ); _ = maker;
    if IsTrue(OpNotEq2(firstMakerFeeRate , MkUndefined() )) {
      maker = OpDiv(parseFloat(firstMakerFeeRate ), MkInteger(100) );
    }
    takerFees := this.SafeValue(market , MkString("fees") , MkArray(&VarArray{})); _ = takerFees;
    firstTakerFee := this.SafeValue(takerFees , MkInteger(0) , MkArray(&VarArray{})); _ = firstTakerFee;
    firstTakerFeeRate := this.SafeNumber(firstTakerFee , MkInteger(1) ); _ = firstTakerFeeRate;
    taker := OpCopy(MkUndefined() ); _ = taker;
    ...
    minAmount := this.SafeNumber(market , MkString("ordermin") ); _ = minAmount;
...
func (this *Kraken) AppendInactiveMarkets(goArgs ...*Variant) *Variant{
  result := GoGetArg(goArgs, 0, MkUndefined()); _ = result;
  precision := MkMap(&VarMap{
        "amount":MkInteger(8) ,
        "price":MkInteger(8) ,
        }); _ = precision;
...
func (this *Kraken) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    ...
    code := this.SafeCurrencyCode(this.SafeString(currency , MkString("altname") )); _ = code;
    precision := this.SafeInteger(currency , MkString("decimals") ); _ = precision;
    active := OpNot(this.InArray(code , *(*this.At(MkString("options")) ).At(MkString("inactiveCurrencies") ))); _ = active;
...
func (this *Kraken) FetchTradingFees(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostTradeVolume"), params ); _ = response;
  tradedVolume := this.SafeNumber(*(response ).At(MkString("result") ), MkString("volume") ); _ = tradedVolume;
  tiers := *(*(*this.At(MkString("fees")) ).At(MkString("trading") )).At(MkString("tiers") ); _ = tiers;
  taker := *(*(tiers ).At(MkString("taker") )).At(MkInteger(1) ); _ = taker;
  maker := *(*(tiers ).At(MkString("maker") )).At(MkInteger(1) ); _ = maker;
...
func (this *Kraken) ParseBidAsk(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(bidask , MkInteger(2) ); _ = timestamp;
...
func (this *Kraken) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  orderbook := this.SafeValue(result , *(market ).At(MkString("id") )); _ = orderbook;
  marketInfo := this.SafeValue(market , MkString("info") , MkMap(&VarMap{})); _ = marketInfo;
  wsName := this.SafeValue(marketInfo , MkString("wsname") ); _ = wsName;
...
func (this *Kraken) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  baseVolume := parseFloat(*(*(ticker ).At(MkString("v") )).At(MkInteger(1) )); _ = baseVolume;
  vwap := parseFloat(*(*(ticker ).At(MkString("p") )).At(MkInteger(1) )); _ = vwap;
  ...
  last := parseFloat(*(*(ticker ).At(MkString("c") )).At(MkInteger(0) )); _ = last;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Kraken) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  marketIds := MkArray(&VarArray{}); _ = marketIds;
  ...
  request := MkMap(&VarMap{"pair":marketIds.Join(MkString(",") )}); _ = request;
  response := this.Call(MkString("publicGetTicker"), this.Extend(request , params )); _ = response;
  tickers := *(response ).At(MkString("result") ); _ = tickers;
...
func (this *Kraken) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  darkpool := OpGtEq(symbol.IndexOf(MkString(".d") ), MkInteger(0) ); _ = darkpool;
  if IsTrue(darkpool ) {
  ...
  request := MkMap(&VarMap{"pair":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetTicker"), this.Extend(request , params )); _ = response;
  ticker := *(*(response ).At(MkString("result") )).At(*(market ).At(MkString("id") )); _ = ticker;
...
func (this *Kraken) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetOHLC"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  ohlcvs := this.SafeValue(result , *(market ).At(MkString("id") ), MkArray(&VarArray{})); _ = ohlcvs;
...
func (this *Kraken) ParseLedgerEntry(goArgs ...*Variant) *Variant{
  ...
  referenceId := this.SafeString(item , MkString("refid") ); _ = referenceId;
  ...
  code := this.SafeCurrencyCode(this.SafeString(item , MkString("asset") ), currency ); _ = code;
  ...
  time := this.SafeNumber(item , MkString("time") ); _ = time;
...
func (this *Kraken) FetchLedger(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostLedgers"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  ledger := this.SafeValue(result , MkString("ledger") , MkMap(&VarMap{})); _ = ledger;
  keys := GoGetKeys(ledger ); _ = keys;
  items := MkArray(&VarArray{}); _ = items;
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    key := *(keys ).At(i ); _ = key;
    value := *(ledger ).At(key ); _ = value;
...
func (this *Kraken) FetchLedgerEntriesByIds(goArgs ...*Variant) *Variant{
  ...
  request := this.Extend(MkMap(&VarMap{"id":ids }), params ); _ = request;
  response := this.Call(MkString("privatePostQueryLedgers"), request ); _ = response;
  result := *(response ).At(MkString("result") ); _ = result;
  keys := GoGetKeys(result ); _ = keys;
  items := MkArray(&VarArray{}); _ = items;
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    key := *(keys ).At(i ); _ = key;
    value := *(result ).At(key ); _ = value;
    *(value ).At(MkString("id") )= OpCopy(key );
    items.Push(value );
  }
  ...
  if IsTrue(GoIsArray(trade )) {
    ...
    if IsTrue(OpEq2(OpType(trade ), MkString("string") )) {
      ...
      if IsTrue(OpHasMember(MkString("ordertxid") , trade )) {
        ...
        foundMarket := this.FindMarketByAltnameOrId(marketId ); _ = foundMarket;
...
func (this *Kraken) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  id := *(market ).At(MkString("id") ); _ = id;
  request := MkMap(&VarMap{"pair":id }); _ = request;
  if IsTrue(OpNotEq2(since , MkUndefined() )) {
  ...
  if IsTrue(OpAnd(OpNotEq2(limit , MkUndefined() ), OpNotEq2(limit , MkInteger(1000) ))) {
    fetchTradesWarning := this.SafeValue(*this.At(MkString("options")) , MkString("fetchTradesWarning") , MkBool(true) ); _ = fetchTradesWarning;
  ...
  result := *(response ).At(MkString("result") ); _ = result;
  trades := *(result ).At(id ); _ = trades;
  length := OpCopy(trades.Length ); _ = length;
  if IsTrue(OpLwEq(length , MkInteger(0) )) {
    return MkArray(&VarArray{});
  }
  lastTrade := *(trades ).At(OpSub(length , MkInteger(1) )); _ = lastTrade;
  lastTradeId := this.SafeString(result , MkString("last") ); _ = lastTradeId;
...
func (this *Kraken) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  balances := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = balances;
  result := MkMap(&VarMap{
        "info":response ,
...
func (this *Kraken) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  clientOrderId := this.SafeString2(params , MkString("userref") , MkString("clientOrderId") ); _ = clientOrderId;
  ...
  if IsTrue(OpEq2(type_ , MkString("limit") )) {
    *(request ).At(MkString("price") )= this.PriceToPrecision(symbol , price );
    ...
    if IsTrue(OpOr(OpEq2(type_ , MkString("stop-loss") ), OpEq2(type_ , MkString("take-profit") ))) {
      stopPrice := this.SafeNumber2(params , MkString("price") , MkString("stopPrice") , price ); _ = stopPrice;
      ...
      if IsTrue(OpOr(OpEq2(type_ , MkString("stop-loss-limit") ), OpEq2(type_ , MkString("take-profit-limit") ))) {
        stopPrice := this.SafeNumber2(params , MkString("price") , MkString("stopPrice") ); _ = stopPrice;
        limitPrice := this.SafeNumber(params , MkString("price2") ); _ = limitPrice;
        stopPriceDefined := OpNotEq2(stopPrice , MkUndefined() ); _ = stopPriceDefined;
        limitPriceDefined := OpNotEq2(limitPrice , MkUndefined() ); _ = limitPriceDefined;
  ...
  response := this.Call(MkString("privatePostAddOrder"), this.Extend(request , params )); _ = response;
...
func (this *Kraken) GetDelistedMarketById(goArgs ...*Variant) *Variant{
  ...
  market := this.SafeValue(*(*this.At(MkString("options")) ).At(MkString("delistedMarketsById") ), id ); _ = market;
  if IsTrue(OpNotEq2(market , MkUndefined() )) {
    return market ;
  }
  baseIdStart := MkInteger(0) ; _ = baseIdStart;
  baseIdEnd := MkInteger(3) ; _ = baseIdEnd;
  quoteIdStart := MkInteger(3) ; _ = quoteIdStart;
  quoteIdEnd := MkInteger(6) ; _ = quoteIdEnd;
  ...
  baseId := id.Slice(baseIdStart , baseIdEnd ); _ = baseId;
  quoteId := id.Slice(quoteIdStart , quoteIdEnd ); _ = quoteId;
  base := this.SafeCurrencyCode(baseId ); _ = base;
  quote := this.SafeCurrencyCode(quoteId ); _ = quote;
...
func (this *Kraken) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  description := this.SafeValue(order , MkString("descr") , MkMap(&VarMap{})); _ = description;
  orderDescription := this.SafeString(description , MkString("order") ); _ = orderDescription;
  side := OpCopy(MkUndefined() ); _ = side;
  type_ := OpCopy(MkUndefined() ); _ = type_;
  ...
  if IsTrue(OpNotEq2(orderDescription , MkUndefined() )) {
    parts := orderDescription.Split(MkString(" ") ); _ = parts;
  ...
  timestamp := this.SafeTimestamp(order , MkString("opentm") ); _ = timestamp;
  amount = this.SafeNumber(order , MkString("vol") , amount );
  filled := this.SafeNumber(order , MkString("vol_exec") ); _ = filled;
  fee := OpCopy(MkUndefined() ); _ = fee;
  cost := this.SafeNumber(order , MkString("cost") ); _ = cost;
  ...
  average := this.SafeNumber(order , MkString("price") ); _ = average;
  if IsTrue(OpNotEq2(market , MkUndefined() )) {
    symbol = *(market ).At(MkString("symbol") );
    if IsTrue(OpHasMember(MkString("fee") , order )) {
      flags := *(order ).At(MkString("oflags") ); _ = flags;
  ...
  if IsTrue(OpEq2(id , MkUndefined() )) {
    txid := this.SafeValue(order , MkString("txid") ); _ = txid;
    id = this.SafeString(txid , MkInteger(0) );
  }
  clientOrderId := this.SafeString(order , MkString("userref") ); _ = clientOrderId;
...
func (this *Kraken) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  clientOrderId := this.SafeValue2(params , MkString("userref") , MkString("clientOrderId") ); _ = clientOrderId;
  request := MkMap(&VarMap{"trades":MkBool(true) }); _ = request;
  ...
  response := this.Call(MkString("privatePostQueryOrders"), this.Extend(request , query )); _ = response;
  ...
  order := this.ParseOrder(this.Extend(MkMap(&VarMap{"id":id }), *(result ).At(id ))); _ = order;
...
func (this *Kraken) FetchOrderTrades(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 3, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 4, MkMap(&VarMap{})); _ = params;
  orderTrades := this.SafeValue(params , MkString("trades") ); _ = orderTrades;
  tradeIds := MkArray(&VarArray{}); _ = tradeIds;
  if IsTrue(OpEq2(orderTrades , MkUndefined() )) {
    ...
    for i := MkInteger(0) ; IsTrue(OpLw(i , orderTrades.Length )); OpIncr(& i ){
      orderTrade := *(orderTrades ).At(i ); _ = orderTrade;
  ...
  this.LoadMarkets();
  options := this.SafeValue(*this.At(MkString("options")) , MkString("fetchOrderTrades") , MkMap(&VarMap{})); _ = options;
  batchSize := this.SafeInteger(options , MkString("batchSize") , MkInteger(20) ); _ = batchSize;
  numTradeIds := OpCopy(tradeIds.Length ); _ = numTradeIds;
  numBatches := parseInt(OpDiv(numTradeIds , batchSize )); _ = numBatches;
  numBatches = this.Sum(numBatches , MkInteger(1) );
  result := MkArray(&VarArray{}); _ = result;
  for j := MkInteger(0) ; IsTrue(OpLw(j , numBatches )); OpIncr(& j ){
    requestIds := MkArray(&VarArray{}); _ = requestIds;
    for k := MkInteger(0) ; IsTrue(OpLw(k , batchSize )); OpIncr(& k ){
      index := this.Sum(OpMulti(j , batchSize ), k ); _ = index;
      if IsTrue(OpLw(index , numTradeIds )) {
        requestIds.Push(*(tradeIds ).At(index ));
      }
    }
    request := MkMap(&VarMap{"txid":requestIds.Join(MkString(",") )}); _ = request;
    response := this.Call(MkString("privatePostQueryTrades"), request ); _ = response;
    rawTrades := this.SafeValue(response , MkString("result") ); _ = rawTrades;
    ids := GoGetKeys(rawTrades ); _ = ids;
    for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
      *(*(rawTrades ).At(*(ids ).At(i ))).At(MkString("id") )= *(ids ).At(i );
    }
    trades := this.ParseTrades(rawTrades , MkUndefined() , since , limit ); _ = trades;
    tradesFilteredBySymbol := this.FilterBySymbol(trades , symbol ); _ = tradesFilteredBySymbol;
...
func (this *Kraken) FetchOrdersByIds(goArgs ...*Variant) *Variant{
  ...
  orderIds := GoGetKeys(result ); _ = orderIds;
  for i := MkInteger(0) ; IsTrue(OpLw(i , orderIds.Length )); OpIncr(& i ){
    id := *(orderIds ).At(i ); _ = id;
    item := *(result ).At(id ); _ = item;
    order := this.ParseOrder(this.Extend(MkMap(&VarMap{"id":id }), item )); _ = order;
    orders.Push(order );
  }
  return orders ;
}
...
func (this *Kraken) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostTradesHistory"), this.Extend(request , params )); _ = response;
  trades := *(*(response ).At(MkString("result") )).At(MkString("trades") ); _ = trades;
...
func (this *Kraken) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  orders := this.SafeValue(result , MkString("open") , MkArray(&VarArray{})); _ = orders;
...
func (this *Kraken) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostClosedOrders"), this.Extend(request , params )); _ = response;
  market := OpCopy(MkUndefined() ); _ = market;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(symbol );
  }
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  orders := this.SafeValue(result , MkString("closed") , MkArray(&VarArray{})); _ = orders;
  return this.ParseOrders(orders , market , since , limit );
}
...
func (this *Kraken) FetchDepositMethods(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostDepositMethods"), this.Extend(request , params )); _ = response;
...
func (this *Kraken) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  id := this.SafeString(transaction , MkString("refid") ); _ = id;
  txid := this.SafeString(transaction , MkString("txid") ); _ = txid;
  timestamp := this.SafeTimestamp(transaction , MkString("time") ); _ = timestamp;
  currencyId := this.SafeString(transaction , MkString("asset") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  address := this.SafeString(transaction , MkString("info") ); _ = address;
...
func (this *Kraken) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"asset":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostDepositStatus"), this.Extend(request , params )); _ = response;
...
func (this *Kraken) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"asset":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostWithdrawStatus"), this.Extend(request , params )); _ = response;
...
func (this *Kraken) CreateDepositAddress(goArgs ...*Variant) *Variant{
  code := GoGetArg(goArgs, 0, MkUndefined()); _ = code;
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"new":MkString("true") }); _ = request;
...
func (this *Kraken) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  result := *(response ).At(MkString("result") ); _ = result;
  numResults := OpCopy(result.Length ); _ = numResults;
  ...
  address := this.SafeString(*(result ).At(MkInteger(0) ), MkString("address") ); _ = address;
  tag := this.SafeString2(*(result ).At(MkInteger(0) ), MkString("tag") , MkString("memo") ); _ = tag;
  this.CheckAddress(address );
  return MkMap(&VarMap{
        "currency":code ,
        "address":address ,
...
func (this *Kraken) Withdraw(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("key") , params )) {
    ...
    result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
    id := this.SafeString(result , MkString("refid") ); _ = id;
    return MkMap(&VarMap{
          "info":result ,
          "id":id ,
          });
...
func (this *Kraken) FetchPositions(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{}); _ = request;
  response := this.Call(MkString("privatePostOpenPositions"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") ); _ = result;
  return result ;
}
...
func (this *Kraken) Sign(goArgs ...*Variant) *Variant{
  ...
  url := OpAdd(MkString("/") , OpAdd(*this.At(MkString("version")) , OpAdd(MkString("/") , OpAdd(api , OpAdd(MkString("/") , path ))))); _ = url;
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    if IsTrue(OpEq2(api , MkString("private") )) {
      ...
      auth := this.Encode(OpAdd(nonce , body )); _ = auth;
      hash := this.Hash(auth , MkString("sha256") , MkString("binary") ); _ = hash;
      binary := this.StringToBinary(this.Encode(url )); _ = binary;
      binhash := this.BinaryConcat(binary , hash ); _ = binhash;
      secret := this.Base64ToBinary(*this.At(MkString("secret")) ); _ = secret;
      signature := this.Hmac(binhash , secret , MkString("sha512") , MkString("base64") ); _ = signature;
  ...
  if IsTrue(OpEq2(*(body ).At(MkInteger(0) ), MkString("{") )) {
    if IsTrue(OpNotEq2(OpType(response ), MkString("string") )) {
      if IsTrue(OpHasMember(MkString("error") , response )) {
        numErrors := *(*(response ).At(MkString("error") )).At(MkString("length") ); _ = numErrors;
        if IsTrue(numErrors ) {
          message := OpAdd(*this.At(MkString("id")) , OpAdd(MkString(" ") , body )); _ = message;
          for i := MkInteger(0) ; IsTrue(OpLw(i , *(*(response ).At(MkString("error") )).At(MkString("length") ))); OpIncr(& i ){
            error := *(*(response ).At(MkString("error") )).At(i ); _ = error;
