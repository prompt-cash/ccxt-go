...
func (this *Lbank) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetAccuracy"), params ); _ = response;
...
func (this *Lbank) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  info := OpCopy(ticker ); _ = info;
  ticker = *(info ).At(MkString("ticker") );
  last := this.SafeNumber(ticker , MkString("latest") ); _ = last;
  ...
  if IsTrue(OpNotEq2(percentage , MkUndefined() )) {
    relativeChange := this.Sum(MkInteger(1) , OpDiv(percentage , MkInteger(100) )); _ = relativeChange;
...
func (this *Lbank) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"symbol":MkString("all") }); _ = request;
  response := this.Call(MkString("publicGetTicker"), this.Extend(request , params )); _ = response;
...
func (this *Lbank) FetchOrderBook(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  limit := GoGetArg(goArgs, 1, MkInteger(60) ); _ = limit;
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  size := MkInteger(60) ; _ = size;
...
func (this *Lbank) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(trade , MkString("date_ms") ); _ = timestamp;
...
func (this *Lbank) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  free := this.SafeValue(info , MkString("free") , MkMap(&VarMap{})); _ = free;
  freeze := this.SafeValue(info , MkString("freeze") , MkMap(&VarMap{})); _ = freeze;
  asset := this.SafeValue(info , MkString("asset") , MkMap(&VarMap{})); _ = asset;
...
func (this *Lbank) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(order , MkString("create_time") ); _ = timestamp;
  price := this.SafeNumber(order , MkString("price") ); _ = price;
  amount := this.SafeNumber(order , MkString("amount") , MkNumber(0.0) ); _ = amount;
  filled := this.SafeNumber(order , MkString("deal_amount") , MkNumber(0.0) ); _ = filled;
...
func (this *Lbank) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostCreateOrder"), this.Extend(order , params )); _ = response;
...
func (this *Lbank) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrdersInfo"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("orders") , MkArray(&VarArray{})); _ = data;
  orders := this.ParseOrders(data , market ); _ = orders;
  numOrders := OpCopy(orders.Length ); _ = numOrders;
...
func (this *Lbank) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrdersInfoHistory"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("orders") , MkArray(&VarArray{})); _ = data;
...
func (this *Lbank) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  closed := this.FilterBy(orders , MkString("status") , MkString("closed") ); _ = closed;
  canceled := this.FilterBy(orders , MkString("status") , MkString("cancelled") ); _ = canceled;
  allOrders := this.ArrayConcat(closed , canceled ); _ = allOrders;
...
func (this *Lbank) ConvertSecretToPem(goArgs ...*Variant) *Variant{
  secret := GoGetArg(goArgs, 0, MkUndefined()); _ = secret;
  lineLength := MkInteger(64) ; _ = lineLength;
  secretLength := OpSub(secret.Length , MkInteger(0) ); _ = secretLength;
  numLines := parseInt(OpDiv(secretLength , lineLength )); _ = numLines;
  numLines = this.Sum(numLines , MkInteger(1) );
  pem := MkString("-----BEGIN PRIVATE KEY-----\n") ; _ = pem;
  for i := MkInteger(0) ; IsTrue(OpLw(i , numLines )); OpIncr(& i ){
    start := OpMulti(i , lineLength ); _ = start;
    end := this.Sum(start , lineLength ); _ = end;
...
func (this *Lbank) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    query := this.Keysort(this.Extend(MkMap(&VarMap{"api_key":*this.At(MkString("apiKey")) }), params )); _ = query;
    queryString := this.Rawencode(query ); _ = queryString;
    message := (this.Hash(this.Encode(queryString ))).Call(MkString("toUpperCase") ); _ = message;
    cacheSecretAsPem := this.SafeValue(*this.At(MkString("options")) , MkString("cacheSecretAsPem") , MkBool(true) ); _ = cacheSecretAsPem;
    pem := OpCopy(MkUndefined() ); _ = pem;
    ...
    sign := this.BinaryToBase64(this.Rsa(message , this.Encode(pem ), MkString("RS256") )); _ = sign;
...
func (this *Lbank) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  success := this.SafeString(response , MkString("result") ); _ = success;
  if IsTrue(OpEq2(success , MkString("false") )) {
    errorCode := this.SafeString(response , MkString("error_code") ); _ = errorCode;
    message := this.SafeString(MkMap(&VarMap{
              ...
              }), errorCode , this.Json(response )); _ = message;
    ErrorClass := this.SafeValue(MkMap(&VarMap{
              ...
              }), errorCode , ExchangeError ); _ = ErrorClass;
