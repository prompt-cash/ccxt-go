...
func (this *Huobi) FetchTradingLimitsById(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"symbol":id }); _ = request;
  response := this.Call(MkString("publicGetCommonExchange"), this.Extend(request , params )); _ = response;
...
func (this *Huobi) ParseTradingLimits(goArgs ...*Variant) *Variant{
  limits := GoGetArg(goArgs, 0, MkUndefined()); _ = limits;
...
func (this *Huobi) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  method := *(*this.At(MkString("options")) ).At(MkString("fetchMarketsMethod") ); _ = method;
  response := this.Call(method , params ); _ = response;
  markets := this.SafeValue(response , MkString("data") ); _ = markets;
  numMarkets := OpCopy(markets.Length ); _ = numMarkets;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    ...
    baseId := this.SafeString(market , MkString("base-currency") ); _ = baseId;
    quoteId := this.SafeString(market , MkString("quote-currency") ); _ = quoteId;
    ...
    maker := OpTrinary(OpEq2(base , MkString("OMG") ), MkInteger(0) , OpDiv(MkNumber(0.2) , MkInteger(100) )); _ = maker;
    taker := OpTrinary(OpEq2(base , MkString("OMG") ), MkInteger(0) , OpDiv(MkNumber(0.2) , MkInteger(100) )); _ = taker;
    minAmount := this.SafeNumber(market , MkString("min-order-amt") , Math.Pow(MkInteger(10) , OpNeg(*(precision ).At(MkString("amount") )))); _ = minAmount;
    maxAmount := this.SafeNumber(market , MkString("max-order-amt") ); _ = maxAmount;
    minCost := this.SafeNumber(market , MkString("min-order-value") , MkInteger(0) ); _ = minCost;
    state := this.SafeString(market , MkString("state") ); _ = state;
    active := OpEq2(state , MkString("online") ); _ = active;
...
func (this *Huobi) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  bid := OpCopy(MkUndefined() ); _ = bid;
  bidVolume := OpCopy(MkUndefined() ); _ = bidVolume;
  ask := OpCopy(MkUndefined() ); _ = ask;
  askVolume := OpCopy(MkUndefined() ); _ = askVolume;
...
func (this *Huobi) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("tick") , response )) {
    ...
    tick := this.SafeValue(response , MkString("tick") ); _ = tick;
    timestamp := this.SafeInteger(tick , MkString("ts") , this.SafeInteger(response , MkString("ts") )); _ = timestamp;
    result := this.ParseOrderBook(tick , symbol , timestamp ); _ = result;
...
func (this *Huobi) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("marketGetDetailMerged"), this.Extend(request , params )); _ = response;
  ticker := this.ParseTicker(*(response ).At(MkString("tick") ), market ); _ = ticker;
  timestamp := this.SafeInteger(response , MkString("ts") ); _ = timestamp;
...
func (this *Huobi) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("marketGetTickers"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , tickers.Length )); OpIncr(& i ){
    ...
    ticker := this.ParseTicker(*(tickers ).At(i ), market ); _ = ticker;
    *(ticker ).At(MkString("timestamp") )= OpCopy(timestamp );
    *(ticker ).At(MkString("datetime") )= this.Iso8601(timestamp );
...
func (this *Huobi) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger2(trade , MkString("ts") , MkString("created-at") ); _ = timestamp;
  order := this.SafeString(trade , MkString("order-id") ); _ = order;
  ...
  if IsTrue(OpNotEq2(type_ , MkUndefined() )) {
    typeParts := type_.Split(MkString("-") ); _ = typeParts;
    side = *(typeParts ).At(MkInteger(0) );
    type_ = *(typeParts ).At(MkInteger(1) );
  }
  takerOrMaker := this.SafeString(trade , MkString("role") ); _ = takerOrMaker;
  priceString := this.SafeString(trade , MkString("price") ); _ = priceString;
  amountString := this.SafeString2(trade , MkString("filled-amount") , MkString("amount") ); _ = amountString;
  ...
  feeCost := this.SafeNumber(trade , MkString("filled-fees") ); _ = feeCost;
  feeCurrency := this.SafeCurrencyCode(this.SafeString(trade , MkString("fee-currency") )); _ = feeCurrency;
  filledPoints := this.SafeNumber(trade , MkString("filled-points") ); _ = filledPoints;
  ...
  tradeId := this.SafeString2(trade , MkString("trade-id") , MkString("tradeId") ); _ = tradeId;
  id := this.SafeString(trade , MkString("id") , tradeId ); _ = id;
  return MkMap(&VarMap{
        "id":id ,
        "info":trade ,
        "order":order ,
...
func (this *Huobi) FetchOrderTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrderOrdersIdMatchresults"), this.Extend(request , params )); _ = response;
...
func (this *Huobi) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrderMatchresults"), this.Extend(request , params )); _ = response;
...
func (this *Huobi) FetchTrades(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  since := GoGetArg(goArgs, 1, MkUndefined() ); _ = since;
  limit := GoGetArg(goArgs, 2, MkInteger(1000) ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  ...
  response := this.Call(MkString("marketGetHistoryTrade"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    trades := this.SafeValue(*(data ).At(i ), MkString("data") , MkArray(&VarArray{})); _ = trades;
    for j := MkInteger(0) ; IsTrue(OpLw(j , trades.Length )); OpIncr(& j ){
      trade := this.ParseTrade(*(trades ).At(j ), market ); _ = trade;
      result.Push(trade );
    }
  }
...
func (this *Huobi) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  since := GoGetArg(goArgs, 2, MkUndefined() ); _ = since;
  limit := GoGetArg(goArgs, 3, MkInteger(1000) ); _ = limit;
  params := GoGetArg(goArgs, 4, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{
  ...
  response := this.Call(MkString("marketGetHistoryKline"), this.Extend(request , params )); _ = response;
...
func (this *Huobi) FetchAccounts(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetAccountAccounts"), params ); _ = response;
  return *(response ).At(MkString("data") );
}

func (this *Huobi) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"language":*(*this.At(MkString("options")) ).At(MkString("language") )}); _ = request;
  response := this.Call(MkString("publicGetSettingsCurrencys"), this.Extend(request , params )); _ = response;
  currencies := this.SafeValue(response , MkString("data") ); _ = currencies;
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    currency := *(currencies ).At(i ); _ = currency;
    id := this.SafeValue(currency , MkString("name") ); _ = id;
    precision := this.SafeInteger(currency , MkString("withdraw-precision") ); _ = precision;
    code := this.SafeCurrencyCode(id ); _ = code;
    active := OpAnd(*(currency ).At(MkString("visible") ), OpAnd(*(currency ).At(MkString("deposit-enabled") ), *(currency ).At(MkString("withdraw-enabled") ))); _ = active;
    name := this.SafeString(currency , MkString("display-name") ); _ = name;
...
func (this *Huobi) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  this.LoadAccounts();
  method := *(*this.At(MkString("options")) ).At(MkString("fetchBalanceMethod") ); _ = method;
  request := MkMap(&VarMap{"id":*(*(*this.At(MkString("accounts")) ).At(MkInteger(0) )).At(MkString("id") )}); _ = request;
  response := this.Call(method , this.Extend(request , params )); _ = response;
  balances := this.SafeValue(*(response ).At(MkString("data") ), MkString("list") , MkArray(&VarArray{})); _ = balances;
...
func (this *Huobi) FetchOrdersByStates(goArgs ...*Variant) *Variant{
  states := GoGetArg(goArgs, 0, MkUndefined()); _ = states;
  symbol := GoGetArg(goArgs, 1, MkUndefined() ); _ = symbol;
  since := GoGetArg(goArgs, 2, MkUndefined() ); _ = since;
  limit := GoGetArg(goArgs, 3, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 4, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"states":states }); _ = request;
  market := OpCopy(MkUndefined() ); _ = market;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(symbol );
    *(request ).At(MkString("symbol") )= *(market ).At(MkString("id") );
  }
  method := this.SafeString(*this.At(MkString("options")) , MkString("fetchOrdersByStatesMethod") , MkString("private_get_order_orders") ); _ = method;
  response := this.Call(method , this.Extend(request , params )); _ = response;
...
func (this *Huobi) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"id":id }); _ = request;
  response := this.Call(MkString("privateGetOrderOrdersId"), this.Extend(request , params )); _ = response;
...
func (this *Huobi) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  method := this.SafeString(*this.At(MkString("options")) , MkString("fetchOpenOrdersMethod") , MkString("fetch_open_orders_v1") ); _ = method;
...
func (this *Huobi) FetchOpenOrdersV2(goArgs ...*Variant) *Variant{
  ...
  accountId := this.SafeString(params , MkString("account-id") ); _ = accountId;
  ...
  *(request ).At(MkString("account-id") )= OpCopy(accountId );
  ...
  omitted := this.Omit(params , MkString("account-id") ); _ = omitted;
  response := this.Call(MkString("privateGetOrderOpenOrders"), this.Extend(request , omitted )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Huobi) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("type") , order )) {
    orderType := (*(order ).At(MkString("type") )).Call(MkString("split") , MkString("-") ); _ = orderType;
  ...
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  timestamp := this.SafeInteger(order , MkString("created-at") ); _ = timestamp;
  clientOrderId := this.SafeString(order , MkString("client-order-id") ); _ = clientOrderId;
  amount := this.SafeNumber(order , MkString("amount") ); _ = amount;
  filled := this.SafeNumber2(order , MkString("filled-amount") , MkString("field-amount") ); _ = filled;
  price := this.SafeNumber(order , MkString("price") ); _ = price;
  if IsTrue(OpEq2(price , MkNumber(0.0) )) {
    price = OpCopy(MkUndefined() );
  }
  cost := this.SafeNumber2(order , MkString("filled-cash-amount") , MkString("field-cash-amount") ); _ = cost;
  feeCost := this.SafeNumber2(order , MkString("filled-fees") , MkString("field-fees") ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
...
func (this *Huobi) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  clientOrderId := this.SafeString2(params , MkString("clientOrderId") , MkString("client-order-id") ); _ = clientOrderId;
  if IsTrue(OpEq2(clientOrderId , MkUndefined() )) {
    broker := this.SafeValue(*this.At(MkString("options")) , MkString("broker") , MkMap(&VarMap{})); _ = broker;
    brokerId := this.SafeString(broker , MkString("id") ); _ = brokerId;
  ...
  method := *(*this.At(MkString("options")) ).At(MkString("createOrderMethod") ); _ = method;
  response := this.Call(method , this.Extend(request , params )); _ = response;
...
func (this *Huobi) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  symbol := GoGetArg(goArgs, 1, MkUndefined() ); _ = symbol;
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("privatePostOrderOrdersIdSubmitcancel"), MkMap(&VarMap{"id":id })); _ = response;
...
func (this *Huobi) CancelOrders(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  clientOrderIds := this.SafeValue2(params , MkString("clientOrderIds") , MkString("client-order-ids") ); _ = clientOrderIds;
  params = this.Omit(params , MkArray(&VarArray{
  ...
  response := this.Call(MkString("privatePostOrderOrdersBatchcancel"), this.Extend(request , params )); _ = response;
  return response ;
}
...
func (this *Huobi) CancelAllOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrderOrdersBatchCancelOpenOrders"), this.Extend(request , params )); _ = response;
  return response ;
}
...
func (this *Huobi) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v2PrivateGetAccountDepositAddress"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Huobi) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetQueryDepositWithdraw"), this.Extend(request , params )); _ = response;
...
func (this *Huobi) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  timestamp := this.SafeInteger(transaction , MkString("created-at") ); _ = timestamp;
  updated := this.SafeInteger(transaction , MkString("updated-at") ); _ = updated;
  code := this.SafeCurrencyCode(this.SafeString(transaction , MkString("currency") )); _ = code;
  ...
  tag := this.SafeString(transaction , MkString("address-tag") ); _ = tag;
...
func (this *Huobi) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostDwWithdrawApiCreate"), this.Extend(request , params )); _ = response;
  id := this.SafeString(response , MkString("data") ); _ = id;
  return MkMap(&VarMap{
        "info":response ,
        "id":id ,
...
func (this *Huobi) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpOr(OpEq2(api , MkString("private") ), OpEq2(api , MkString("v2Private") ))) {
    ...
    timestamp := this.Ymdhms(this.Milliseconds(), MkString("T") ); _ = timestamp;
    request := MkMap(&VarMap{
    ...
    auth := this.Urlencode(request ); _ = auth;
...
func (this *Huobi) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("status") , response )) {
    ...
    if IsTrue(OpEq2(status , MkString("error") )) {
      code := this.SafeString(response , MkString("err-code") ); _ = code;
      ...
      message := this.SafeString(response , MkString("err-msg") ); _ = message;
