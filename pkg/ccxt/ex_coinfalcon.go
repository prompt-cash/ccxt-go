...
func (this *Coinfalcon) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	tickers := this.FetchTickers(params)
	_ = tickers
...
func (this *Coinfalcon) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetMarketsMarketOrders"), this.Extend(request, params))
	_ = response
...
func (this *Coinfalcon) ParseTrade(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCurrencyCode := this.SafeString(trade, MkString("fee_currency_code"))
		_ = feeCurrencyCode
		fee = MkMap(&VarMap{
			"cost":     feeCost,
...
func (this *Coinfalcon) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetUserAccounts"), params)
	_ = response
...
func (this *Coinfalcon) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = amount
	filled := this.SafeNumber(order, MkString("size_filled"))
	_ = filled
	status := this.ParseOrderStatus(this.SafeString(order, MkString("status")))
	_ = status
	type_ := this.SafeString(order, MkString("operation_type"))
	_ = type_
...
func (this *Coinfalcon) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateDeleteUserOrdersId"), this.Extend(request, params))
	_ = response
	market := this.Market(symbol)
	_ = market
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
...
func (this *Coinfalcon) FetchOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"id": id})
	_ = request
	response := this.Call(MkString("privateGetUserOrdersId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
...
func (this *Coinfalcon) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetUserOrders"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
...
func (this *Coinfalcon) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		seconds := (this.Seconds()).Call(MkString("toString"))
		_ = seconds
		...
		signature := this.Hmac(this.Encode(payload), this.Encode(*this.At(MkString("secret"))))
		_ = signature
