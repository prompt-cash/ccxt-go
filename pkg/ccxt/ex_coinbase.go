...
func (this *Coinbase) FetchAccounts(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"limit": MkInteger(100)})
	_ = request
	...
	for i := MkInteger(0); IsTrue(OpLw(i, data.Length)); OpIncr(&i) {
		account := *(data).At(i)
		_ = account
		currency := this.SafeValue(account, MkString("currency"), MkMap(&VarMap{}))
		_ = currency
		currencyId := this.SafeString(currency, MkString("code"))
		_ = currencyId
...
func (this *Coinbase) CreateDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = params
	accountId := this.SafeString(params, MkString("account_id"))
	_ = accountId
	...
	request := MkMap(&VarMap{"account_id": accountId})
	_ = request
	response := this.Call(MkString("privatePostAccountsAccountIdAddresses"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	tag := this.SafeString(data, MkString("destination_tag"))
	_ = tag
...
func (this *Coinbase) FetchMySells(goArgs ...*Variant) *Variant {
	...
	_ = params
	request := this.PrepareAccountRequest(limit, params)
	_ = request
	this.LoadMarkets()
	...
	_ = query
	sells := this.Call(MkString("privateGetAccountsAccountIdSells"), this.Extend(request, query))
	_ = sells
...
func (this *Coinbase) FetchMyBuys(goArgs ...*Variant) *Variant {
	...
	_ = query
	buys := this.Call(MkString("privateGetAccountsAccountIdBuys"), this.Extend(request, query))
	_ = buys
...
func (this *Coinbase) FetchTransactionsWithMethod(goArgs ...*Variant) *Variant {
	...
	_ = params
	request := this.PrepareAccountRequestWithCurrencyCode(code, limit, params)
	_ = request
	this.LoadMarkets()
	query := this.Omit(params, MkArray(&VarArray{
		MkString("account_id"),
...
func (this *Coinbase) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = market
	subtotalObject := this.SafeValue(transaction, MkString("subtotal"), MkMap(&VarMap{}))
	_ = subtotalObject
	feeObject := this.SafeValue(transaction, MkString("fee"), MkMap(&VarMap{}))
	_ = feeObject
	id := this.SafeString(transaction, MkString("id"))
	_ = id
	timestamp := this.Parse8601(this.SafeValue(transaction, MkString("created_at")))
	_ = timestamp
	updated := this.Parse8601(this.SafeValue(transaction, MkString("updated_at")))
	_ = updated
	type_ := this.SafeString(transaction, MkString("resource"))
	_ = type_
	amount := this.SafeNumber(subtotalObject, MkString("amount"))
	_ = amount
	currencyId := this.SafeString(subtotalObject, MkString("currency"))
	_ = currencyId
	currency := this.SafeCurrencyCode(currencyId)
	_ = currency
	feeCost := this.SafeNumber(feeObject, MkString("amount"))
	_ = feeCost
	feeCurrencyId := this.SafeString(feeObject, MkString("currency"))
	_ = feeCurrencyId
	...
	if IsTrue(OpEq2(status, MkUndefined())) {
		committed := this.SafeValue(transaction, MkString("committed"))
		_ = committed
...
func (this *Coinbase) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = symbol
	totalObject := this.SafeValue(trade, MkString("total"), MkMap(&VarMap{}))
	_ = totalObject
	amountObject := this.SafeValue(trade, MkString("amount"), MkMap(&VarMap{}))
	_ = amountObject
	subtotalObject := this.SafeValue(trade, MkString("subtotal"), MkMap(&VarMap{}))
	_ = subtotalObject
	feeObject := this.SafeValue(trade, MkString("fee"), MkMap(&VarMap{}))
	_ = feeObject
	id := this.SafeString(trade, MkString("id"))
	_ = id
	timestamp := this.Parse8601(this.SafeValue(trade, MkString("created_at")))
	_ = timestamp
	if IsTrue(OpEq2(market, MkUndefined())) {
		baseId := this.SafeString(amountObject, MkString("currency"))
		_ = baseId
		quoteId := this.SafeString(totalObject, MkString("currency"))
		_ = quoteId
	...
	_ = orderId
	side := this.SafeString(trade, MkString("resource"))
	_ = side
	type_ := OpCopy(MkUndefined())
	_ = type_
	costString := this.SafeString(subtotalObject, MkString("amount"))
	_ = costString
	amountString := this.SafeString(amountObject, MkString("amount"))
	_ = amountString
...
func (this *Coinbase) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = response
	currencies := this.SafeValue(response, MkString("currencies"), MkMap(&VarMap{}))
	_ = currencies
	exchangeRates := this.SafeValue(response, MkString("exchangeRates"), MkMap(&VarMap{}))
	_ = exchangeRates
	data := this.SafeValue(currencies, MkString("data"), MkArray(&VarArray{}))
	_ = data
	dataById := this.IndexBy(data, MkString("id"))
	_ = dataById
	rates := this.SafeValue(this.SafeValue(exchangeRates, MkString("data"), MkMap(&VarMap{})), MkString("rates"), MkMap(&VarMap{}))
	_ = rates
	baseIds := GoGetKeys(rates)
	_ = baseIds
	result := MkArray(&VarArray{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, baseIds.Length)); OpIncr(&i) {
		...
		base := this.SafeCurrencyCode(baseId)
		_ = base
		type_ := OpTrinary(OpHasMember(baseId, dataById), MkString("fiat"), MkString("crypto"))
		_ = type_
		if IsTrue(OpEq2(type_, MkString("crypto"))) {
			for j := MkInteger(0); IsTrue(OpLw(j, data.Length)); OpIncr(&j) {
				quoteCurrency := *(data).At(j)
				_ = quoteCurrency
				quoteId := this.SafeString(quoteCurrency, MkString("id"))
				_ = quoteId
				quote := this.SafeCurrencyCode(quoteId)
				_ = quote
				symbol := OpAdd(base, OpAdd(MkString("/"), quote))
...
func (this *Coinbase) FetchCurrenciesFromCache(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpOr(OpEq2(timestamp, MkUndefined()), OpGt(OpSub(now, timestamp), expires))) {
		currencies := this.Call(MkString("publicGetCurrencies"), params)
		_ = currencies
		exchangeRates := this.Call(MkString("publicGetExchangeRates"), params)
		_ = exchangeRates
...
func (this *Coinbase) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, keys.Length)); OpIncr(&i) {
		key := *(keys).At(i)
		_ = key
		type_ := OpTrinary(OpHasMember(key, dataById), MkString("fiat"), MkString("crypto"))
		_ = type_
		currency := this.SafeValue(dataById, key, MkMap(&VarMap{}))
		_ = currency
		id := this.SafeString(currency, MkString("id"), key)
		_ = id
...
func (this *Coinbase) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := this.Extend(MkMap(&VarMap{"symbol": *(market).At(MkString("id"))}), params)
	_ = request
	buy := this.Call(MkString("publicGetPricesSymbolBuy"), request)
	_ = buy
	sell := this.Call(MkString("publicGetPricesSymbolSell"), request)
	_ = sell
	spot := this.Call(MkString("publicGetPricesSymbolSpot"), request)
	_ = spot
	ask := this.SafeNumber(*(buy).At(MkString("data")), MkString("amount"))
	_ = ask
	bid := this.SafeNumber(*(sell).At(MkString("data")), MkString("amount"))
	_ = bid
	last := this.SafeNumber(*(spot).At(MkString("data")), MkString("amount"))
	_ = last
	return MkMap(&VarMap{
		"symbol":        symbol,
		"timestamp":     timestamp,
...
func (this *Coinbase) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = balances
	accounts := this.SafeValue(params, MkString("type"), *(*this.At(MkString("options"))).At(MkString("accounts")))
	_ = accounts
	result := MkMap(&VarMap{"info": response})
	_ = result
	for b := MkInteger(0); IsTrue(OpLw(b, balances.Length)); OpIncr(&b) {
		balance := *(balances).At(b)
		_ = balance
		type_ := this.SafeString(balance, MkString("type"))
		_ = type_
		if IsTrue(this.InArray(type_, accounts)) {
			value := this.SafeValue(balance, MkString("balance"))
			_ = value
			if IsTrue(OpNotEq2(value, MkUndefined())) {
				currencyId := this.SafeString(value, MkString("currency"))
				_ = currencyId
				code := this.SafeCurrencyCode(currencyId)
				_ = code
				total := this.SafeString(value, MkString("amount"))
				_ = total
				free := OpCopy(total)
				_ = free
				account := this.SafeValue(result, code)
				_ = account
...
func (this *Coinbase) FetchLedger(goArgs ...*Variant) *Variant {
	...
	_ = query
	response := this.Call(MkString("privateGetAccountsAccountIdTransactions"), this.Extend(request, query))
	_ = response
...
func (this *Coinbase) ParseLedgerEntry(goArgs ...*Variant) *Variant {
	...
	_ = currency
	amountInfo := this.SafeValue(item, MkString("amount"), MkMap(&VarMap{}))
	_ = amountInfo
	amount := this.SafeNumber(amountInfo, MkString("amount"))
	_ = amount
	...
	currencyId := this.SafeString(amountInfo, MkString("currency"))
	_ = currencyId
	code := this.SafeCurrencyCode(currencyId, currency)
	_ = code
	fee := OpCopy(MkUndefined())
	_ = fee
	networkInfo := this.SafeValue(item, MkString("network"), MkMap(&VarMap{}))
	_ = networkInfo
	feeInfo := this.SafeValue(networkInfo, MkString("transaction_fee"))
	_ = feeInfo
	if IsTrue(OpNotEq2(feeInfo, MkUndefined())) {
		feeCurrencyId := this.SafeString(feeInfo, MkString("currency"))
		_ = feeCurrencyId
		feeCurrencyCode := this.SafeCurrencyCode(feeCurrencyId, currency)
		_ = feeCurrencyCode
		feeAmount := this.SafeNumber(feeInfo, MkString("amount"))
		_ = feeAmount
		fee = MkMap(&VarMap{
			"cost":     feeAmount,
	...
	timestamp := this.Parse8601(this.SafeValue(item, MkString("created_at")))
	_ = timestamp
	id := this.SafeString(item, MkString("id"))
	_ = id
	type_ := this.ParseLedgerEntryType(this.SafeString(item, MkString("type")))
	_ = type_
	status := this.ParseLedgerEntryStatus(this.SafeString(item, MkString("status")))
	_ = status
	path := this.SafeString(item, MkString("resource_path"))
	_ = path
	accountId := OpCopy(MkUndefined())
	_ = accountId
	if IsTrue(OpNotEq2(path, MkUndefined())) {
		parts := path.Split(MkString("/"))
		_ = parts
...
func (this *Coinbase) PrepareAccountRequest(goArgs ...*Variant) *Variant {
	limit := GoGetArg(goArgs, 0, MkUndefined())
	_ = limit
	params := GoGetArg(goArgs, 1, MkMap(&VarMap{}))
	_ = params
	accountId := this.SafeString2(params, MkString("account_id"), MkString("accountId"))
	_ = accountId
	if IsTrue(OpEq2(accountId, MkUndefined())) {
...
func (this *Coinbase) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	fullPath := OpAdd(MkString("/"), OpAdd(*this.At(MkString("version")), OpAdd(MkString("/"), this.ImplodeParams(path, params))))
	_ = fullPath
	...
	url := OpAdd(*(*this.At(MkString("urls"))).At(MkString("api")), fullPath)
	_ = url
	if IsTrue(OpEq2(api, MkString("private"))) {
		authorization := this.SafeString(*this.At(MkString("headers")), MkString("Authorization"))
		_ = authorization
		if IsTrue(OpNotEq2(authorization, MkUndefined())) {
			...
			if IsTrue(*this.At(MkString("token"))) {
				...
				auth := OpAdd(nonce, OpAdd(method, OpAdd(fullPath, payload)))
				_ = auth
...
func (this *Coinbase) HandleErrors(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpNotEq2(errorCode, MkUndefined())) {
		errorMessage := this.SafeString(response, MkString("error_description"))
		_ = errorMessage
	...
	if IsTrue(OpNotEq2(errors, MkUndefined())) {
		if IsTrue(GoIsArray(errors)) {
			...
			if IsTrue(OpGt(numErrors, MkInteger(0))) {
				...
				errorMessage := this.SafeString(*(errors).At(MkInteger(0)), MkString("message"))
				_ = errorMessage
				if IsTrue(OpNotEq2(errorCode, MkUndefined())) {
					this.ThrowExactlyMatchedException(*(*this.At(MkString("exceptions"))).At(MkString("exact")), errorCode, feedback)
					this.ThrowBroadlyMatchedException(*(*this.At(MkString("exceptions"))).At(MkString("broad")), errorMessage, feedback)
