...
func (this *Hbtc) ParseMarket(goArgs ...*Variant) *Variant{
  ...
  type_ := GoGetArg(goArgs, 1, MkString("spot") ); _ = type_;
  ...
  margin := this.SafeValue(market , MkString("allowMargin") , MkUndefined() ); _ = margin;
  isAggregate := this.SafeValue(market , MkString("isAggregate") , MkUndefined() ); _ = isAggregate;
  active := OpCopy(MkBool(true) ); _ = active;
  if IsTrue(OpEq2(isAggregate , MkBool(true) )) {
    active = OpCopy(MkBool(false) );
  }
  amountMin := OpCopy(MkUndefined() ); _ = amountMin;
  priceMin := OpCopy(MkUndefined() ); _ = priceMin;
  priceMax := OpCopy(MkUndefined() ); _ = priceMax;
  costMin := OpCopy(MkUndefined() ); _ = costMin;
  pricePrecision := OpCopy(MkUndefined() ); _ = pricePrecision;
  amountPrecision := OpCopy(MkUndefined() ); _ = amountPrecision;
  for j := MkInteger(0) ; IsTrue(OpLw(j , filters.Length )); OpIncr(& j ){
    filter := *(filters ).At(j ); _ = filter;
    filterType := this.SafeString(filter , MkString("filterType") ); _ = filterType;
...
func (this *Hbtc) FetchMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetBrokerInfo"), params ); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  symbols := this.SafeValue(response , MkString("symbols") , MkArray(&VarArray{})); _ = symbols;
  for i := MkInteger(0) ; IsTrue(OpLw(i , symbols.Length )); OpIncr(& i ){
    market := this.ParseMarket(*(symbols ).At(i ), MkString("spot") ); _ = market;
    result.Push(market );
  }
  options := this.SafeValue(response , MkString("options") , MkArray(&VarArray{})); _ = options;
  for i := MkInteger(0) ; IsTrue(OpLw(i , options.Length )); OpIncr(& i ){
    market := this.ParseMarket(*(options ).At(i ), MkString("option") ); _ = market;
    result.Push(market );
  }
  contracts := this.SafeValue(response , MkString("contracts") , MkArray(&VarArray{})); _ = contracts;
  for i := MkInteger(0) ; IsTrue(OpLw(i , contracts.Length )); OpIncr(& i ){
    market := this.ParseMarket(*(contracts ).At(i ), MkString("future") ); _ = market;
    result.Push(market );
  }
  return result ;
}
...
func (this *Hbtc) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("quoteGetDepth"), this.Extend(request , params )); _ = response;
  timestamp := this.SafeInteger(response , MkString("time") ); _ = timestamp;
...
func (this *Hbtc) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("quoteGetTicker24hr"), this.Extend(request , params )); _ = response;
...
func (this *Hbtc) FetchBidAsk(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("quoteGetTickerBookTicker"), this.Extend(request , params )); _ = response;
  return this.ParseTicker(response , market );
}
...
func (this *Hbtc) FetchBidsAsks(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("quoteGetTickerBookTicker"), params ); _ = response;
...
func (this *Hbtc) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  defaultMethod := this.SafeString(options , MkString("method") , MkString("quoteGetTicker24hr") ); _ = defaultMethod;
  defaultType := this.SafeString(options , MkString("type") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  query := this.Omit(params , MkString("type") ); _ = query;
  method := OpCopy(defaultMethod ); _ = method;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
...
func (this *Hbtc) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(balances , MkUndefined() )) {
    for i := MkInteger(0) ; IsTrue(OpLw(i , balances.Length )); OpIncr(& i ){
      ...
      currencyId := this.SafeString2(balance , MkString("asset") , MkString("tokenName") ); _ = currencyId;
    ...
    for i := MkInteger(0) ; IsTrue(OpLw(i , currencyIds.Length )); OpIncr(& i ){
      ...
      code := this.SafeCurrencyCode(currencyId ); _ = code;
      balance := *(response ).At(currencyId ); _ = balance;
      account := this.Account(); _ = account;
...
func (this *Hbtc) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("quoteGetTrades"), this.Extend(request , params )); _ = response;
...
func (this *Hbtc) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("quoteGetKlines"), this.Extend(request , params )); _ = response;
...
func (this *Hbtc) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  defaultType := this.SafeString(*this.At(MkString("options")) , MkString("type") , MkString("spot") ); _ = defaultType;
  options := this.SafeValue(*this.At(MkString("options")) , MkString("fetchMyTrades") , MkMap(&VarMap{})); _ = options;
  fetchMyTradesType := this.SafeString(options , MkString("type") , defaultType ); _ = fetchMyTradesType;
  type_ := this.SafeString(params , MkString("type") , fetchMyTradesType ); _ = type_;
  ...
  method := MkString("privateGetMyTrades") ; _ = method;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
...
func (this *Hbtc) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  orderSide := side.ToUpperCase(); _ = orderSide;
  orderType := type_.ToUpperCase(); _ = orderType;
  request := MkMap(&VarMap{
        "symbol":*(market ).At(MkString("id") ),
        "side":orderSide ,
        }); _ = request;
  ...
  if IsTrue(OpEq2(*(market ).At(MkString("type") ), MkString("future") )) {
    ...
    leverage := this.SafeValue(params , MkString("leverage") ); _ = leverage;
    ...
    priceType := this.SafeString(params , MkString("priceType") ); _ = priceType;
    ...
    *(request ).At(MkString("clientOrderId") )= OpCopy(clientOrderId );
    ...
    newClientOrderId := this.SafeValue2(params , MkString("clientOrderId") , MkString("newClientOrderId") ); _ = newClientOrderId;
...
func (this *Hbtc) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  cancelOrderType := this.SafeString(options , MkString("type") , defaultType ); _ = cancelOrderType;
  type_ := this.SafeString(params , MkString("type") , cancelOrderType ); _ = type_;
  query := this.Omit(params , MkString("type") ); _ = query;
  ...
  orderType := this.SafeString(query , MkString("orderType") ); _ = orderType;
...
func (this *Hbtc) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  fetchOpenOrdersType := this.SafeString(options , MkString("type") , defaultType ); _ = fetchOpenOrdersType;
  type_ := this.SafeString(params , MkString("type") , fetchOpenOrdersType ); _ = type_;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(symbol );
    *(request ).At(MkString("symbol") )= *(market ).At(MkString("id") );
    type_ = *(market ).At(MkString("type") );
...
func (this *Hbtc) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  fetchClosedOrdersType := this.SafeString(options , MkString("type") , defaultType ); _ = fetchClosedOrdersType;
  type_ := this.SafeString(params , MkString("type") , fetchClosedOrdersType ); _ = type_;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(symbol );
    *(request ).At(MkString("symbol") )= *(market ).At(MkString("id") );
    type_ = *(market ).At(MkString("type") );
  ...
  method := MkString("privateGetHistoryOrders") ; _ = method;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
...
func (this *Hbtc) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  fetchOrderType := this.SafeString(options , MkString("type") , defaultType ); _ = fetchOrderType;
  type_ := this.SafeString(params , MkString("type") , fetchOrderType ); _ = type_;
  query := this.Omit(params , MkString("type") ); _ = query;
  if IsTrue(OpNotEq2(clientOrderId , MkUndefined() )) {
    *(request ).At(MkString("origClientOrderId") )= OpCopy(clientOrderId );
    query = this.Omit(query , MkArray(&VarArray{
...
func (this *Hbtc) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetDepositOrders"), this.Extend(request , params )); _ = response;
...
func (this *Hbtc) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetWithdrawalOrders"), this.Extend(request , params )); _ = response;
  return this.ParseTransactions(response , currency , since , limit );
}
...
func (this *Hbtc) Withdraw(goArgs ...*Variant) *Variant{
  ...
  clientOrderId := this.SafeString(params , MkString("clientOrderId") , this.Uuid()); _ = clientOrderId;
  request := MkMap(&VarMap{
...
func (this *Hbtc) FetchAccounts(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("privatePostSubAccountQuery"), params ); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    account := *(response ).At(i ); _ = account;
    accountId := this.SafeString(account , MkString("accountId") ); _ = accountId;
    accountType := this.SafeString(account , MkString("accountType") ); _ = accountType;
    type_ := OpCopy(accountType ); _ = type_;
...
func (this *Hbtc) FetchLedger(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetBalanceFlow"), this.Extend(request , params )); _ = response;
...
func (this *Hbtc) ParseLedgerEntry(goArgs ...*Variant) *Variant{
  ...
  currencyId := this.SafeString(item , MkString("tokenId") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  amount := this.SafeNumber(item , MkString("change") ); _ = amount;
  after := this.SafeNumber(item , MkString("total") ); _ = after;
  ...
  timestamp := this.SafeInteger(item , MkString("created") ); _ = timestamp;
  type_ := this.ParseLedgerEntryType(this.SafeString(item , MkString("flowType") )); _ = type_;
...
func (this *Hbtc) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  id := this.SafeString(transaction , MkString("orderId") ); _ = id;
  address := this.SafeString(transaction , MkString("address") ); _ = address;
  tag := this.SafeString2(transaction , MkString("addressExt") , MkString("addressTag") ); _ = tag;
  if IsTrue(OpNotEq2(tag , MkUndefined() )) {
    if IsTrue(OpLw(tag.Length , MkInteger(1) )) {
      tag = OpCopy(MkUndefined() );
    }
  }
  addressFrom := this.SafeString(transaction , MkString("fromAddress") ); _ = addressFrom;
  tagFrom := this.SafeString(transaction , MkString("fromAddressTag") ); _ = tagFrom;
  if IsTrue(OpNotEq2(tagFrom , MkUndefined() )) {
    if IsTrue(OpLw(tagFrom.Length , MkInteger(1) )) {
      tagFrom = OpCopy(MkUndefined() );
    }
  }
  currencyId := this.SafeString(transaction , MkString("tokenId") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  ...
  status := this.ParseTransactionStatus(this.SafeString(transaction , MkString("statusCode") )); _ = status;
  ...
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyId := this.SafeString(transaction , MkString("feeTokenId") ); _ = feeCurrencyId;
...
func (this *Hbtc) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeNumber(trade , MkString("time") ); _ = timestamp;
  type_ := OpCopy(MkUndefined() ); _ = type_;
...
func (this *Hbtc) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  fees := this.SafeValue(order , MkString("fees") , MkArray(&VarArray{})); _ = fees;
  ...
  if IsTrue(OpGt(numFees , MkInteger(0) )) {
    *(result ).At(MkString("fees") )= MkArray(&VarArray{});
    for i := MkInteger(0) ; IsTrue(OpLw(i , fees.Length )); OpIncr(& i ){
      feeCost := this.SafeNumber(*(fees ).At(i ), MkString("fee") ); _ = feeCost;
      if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
        feeCurrencyId := this.SafeString(*(fees ).At(i ), MkString("feeToken") ); _ = feeCurrencyId;
        feeCurrencyCode := this.SafeCurrencyCode(feeCurrencyId ); _ = feeCurrencyCode;
...
func (this *Hbtc) Sign(goArgs ...*Variant) *Variant{
  ...
  isPublicContract := OpAnd(OpEq2(api , MkString("contract") ), OpOr(OpEq2(path , MkString("insurance") ), OpEq2(path , MkString("fundingRate") ))); _ = isPublicContract;
  if IsTrue(OpOr(OpEq2(api , MkString("public") ), OpOr(OpEq2(api , MkString("quote") ), isPublicContract ))) {
    ...
    request := this.Extend(MkMap(&VarMap{"timestamp":timestamp }), query ); _ = request;
