...
func (this *Delta) FetchTime(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetSettings"), params ); _ = response;
...
func (this *Delta) FetchStatus(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetSettings"), params ); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  underMaintenance := this.SafeValue(result , MkString("under_maintenance") ); _ = underMaintenance;
  status := OpTrinary(OpEq2(underMaintenance , MkString("true") ), MkString("maintenance") , MkString("ok") ); _ = status;
  updated := this.SafeIntegerProduct(result , MkString("server_time") , MkNumber(0.001) ); _ = updated;
...
func (this *Delta) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    ...
    depositStatus := this.SafeString(currency , MkString("deposit_status") ); _ = depositStatus;
    withdrawalStatus := this.SafeString(currency , MkString("withdrawal_status") ); _ = withdrawalStatus;
    depositsEnabled := OpEq2(depositStatus , MkString("enabled") ); _ = depositsEnabled;
    withdrawalsEnabled := OpEq2(withdrawalStatus , MkString("enabled") ); _ = withdrawalsEnabled;
    active := OpAnd(depositsEnabled , withdrawalsEnabled ); _ = active;
...
func (this *Delta) LoadMarkets(goArgs ...*Variant) *Variant{
  ...
  marketsByNumericId := this.SafeValue(*this.At(MkString("options")) , MkString("marketsByNumericId") ); _ = marketsByNumericId;
...
func (this *Delta) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    ...
    type_ := this.SafeString(market , MkString("contract_type") ); _ = type_;
    quotingAsset := this.SafeValue(market , MkString("quoting_asset") , MkMap(&VarMap{})); _ = quotingAsset;
    underlyingAsset := this.SafeValue(market , MkString("underlying_asset") , MkMap(&VarMap{})); _ = underlyingAsset;
    baseId := this.SafeString(underlyingAsset , MkString("symbol") ); _ = baseId;
    quoteId := this.SafeString(quotingAsset , MkString("symbol") ); _ = quoteId;
    ...
    active := OpEq2(state , MkString("live") ); _ = active;
    maker := this.SafeNumber(market , MkString("maker_commission_rate") ); _ = maker;
    taker := this.SafeNumber(market , MkString("taker_commission_rate") ); _ = taker;
...
func (this *Delta) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeIntegerProduct(ticker , MkString("timestamp") , MkNumber(0.001) ); _ = timestamp;
  ...
  quoteVolume := this.SafeNumber(ticker , MkString("turnover") ); _ = quoteVolume;
...
func (this *Delta) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTickersSymbol"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
...
func (this *Delta) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetL2orderbookSymbol"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
...
func (this *Delta) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  product := this.SafeValue(trade , MkString("product") , MkMap(&VarMap{})); _ = product;
  marketId := this.SafeString(product , MkString("symbol") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  sellerRole := this.SafeString(trade , MkString("seller_role") ); _ = sellerRole;
  ...
  metaData := this.SafeValue(trade , MkString("meta_data") , MkMap(&VarMap{})); _ = metaData;
  type_ := this.SafeString(metaData , MkString("order_type") ); _ = type_;
  ...
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    settlingAsset := this.SafeValue(product , MkString("settling_asset") , MkMap(&VarMap{})); _ = settlingAsset;
    feeCurrencyId := this.SafeString(settlingAsset , MkString("symbol") ); _ = feeCurrencyId;
...
func (this *Delta) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetHistoryCandles"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkArray(&VarArray{})); _ = result;
...
func (this *Delta) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  currenciesByNumericId := this.SafeValue(*this.At(MkString("options")) , MkString("currenciesByNumericId") , MkMap(&VarMap{})); _ = currenciesByNumericId;
  for i := MkInteger(0) ; IsTrue(OpLw(i , balances.Length )); OpIncr(& i ){
    balance := *(balances ).At(i ); _ = balance;
    currencyId := this.SafeString(balance , MkString("asset_id") ); _ = currencyId;
    currency := this.SafeValue(currenciesByNumericId , currencyId ); _ = currency;
    code := OpTrinary(OpEq2(currency , MkUndefined() ), currencyId , *(currency ).At(MkString("code") )); _ = code;
...
func (this *Delta) FetchPosition(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  params := GoGetArg(goArgs, 1, MkUndefined() ); _ = params;
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"product_id":*(market ).At(MkString("numericId") )}); _ = request;
...
func (this *Delta) FetchPositions(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetPositionsMargined"), params ); _ = response;
  result := this.SafeValue(response , MkString("result") , MkArray(&VarArray{})); _ = result;
  return result ;
}
...
func (this *Delta) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  marketsByNumericId := this.SafeValue(*this.At(MkString("options")) , MkString("marketsByNumericId") , MkMap(&VarMap{})); _ = marketsByNumericId;
  market = this.SafeValue(marketsByNumericId , marketId , market );
  symbol := OpTrinary(OpEq2(market , MkUndefined() ), marketId , *(market ).At(MkString("symbol") )); _ = symbol;
  ...
  remaining := this.SafeNumber(order , MkString("unfilled_size") ); _ = remaining;
  average := this.SafeNumber(order , MkString("average_fill_price") ); _ = average;
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(order , MkString("paid_commission") ); _ = feeCost;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyCode := OpCopy(MkUndefined() ); _ = feeCurrencyCode;
    if IsTrue(OpNotEq2(market , MkUndefined() )) {
      settlingAsset := this.SafeValue(*(market ).At(MkString("info") ), MkString("settling_asset") , MkMap(&VarMap{})); _ = settlingAsset;
      feeCurrencyId := this.SafeString(settlingAsset , MkString("symbol") ); _ = feeCurrencyId;
...
func (this *Delta) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  orderType := OpAdd(type_ , MkString("_order") ); _ = orderType;
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{
...
func (this *Delta) EditOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePutOrders"), this.Extend(request , params )); _ = response;
...
func (this *Delta) CancelAllOrders(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"product_id":*(market ).At(MkString("numericId") )}); _ = request;
  response := this.Call(MkString("privateDeleteOrdersAll"), this.Extend(request , params )); _ = response;
  return response ;
}
...
func (this *Delta) ParseLedgerEntry(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString(item , MkString("uuid") ); _ = id;
  direction := OpCopy(MkUndefined() ); _ = direction;
  account := OpCopy(MkUndefined() ); _ = account;
  metaData := this.SafeValue(item , MkString("meta_data") , MkMap(&VarMap{})); _ = metaData;
  referenceId := this.SafeString(metaData , MkString("transaction_id") ); _ = referenceId;
  referenceAccount := OpCopy(MkUndefined() ); _ = referenceAccount;
  type_ := this.SafeString(item , MkString("transaction_type") ); _ = type_;
  ...
  currencyId := this.SafeInteger(item , MkString("asset_id") ); _ = currencyId;
  currenciesByNumericId := this.SafeValue(*this.At(MkString("options")) , MkString("currenciesByNumericId") ); _ = currenciesByNumericId;
  ...
  timestamp := this.Parse8601(this.SafeString(item , MkString("created_at") )); _ = timestamp;
  after := this.SafeNumber(item , MkString("balance") ); _ = after;
  before := Math.Max(MkInteger(0) , OpSub(after , amount )); _ = before;
...
func (this *Delta) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetDepositsAddress"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
...
func (this *Delta) Sign(goArgs ...*Variant) *Variant{
  ...
  requestPath := OpAdd(MkString("/") , OpAdd(*this.At(MkString("version")) , OpAdd(MkString("/") , this.ImplodeParams(path , params )))); _ = requestPath;
  url := OpAdd(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(api ), requestPath ); _ = url;
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    if IsTrue(OpEq2(api , MkString("private") )) {
      ...
      timestamp := (this.Seconds()).Call(MkString("toString") ); _ = timestamp;
      headers = MkMap(&VarMap{
          "api-key":*this.At(MkString("apiKey")) ,
          "timestamp":timestamp ,
          });
      auth := OpAdd(method , OpAdd(timestamp , requestPath )); _ = auth;
      if IsTrue(OpOr(OpEq2(method , MkString("GET") ), OpEq2(method , MkString("DELETE") ))) {
        if IsTrue(*(GoGetKeys(query )).At(MkString("length") )) {
          queryString := OpAdd(MkString("?") , this.Urlencode(query )); _ = queryString;
