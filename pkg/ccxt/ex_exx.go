...
func (this *Exx) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    ...
    active := OpEq2(*(market ).At(MkString("isOpen") ), MkBool(true) ); _ = active;
...
func (this *Exx) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"currency":this.MarketId(symbol )}); _ = request;
...
func (this *Exx) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  balances := this.SafeValue(response , MkString("funds") ); _ = balances;
  currencies := GoGetKeys(balances ); _ = currencies;
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    currencyId := *(currencies ).At(i ); _ = currencyId;
...
func (this *Exx) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  symbol := *(market ).At(MkString("symbol") ); _ = symbol;
  timestamp := parseInt(*(order ).At(MkString("trade_date") )); _ = timestamp;
  ...
  filled := this.SafeNumber(order , MkString("trade_amount") , MkNumber(0.0) ); _ = filled;
  status := this.SafeInteger(order , MkString("status") ); _ = status;
...
func (this *Exx) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    signed := this.Hmac(this.Encode(query ), this.Encode(*this.At(MkString("secret")) ), MkString("sha512") ); _ = signed;
