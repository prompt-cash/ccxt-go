...
func (this *Crex24) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response2 := this.Call(MkString("publicGetTradingFeeSchedules"), params ); _ = response2;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    minPrice := this.SafeNumber(market , MkString("minPrice") ); _ = minPrice;
    maxPrice := this.SafeNumber(market , MkString("maxPrice") ); _ = maxPrice;
    minAmount := this.SafeNumber(market , MkString("minVolume") ); _ = minAmount;
    maxAmount := this.SafeNumber(market , MkString("maxVolume") ); _ = maxAmount;
    minCost := this.SafeNumber(market , MkString("minQuoteVolume") ); _ = minCost;
    maxCost := this.SafeNumber(market , MkString("maxQuoteVolume") ); _ = maxCost;
    ...
    feeSchedule := this.SafeString(market , MkString("feeSchedule") ); _ = feeSchedule;
    for j := MkInteger(0) ; IsTrue(OpLw(j , response2.Length )); OpIncr(& j ){
      feeScheduleName := this.SafeString(*(response2 ).At(j ), MkString("name") ); _ = feeScheduleName;
      if IsTrue(OpEq2(feeScheduleName , feeSchedule )) {
        feeRates := this.SafeValue(*(response2 ).At(j ), MkString("feeRates") , MkArray(&VarArray{})); _ = feeRates;
        for k := MkInteger(0) ; IsTrue(OpLw(k , feeRates.Length )); OpIncr(& k ){
          volumeThreshold := this.SafeNumber(*(feeRates ).At(k ), MkString("volumeThreshold") ); _ = volumeThreshold;
    ...
    active := OpEq2(*(market ).At(MkString("state") ), MkString("active") ); _ = active;
...
func (this *Crex24) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    withdrawalPrecision := this.SafeInteger(currency , MkString("withdrawalPrecision") ); _ = withdrawalPrecision;
    precision := Math.Pow(MkInteger(10) , OpNeg(withdrawalPrecision )); _ = precision;
    address := this.SafeValue(currency , MkString("BaseAddress") ); _ = address;
    active := OpAnd(*(currency ).At(MkString("depositsAllowed") ), OpAnd(*(currency ).At(MkString("withdrawalsAllowed") ), OpNot(*(currency ).At(MkString("isDelisted") )))); _ = active;
    type_ := OpTrinary(*(currency ).At(MkString("isFiat") ), MkString("fiat") , MkString("crypto") ); _ = type_;
...
func (this *Crex24) FetchFundingFees(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  response := this.Call(MkString("publicGetCurrenciesWithdrawalFees"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    networkList := this.SafeValue(entry , MkString("fees") ); _ = networkList;
    *(withdrawFees ).At(code )= MkMap(&VarMap{});
    for j := MkInteger(0) ; IsTrue(OpLw(j , networkList.Length )); OpIncr(& j ){
      ...
      networkId := this.SafeString(networkEntry , MkString("feeCurrency") ); _ = networkId;
      networkCode := this.SafeCurrencyCode(networkId ); _ = networkCode;
      fee := this.SafeNumber(networkEntry , MkString("amount") ); _ = fee;
...
func (this *Crex24) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("accountGetBalance"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"instrument":*(market ).At(MkString("id") )}); _ = request;
...
func (this *Crex24) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(ticker , MkString("instrument") ); _ = marketId;
...
func (this *Crex24) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  numTickers := OpCopy(response.Length ); _ = numTickers;
...
func (this *Crex24) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  feeCode := this.SafeCurrencyCode(feeCurrencyId ); _ = feeCode;
...
func (this *Crex24) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetRecentTrades"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetOhlcv"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  remaining := this.SafeNumber(order , MkString("remainingVolume") ); _ = remaining;
  lastTradeTimestamp := this.Parse8601(this.SafeString(order , MkString("lastUpdate") )); _ = lastTradeTimestamp;
...
func (this *Crex24) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("tradingPostPlaceOrder"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("tradingGetOrderStatus"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  method := this.SafeString(*this.At(MkString("options")) , MkString("fetchOrdersMethod") , MkString("tradingGetOrderHistory") ); _ = method;
...
func (this *Crex24) FetchOrdersByIds(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"id":ids.Join(MkString(",") )}); _ = request;
  response := this.Call(MkString("tradingGetOrderStatus"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("tradingGetActiveOrders"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  method := this.SafeString(*this.At(MkString("options")) , MkString("fetchClosedOrdersMethod") , MkString("tradingGetOrderHistory") ); _ = method;
  response := this.Call(method , this.Extend(request , params )); _ = response;
  return this.ParseOrders(response , market , since , limit );
}
...
func (this *Crex24) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("tradingPostCancelOrdersById"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) CancelAllOrders(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined() ); _ = symbol;
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("tradingPostCancelAllOrders"), params ); _ = response;
  return response ;
}
...
func (this *Crex24) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("tradingGetTradeHistory"), this.Extend(request , params )); _ = response;
  return this.ParseTrades(response , market , since , limit );
}
...
func (this *Crex24) FetchTransactions(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("accountGetMoneyTransfers"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"type":MkString("deposit") }); _ = request;
...
func (this *Crex24) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"type":MkString("withdrawal") }); _ = request;
  return this.FetchTransactions(code , since , limit , this.Extend(request , params ));
}
...
func (this *Crex24) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  txid := this.SafeValue(transaction , MkString("txId") ); _ = txid;
  ...
  updated := this.Parse8601(this.SafeString(transaction , MkString("processedAt") )); _ = updated;
...
func (this *Crex24) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("accountPostWithdraw"), this.Extend(request , params )); _ = response;
...
func (this *Crex24) Sign(goArgs ...*Variant) *Variant{
  ...
  request := OpAdd(MkString("/") , OpAdd(*this.At(MkString("version")) , OpAdd(MkString("/") , OpAdd(api , OpAdd(MkString("/") , this.ImplodeParams(path , params )))))); _ = request;
  ...
  if IsTrue(OpOr(OpEq2(api , MkString("trading") ), OpEq2(api , MkString("account") ))) {
    ...
    auth := OpAdd(request , nonce ); _ = auth;
...
func (this *Crex24) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  message := this.SafeString(response , MkString("errorDescription") ); _ = message;
