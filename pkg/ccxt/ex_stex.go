...
func (this *Stex) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	result := MkMap(&VarMap{})
	_ = result
	currencies := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = currencies
	for i := MkInteger(0); IsTrue(OpLw(i, currencies.Length)); OpIncr(&i) {
		...
		_ = numericId
		code := this.SafeCurrencyCode(this.SafeString(currency, MkString("code")))
		_ = code
		precision := this.SafeString(currency, MkString("precision"))
		_ = precision
		amountLimit := this.ParsePrecision(precision)
		_ = amountLimit
		fee := this.SafeNumber(currency, MkString("withdrawal_fee_const"))
		_ = fee
		active := this.SafeValue(currency, MkString("active"), MkBool(true))
		_ = active
...
func (this *Stex) FetchMarkets(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"code": MkString("ALL")})
	_ = request
	response := this.Call(MkString("publicGetCurrencyPairsListCode"), this.Extend(request, params))
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		...
		_ = numericId
		baseId := this.SafeString(market, MkString("currency_id"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("market_currency_id"))
		_ = quoteId
		baseNumericId := this.SafeInteger(market, MkString("currency_id"))
		_ = baseNumericId
		quoteNumericId := this.SafeInteger(market, MkString("market_currency_id"))
		_ = quoteNumericId
		base := this.SafeCurrencyCode(this.SafeString(market, MkString("currency_code")))
		_ = base
		quote := this.SafeCurrencyCode(this.SafeString(market, MkString("market_code")))
		_ = quote
		...
		_ = active
		minBuyPrice := this.SafeNumber(market, MkString("min_buy_price"))
		_ = minBuyPrice
		minSellPrice := this.SafeNumber(market, MkString("min_sell_price"))
		_ = minSellPrice
		minPrice := Math.Max(minBuyPrice, minSellPrice)
		_ = minPrice
		buyFee := OpDiv(this.SafeNumber(market, MkString("buy_fee_percent")), MkInteger(100))
		_ = buyFee
		sellFee := OpDiv(this.SafeNumber(market, MkString("sell_fee_percent")), MkInteger(100))
		_ = sellFee
		fee := Math.Max(buyFee, sellFee)
		_ = fee
...
func (this *Stex) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"currencyPairId": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetTickerCurrencyPairId"), this.Extend(request, params))
	_ = response
	ticker := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = ticker
...
func (this *Stex) FetchTime(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("publicGetPing"), params)
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	serverDatetime := this.SafeValue(data, MkString("server_datetime"), MkMap(&VarMap{}))
	_ = serverDatetime
...
func (this *Stex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetOrderbookCurrencyPairId"), this.Extend(request, params))
	_ = response
...
func (this *Stex) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	marketId := this.SafeString2(ticker, MkString("id"), MkString("symbol"))
	_ = marketId
...
func (this *Stex) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	_ = duration
	timerange := OpMulti(limit, duration)
	_ = timerange
	...
	response := this.Call(MkString("publicGetChartCurrencyPairIdCandlesType"), this.Extend(request, params))
	_ = response
...
func (this *Stex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	side := this.SafeStringLower2(trade, MkString("type"), MkString("trade_type"))
	_ = side
	return MkMap(&VarMap{
		"info":         trade,
		"timestamp":    timestamp,
...
func (this *Stex) FetchTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetTradesCurrencyPairId"), this.Extend(request, params))
	_ = response
...
func (this *Stex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("profileGetWallets"), params)
	_ = response
	result := MkMap(&VarMap{
		"info":      response,
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = balance
		code := this.SafeCurrencyCode(this.SafeString(balance, MkString("currency_id")))
		_ = code
...
func (this *Stex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = status
	marketId := this.SafeString2(order, MkString("currency_pair_id"), MkString("currency_pair_name"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("_"))
	_ = symbol
	...
	_ = price
	amount := this.SafeNumber(order, MkString("initial_amount"))
	_ = amount
	filled := this.SafeNumber(order, MkString("processed_amount"))
	_ = filled
	...
	type_ := this.SafeString(order, MkString("original_type"))
	_ = type_
	...
	_ = result
	fees := this.SafeValue(order, MkString("fees"))
	_ = fees
		...
		if IsTrue(OpGt(numFees, MkInteger(0))) {
			*(result).At(MkString("fees")) = MkArray(&VarArray{})
			for i := MkInteger(0); IsTrue(OpLw(i, fees.Length)); OpIncr(&i) {
				feeCost := this.SafeNumber(*(fees).At(i), MkString("amount"))
				_ = feeCost
				if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
					feeCurrencyId := this.SafeString(*(fees).At(i), MkString("currency_id"))
					_ = feeCurrencyId
...
func (this *Stex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("tradingPostOrdersCurrencyPairId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
...
func (this *Stex) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("tradingGetOrderOrderId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
...
func (this *Stex) FetchClosedOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"orderId": id})
	_ = request
	response := this.Call(MkString("reportsGetOrdersOrderId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	market := OpCopy(MkUndefined())
...
func (this *Stex) FetchOrderTrades(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 4, MkMap(&VarMap{}))
	_ = params
	order := this.FetchClosedOrder(id, symbol, params)
	_ = order
...
func (this *Stex) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	market := OpCopy(MkUndefined())
	_ = market
	method := MkString("tradingGetOrders")
	_ = method
	request := MkMap(&VarMap{})
	_ = request
	if IsTrue(OpNotEq2(symbol, MkUndefined())) {
...
func (this *Stex) CancelOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"orderId": id})
	_ = request
	response := this.Call(MkString("tradingDeleteOrderOrderId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	acceptedOrders := this.SafeValue(data, MkString("put_into_processing_queue"), MkArray(&VarArray{}))
	_ = acceptedOrders
	rejectedOrders := this.SafeValue(data, MkString("not_put_into_processing_queue"), MkArray(&VarArray{}))
	_ = rejectedOrders
	numAcceptedOrders := OpCopy(acceptedOrders.Length)
	_ = numAcceptedOrders
	numRejectedOrders := OpCopy(rejectedOrders.Length)
	_ = numRejectedOrders
...
func (this *Stex) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	method := MkString("tradingDeleteOrders")
	_ = method
	if IsTrue(OpNotEq2(symbol, MkUndefined())) {
		market := this.Market(symbol)
		_ = market
...
func (this *Stex) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("reportsGetTradesCurrencyPairId"), this.Extend(request, params))
	_ = response
	trades := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = trades
	return this.ParseTrades(trades, market, since, limit)
...
func (this *Stex) CreateDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("profilePostWalletsCurrencyId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	depositAddress := this.SafeValue(data, MkString("deposit_address"), MkMap(&VarMap{}))
	_ = depositAddress
	address := this.SafeString(depositAddress, MkString("address"))
	_ = address
	tag := this.SafeString(depositAddress, MkString("additional_address_parameter"))
	_ = tag
...
func (this *Stex) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = balance
	wallets := this.SafeValue(*(balance).At(MkString("info")), MkString("data"), MkArray(&VarArray{}))
	_ = wallets
	walletsByCurrencyId := this.IndexBy(wallets, MkString("currency_id"))
	_ = walletsByCurrencyId
	currency := this.Currency(code)
	_ = currency
	wallet := this.SafeValue(walletsByCurrencyId, *(currency).At(MkString("id")))
	_ = wallet
	...
	walletId := this.SafeInteger(wallet, MkString("id"))
	_ = walletId
	if IsTrue(OpEq2(walletId, MkUndefined())) {
		panic(NewExchangeError(OpAdd(*this.At(MkString("id")), OpAdd(MkString(" fetchDepositAddress() could not find the wallet id for currency code "), OpAdd(code, MkString(", try to call createDepositAddress() first"))))))
	}
	request := MkMap(&VarMap{"walletId": walletId})
	_ = request
	response := this.Call(MkString("profileGetWalletsWalletId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
	depositAddress := this.SafeValue(data, MkString("deposit_address"), MkMap(&VarMap{}))
...
func (this *Stex) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	url := OpAdd(*(*this.At(MkString("urls"))).At(MkString("api")), OpAdd(MkString("/"), OpAdd(api, OpAdd(MkString("/"), this.ImplodeParams(path, params)))))
	_ = url
...
func (this *Stex) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = id
	withdrawalAddress := this.SafeValue(transaction, MkString("withdrawal_address"), MkMap(&VarMap{}))
	_ = withdrawalAddress
	address := this.SafeString(withdrawalAddress, MkString("address"))
	_ = address
	tag := this.SafeString(withdrawalAddress, MkString("additional_address_parameter"))
	_ = tag
	...
	type_ := OpTrinary(OpHasMember(MkString("deposit_status_id"), transaction), MkString("deposit"), MkString("withdrawal"))
	_ = type_
	...
	_ = status
	timestamp := this.SafeTimestamp2(transaction, MkString("timestamp"), MkString("created_ts"))
	_ = timestamp
	updated := this.SafeTimestamp(transaction, MkString("updated_ts"))
	_ = updated
	...
	_ = feeCost
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCurrencyId := this.SafeString(transaction, MkString("fee_currency_id"), MkString("deposit_fee_currency_id"))
		_ = feeCurrencyId
		feeCurrencyCode := this.SafeCurrencyCode(feeCurrencyId)
		_ = feeCurrencyCode
		fee = MkMap(&VarMap{
...
func (this *Stex) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("profileGetDeposits"), this.Extend(request, params))
	_ = response
	deposits := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = deposits
...
func (this *Stex) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("profileGetWithdrawals"), this.Extend(request, params))
	_ = response
	withdrawals := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = withdrawals
...
func (this *Stex) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("profilePostWithdraw"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
...
func (this *Stex) FetchFundingFees(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, data.Length)); OpIncr(&i) {
		id := this.SafeString(*(data).At(i), MkString("id"))
		_ = id
