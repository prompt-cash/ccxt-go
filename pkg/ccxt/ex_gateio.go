...
func (this *Gateio) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicSpotGetCurrencyPairs"), params ); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    entry := *(response ).At(i ); _ = entry;
    id := this.SafeString(entry , MkString("id") ); _ = id;
    baseId := this.SafeString(entry , MkString("base") ); _ = baseId;
    quoteId := this.SafeString(entry , MkString("quote") ); _ = quoteId;
    ...
    taker := OpDiv(this.SafeNumber(entry , MkString("fee") ), MkInteger(100) ); _ = taker;
    maker := OpCopy(taker ); _ = maker;
    tradeStatus := this.SafeString(entry , MkString("trade_status") ); _ = tradeStatus;
    active := OpEq2(tradeStatus , MkString("tradable") ); _ = active;
    amountPrecision := this.SafeString(entry , MkString("amount_precision") ); _ = amountPrecision;
    pricePrecision := this.SafeString(entry , MkString("precision") ); _ = pricePrecision;
    amountLimit := this.ParsePrecision(amountPrecision ); _ = amountLimit;
...
func (this *Gateio) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicSpotGetCurrencies"), params ); _ = response;
  result := MkMap(&VarMap{}); _ = result;
  amountPrecision := MkInteger(6) ; _ = amountPrecision;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    entry := *(response ).At(i ); _ = entry;
    currencyId := this.SafeString(entry , MkString("currency") ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    delisted := this.SafeValue(entry , MkString("delisted") ); _ = delisted;
    withdraw_disabled := this.SafeValue(entry , MkString("withdraw_disabled") ); _ = withdraw_disabled;
    deposit_disabled := this.SafeValue(entry , MkString("disabled_disabled") ); _ = deposit_disabled;
    trade_disabled := this.SafeValue(entry , MkString("trade_disabled") ); _ = trade_disabled;
    active := OpNot(OpAnd(delisted , OpAnd(withdraw_disabled , OpAnd(deposit_disabled , trade_disabled )))); _ = active;
...
func (this *Gateio) FetchNetworkDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateWalletGetDepositAddress"), this.Extend(request , params )); _ = response;
  addresses := this.SafeValue(response , MkString("multichain_addresses") ); _ = addresses;
  currencyId := this.SafeString(response , MkString("currency") ); _ = currencyId;
  code = this.SafeCurrencyCode(currencyId );
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , addresses.Length )); OpIncr(& i ){
    entry := *(addresses ).At(i ); _ = entry;
    obtainFailed := this.SafeInteger(entry , MkString("obtain_failed") ); _ = obtainFailed;
    if IsTrue(obtainFailed ) {
      continue ;
    }
    network := this.SafeString(entry , MkString("chain") ); _ = network;
    address := this.SafeString(entry , MkString("address") ); _ = address;
    tag := this.SafeString(entry , MkString("payment_id") ); _ = tag;
    tagLength := OpCopy(tag.Length ); _ = tagLength;
...
func (this *Gateio) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currencyId := this.SafeString(response , MkString("currency") ); _ = currencyId;
  code = this.SafeCurrencyCode(currencyId );
  addressField := this.SafeString(response , MkString("address") ); _ = addressField;
  ...
  if IsTrue(OpGt(addressField.IndexOf(MkString(" ") ), OpNeg(MkInteger(1) ))) {
    splitted := addressField.Split(MkString(" ") ); _ = splitted;
...
func (this *Gateio) FetchTradingFees(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateWalletGetFee"), params ); _ = response;
  result := MkMap(&VarMap{}); _ = result;
  taker := this.SafeNumber(response , MkString("taker_fee") ); _ = taker;
  maker := this.SafeNumber(response , MkString("maker_fee") ); _ = maker;
...
func (this *Gateio) FetchFundingFees(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateWalletGetWithdrawStatus"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    *(withdrawFees ).At(code )= MkMap(&VarMap{});
    withdrawFix := this.SafeValue(entry , MkString("withdraw_fix_on_chains") ); _ = withdrawFix;
    ...
    keys := GoGetKeys(withdrawFix ); _ = keys;
...
func (this *Gateio) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"currency_pair":*(market ).At(MkString("id") )}); _ = request;
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
    *(request ).At(MkString("limit") )= OpCopy(limit );
  }
  response := this.Call(MkString("publicSpotGetOrderBook"), this.Extend(request , params )); _ = response;
  timestamp := this.SafeInteger(response , MkString("current") ); _ = timestamp;
...
func (this *Gateio) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"currency_pair":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicSpotGetTickers"), this.Extend(request , params )); _ = response;
...
func (this *Gateio) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(ticker , MkString("currency_pair") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  last := this.SafeNumber(ticker , MkString("last") ); _ = last;
  ask := this.SafeNumber(ticker , MkString("lowest_ask") ); _ = ask;
  bid := this.SafeNumber(ticker , MkString("highest_bid") ); _ = bid;
  high := this.SafeNumber(ticker , MkString("high_24h") ); _ = high;
  low := this.SafeNumber(ticker , MkString("low_24h") ); _ = low;
  ...
  percentage := this.SafeNumber(ticker , MkString("change_percentage") ); _ = percentage;
  return MkMap(&VarMap{
        "symbol":symbol ,
...
func (this *Gateio) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicSpotGetTickers"), params ); _ = response;
...
func (this *Gateio) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateSpotGetAccounts"), params ); _ = response;
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    entry := *(response ).At(i ); _ = entry;
...
func (this *Gateio) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicSpotGetCandlesticks"), this.Extend(request , params )); _ = response;
...
func (this *Gateio) ParseOHLCV(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeTimestamp(ohlcv , MkInteger(0) ); _ = timestamp;
  volume := this.SafeNumber(ohlcv , MkInteger(1) ); _ = volume;
  close := this.SafeNumber(ohlcv , MkInteger(2) ); _ = close;
  high := this.SafeNumber(ohlcv , MkInteger(3) ); _ = high;
  low := this.SafeNumber(ohlcv , MkInteger(4) ); _ = low;
  open := this.SafeNumber(ohlcv , MkInteger(5) ); _ = open;
...
func (this *Gateio) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"currency_pair":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicSpotGetTrades"), this.Extend(request , params )); _ = response;
...
func (this *Gateio) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateSpotGetMyTrades"), this.Extend(request , params )); _ = response;
  return this.ParseTrades(response , market , since , limit );
}
...
func (this *Gateio) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  timestampString := this.SafeString2(trade , MkString("create_time_ms") , MkString("time") ); _ = timestampString;
  timestamp := OpCopy(MkUndefined() ); _ = timestamp;
  if IsTrue(OpGt(timestampString.IndexOf(MkString(".") ), MkInteger(0) )) {
    milliseconds := timestampString.Split(MkString(".") ); _ = milliseconds;
    timestamp = parseInt(*(milliseconds ).At(MkInteger(0) ));
  }
  marketId := this.SafeString(trade , MkString("currency_pair") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  ...
  gtFee := this.SafeString(trade , MkString("gt_fee") ); _ = gtFee;
...
func (this *Gateio) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateWalletGetDeposits"), this.Extend(request , params )); _ = response;
...
func (this *Gateio) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateWalletGetWithdrawals"), this.Extend(request , params )); _ = response;
  return this.ParseTransactions(response , currency );
}
...
func (this *Gateio) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateWithdrawalsPost"), this.Extend(request , params )); _ = response;
  currencyId := this.SafeString(response , MkString("currency") ); _ = currencyId;
...
func (this *Gateio) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateSpotPostOrders"), this.Extend(request , params )); _ = response;
...
func (this *Gateio) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(order , MkString("currency_pair") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  timestamp := this.SafeTimestamp(order , MkString("create_time") ); _ = timestamp;
  timestamp = this.SafeInteger(order , MkString("create_time_ms") , timestamp );
  lastTradeTimestamp := this.SafeTimestamp(order , MkString("update_time") ); _ = lastTradeTimestamp;
  ...
  remaining := this.SafeNumber(order , MkString("left") ); _ = remaining;
  cost := this.SafeNumber(order , MkString("filled_total") ); _ = cost;
  ...
  timeInForce := this.SafeStringUpper(order , MkString("time_in_force") ); _ = timeInForce;
  fees := MkArray(&VarArray{}); _ = fees;
  ...
  rebate := this.SafeString(order , MkString("rebated_fee") ); _ = rebate;
  fees.Push(MkMap(&VarMap{
...
func (this *Gateio) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateSpotGetOrdersOrderId"), this.Extend(request , params )); _ = response;
  return this.ParseOrder(response , market );
}
...
func (this *Gateio) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(symbol , MkUndefined() )) {
    ...
    response := this.Call(MkString("privateSpotGetOpenOrders"), this.Extend(request , params )); _ = response;
    allOrders := MkArray(&VarArray{}); _ = allOrders;
    for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
      entry := *(response ).At(i ); _ = entry;
      orders := this.SafeValue(entry , MkString("orders") , MkArray(&VarArray{})); _ = orders;
      parsed := this.ParseOrders(orders , MkUndefined() , since , limit ); _ = parsed;
...
func (this *Gateio) FetchOrdersByStatus(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateSpotGetOrders"), this.Extend(request , params )); _ = response;
...
func (this *Gateio) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateSpotDeleteOrdersOrderId"), this.Extend(request , params )); _ = response;
...
func (this *Gateio) Transfer(goArgs ...*Variant) *Variant{
  ...
  truncated := this.CurrencyToPrecision(code , amount ); _ = truncated;
  request := MkMap(&VarMap{
        "currency":*(currency ).At(MkString("id") ),
        "from":fromId ,
        "to":toId ,
  ...
  response := this.Call(MkString("privateWalletPostTransfers"), this.Extend(request , params )); _ = response;
  return MkMap(&VarMap{
        "info":response ,
        "from":fromId ,
        "to":toId ,
...
func (this *Gateio) Sign(goArgs ...*Variant) *Variant{
  ...
  api := GoGetArg(goArgs, 1, MkArray(&VarArray{})); _ = api;
  ...
  authentication := *(api ).At(MkInteger(0) ); _ = authentication;
  type_ := *(api ).At(MkInteger(1) ); _ = type_;
  query := this.Omit(params , this.ExtractParams(path )); _ = query;
  path = this.ImplodeParams(path , params );
  endPart := OpTrinary(OpEq2(path , MkString("") ), MkString("") , OpAdd(MkString("/") , path )); _ = endPart;
  entirePath := OpAdd(MkString("/") , OpAdd(type_ , endPart )); _ = entirePath;
  url := OpAdd(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(authentication ), entirePath ); _ = url;
  ...
  if IsTrue(OpEq2(authentication , MkString("public") )) {
    ...
    bodyPayload := OpTrinary(OpEq2(body , MkUndefined() ), MkString("") , body ); _ = bodyPayload;
    bodySignature := this.Hash(this.Encode(bodyPayload ), MkString("sha512") ); _ = bodySignature;
    timestamp := this.Seconds(); _ = timestamp;
    timestampString := timestamp.ToString(); _ = timestampString;
    signaturePath := OpAdd(MkString("/api/v4") , entirePath ); _ = signaturePath;
    ...
    signature := this.Hmac(this.Encode(payload ), this.Encode(*this.At(MkString("secret")) ), MkString("sha512") ); _ = signature;
...
func (this *Gateio) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  label := this.SafeString(response , MkString("label") ); _ = label;
  if IsTrue(OpNotEq2(label , MkUndefined() )) {
    message := this.SafeString(response , MkString("message") ); _ = message;
    Error := this.SafeValue(*this.At(MkString("exceptions")) , label , ExchangeError ); _ = Error;
