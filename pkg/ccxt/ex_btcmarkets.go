...
func (this *Btcmarkets) ParseTransactionStatus(goArgs ...*Variant) *Variant {
	...
	_ = status
	statuses := MkMap(&VarMap{"Complete": MkString("ok")})
	_ = statuses
...
func (this *Btcmarkets) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = currency
	timestamp := this.Parse8601(this.SafeString(transaction, MkString("creationTime")))
	_ = timestamp
	lastUpdate := this.Parse8601(this.SafeString(transaction, MkString("lastUpdate")))
	_ = lastUpdate
	type_ := this.ParseTransactionType(this.SafeStringLower(transaction, MkString("type")))
	_ = type_
	if IsTrue(OpEq2(type_, MkString("withdraw"))) {
		type_ = MkString("withdrawal")
	}
	cryptoPaymentDetail := this.SafeValue(transaction, MkString("paymentDetail"), MkMap(&VarMap{}))
	_ = cryptoPaymentDetail
	txid := this.SafeString(cryptoPaymentDetail, MkString("txId"))
	_ = txid
	address := this.SafeString(cryptoPaymentDetail, MkString("address"))
	_ = address
	...
	_ = status
	currencyId := this.SafeString(transaction, MkString("assetName"))
	_ = currencyId
...
func (this *Btcmarkets) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = market
		baseId := this.SafeString(market, MkString("baseAssetName"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("quoteAssetName"))
		_ = quoteId
		...
		_ = symbol
		fees := this.SafeValue(this.SafeValue(*this.At(MkString("options")), MkString("fees"), MkMap(&VarMap{})), quote, *this.At(MkString("fees")))
		_ = fees
		pricePrecision := this.SafeInteger(market, MkString("priceDecimals"))
		_ = pricePrecision
		amountPrecision := this.SafeInteger(market, MkString("amountDecimals"))
		_ = amountPrecision
		minAmount := this.SafeNumber(market, MkString("minOrderAmount"))
		_ = minAmount
		maxAmount := this.SafeNumber(market, MkString("maxOrderAmount"))
		_ = maxAmount
...
func (this *Btcmarkets) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetAccountsMeBalances"), params)
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = balance
		currencyId := this.SafeString(balance, MkString("assetName"))
		_ = currencyId
		code := this.SafeCurrencyCode(currencyId)
		_ = code
...
func (this *Btcmarkets) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetMarketsMarketIdCandles"), this.Extend(request, params))
	_ = response
...
func (this *Btcmarkets) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetMarketsMarketIdOrderbook"), this.Extend(request, params))
	_ = response
	timestamp := this.SafeIntegerProduct(response, MkString("snapshotId"), MkNumber(0.001))
	_ = timestamp
...
func (this *Btcmarkets) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = symbol
	marketId := this.SafeString(ticker, MkString("marketId"))
	_ = marketId
	...
	_ = baseVolume
	quoteVolume := this.SafeNumber(ticker, MkString("volumeQte24h"))
	_ = quoteVolume
	vwap := this.Vwap(baseVolume, quoteVolume)
	_ = vwap
	change := this.SafeNumber(ticker, MkString("price24h"))
	_ = change
	percentage := this.SafeNumber(ticker, MkString("pricePct24h"))
	_ = percentage
	return MkMap(&VarMap{
		"symbol":        symbol,
		"timestamp":     timestamp,
...
func (this *Btcmarkets) FetchTicker(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"marketId": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetMarketsMarketIdTicker"), this.Extend(request, params))
	_ = response
...
func (this *Btcmarkets) FetchTicker2(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetMarketIdTick"), this.Extend(request, params))
	_ = response
	return this.ParseTicker(response, market)
}
...
func (this *Btcmarkets) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	marketId := this.SafeString(trade, MkString("marketId"))
	_ = marketId
	symbol := OpCopy(MkUndefined())
	_ = symbol
	...
	takerOrMaker := this.SafeStringLower(trade, MkString("liquidityType"))
	_ = takerOrMaker
	return MkMap(&VarMap{
		"info":         trade,
		"id":           id,
...
func (this *Btcmarkets) FetchTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"marketId": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetMarketsMarketIdTrades"), this.Extend(request, params))
	_ = response
...
func (this *Btcmarkets) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	lowercaseType := type_.ToLowerCase()
	_ = lowercaseType
	...
	*(request).At(MkString("type")) = this.SafeString(orderTypes, lowercaseType, type_)
	...
	_ = priceIsRequired
	triggerPriceIsRequired := OpCopy(MkBool(false))
	_ = triggerPriceIsRequired
...
func (this *Btcmarkets) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.Parse8601(this.SafeString(order, MkString("creationTime")))
	_ = timestamp
	...
	_ = amount
	remaining := this.SafeNumber(order, MkString("openAmount"))
	_ = remaining
...
func (this *Btcmarkets) Sign(goArgs ...*Variant) *Variant {
	...
	_ = request
	query := this.Keysort(this.Omit(params, this.ExtractParams(path)))
	_ = query
	if IsTrue(OpEq2(api, MkString("private"))) {
		this.CheckRequiredCredentials()
		nonce := (this.Nonce()).Call(MkString("toString"))
		_ = nonce
		secret := this.Base64ToBinary(this.Encode(*this.At(MkString("secret"))))
		_ = secret
		auth := OpAdd(method, OpAdd(request, nonce))
		_ = auth
		...
		signature := this.Hmac(this.Encode(auth), secret, MkString("sha512"), MkString("base64"))
		_ = signature
...
func (this *Btcmarkets) HandleErrors(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpHasMember(MkString("success"), response)) {
		if IsTrue(OpNot(*(response).At(MkString("success")))) {
			error := this.SafeString(response, MkString("errorCode"))
			_ = error
