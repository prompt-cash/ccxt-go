...
func (this *Bitfinex2) FetchStatus(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("publicGetPlatformStatus"), params)
	_ = response
	status := this.SafeInteger(response, MkInteger(0))
	_ = status
	formattedStatus := OpTrinary(OpEq2(status, MkInteger(1)), MkString("ok"), MkString("maintenance"))
	_ = formattedStatus
...
func (this *Bitfinex2) FetchMarkets(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	v2response := this.Call(MkString("publicGetConfPubListPairFutures"), params)
	_ = v2response
	v1response := this.Call(MkString("v1GetSymbolsDetails"), params)
	_ = v1response
	futuresMarketIds := this.SafeValue(v2response, MkInteger(0), MkArray(&VarArray{}))
	_ = futuresMarketIds
	result := MkArray(&VarArray{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, v1response.Length)); OpIncr(&i) {
		market := *(v1response).At(i)
		_ = market
		id := this.SafeStringUpper(market, MkString("pair"))
		_ = id
		...
		futures := OpNot(spot)
		_ = futures
		type_ := OpTrinary(spot, MkString("spot"), MkString("futures"))
		_ = type_
		...
		_ = precision
		minOrderSizeString := this.SafeString(market, MkString("minimum_order_size"))
		_ = minOrderSizeString
		maxOrderSizeString := this.SafeString(market, MkString("maximum_order_size"))
		_ = maxOrderSizeString
		limits := MkMap(&VarMap{
			"amount": MkMap(&VarMap{
...
func (this *Bitfinex2) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	_ = labels
	config := labels.Join(MkString(","))
	_ = config
	request := MkMap(&VarMap{"config": config})
	_ = request
	response := this.Call(MkString("publicGetConfConfig"), this.Extend(request, params))
	_ = response
	...
	_ = indexed
	ids := this.SafeValue(response, MkInteger(0), MkArray(&VarArray{}))
	_ = ids
	result := MkMap(&VarMap{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		...
		_ = code
		label := this.SafeValue(*(indexed).At(MkString("label")), id, MkArray(&VarArray{}))
		_ = label
		name := this.SafeString(label, MkInteger(1))
		_ = name
		pool := this.SafeValue(*(indexed).At(MkString("pool")), id, MkArray(&VarArray{}))
		_ = pool
		type_ := this.SafeString(pool, MkInteger(1))
		_ = type_
		feeValues := this.SafeValue(*(indexed).At(MkString("fees")), id, MkArray(&VarArray{}))
		_ = feeValues
		fees := this.SafeValue(feeValues, MkInteger(1), MkArray(&VarArray{}))
		_ = fees
		fee := this.SafeNumber(fees, MkInteger(1))
		_ = fee
		undl := this.SafeValue(*(indexed).At(MkString("undl")), id, MkArray(&VarArray{}))
		_ = undl
		precision := MkInteger(8)
		_ = precision
		fid := OpAdd(MkString("f"), id)
		_ = fid
...
func (this *Bitfinex2) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	accountsByType := this.SafeValue(*this.At(MkString("options")), MkString("v2AccountsByType"), MkMap(&VarMap{}))
	_ = accountsByType
	...
	_ = query
	response := this.Call(MkString("privatePostAuthRWallets"), query)
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = balance
		type_ := this.SafeString(balance, MkInteger(0))
		_ = type_
		currencyId := this.SafeStringLower(balance, MkInteger(1), MkString(""))
		_ = currencyId
...
func (this *Bitfinex2) Transfer(goArgs ...*Variant) *Variant {
	...
	_ = currency
	fromCurrencyId := this.ConvertDerivativesId(currency, fromAccount)
	_ = fromCurrencyId
	toCurrencyId := this.ConvertDerivativesId(currency, toAccount)
	_ = toCurrencyId
	...
	_ = request
	response := this.Call(MkString("privatePostAuthWTransfer"), this.Extend(request, params))
	_ = response
	timestamp := this.SafeInteger(response, MkInteger(0))
	_ = timestamp
	error := this.SafeString(response, MkInteger(0))
	_ = error
	if IsTrue(OpEq2(error, MkString("error"))) {
		message := this.SafeString(response, MkInteger(2), MkString(""))
		_ = message
	...
	info := this.SafeValue(response, MkInteger(4))
	_ = info
	fromResponse := this.SafeString(info, MkInteger(1))
	_ = fromResponse
	toResponse := this.SafeString(info, MkInteger(2))
	_ = toResponse
	toCode := this.SafeCurrencyCode(this.SafeString(info, MkInteger(5)))
	_ = toCode
	success := this.SafeString(response, MkInteger(6))
	_ = success
	status := OpTrinary(OpEq2(success, MkString("SUCCESS")), MkString("ok"), MkUndefined())
	_ = status
	return MkMap(&VarMap{
		"info":        response,
		"timestamp":   timestamp,
...
func (this *Bitfinex2) ConvertDerivativesId(goArgs ...*Variant) *Variant {
	...
	_ = type_
	info := this.SafeValue(currency, MkString("info"))
	_ = info
	transferId := this.SafeString(info, MkInteger(0))
	_ = transferId
	underlying := this.SafeValue(info, MkInteger(4), MkArray(&VarArray{}))
	_ = underlying
...
func (this *Bitfinex2) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	precision := this.SafeValue(*this.At(MkString("options")), MkString("precision"), MkString("R0"))
	_ = precision
	request := MkMap(&VarMap{
	...
	_ = fullRequest
	orderbook := this.Call(MkString("publicGetBookSymbolPrecision"), fullRequest)
	_ = orderbook
	...
	_ = result
	priceIndex := OpTrinary(OpEq2(*(fullRequest).At(MkString("precision")), MkString("R0")), MkInteger(1), MkInteger(0))
	_ = priceIndex
	for i := MkInteger(0); IsTrue(OpLw(i, orderbook.Length)); OpIncr(&i) {
		order := *(orderbook).At(i)
		_ = order
		price := this.SafeNumber(order, priceIndex)
		_ = price
		signedAmount := this.SafeNumber(order, MkInteger(2))
		_ = signedAmount
		amount := Math.Abs(signedAmount)
		_ = amount
		side := OpTrinary(OpGt(signedAmount, MkInteger(0)), MkString("bids"), MkString("asks"))
		_ = side
...
func (this *Bitfinex2) ParseTicker(goArgs ...*Variant) *Variant {
	...
	length := OpCopy(ticker.Length)
	_ = length
	last := this.SafeNumber(ticker, OpSub(length, MkInteger(4)))
	_ = last
	return MkMap(&VarMap{
		"symbol":        symbol,
		"timestamp":     timestamp,
...
func (this *Bitfinex2) FetchTickers(goArgs ...*Variant) *Variant {
	...
	tickers := this.Call(MkString("publicGetTickers"), this.Extend(request, params))
	_ = tickers
	result := MkMap(&VarMap{})
	_ = result
...
func (this *Bitfinex2) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = request
	ticker := this.Call(MkString("publicGetTickerSymbol"), this.Extend(request, params))
	_ = ticker
...
func (this *Bitfinex2) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = tradeLength
	isPrivate := OpGt(tradeLength, MkInteger(5))
	_ = isPrivate
	id := this.SafeString(trade, MkInteger(0))
	_ = id
	amountIndex := OpTrinary(isPrivate, MkInteger(4), MkInteger(2))
	_ = amountIndex
	side := OpCopy(MkUndefined())
	_ = side
	amountString := this.SafeString(trade, amountIndex)
	_ = amountString
	priceIndex := OpTrinary(isPrivate, MkInteger(5), MkInteger(3))
	_ = priceIndex
	priceString := this.SafeString(trade, priceIndex)
	_ = priceString
	...
	symbol := OpCopy(MkUndefined())
	_ = symbol
	timestampIndex := OpTrinary(isPrivate, MkInteger(2), MkInteger(1))
	_ = timestampIndex
	timestamp := this.SafeInteger(trade, timestampIndex)
	_ = timestamp
	if IsTrue(isPrivate) {
		marketId := *(trade).At(MkInteger(1))
		_ = marketId
		...
		orderType := *(trade).At(MkInteger(6))
		_ = orderType
...
func (this *Bitfinex2) FetchTrades(goArgs ...*Variant) *Variant {
	...
	*(request).At(MkString("sort")) = OpCopy(sort)
	response := this.Call(MkString("publicGetTradesSymbolHist"), this.Extend(request, params))
	_ = response
	trades := this.SortBy(response, MkInteger(1))
	_ = trades
...
func (this *Bitfinex2) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	_ = since
	limit := GoGetArg(goArgs, 3, MkInteger(100))
	_ = limit
	params := GoGetArg(goArgs, 4, MkMap(&VarMap{}))
	_ = params
	this.LoadMarkets()
	...
	_ = request
	response := this.Call(MkString("publicGetCandlesTradeTimeframeSymbolHist"), this.Extend(request, params))
	_ = response
...
func (this *Bitfinex2) ParseOrderStatus(goArgs ...*Variant) *Variant {
	...
	_ = parts
	state := this.SafeString(parts, MkInteger(0))
	_ = state
...
func (this *Bitfinex2) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	id := this.SafeString(order, MkInteger(0))
	_ = id
	symbol := OpCopy(MkUndefined())
	_ = symbol
	marketId := this.SafeString(order, MkInteger(3))
	_ = marketId
	if IsTrue(OpHasMember(marketId, *this.At(MkString("markets_by_id")))) {
		market = *(*this.At(MkString("markets_by_id"))).At(marketId)
	} else {
	...
	timestamp := this.SafeInteger(order, MkInteger(5))
	_ = timestamp
	remaining := Math.Abs(this.SafeNumber(order, MkInteger(6)))
	_ = remaining
	signedAmount := this.SafeNumber(order, MkInteger(7))
	_ = signedAmount
	amount := Math.Abs(signedAmount)
	_ = amount
	side := OpTrinary(OpLw(signedAmount, MkInteger(0)), MkString("sell"), MkString("buy"))
	_ = side
	orderType := this.SafeString(order, MkInteger(8))
	_ = orderType
	type_ := this.SafeString(this.SafeValue(*this.At(MkString("options")), MkString("exchangeTypes")), orderType)
	_ = type_
	status := OpCopy(MkUndefined())
	_ = status
	statusString := this.SafeString(order, MkInteger(13))
	_ = statusString
	if IsTrue(OpNotEq2(statusString, MkUndefined())) {
		parts := statusString.Split(MkString(" @ "))
		_ = parts
		status = this.ParseOrderStatus(this.SafeString(parts, MkInteger(0)))
	}
	price := this.SafeNumber(order, MkInteger(16))
	_ = price
	average := this.SafeNumber(order, MkInteger(17))
	_ = average
	clientOrderId := this.SafeString(order, MkInteger(2))
	_ = clientOrderId
...
func (this *Bitfinex2) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = orderTypes
	orderType := this.SafeStringUpper(orderTypes, type_, type_)
	_ = orderType
	...
	if IsTrue(OpOr(OpEq2(orderType, MkString("LIMIT")), OpEq2(orderType, MkString("EXCHANGE LIMIT")))) {
		*(request).At(MkString("price")) = this.NumberToString(price)
		...
		if IsTrue(OpOr(OpEq2(orderType, MkString("STOP")), OpEq2(orderType, MkString("EXCHANGE STOP")))) {
			stopPrice := this.SafeNumber(params, MkString("stopPrice"), price)
			_ = stopPrice
			*(request).At(MkString("price")) = this.NumberToString(stopPrice)
			...
			if IsTrue(OpOr(OpEq2(orderType, MkString("STOP LIMIT")), OpEq2(orderType, MkString("EXCHANGE STOP LIMIT")))) {
				priceAuxLimit := this.SafeNumber(params, MkString("price_aux_limit"))
				_ = priceAuxLimit
				...
				*(request).At(MkString("price")) = this.NumberToString(stopPrice)
				...
				if IsTrue(OpOr(OpEq2(orderType, MkString("TRAILING STOP")), OpEq2(orderType, MkString("EXCHANGE TRAILING STOP")))) {
					priceTrailing := this.SafeNumber(params, MkString("price_trailing"))
					_ = priceTrailing
	...
	clientOrderId := this.SafeValue2(params, MkString("cid"), MkString("clientOrderId"))
	_ = clientOrderId
	...
	response := this.Call(MkString("privatePostAuthWOrderSubmit"), this.Extend(request, params))
	_ = response
	status := this.SafeString(response, MkInteger(6))
	_ = status
	if IsTrue(OpNotEq2(status, MkString("SUCCESS"))) {
		errorCode := *(response).At(MkInteger(5))
		_ = errorCode
		errorText := *(response).At(MkInteger(7))
		_ = errorText
	...
	orders := this.SafeValue(response, MkInteger(4), MkArray(&VarArray{}))
	_ = orders
...
func (this *Bitfinex2) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 1, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"all": MkInteger(1)})
	_ = request
	response := this.Call(MkString("privatePostAuthWOrderCancelMulti"), this.Extend(request, params))
	_ = response
	orders := this.SafeValue(response, MkInteger(4), MkArray(&VarArray{}))
	_ = orders
...
func (this *Bitfinex2) CancelOrder(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 2, MkMap(&VarMap{}))
	_ = params
	cid := this.SafeValue2(params, MkString("cid"), MkString("clientOrderId"))
	_ = cid
	request := OpCopy(MkUndefined())
	_ = request
	if IsTrue(OpNotEq2(cid, MkUndefined())) {
		cidDate := this.SafeValue(params, MkString("cidDate"))
		_ = cidDate
	...
	response := this.Call(MkString("privatePostAuthWOrderCancel"), this.Extend(request, params))
	_ = response
	order := this.SafeValue(response, MkInteger(4))
	_ = order
...
func (this *Bitfinex2) FetchOpenOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	orders := this.FetchOpenOrders(symbol, MkUndefined(), MkUndefined(), this.Extend(request, params))
	_ = orders
	order := this.SafeValue(orders, MkInteger(0))
	_ = order
...
func (this *Bitfinex2) FetchClosedOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	orders := this.FetchClosedOrders(symbol, MkUndefined(), MkUndefined(), this.Extend(request, params))
	_ = orders
	order := this.SafeValue(orders, MkInteger(0))
	_ = order
	if IsTrue(OpEq2(order, MkUndefined())) {
...
func (this *Bitfinex2) FetchOrderTrades(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostAuthROrderSymbolIdTrades"), this.Extend(request, params))
	_ = response
...
func (this *Bitfinex2) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	market := OpCopy(MkUndefined())
	_ = market
	request := MkMap(&VarMap{"end": this.Milliseconds()})
	_ = request
	if IsTrue(OpNotEq2(since, MkUndefined())) {
		*(request).At(MkString("start")) = OpCopy(since)
	}
	...
	method := MkString("privatePostAuthRTradesHist")
	_ = method
...
func (this *Bitfinex2) CreateDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"op_renew": MkInteger(1)})
	_ = request
...
func (this *Bitfinex2) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	name := this.Call(MkString("getCurrencyName"), code)
	_ = name
	request := MkMap(&VarMap{
		"method":   name,
	...
	_ = request
	response := this.Call(MkString("privatePostAuthWDepositAddress"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkInteger(4), MkArray(&VarArray{}))
	_ = result
	poolAddress := this.SafeString(result, MkInteger(5))
	_ = poolAddress
	address := OpTrinary(OpEq2(poolAddress, MkUndefined()), this.SafeString(result, MkInteger(4)), poolAddress)
	_ = address
	tag := OpTrinary(OpEq2(poolAddress, MkUndefined()), MkUndefined(), this.SafeString(result, MkInteger(4)))
	_ = tag
...
func (this *Bitfinex2) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = currency
	transactionLength := OpCopy(transaction.Length)
	_ = transactionLength
	...
	_ = txid
	addressTo := OpCopy(MkUndefined())
	_ = addressTo
	if IsTrue(OpLw(transactionLength, MkInteger(9))) {
		data := this.SafeValue(transaction, MkInteger(4), MkArray(&VarArray{}))
		_ = data
...
func (this *Bitfinex2) FetchTransactions(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	method := MkString("privatePostAuthRMovementsHist")
	_ = method
...
func (this *Bitfinex2) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostAuthWWithdraw"), this.Extend(request, params))
	_ = response
	text := this.SafeString(response, MkInteger(7))
	_ = text
...
func (this *Bitfinex2) Sign(goArgs ...*Variant) *Variant {
	...
	url := OpAdd(*(*(*this.At(MkString("urls"))).At(MkString("api"))).At(api), OpAdd(MkString("/"), request))
	_ = url
	...
	if IsTrue(OpEq2(api, MkString("private"))) {
		...
		auth := OpAdd(MkString("/api/"), OpAdd(request, OpAdd(nonce, body)))
		_ = auth
		signature := this.Hmac(this.Encode(auth), this.Encode(*this.At(MkString("secret"))), MkString("sha384"))
		_ = signature
...
func (this *Bitfinex2) HandleErrors(goArgs ...*Variant) *Variant {
	statusCode := GoGetArg(goArgs, 0, MkUndefined())
	_ = statusCode
	statusText := GoGetArg(goArgs, 1, MkUndefined())
	_ = statusText
	url := GoGetArg(goArgs, 2, MkUndefined())
	_ = url
	method := GoGetArg(goArgs, 3, MkUndefined())
	_ = method
	responseHeaders := GoGetArg(goArgs, 4, MkUndefined())
	_ = responseHeaders
	responseBody := GoGetArg(goArgs, 5, MkUndefined())
	_ = responseBody
	...
	if IsTrue(OpEq2(statusCode, MkInteger(500))) {
		errorCode := this.NumberToString(*(response).At(MkInteger(1)))
		_ = errorCode
		errorText := *(response).At(MkInteger(2))
		_ = errorText
		feedback := OpAdd(*this.At(MkString("id")), OpAdd(MkString(" "), errorText))
		_ = feedback
