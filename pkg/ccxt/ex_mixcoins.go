...
func (this *Mixcoins) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  response := this.Call(MkString("privatePostInfo"), params ); _ = response;
  balances := this.SafeValue(*(response ).At(MkString("result") ), MkString("wallet") ); _ = balances;
...
func (this *Mixcoins) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  ticker := this.SafeValue(response , MkString("result") ); _ = ticker;
