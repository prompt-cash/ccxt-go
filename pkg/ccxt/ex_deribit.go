...
func (this *Deribit) FetchTime(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetGetTime"), params ); _ = response;
  return this.SafeInteger(response , MkString("result") );
}

func (this *Deribit) CodeFromOptions(goArgs ...*Variant) *Variant{
  methodName := GoGetArg(goArgs, 0, MkUndefined()); _ = methodName;
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  defaultCode := this.SafeValue(*this.At(MkString("options")) , MkString("code") , MkString("BTC") ); _ = defaultCode;
  options := this.SafeValue(*this.At(MkString("options")) , methodName , MkMap(&VarMap{})); _ = options;
  code := this.SafeValue(options , MkString("code") , defaultCode ); _ = code;
...
func (this *Deribit) FetchMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  currenciesResponse := this.Call(MkString("publicGetGetCurrencies"), params ); _ = currenciesResponse;
  currenciesResult := this.SafeValue(currenciesResponse , MkString("result") , MkArray(&VarArray{})); _ = currenciesResult;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , currenciesResult.Length )); OpIncr(& i ){
    currencyId := this.SafeString(*(currenciesResult ).At(i ), MkString("currency") ); _ = currencyId;
    request := MkMap(&VarMap{"currency":currencyId }); _ = request;
    instrumentsResponse := this.Call(MkString("publicGetGetInstruments"), this.Extend(request , params )); _ = instrumentsResponse;
    instrumentsResult := this.SafeValue(instrumentsResponse , MkString("result") , MkArray(&VarArray{})); _ = instrumentsResult;
    for k := MkInteger(0) ; IsTrue(OpLw(k , instrumentsResult.Length )); OpIncr(& k ){
      market := *(instrumentsResult ).At(k ); _ = market;
      id := this.SafeString(market , MkString("instrument_name") ); _ = id;
      ...
      type_ := this.SafeString(market , MkString("kind") ); _ = type_;
      future := OpEq2(type_ , MkString("future") ); _ = future;
      option := OpEq2(type_ , MkString("option") ); _ = option;
      active := this.SafeValue(market , MkString("is_active") ); _ = active;
      minTradeAmount := this.SafeNumber(market , MkString("min_trade_amount") ); _ = minTradeAmount;
      tickSize := this.SafeNumber(market , MkString("tick_size") ); _ = tickSize;
...
func (this *Deribit) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  code := this.CodeFromOptions(MkString("fetchBalance") , params ); _ = code;
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetGetAccountSummary"), this.Extend(request , params )); _ = response;
  result := MkMap(&VarMap{"info":response }); _ = result;
  balance := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = balance;
...
func (this *Deribit) CreateDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetCreateDepositAddress"), this.Extend(request , params )); _ = response;
...
func (this *Deribit) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetGetCurrentDepositAddress"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  address := this.SafeString(result , MkString("address") ); _ = address;
  this.CheckAddress(address );
  return MkMap(&VarMap{
...
func (this *Deribit) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger2(ticker , MkString("timestamp") , MkString("creation_timestamp") ); _ = timestamp;
  marketId := this.SafeString(ticker , MkString("instrument_name") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  last := this.SafeNumber2(ticker , MkString("last_price") , MkString("last") ); _ = last;
  stats := this.SafeValue(ticker , MkString("stats") , ticker ); _ = stats;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Deribit) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"instrument_name":*(market ).At(MkString("id") )}); _ = request;
...
func (this *Deribit) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  code := this.CodeFromOptions(MkString("fetchTickers") , params ); _ = code;
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetGetBookSummaryByCurrency"), this.Extend(request , params )); _ = response;
...
func (this *Deribit) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetGetTradingviewChartData"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  ohlcvs := this.ConvertTradingViewToOHLCV(result , MkString("ticks") , MkString("open") , MkString("high") , MkString("low") , MkString("close") , MkString("volume") , MkBool(true) ); _ = ohlcvs;
...
func (this *Deribit) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(trade , MkString("instrument_name") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  ...
  liquidity := this.SafeString(trade , MkString("liquidity") ); _ = liquidity;
...
func (this *Deribit) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  method := OpTrinary(OpEq2(since , MkUndefined() ), MkString("publicGetGetLastTradesByInstrument") , MkString("publicGetGetLastTradesByInstrumentAndTime") ); _ = method;
...
func (this *Deribit) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  timestamp := this.SafeInteger(result , MkString("timestamp") ); _ = timestamp;
  nonce := this.SafeInteger(result , MkString("change_id") ); _ = nonce;
  orderbook := this.ParseOrderBook(result , symbol , timestamp ); _ = orderbook;
...
func (this *Deribit) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeInteger(order , MkString("creation_timestamp") ); _ = timestamp;
  lastUpdate := this.SafeInteger(order , MkString("last_update_timestamp") ); _ = lastUpdate;
  ...
  status := this.ParseOrderStatus(this.SafeString(order , MkString("order_state") )); _ = status;
  marketId := this.SafeString(order , MkString("instrument_name") ); _ = marketId;
  market = this.SafeMarket(marketId , market );
  side := this.SafeStringLower(order , MkString("direction") ); _ = side;
  feeCost := this.SafeNumber(order , MkString("commission") ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
  ...
  stopPrice := this.SafeValue(order , MkString("stop_price") ); _ = stopPrice;
...
func (this *Deribit) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetOrderState"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") ); _ = result;
...
func (this *Deribit) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  method := OpAdd(MkString("privateGet") , this.Capitalize(side )); _ = method;
  response := this.Call(method , this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  order := this.SafeValue(result , MkString("order") ); _ = order;
  trades := this.SafeValue(result , MkString("trades") , MkArray(&VarArray{})); _ = trades;
...
func (this *Deribit) EditOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetEdit"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  order := this.SafeValue(result , MkString("order") ); _ = order;
  trades := this.SafeValue(result , MkString("trades") , MkArray(&VarArray{})); _ = trades;
  *(order ).At(MkString("trades") )= OpCopy(trades );
...
func (this *Deribit) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"order_id":id }); _ = request;
  response := this.Call(MkString("privateGetCancel"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  return this.ParseOrder(result );
}
...
func (this *Deribit) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  method := OpCopy(MkUndefined() ); _ = method;
  if IsTrue(OpEq2(symbol , MkUndefined() )) {
    code := this.CodeFromOptions(MkString("fetchOpenOrders") , params ); _ = code;
    currency := this.Currency(code ); _ = currency;
...
func (this *Deribit) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  method := OpCopy(MkUndefined() ); _ = method;
  if IsTrue(OpEq2(symbol , MkUndefined() )) {
    code := this.CodeFromOptions(MkString("fetchClosedOrders") , params ); _ = code;
    currency := this.Currency(code ); _ = currency;
    *(request ).At(MkString("currency") )= *(currency ).At(MkString("id") );
...
func (this *Deribit) FetchOrderTrades(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"order_id":id }); _ = request;
  response := this.Call(MkString("privateGetGetUserTradesByOrder"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  trades := this.SafeValue(result , MkString("trades") , MkArray(&VarArray{})); _ = trades;
...
func (this *Deribit) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"include_old":MkBool(true) }); _ = request;
  market := OpCopy(MkUndefined() ); _ = market;
  method := OpCopy(MkUndefined() ); _ = method;
  if IsTrue(OpEq2(symbol , MkUndefined() )) {
    code := this.CodeFromOptions(MkString("fetchMyTrades") , params ); _ = code;
    currency := this.Currency(code ); _ = currency;
    *(request ).At(MkString("currency") )= *(currency ).At(MkString("id") );
    if IsTrue(OpEq2(since , MkUndefined() )) {
...
func (this *Deribit) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetWithdrawals"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  data := this.SafeValue(result , MkString("data") , MkArray(&VarArray{})); _ = data;
  return this.ParseTransactions(data , currency , since , limit , params );
}
...
func (this *Deribit) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger2(transaction , MkString("created_timestamp") , MkString("received_timestamp") ); _ = timestamp;
  updated := this.SafeInteger(transaction , MkString("updated_timestamp") ); _ = updated;
  ...
  type_ := MkString("deposit") ; _ = type_;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
...
func (this *Deribit) FetchPosition(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"instrument_name":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetGetPosition"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") ); _ = result;
  return result ;
}
...
func (this *Deribit) FetchPositions(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  code := this.CodeFromOptions(MkString("fetchPositions") , params ); _ = code;
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetGetPositions"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkArray(&VarArray{})); _ = result;
  return result ;
}
...
func (this *Deribit) Sign(goArgs ...*Variant) *Variant{
  ...
  request := OpAdd(MkString("/") , OpAdd(MkString("api/") , OpAdd(*this.At(MkString("version")) , OpAdd(MkString("/") , OpAdd(api , OpAdd(MkString("/") , path )))))); _ = request;
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    requestBody := MkString("") ; _ = requestBody;
    if IsTrue(*(GoGetKeys(params )).At(MkString("length") )) {
      OpAddAssign(& request , OpAdd(MkString("?") , this.Urlencode(params )));
    }
