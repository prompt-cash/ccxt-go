...
func (this *Coinmate) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, data.Length)); OpIncr(&i) {
		...
		_ = id
		baseId := this.SafeString(market, MkString("firstCurrency"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("secondCurrency"))
		_ = quoteId
		...
		_ = symbol
		promotionalMarkets := this.SafeValue(*this.At(MkString("options")), MkString("promotionalMarkets"), MkArray(&VarArray{}))
		_ = promotionalMarkets
		fees := this.SafeValue(*this.At(MkString("fees")), MkString("trading"))
		_ = fees
		if IsTrue(this.InArray(symbol, promotionalMarkets)) {
			promotionalFees := this.SafeValue(*this.At(MkString("fees")), MkString("promotional"), MkMap(&VarMap{}))
			_ = promotionalFees
...
func (this *Coinmate) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privatePostBalances"), params)
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, currencyIds.Length)); OpIncr(&i) {
		...
		_ = code
		balance := this.SafeValue(balances, currencyId)
		_ = balance
...
func (this *Coinmate) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = response
	orderbook := *(response).At(MkString("data"))
	_ = orderbook
...
func (this *Coinmate) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = response
	ticker := this.SafeValue(response, MkString("data"))
	_ = ticker
...
func (this *Coinmate) FetchTransactions(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"limit": MkInteger(1000)})
	_ = request
	...
	response := this.Call(MkString("privatePostTransferHistory"), this.Extend(request, params))
	_ = response
	items := *(response).At(MkString("data"))
	_ = items
...
func (this *Coinmate) ParseTransactionStatus(goArgs ...*Variant) *Variant {
	...
	_ = status
	statuses := MkMap(&VarMap{"COMPLETED": MkString("ok")})
	_ = statuses
...
func (this *Coinmate) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = amount
	fee := this.SafeNumber(item, MkString("fee"))
	_ = fee
	txid := this.SafeString(item, MkString("txid"))
	_ = txid
	address := this.SafeString(item, MkString("destination"))
	_ = address
	tag := this.SafeString(item, MkString("destinationTag"))
	_ = tag
	currencyId := this.SafeString(item, MkString("amountCurrency"))
	_ = currencyId
	code := this.SafeCurrencyCode(currencyId, currency)
	_ = code
	type_ := this.SafeStringLower(item, MkString("transferType"))
	_ = type_
	status := this.ParseTransactionStatus(this.SafeString(item, MkString("transferStatus")))
	_ = status
...
func (this *Coinmate) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"limit": limit})
	_ = request
...
func (this *Coinmate) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = cost
	side := this.SafeStringLower2(trade, MkString("type"), MkString("tradeType"))
	_ = side
	type_ := this.SafeStringLower(trade, MkString("orderType"))
	_ = type_
	orderId := this.SafeString(trade, MkString("orderId"))
	_ = orderId
	id := this.SafeString(trade, MkString("transactionId"))
	_ = id
	timestamp := this.SafeInteger2(trade, MkString("timestamp"), MkString("createdTimestamp"))
	_ = timestamp
	...
	takerOrMaker := this.SafeString(trade, MkString("feeType"))
	_ = takerOrMaker
...
func (this *Coinmate) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetTransactions"), this.Extend(request, params))
	_ = response
...
func (this *Coinmate) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("privatePostOpenOrders"), this.Extend(MkMap(&VarMap{}), params))
	_ = response
...
func (this *Coinmate) FetchOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostOrderHistory"), this.Extend(request, params))
	_ = response
...
func (this *Coinmate) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = price
	amount := this.SafeNumber(order, MkString("originalAmount"))
	_ = amount
	remaining := this.SafeNumber(order, MkString("remainingAmount"))
	_ = remaining
	...
	_ = status
	type_ := this.ParseOrderType(this.SafeString(order, MkString("orderTradeType")))
	_ = type_
...
func (this *Coinmate) FetchOrder(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostOrderById"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"))
	_ = data
...
func (this *Coinmate) CancelOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"orderId": id})
	_ = request
	response := this.Call(MkString("privatePostCancelOrderWithInfo"), this.Extend(request, params))
	_ = response
...
func (this *Coinmate) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	url := OpAdd(*(*this.At(MkString("urls"))).At(MkString("api")), OpAdd(MkString("/"), path))
	_ = url
	...
	if IsTrue(OpNotEq2(response, MkUndefined())) {
		if IsTrue(OpHasMember(MkString("error"), response)) {
			if IsTrue(*(response).At(MkString("error"))) {
				message := this.SafeString(response, MkString("errorMessage"))
				_ = message
