...
func (this *Coinegg) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  quoteIds := *(*this.At(MkString("options")) ).At(MkString("quoteIds") ); _ = quoteIds;
  result := MkArray(&VarArray{}); _ = result;
  for b := MkInteger(0) ; IsTrue(OpLw(b , quoteIds.Length )); OpIncr(& b ){
    quoteId := *(quoteIds ).At(b ); _ = quoteId;
    response := this.Call(MkString("webGetSymbolTickerRightCoinQuote"), MkMap(&VarMap{"quote":quoteId })); _ = response;
    ...
    for i := MkInteger(0) ; IsTrue(OpLw(i , tickers.Length )); OpIncr(& i ){
      ...
      id := *(ticker ).At(MkString("symbol") ); _ = id;
      baseId := *(id.Split(MkString("_") )).At(MkInteger(0) ); _ = baseId;
...
func (this *Coinegg) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(percentage , MkUndefined() )) {
    relativeChange := OpDiv(percentage , MkInteger(100) ); _ = relativeChange;
...
func (this *Coinegg) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTickerRegionQuote"), this.Extend(request , params )); _ = response;
...
func (this *Coinegg) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetDepthRegionQuote"), this.Extend(request , params )); _ = response;
...
func (this *Coinegg) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetOrdersRegionQuote"), this.Extend(request , params )); _ = response;
...
func (this *Coinegg) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  balances := this.Omit(data , MkString("uid") ); _ = balances;
  keys := GoGetKeys(balances ); _ = keys;
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    key := *(keys ).At(i ); _ = key;
    currencyId := MkUndefined(); _ = currencyId;
    accountType := MkUndefined(); _ = accountType;
    ...
    type_ := OpTrinary(OpEq2(accountType , MkString("lock") ), MkString("used") , MkString("free") ); _ = type_;
...
func (this *Coinegg) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  amount := this.SafeNumber(order , MkString("amount_original") ); _ = amount;
  remaining := this.SafeNumber(order , MkString("amount_outstanding") ); _ = remaining;
  ...
  info := this.SafeValue(order , MkString("info") , order ); _ = info;
  type_ := MkString("limit") ; _ = type_;
...
func (this *Coinegg) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostTradeAddRegionQuote"), this.Extend(request , params )); _ = response;
...
func (this *Coinegg) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostTradeViewRegionQuote"), this.Extend(request , params )); _ = response;
...
func (this *Coinegg) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostTradeListRegionQuote"), this.Extend(request , params )); _ = response;
...
func (this *Coinegg) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"type":MkString("open") }); _ = request;
...
func (this *Coinegg) Sign(goArgs ...*Variant) *Variant{
  ...
  apiType := MkString("rest") ; _ = apiType;
  if IsTrue(OpEq2(api , MkString("web") )) {
    apiType = OpCopy(api );
  }
  url := OpAdd(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(apiType ), OpAdd(MkString("/") , this.ImplodeParams(path , params ))); _ = url;
  ...
  if IsTrue(OpOr(OpEq2(api , MkString("public") ), OpEq2(api , MkString("web") ))) {
    ...
    secret := this.Hash(this.Encode(*this.At(MkString("secret")) )); _ = secret;
    signature := this.Hmac(this.Encode(query ), this.Encode(secret )); _ = signature;
...
func (this *Coinegg) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  errorMessages := OpCopy(*this.At(MkString("errorMessages")) ); _ = errorMessages;
  message := this.SafeString(errorMessages , errorCode , MkString("Unknown Error") ); _ = message;
