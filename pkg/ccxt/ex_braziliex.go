...
func (this *Braziliex) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  *(*this.At(MkString("options")) ).At(MkString("currencies") )= MkMap(&VarMap{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    ...
    precision := this.SafeInteger(currency , MkString("decimal") ); _ = precision;
    code := this.SafeCurrencyCode(id ); _ = code;
    active := OpEq2(this.SafeInteger(currency , MkString("active") ), MkInteger(1) ); _ = active;
    maintenance := this.SafeInteger(currency , MkString("under_maintenance") ); _ = maintenance;
    if IsTrue(OpNotEq2(maintenance , MkInteger(0) )) {
      active = OpCopy(MkBool(false) );
    }
    canWithdraw := OpEq2(this.SafeInteger(currency , MkString("is_withdrawal_active") ), MkInteger(1) ); _ = canWithdraw;
    canDeposit := OpEq2(this.SafeInteger(currency , MkString("is_deposit_active") ), MkInteger(1) ); _ = canDeposit;
...
func (this *Braziliex) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTicker")); _ = response;
  ids := GoGetKeys(response ); _ = ids;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    ...
    uppercaseBaseId := baseId.ToUpperCase(); _ = uppercaseBaseId;
    uppercaseQuoteId := quoteId.ToUpperCase(); _ = uppercaseQuoteId;
    base := this.SafeCurrencyCode(uppercaseBaseId ); _ = base;
    quote := this.SafeCurrencyCode(uppercaseQuoteId ); _ = quote;
    symbol := OpAdd(base , OpAdd(MkString("/") , quote )); _ = symbol;
    baseCurrency := this.SafeValue(currencies , baseId , MkMap(&VarMap{})); _ = baseCurrency;
    quoteCurrency := this.SafeValue(currencies , quoteId , MkMap(&VarMap{})); _ = quoteCurrency;
    quoteIsFiat := this.SafeInteger(quoteCurrency , MkString("is_fiat") , MkInteger(0) ); _ = quoteIsFiat;
    ...
    isActive := this.SafeInteger(market , MkString("active") ); _ = isActive;
    active := OpEq2(isActive , MkInteger(1) ); _ = active;
    precision := MkMap(&VarMap{
          "amount":MkInteger(8) ,
          "price":MkInteger(8) ,
          }); _ = precision;
...
func (this *Braziliex) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTickerMarket"), this.Extend(request , params )); _ = response;
...
func (this *Braziliex) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetOrderbookMarket"), this.Extend(request , params )); _ = response;
...
func (this *Braziliex) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.Parse8601(this.SafeString2(trade , MkString("date_exec") , MkString("date") )); _ = timestamp;
  ...
  orderId := this.SafeString(trade , MkString("order_number") ); _ = orderId;
  type_ := MkString("limit") ; _ = type_;
  side := this.SafeString(trade , MkString("type") ); _ = side;
  id := this.SafeString(trade , MkString("_id") ); _ = id;
  return MkMap(&VarMap{
        "id":id ,
        "info":trade ,
        "timestamp":timestamp ,
...
func (this *Braziliex) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"market":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetTradehistoryMarket"), this.Extend(request , params )); _ = response;
...
func (this *Braziliex) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  balances := this.Call(MkString("privatePostCompleteBalance"), params ); _ = balances;
...
func (this *Braziliex) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  cost := this.SafeNumber(order , MkString("total") ); _ = cost;
  amount := this.SafeNumber(order , MkString("amount") ); _ = amount;
  filledPercentage := this.SafeNumber(order , MkString("progress") ); _ = filledPercentage;
  filled := OpMulti(amount , filledPercentage ); _ = filled;
  id := this.SafeString(order , MkString("order_number") ); _ = id;
  fee := this.SafeValue(order , MkString("fee") ); _ = fee;
  status := OpTrinary(OpEq2(filledPercentage , MkNumber(1.0) ), MkString("closed") , MkString("open") ); _ = status;
...
func (this *Braziliex) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  parts := message.Split(MkString(" / ") ); _ = parts;
  parts = parts.Slice(MkInteger(1) );
  feeParts := (*(parts ).At(MkInteger(5) )).Call(MkString("split") , MkString(" ") ); _ = feeParts;
  amountParts := (*(parts ).At(MkInteger(2) )).Call(MkString("split") , MkString(" ") ); _ = amountParts;
  priceParts := (*(parts ).At(MkInteger(3) )).Call(MkString("split") , MkString(" ") ); _ = priceParts;
  totalParts := (*(parts ).At(MkInteger(4) )).Call(MkString("split") , MkString(" ") ); _ = totalParts;
...
func (this *Braziliex) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  orders := this.SafeValue(response , MkString("order_open") , MkArray(&VarArray{})); _ = orders;
...
func (this *Braziliex) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  trades := this.SafeValue(response , MkString("trade_history") , MkArray(&VarArray{})); _ = trades;
...
func (this *Braziliex) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostDepositAddress"), this.Extend(request , params )); _ = response;
  address := this.SafeString(response , MkString("deposit_address") ); _ = address;
  this.CheckAddress(address );
  tag := this.SafeString(response , MkString("payment_id") ); _ = tag;
  return MkMap(&VarMap{
        "currency":code ,
        "address":address ,
        "tag":tag ,
...
func (this *Braziliex) Sign(goArgs ...*Variant) *Variant{
  ...
  url := OpAdd(*(*this.At(MkString("urls")) ).At(MkString("api") ), OpAdd(MkString("/") , api )); _ = url;
