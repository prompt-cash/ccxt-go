...
func (this *Yobit) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = balances
	timestamp := this.SafeInteger(balances, MkString("server_time"))
	_ = timestamp
	result := MkMap(&VarMap{
		"info":      response,
		"timestamp": timestamp,
	...
	_ = result
	free := this.SafeValue(balances, MkString("funds"), MkMap(&VarMap{}))
	_ = free
	total := this.SafeValue(balances, MkString("funds_incl_orders"), MkMap(&VarMap{}))
	_ = total
	currencyIds := GoGetKeys(this.Extend(free, total))
	_ = currencyIds
...
func (this *Yobit) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = response
	markets := this.SafeValue(response, MkString("pairs"))
	_ = markets
	...
	for i := MkInteger(0); IsTrue(OpLw(i, keys.Length)); OpIncr(&i) {
		...
		_ = priceLimits
		costLimits := MkMap(&VarMap{"min": this.SafeNumber(market, MkString("min_total"))})
		_ = costLimits
		limits := MkMap(&VarMap{
			"amount": amountLimits,
			"price":  priceLimits,
		...
		_ = active
		feeString := this.SafeString(market, MkString("fee"))
		_ = feeString
		feeString = Precise.StringDiv(feeString, MkString("100"))
		takerFee := this.ParseNumber(feeString)
		_ = takerFee
		makerFee := this.ParseNumber(feeString)
		_ = makerFee
...
func (this *Yobit) FetchTickers(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"pair": ids})
	_ = request
	tickers := this.Call(MkString("publicGetTickerPair"), this.Extend(request, params))
	_ = tickers
	result := MkMap(&VarMap{})
	_ = result
	keys := GoGetKeys(tickers)
	_ = keys
...
func (this *Yobit) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = response
	id = id.ToString()
	orders := this.SafeValue(response, MkString("return"), MkMap(&VarMap{}))
	_ = orders
...
func (this *Yobit) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	_ = response
	trades := this.SafeValue(response, MkString("return"), MkMap(&VarMap{}))
	_ = trades
	...
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		id := *(ids).At(i)
		_ = id
		trade := this.ParseTrade(this.Extend(*(trades).At(id), MkMap(&VarMap{"trade_id": id})), market)
		_ = trade
		result.Push(trade)
	}
...
func (this *Yobit) CreateDepositAddress(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 1, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"need_new": MkInteger(1)})
	_ = request
...
func (this *Yobit) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostGetDepositAddress"), this.Extend(request, params))
	_ = response
	address := this.SafeString(*(response).At(MkString("return")), MkString("address"))
	_ = address
	this.CheckAddress(address)
	return MkMap(&VarMap{
		"currency": code,
...
func (this *Yobit) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostWithdrawCoinsToAddress"), this.Extend(request, params))
	_ = response
	return MkMap(&VarMap{
		"info": response,
		"id":   MkUndefined(),
