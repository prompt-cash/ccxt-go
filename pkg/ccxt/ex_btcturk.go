...
func (this *Btcturk) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetServerExchangeinfo"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  markets := this.SafeValue(data , MkString("symbols") , MkArray(&VarArray{})); _ = markets;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    entry := *(markets ).At(i ); _ = entry;
    id := this.SafeString(entry , MkString("name") ); _ = id;
    baseId := this.SafeString(entry , MkString("numerator") ); _ = baseId;
    quoteId := this.SafeString(entry , MkString("denominator") ); _ = quoteId;
    ...
    filters := this.SafeValue(entry , MkString("filters") ); _ = filters;
...
func (this *Btcturk) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetUsersBalances"), params ); _ = response;
...
func (this *Btcturk) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"pairSymbol":*(market ).At(MkString("id") )}); _ = request;
...
func (this *Btcturk) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeInteger2(trade , MkString("date") , MkString("timestamp") ); _ = timestamp;
  id := this.SafeString2(trade , MkString("tid") , MkString("id") ); _ = id;
  order := this.SafeString(trade , MkString("orderId") ); _ = order;
  priceString := this.SafeString(trade , MkString("price") ); _ = priceString;
  amountString := Precise.StringAbs(this.SafeString(trade , MkString("amount") )); _ = amountString;
  ...
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  side := this.SafeString2(trade , MkString("side") , MkString("orderType") ); _ = side;
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeAmountString := this.SafeString(trade , MkString("fee") ); _ = feeAmountString;
  if IsTrue(OpNotEq2(feeAmountString , MkUndefined() )) {
    feeCurrency := this.SafeString(trade , MkString("denominatorSymbol") ); _ = feeCurrency;
    fee = MkMap(&VarMap{
...
func (this *Btcturk) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("graphGetOhlcs"), this.Extend(request , params )); _ = response;
...
func (this *Btcturk) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  data := this.SafeValue(response , MkString("data") ); _ = data;
  bids := this.SafeValue(data , MkString("bids") , MkArray(&VarArray{})); _ = bids;
  asks := this.SafeValue(data , MkString("asks") , MkArray(&VarArray{})); _ = asks;
...
func (this *Btcturk) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetAllOrders"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
...
func (this *Btcturk) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  precisePrice := NewPrecise(priceString ); _ = precisePrice;
  price := OpCopy(MkUndefined() ); _ = price;
  isZero := OpEq2(precisePrice.ToString(), MkString("0") ); _ = isZero;
  ...
  amount := this.ParseNumber(Precise.StringAbs(amountString )); _ = amount;
  remaining := this.SafeNumber(order , MkString("leftAmount") ); _ = remaining;
  marketId := this.SafeNumber(order , MkString("pairSymbol") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  side := this.SafeString(order , MkString("type") ); _ = side;
  type_ := this.SafeString(order , MkString("method") ); _ = type_;
  clientOrderId := this.SafeString(order , MkString("orderClientId") ); _ = clientOrderId;
  timestamp := this.SafeInteger2(order , MkString("updateTime") , MkString("datetime") ); _ = timestamp;
...
func (this *Btcturk) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetUsersTransactionsTrade")); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  return this.ParseTrades(data , market , since , limit );
}
...
func (this *Btcturk) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    auth := OpAdd(*this.At(MkString("apiKey")) , nonce ); _ = auth;
...
func (this *Btcturk) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  errorCode := this.SafeString(response , MkString("code") , MkString("0") ); _ = errorCode;
  message := this.SafeString(response , MkString("message") ); _ = message;
  output := OpTrinary(OpEq2(message , MkUndefined() ), body , message ); _ = output;
