...
func (this *Bl3p) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privatePostGENMKTMoneyInfo"), params)
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	wallets := this.SafeValue(data, MkString("wallets"))
	_ = wallets
	result := MkMap(&VarMap{"info": data})
	_ = result
	...
	for i := MkInteger(0); IsTrue(OpLw(i, codes.Length)); OpIncr(&i) {
		...
		_ = currencyId
		wallet := this.SafeValue(wallets, currencyId, MkMap(&VarMap{}))
		_ = wallet
		available := this.SafeValue(wallet, MkString("available"), MkMap(&VarMap{}))
		_ = available
		balance := this.SafeValue(wallet, MkString("balance"), MkMap(&VarMap{}))
		_ = balance
...
func (this *Bl3p) ParseBidAsk(goArgs ...*Variant) *Variant {
	...
	_ = price
	size := this.SafeNumber(bidask, amountKey)
	_ = size
...
func (this *Bl3p) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = request
	ticker := this.Call(MkString("publicGetMarketTicker"), this.Extend(request, params))
	_ = ticker
...
func (this *Bl3p) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = id
	timestamp := this.SafeInteger(trade, MkString("date"))
	_ = timestamp
	priceString := this.SafeString(trade, MkString("price_int"))
	_ = priceString
	priceString = Precise.StringDiv(priceString, MkString("100000"))
	amountString := this.SafeString(trade, MkString("amount_int"))
	_ = amountString
...
func (this *Bl3p) FetchTrades(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	response := this.Call(MkString("publicGetMarketTrades"), this.Extend(MkMap(&VarMap{"market": *(market).At(MkString("id"))}), params))
	_ = response
	result := this.ParseTrades(*(*(response).At(MkString("data"))).At(MkString("trades")), market, since, limit)
	_ = result
	return result
}
...
func (this *Bl3p) CreateOrder(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostMarketMoneyOrderAdd"), this.Extend(order, params))
	_ = response
	orderId := this.SafeString(*(response).At(MkString("data")), MkString("order_id"))
	_ = orderId
	return MkMap(&VarMap{
		"info": response,
		"id":   orderId,
...
func (this *Bl3p) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		_ = secret
		auth := OpAdd(request, OpAdd(MkString("\000"), body))
		_ = auth
