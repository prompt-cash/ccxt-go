...
func (this *Bitfinex) FetchFundingFees(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  response := this.Call(MkString("privatePostAccountFees"), params ); _ = response;
  fees := *(response ).At(MkString("withdraw") ); _ = fees;
  withdraw := MkMap(&VarMap{}); _ = withdraw;
  ids := GoGetKeys(fees ); _ = ids;
...
func (this *Bitfinex) FetchTradingFees(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostSummary"), params ); _ = response;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Bitfinex) FetchMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  ids := this.Call(MkString("publicGetSymbols")); _ = ids;
  details := this.Call(MkString("publicGetSymbolsDetails")); _ = details;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , details.Length )); OpIncr(& i ){
    market := *(details ).At(i ); _ = market;
    ...
    minAmountString := this.SafeString(market , MkString("minimum_order_size") ); _ = minAmountString;
    maxAmountString := this.SafeString(market , MkString("maximum_order_size") ); _ = maxAmountString;
    limits := MkMap(&VarMap{
          "amount":MkMap(&VarMap{
...
func (this *Bitfinex) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostBalances"), query ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    currencyId := this.SafeStringLower(balance , MkString("currency") , MkString("") ); _ = currencyId;
...
func (this *Bitfinex) Transfer(goArgs ...*Variant) *Variant{
  ...
  fromCurrencyId := this.ConvertDerivativesId(*(currency ).At(MkString("id") ), fromAccount ); _ = fromCurrencyId;
  toCurrencyId := this.ConvertDerivativesId(*(currency ).At(MkString("id") ), toAccount ); _ = toCurrencyId;
  ...
  result := this.SafeValue(response , MkInteger(0) ); _ = result;
  status := this.SafeString(result , MkString("status") ); _ = status;
  message := this.SafeString(result , MkString("message") ); _ = message;
...
func (this *Bitfinex) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetBookSymbol"), this.Extend(request , params )); _ = response;
...
func (this *Bitfinex) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  ticker := this.Call(MkString("publicGetPubtickerSymbol"), this.Extend(request , params )); _ = ticker;
...
func (this *Bitfinex) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeNumber(ticker , MkString("timestamp") ); _ = timestamp;
    ...
    if IsTrue(OpHasMember(MkString("pair") , ticker )) {
      ...
      if IsTrue(OpNotEq2(marketId , MkUndefined() )) {
        if IsTrue(OpHasMember(marketId , *this.At(MkString("markets_by_id")) )) {
          ...
          baseId := marketId.Slice(MkInteger(0) , MkInteger(3) ); _ = baseId;
          quoteId := marketId.Slice(MkInteger(3) , MkInteger(6) ); _ = quoteId;
          base := this.SafeCurrencyCode(baseId ); _ = base;
          quote := this.SafeCurrencyCode(quoteId ); _ = quote;
...
func (this *Bitfinex) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("fee_amount") , trade )) {
    feeCost := OpNeg(this.SafeNumber(trade , MkString("fee_amount") )); _ = feeCost;
...
func (this *Bitfinex) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostMytrades"), this.Extend(request , params )); _ = response;
  return this.ParseTrades(response , market , since , limit );
}
...
func (this *Bitfinex) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrderNew"), this.Extend(request , params )); _ = response;
...
func (this *Bitfinex) EditOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  order := MkMap(&VarMap{"order_id":parseInt(id )}); _ = order;
  ...
  response := this.Call(MkString("privatePostOrderCancelReplace"), this.Extend(order , params )); _ = response;
  return this.ParseOrder(response );
}
...
func (this *Bitfinex) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  open := this.SafeValue(order , MkString("is_live") ); _ = open;
  canceled := this.SafeValue(order , MkString("is_cancelled") ); _ = canceled;
  ...
  marketId := this.SafeStringUpper(order , MkString("symbol") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  orderType := this.SafeString(order , MkString("type") , MkString("") ); _ = orderType;
  exchange := OpGtEq(orderType.IndexOf(MkString("exchange ") ), MkInteger(0) ); _ = exchange;
  if IsTrue(exchange ) {
    parts := (*(order ).At(MkString("type") )).Call(MkString("split") , MkString(" ") ); _ = parts;
...
func (this *Bitfinex) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrders"), params ); _ = response;
  orders := this.ParseOrders(response , MkUndefined() , since , limit ); _ = orders;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
...
func (this *Bitfinex) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrdersHist"), this.Extend(request , params )); _ = response;
  orders := this.ParseOrders(response , MkUndefined() , since , limit ); _ = orders;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    orders = this.FilterBy(orders , MkString("symbol") , symbol );
  }
...
func (this *Bitfinex) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  v2id := OpAdd(MkString("t") , *(market ).At(MkString("id") )); _ = v2id;
  request := MkMap(&VarMap{
        "symbol":v2id ,
  ...
  response := this.Call(MkString("v2GetCandlesTradeTimeframeSymbolHist"), this.Extend(request , params )); _ = response;
...
func (this *Bitfinex) CreateDepositAddress(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"renew":MkInteger(1) }); _ = request;
...
func (this *Bitfinex) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostDepositNew"), this.Extend(request , params )); _ = response;
...
func (this *Bitfinex) FetchTransactions(goArgs ...*Variant) *Variant{
  ...
  query := this.Omit(params , MkString("currency") ); _ = query;
  ...
  *(query ).At(MkString("currency") )= OpCopy(currencyId );
  ...
  response := this.Call(MkString("privatePostHistoryMovements"), this.Extend(query , params )); _ = response;
...
func (this *Bitfinex) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeTimestamp(transaction , MkString("timestamp_created") ); _ = timestamp;
  updated := this.SafeTimestamp(transaction , MkString("timestamp") ); _ = updated;
...
func (this *Bitfinex) Withdraw(goArgs ...*Variant) *Variant{
  ...
  responses := this.Call(MkString("privatePostWithdraw"), this.Extend(request , params )); _ = responses;
  response := *(responses ).At(MkInteger(0) ); _ = response;
  id := this.SafeString(response , MkString("withdrawal_id") ); _ = id;
  message := this.SafeString(response , MkString("message") ); _ = message;
  errorMessage := this.FindBroadlyMatchedKey(*(*this.At(MkString("exceptions")) ).At(MkString("broad") ), message ); _ = errorMessage;
  if IsTrue(OpEq2(id , MkInteger(0) )) {
    if IsTrue(OpNotEq2(errorMessage , MkUndefined() )) {
      ExceptionClass := *(*(*this.At(MkString("exceptions")) ).At(MkString("broad") )).At(errorMessage ); _ = ExceptionClass;
...
func (this *Bitfinex) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    payload := this.StringToBase64(body ); _ = payload;
    secret := this.Encode(*this.At(MkString("secret")) ); _ = secret;
    signature := this.Hmac(payload , secret , MkString("sha384") ); _ = signature;
...
func (this *Bitfinex) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpGtEq(code , MkInteger(400) )) {
    if IsTrue(OpEq2(*(body ).At(MkInteger(0) ), MkString("{") )) {
      ...
      message := this.SafeString2(response , MkString("message") , MkString("error") ); _ = message;
