...
func (this *Bigone) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		...
		_ = id
		uuid := this.SafeString(market, MkString("id"))
		_ = uuid
		baseAsset := this.SafeValue(market, MkString("base_asset"), MkMap(&VarMap{}))
		_ = baseAsset
		quoteAsset := this.SafeValue(market, MkString("quote_asset"), MkMap(&VarMap{}))
		_ = quoteAsset
		baseId := this.SafeString(baseAsset, MkString("symbol"))
		_ = baseId
		quoteId := this.SafeString(quoteAsset, MkString("symbol"))
		_ = quoteId
		...
		_ = symbol
		amountPrecisionString := this.SafeString(market, MkString("base_scale"))
		_ = amountPrecisionString
		pricePrecisionString := this.SafeString(market, MkString("quote_scale"))
		...
		_ = precision
		minCost := this.SafeNumber(market, MkString("min_quote_value"))
		_ = minCost
		entry := MkMap(&VarMap{
			"id":        id,
			"uuid":      uuid,
...
func (this *Bigone) LoadMarkets(goArgs ...*Variant) *Variant {
	...
	_ = markets
	marketsByUuid := this.SafeValue(*this.At(MkString("options")), MkString("marketsByUuid"))
	_ = marketsByUuid
	if IsTrue(OpOr(OpEq2(marketsByUuid, MkUndefined()), reload)) {
		...
		for i := MkInteger(0); IsTrue(OpLw(i, (*((this).At(MkString("symbols")))).Length)); OpIncr(&i) {
			...
			_ = market
			uuid := this.SafeString(market, MkString("uuid"))
			_ = uuid
...
func (this *Bigone) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	marketId := this.SafeString(ticker, MkString("asset_pair_name"))
	_ = marketId
...
func (this *Bigone) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"asset_pair_name": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetAssetPairsAssetPairNameTicker"), this.Extend(request, params))
	_ = response
...
func (this *Bigone) FetchTickers(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetAssetPairsTickers"), this.Extend(request, params))
	_ = response
...
func (this *Bigone) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetAssetPairsAssetPairNameDepth"), this.Extend(request, params))
	_ = response
...
func (this *Bigone) ParseTrade(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.Parse8601(this.SafeString2(trade, MkString("created_at"), MkString("inserted_at")))
	_ = timestamp
	...
	_ = cost
	marketId := this.SafeString(trade, MkString("asset_pair_name"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("-"))
	_ = symbol
	...
	_ = result
	makerCurrencyCode := OpCopy(MkUndefined())
	_ = makerCurrencyCode
	takerCurrencyCode := OpCopy(MkUndefined())
	_ = takerCurrencyCode
	...
	makerFeeCost := this.SafeNumber(trade, MkString("maker_fee"))
	_ = makerFeeCost
	takerFeeCost := this.SafeNumber(trade, MkString("taker_fee"))
	_ = takerFeeCost
...
func (this *Bigone) FetchTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"asset_pair_name": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetAssetPairsAssetPairNameTrades"), this.Extend(request, params))
	_ = response
...
func (this *Bigone) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpNotEq2(since, MkUndefined())) {
		...
		_ = duration
		end := this.Sum(since, OpMulti(limit, OpMulti(duration, MkInteger(1000))))
		_ = end
		*(request).At(MkString("time")) = this.Iso8601(end)
	}
	response := this.Call(MkString("publicGetAssetPairsAssetPairNameCandles"), this.Extend(request, params))
	_ = response
...
func (this *Bigone) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	type_ := this.SafeString(params, MkString("type"), MkString(""))
	_ = type_
	params = this.Omit(params, MkString("type"))
	method := OpAdd(MkString("privateGet"), OpAdd(this.Capitalize(type_), MkString("Accounts")))
	_ = method
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = balance
		symbol := this.SafeString(balance, MkString("asset_symbol"))
		_ = symbol
...
func (this *Bigone) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = id
	marketId := this.SafeString(order, MkString("asset_pair_name"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("-"))
	_ = symbol
	...
	lastTradeTimestamp := this.Parse8601(this.SafeString(order, MkString("updated_at")))
	_ = lastTradeTimestamp
	average := this.SafeNumber(order, MkString("avg_deal_price"))
	_ = average
...
func (this *Bigone) CreateOrder(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(uppercaseType, MkString("LIMIT"))) {
		*(request).At(MkString("price")) = this.PriceToPrecision(symbol, price)
		...
		isStopLimit := OpEq2(uppercaseType, MkString("STOP_LIMIT"))
		_ = isStopLimit
		isStopMarket := OpEq2(uppercaseType, MkString("STOP_MARKET"))
		_ = isStopMarket
...
func (this *Bigone) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostOrdersIdCancel"), this.Extend(request, params))
	_ = response
	order := this.SafeValue(response, MkString("data"))
	_ = order
...
func (this *Bigone) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = response
	order := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = order
	return this.ParseOrder(order)
}
...
func (this *Bigone) FetchOrders(goArgs ...*Variant) *Variant {
	...
	_ = response
	orders := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = orders
...
func (this *Bigone) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"state": MkString("FILLED")})
	_ = request
	return this.FetchOrders(symbol, since, limit, this.Extend(request, params))
}
...
func (this *Bigone) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetAssetsAssetSymbolAddress"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
	dataLength := OpCopy(data.Length)
	_ = dataLength
	...
	firstElement := *(data).At(MkInteger(0))
	_ = firstElement
	address := this.SafeString(firstElement, MkString("value"))
	_ = address
	tag := this.SafeString(firstElement, MkString("memo"))
	_ = tag
...
func (this *Bigone) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = currency
	currencyId := this.SafeString(transaction, MkString("asset_symbol"))
	_ = currencyId
	code := this.SafeCurrencyCode(currencyId)
	_ = code
	id := this.SafeInteger(transaction, MkString("id"))
	_ = id
	...
	_ = status
	timestamp := this.Parse8601(this.SafeString(transaction, MkString("inserted_at")))
	_ = timestamp
	updated := this.Parse8601(this.SafeString2(transaction, MkString("updated_at"), MkString("completed_at")))
	_ = updated
	txid := this.SafeString(transaction, MkString("txid"))
	_ = txid
	address := this.SafeString(transaction, MkString("target_address"))
	_ = address
	tag := this.SafeString(transaction, MkString("memo"))
	_ = tag
	type_ := OpTrinary(OpHasMember(MkString("customer_id"), transaction), MkString("deposit"), MkString("withdrawal"))
	_ = type_
	return MkMap(&VarMap{
		"info":        transaction,
		"id":          id,
...
func (this *Bigone) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetWithdrawals"), this.Extend(request, params))
	_ = response
