...
func (this *Timex) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	period := this.SafeString(*(*this.At(MkString("options"))).At(MkString("fetchTickers")), MkString("period"), MkString("1d"))
	_ = period
	request := MkMap(&VarMap{"period": *(*this.At(MkString("timeframes"))).At(period)})
	_ = request
...
func (this *Timex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetOrderbookV2"), this.Extend(request, params))
	_ = response
	timestamp := this.Parse8601(this.SafeString(response, MkString("timestamp")))
	_ = timestamp
...
func (this *Timex) FetchTrades(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	options := this.SafeValue(*this.At(MkString("options")), MkString("fetchTrades"), MkMap(&VarMap{}))
	_ = options
	defaultSort := this.SafeValue(options, MkString("sort"), MkString("timestamp,asc"))
	_ = defaultSort
	sort := this.SafeString(params, MkString("sort"), defaultSort)
	_ = sort
	query := this.Omit(params, MkString("sort"))
	_ = query
	request := MkMap(&VarMap{
		"market": *(market).At(MkString("id")),
		"sort":   sort,
	...
	response := this.Call(MkString("publicGetTrades"), this.Extend(request, query))
	_ = response
...
func (this *Timex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("tradingGetBalances"), params)
	_ = response
	result := MkMap(&VarMap{
		"info":      response,
...
func (this *Timex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(uppercaseType, MkString("LIMIT"))) {
		*(request).At(MkString("price")) = this.PriceToPrecision(symbol, price)
		defaultExpireIn := this.SafeInteger(*this.At(MkString("options")), MkString("expireIn"))
		_ = defaultExpireIn
		expireTime := this.SafeValue(params, MkString("expireTime"))
		_ = expireTime
		expireIn := this.SafeValue(params, MkString("expireIn"), defaultExpireIn)
		_ = expireIn
	...
	response := this.Call(MkString("tradingPostOrders"), this.Extend(request, query))
	_ = response
	orders := this.SafeValue(response, MkString("orders"), MkArray(&VarArray{}))
	_ = orders
	order := this.SafeValue(orders, MkInteger(0), MkMap(&VarMap{}))
	_ = order
...
func (this *Timex) EditOrder(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("tradingPutOrders"), this.Extend(request, params))
	_ = response
	if IsTrue(OpHasMember(MkString("unchangedOrders"), response)) {
		orderIds := this.SafeValue(response, MkString("unchangedOrders"), MkArray(&VarArray{}))
		_ = orderIds
		orderId := this.SafeString(orderIds, MkInteger(0))
		_ = orderId
		return MkMap(&VarMap{
			"id":   orderId,
			"info": response,
	...
	orders := this.SafeValue(response, MkString("changedOrders"), MkArray(&VarArray{}))
	_ = orders
	firstOrder := this.SafeValue(orders, MkInteger(0), MkMap(&VarMap{}))
	_ = firstOrder
	order := this.SafeValue(firstOrder, MkString("newOrder"), MkMap(&VarMap{}))
	_ = order
	return this.ParseOrder(order, market)
}
...
func (this *Timex) CancelOrders(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"id": ids})
	_ = request
	response := this.Call(MkString("tradingDeleteOrders"), this.Extend(request, params))
	_ = response
	return response
}
...
func (this *Timex) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"orderHash": id})
	_ = request
	response := this.Call(MkString("historyGetOrdersDetails"), request)
	_ = response
	order := this.SafeValue(response, MkString("order"), MkMap(&VarMap{}))
	_ = order
...
func (this *Timex) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	_ = options
	defaultSort := this.SafeValue(options, MkString("sort"), MkString("createdAt,asc"))
	_ = defaultSort
	sort := this.SafeString(params, MkString("sort"), defaultSort)
	_ = sort
	query := this.Omit(params, MkString("sort"))
	_ = query
	request := MkMap(&VarMap{"sort": sort})
	_ = request
	...
	response := this.Call(MkString("tradingGetOrders"), this.Extend(request, query))
	_ = response
	orders := this.SafeValue(response, MkString("orders"), MkArray(&VarArray{}))
	_ = orders
...
func (this *Timex) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("historyGetOrders"), this.Extend(request, query))
	_ = response
	orders := this.SafeValue(response, MkString("orders"), MkArray(&VarArray{}))
	_ = orders
	return this.ParseOrders(orders, market, since, limit)
...
func (this *Timex) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("historyGetTrades"), this.Extend(request, query))
	_ = response
	trades := this.SafeValue(response, MkString("trades"), MkArray(&VarArray{}))
	_ = trades
...
func (this *Timex) FetchTradingFee(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	request := MkMap(&VarMap{"markets": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("tradingGetFees"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkInteger(0), MkMap(&VarMap{}))
	_ = result
	return MkMap(&VarMap{
		"info":  response,
...
func (this *Timex) ParseMarket(goArgs ...*Variant) *Variant {
	...
	_ = market
	locked := this.SafeValue(market, MkString("locked"))
	_ = locked
	active := OpNot(locked)
	_ = active
	...
	_ = precision
	amountIncrement := this.SafeNumber(market, MkString("quantityIncrement"))
	_ = amountIncrement
	minBase := this.SafeNumber(market, MkString("baseMinSize"))
	_ = minBase
	minAmount := Math.Max(amountIncrement, minBase)
	_ = minAmount
	priceIncrement := this.SafeNumber(market, MkString("tickSize"))
	_ = priceIncrement
	minCost := this.SafeNumber(market, MkString("quoteMinSize"))
	_ = minCost
	limits := MkMap(&VarMap{
		"amount": MkMap(&VarMap{
			"min": minAmount,
...
func (this *Timex) ParseCurrency(goArgs ...*Variant) *Variant {
	...
	_ = precision
	active := this.SafeValue(currency, MkString("active"))
	_ = active
	feeString := this.SafeString(currency, MkString("withdrawalFee"))
	_ = feeString
	tradeDecimals := this.SafeInteger(currency, MkString("tradeDecimals"))
	_ = tradeDecimals
	fee := OpCopy(MkUndefined())
	_ = fee
	if IsTrue(OpAnd(OpNotEq2(feeString, MkUndefined()), OpNotEq2(tradeDecimals, MkUndefined()))) {
		feeStringLen := OpCopy(feeString.Length)
		_ = feeStringLen
		dotIndex := OpSub(feeStringLen, tradeDecimals)
		_ = dotIndex
		if IsTrue(OpGt(dotIndex, MkInteger(0))) {
			whole := feeString.Slice(MkInteger(0), dotIndex)
			_ = whole
			fraction := feeString.Slice(OpNeg(dotIndex))
			_ = fraction
...
func (this *Timex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = id
	side := this.SafeStringLower2(trade, MkString("direction"), MkString("side"))
	_ = side
	takerOrMaker := this.SafeStringLower(trade, MkString("makerOrTaker"))
	_ = takerOrMaker
	...
	_ = feeCost
	feeCurrency := this.SafeCurrencyCode(this.SafeString(trade, MkString("feeToken")))
	_ = feeCurrency
...
func (this *Timex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = amount
	filled := this.SafeNumber(order, MkString("filledQuantity"))
	_ = filled
	canceledQuantity := this.SafeNumber(order, MkString("cancelledQuantity"))
	_ = canceledQuantity
...
func (this *Timex) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	url := OpAdd(*(*this.At(MkString("urls"))).At(MkString("api")), OpAdd(MkString("/"), OpAdd(api, OpAdd(MkString("/"), path))))
	_ = url
	...
	if IsTrue(OpNotEq2(api, MkString("public"))) {
		...
		_ = auth
		secret := OpAdd(MkString("Basic "), this.Decode(auth))
		_ = secret
...
func (this *Timex) HandleErrors(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpGtEq(statusCode, MkInteger(400))) {
		...
		code := this.SafeString2(error, MkString("code"), MkString("status"))
		_ = code
		message := this.SafeString2(error, MkString("message"), MkString("debugMessage"))
		_ = message
