...
func (this *Bitbns) FetchStatus(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v1GetPlatformStatus"), params ); _ = response;
...
func (this *Bitbns) FetchMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("ccxtGetFetchMarkets"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    marketPrecision := this.SafeValue(market , MkString("precision") , MkMap(&VarMap{})); _ = marketPrecision;
    precision := MkMap(&VarMap{
          "amount":this.SafeInteger(marketPrecision , MkString("amount") ),
          "price":this.SafeInteger(marketPrecision , MkString("price") ),
          }); _ = precision;
    marketLimits := this.SafeValue(market , MkString("limits") , MkMap(&VarMap{})); _ = marketLimits;
    amountLimits := this.SafeValue(marketLimits , MkString("amount") , MkMap(&VarMap{})); _ = amountLimits;
    priceLimits := this.SafeValue(marketLimits , MkString("price") , MkMap(&VarMap{})); _ = priceLimits;
    costLimits := this.SafeValue(marketLimits , MkString("cost") , MkMap(&VarMap{})); _ = costLimits;
    usdt := OpEq2(quoteId , MkString("USDT") ); _ = usdt;
    uppercaseId := OpTrinary(usdt , OpAdd(baseId , OpAdd(MkString("_") , quoteId )), baseId ); _ = uppercaseId;
...
func (this *Bitbns) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("ccxtGetFetchOrderbook"), this.Extend(request , params )); _ = response;
...
func (this *Bitbns) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  response := this.Call(MkString("ccxtGetFetchTickers"), params ); _ = response;
...
func (this *Bitbns) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("v1PostCurrentCoinBalanceEVERYTHING"), params ); _ = response;
  ...
  keys := GoGetKeys(data ); _ = keys;
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    key := *(keys ).At(i ); _ = key;
    parts := key.Split(MkString("availableorder") ); _ = parts;
    numParts := OpCopy(parts.Length ); _ = numParts;
    if IsTrue(OpGt(numParts , MkInteger(1) )) {
      currencyId := this.SafeString(parts , MkInteger(1) ); _ = currencyId;
...
func (this *Bitbns) ParseOrderStatus(goArgs ...*Variant) *Variant{
  ...
  statuses := MkMap(&VarMap{"0":MkString("open") }); _ = statuses;
...
func (this *Bitbns) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  id := this.SafeString2(order , MkString("id") , MkString("entry_id") ); _ = id;
  ...
  amount := this.SafeNumber2(order , MkString("amount") , MkString("btc") ); _ = amount;
  filled := this.SafeNumber(order , MkString("filled") ); _ = filled;
  remaining := this.SafeNumber(order , MkString("remaining") ); _ = remaining;
  average := this.SafeNumber(order , MkString("avg_cost") ); _ = average;
...
func (this *Bitbns) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v2PostOrders"), this.Extend(request , params )); _ = response;
...
func (this *Bitbns) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  quoteSide := OpTrinary(OpEq2(*(market ).At(MkString("quoteId") ), MkString("USDT") ), MkString("usdtcancelOrder") , MkString("cancelOrder") ); _ = quoteSide;
  request := MkMap(&VarMap{
        "entry_id":id ,
        "symbol":*(market ).At(MkString("uppercaseId") ),
        "side":quoteSide ,
        }); _ = request;
  response := this.Call(MkString("v2PostCancel"), this.Extend(request , params )); _ = response;
  return this.ParseOrder(response , market );
}
...
func (this *Bitbns) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v1PostOrderStatusSymbol"), this.Extend(request , params )); _ = response;
...
func (this *Bitbns) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  quoteSide := OpTrinary(OpEq2(*(market ).At(MkString("quoteId") ), MkString("USDT") ), MkString("usdtListOpenOrders") , MkString("listOpenOrders") ); _ = quoteSide;
  request := MkMap(&VarMap{
        "symbol":*(market ).At(MkString("uppercaseId") ),
        "side":quoteSide ,
        "page":MkInteger(0) ,
        }); _ = request;
  response := this.Call(MkString("v2PostGetordersnew"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Bitbns) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  orderId := this.SafeString(trade , MkString("id") ); _ = orderId;
  ...
  factor := this.SafeString(trade , MkString("factor") ); _ = factor;
  amountScaled := Precise.StringDiv(amountString , factor ); _ = amountScaled;
  amount := this.ParseNumber(amountScaled ); _ = amount;
  cost := this.ParseNumber(Precise.StringMul(priceString , amountScaled )); _ = cost;
  ...
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyCode := *(market ).At(MkString("quote") ); _ = feeCurrencyCode;
    fee = MkMap(&VarMap{
        "cost":feeCost ,
        "currency":feeCurrencyCode ,
        });
...
func (this *Bitbns) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v1PostListExecutedOrdersSymbol"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Bitbns) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v1PostDepositHistorySymbol"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Bitbns) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v1PostWithdrawHistorySymbol"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  return this.ParseTransactions(data , currency , since , limit );
}
...
func (this *Bitbns) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  currencyId := this.SafeString(transaction , MkString("unit") ); _ = currencyId;
...
func (this *Bitbns) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"symbol":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("v1PostGetCoinAddressSymbol"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  address := this.SafeString(data , MkString("token") ); _ = address;
...
func (this *Bitbns) Sign(goArgs ...*Variant) *Variant{
  ...
  api := GoGetArg(goArgs, 1, MkString("v1") ); _ = api;
  ...
  if IsTrue(OpEq2(method , MkString("GET") )) {
    ...
    if IsTrue(OpEq2(method , MkString("POST") )) {
      ...
      payload := this.StringToBase64(this.Json(auth )); _ = payload;
      signature := this.Hmac(payload , this.Encode(*this.At(MkString("secret")) ), MkString("sha512") ); _ = signature;
...
func (this *Bitbns) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  error := OpAnd(OpNotEq2(code , MkUndefined() ), OpNotEq2(code , MkString("200") )); _ = error;
