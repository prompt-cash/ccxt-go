...
func (this *Paymium) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetDataCurrencyDepth"), this.Extend(request , params )); _ = response;
...
func (this *Paymium) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"currency":this.MarketId(symbol )}); _ = request;
  ticker := this.Call(MkString("publicGetDataCurrencyTicker"), this.Extend(request , params )); _ = ticker;
...
func (this *Paymium) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeTimestamp(trade , MkString("created_at_int") ); _ = timestamp;
  id := this.SafeString(trade , MkString("uuid") ); _ = id;
  ...
  amountField := OpAdd(MkString("traded_") , (*(market ).At(MkString("base") )).Call(MkString("toLowerCase") )); _ = amountField;
  amountString := this.SafeString(trade , amountField ); _ = amountString;
...
func (this *Paymium) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetDataCurrencyTrades"), this.Extend(request , params )); _ = response;
