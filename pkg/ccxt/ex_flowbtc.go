...
func (this *Flowbtc) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("publicPostGetProductPairs"), params)
	_ = response
	markets := this.SafeValue(response, MkString("productPairs"))
	_ = markets
	result := MkMap(&VarMap{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		...
		_ = id
		baseId := this.SafeString(market, MkString("product1Label"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("product2Label"))
		_ = quoteId
...
func (this *Flowbtc) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = response
	balances := this.SafeValue(response, MkString("currencies"))
	_ = balances
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = balance
		currencyId := *(balance).At(MkString("name"))
		_ = currencyId
...
func (this *Flowbtc) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"productPair": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicPostGetOrderBook"), this.Extend(request, params))
	_ = response
...
func (this *Flowbtc) FetchTicker(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"productPair": *(market).At(MkString("id"))})
	_ = request
	ticker := this.Call(MkString("publicPostGetTicker"), this.Extend(request, params))
	_ = ticker
...
func (this *Flowbtc) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = market
	timestamp := this.SafeTimestamp(trade, MkString("unixtime"))
	_ = timestamp
	side := OpTrinary(OpEq2(*(trade).At(MkString("incomingOrderSide")), MkInteger(0)), MkString("buy"), MkString("sell"))
	_ = side
	id := this.SafeString(trade, MkString("tid"))
	_ = id
	priceString := this.SafeString(trade, MkString("px"))
	_ = priceString
...
func (this *Flowbtc) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicPostGetTrades"), this.Extend(request, params))
	_ = response
...
func (this *Flowbtc) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	orderType := OpTrinary(OpEq2(type_, MkString("market")), MkInteger(1), MkInteger(0))
	_ = orderType
	request := MkMap(&VarMap{
	...
	_ = request
	response := this.Call(MkString("privatePostCreateOrder"), this.Extend(request, params))
	_ = response
	return MkMap(&VarMap{
		"info": response,
...
func (this *Flowbtc) CancelOrder(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpHasMember(MkString("ins"), params)) {
		request := MkMap(&VarMap{"serverOrderId": id})
		_ = request
...
func (this *Flowbtc) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		nonce := this.Nonce()
		_ = nonce
		auth := OpAdd(nonce.ToString(), OpAdd(*this.At(MkString("uid")), *this.At(MkString("apiKey"))))
		_ = auth
...
func (this *Flowbtc) HandleErrors(goArgs ...*Variant) *Variant {
	...
	isAccepted := this.SafeValue(response, MkString("isAccepted"), MkBool(true))
	_ = isAccepted
