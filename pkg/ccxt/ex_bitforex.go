...
func (this *Bitforex) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetApiV1MarketSymbols"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    ...
    symbolParts := id.Split(MkString("-") ); _ = symbolParts;
    baseId := *(symbolParts ).At(MkInteger(2) ); _ = baseId;
    quoteId := *(symbolParts ).At(MkInteger(1) ); _ = quoteId;
...
func (this *Bitforex) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  sideId := this.SafeInteger(trade , MkString("direction") ); _ = sideId;
  side := this.ParseSide(sideId ); _ = side;
  return MkMap(&VarMap{
        "info":trade ,
        "id":id ,
        "timestamp":timestamp ,
...
func (this *Bitforex) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetApiV1MarketTrades"), this.Extend(request , params )); _ = response;
...
func (this *Bitforex) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostApiV1FundAllAccount"), params ); _ = response;
  data := *(response ).At(MkString("data") ); _ = data;
...
func (this *Bitforex) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetApiV1MarketTicker"), this.Extend(request , params )); _ = response;
  data := *(response ).At(MkString("data") ); _ = data;
...
func (this *Bitforex) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetApiV1MarketKline"), this.Extend(request , params )); _ = response;
...
func (this *Bitforex) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetApiV1MarketDepth"), this.Extend(request , params )); _ = response;
...
func (this *Bitforex) ParseSide(goArgs ...*Variant) *Variant{
  sideId := GoGetArg(goArgs, 0, MkUndefined()); _ = sideId;
...
func (this *Bitforex) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeNumber(order , MkString("createTime") ); _ = timestamp;
  lastTradeTimestamp := this.SafeNumber(order , MkString("lastTime") ); _ = lastTradeTimestamp;
  symbol := *(market ).At(MkString("symbol") ); _ = symbol;
  sideId := this.SafeInteger(order , MkString("tradeType") ); _ = sideId;
  side := this.ParseSide(sideId ); _ = side;
  type_ := OpCopy(MkUndefined() ); _ = type_;
  price := this.SafeNumber(order , MkString("orderPrice") ); _ = price;
  average := this.SafeNumber(order , MkString("avgPrice") ); _ = average;
  amount := this.SafeNumber(order , MkString("orderAmount") ); _ = amount;
  filled := this.SafeNumber(order , MkString("dealAmount") ); _ = filled;
  status := this.ParseOrderStatus(this.SafeString(order , MkString("orderState") )); _ = status;
  feeSide := OpTrinary(OpEq2(side , MkString("buy") ), MkString("base") , MkString("quote") ); _ = feeSide;
  feeCurrency := *(market ).At(feeSide ); _ = feeCurrency;
  fee := MkMap(&VarMap{
...
func (this *Bitforex) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostApiV1TradeOrderInfo"), this.Extend(request , params )); _ = response;
  order := this.ParseOrder(*(response ).At(MkString("data") ), market ); _ = order;
  return order ;
}
...
func (this *Bitforex) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostApiV1TradeOrderInfos"), this.Extend(request , params )); _ = response;
...
func (this *Bitforex) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  sideId := OpCopy(MkUndefined() ); _ = sideId;
  ...
  response := this.Call(MkString("privatePostApiV1TradePlaceOrder"), this.Extend(request , params )); _ = response;
  data := *(response ).At(MkString("data") ); _ = data;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Bitforex) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  results := this.Call(MkString("privatePostApiV1TradeCancelOrder"), this.Extend(request , params )); _ = results;
  success := *(results ).At(MkString("success") ); _ = success;
...
func (this *Bitforex) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    payload := this.Urlencode(MkMap(&VarMap{"accessKey":*this.At(MkString("apiKey")) })); _ = payload;
    *(query ).At(MkString("nonce") )= this.Milliseconds();
    if IsTrue(*(GoGetKeys(query )).At(MkString("length") )) {
      OpAddAssign(& payload , OpAdd(MkString("&") , this.Urlencode(this.Keysort(query ))));
    }
    message := OpAdd(MkString("/") , OpAdd(path , OpAdd(MkString("?") , payload ))); _ = message;
