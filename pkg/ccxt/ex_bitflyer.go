...
func (this *Bitflyer) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  jp_markets := this.Call(MkString("publicGetGetmarkets"), params ); _ = jp_markets;
  us_markets := this.Call(MkString("publicGetGetmarketsUsa"), params ); _ = us_markets;
  eu_markets := this.Call(MkString("publicGetGetmarketsEu"), params ); _ = eu_markets;
  markets := this.ArrayConcat(jp_markets , us_markets ); _ = markets;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    ...
    id := this.SafeString(market , MkString("product_code") ); _ = id;
    currencies := id.Split(MkString("_") ); _ = currencies;
    ...
    numCurrencies := OpCopy(currencies.Length ); _ = numCurrencies;
    ...
    symbol := OpTrinary(OpEq2(numCurrencies , MkInteger(2) ), OpAdd(base , OpAdd(MkString("/") , quote )), id ); _ = symbol;
    fees := this.SafeValue(*this.At(MkString("fees")) , symbol , *(*this.At(MkString("fees")) ).At(MkString("trading") )); _ = fees;
    maker := this.SafeValue(fees , MkString("maker") , *(*(*this.At(MkString("fees")) ).At(MkString("trading") )).At(MkString("maker") )); _ = maker;
    taker := this.SafeValue(fees , MkString("taker") , *(*(*this.At(MkString("fees")) ).At(MkString("trading") )).At(MkString("taker") )); _ = taker;
...
func (this *Bitflyer) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetGetbalance"), params ); _ = response;
...
func (this *Bitflyer) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"product_code":this.MarketId(symbol )}); _ = request;
  orderbook := this.Call(MkString("publicGetGetboard"), this.Extend(request , params )); _ = orderbook;
...
func (this *Bitflyer) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"product_code":this.MarketId(symbol )}); _ = request;
  ticker := this.Call(MkString("publicGetGetticker"), this.Extend(request , params )); _ = ticker;
  timestamp := this.Parse8601(this.SafeString(ticker , MkString("timestamp") )); _ = timestamp;
  last := this.SafeNumber(ticker , MkString("ltp") ); _ = last;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Bitflyer) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(side , MkUndefined() )) {
    id := OpAdd(side , MkString("_child_order_acceptance_id") ); _ = id;
  ...
  timestamp := this.Parse8601(this.SafeString(trade , MkString("exec_date") )); _ = timestamp;
...
func (this *Bitflyer) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"product_code":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetGetexecutions"), this.Extend(request , params )); _ = response;
...
func (this *Bitflyer) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  result := this.Call(MkString("privatePostSendchildorder"), this.Extend(request , params )); _ = result;
  id := this.SafeString(result , MkString("child_order_acceptance_id") ); _ = id;
  return MkMap(&VarMap{
        "info":result ,
        "id":id ,
        });
...
func (this *Bitflyer) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.Parse8601(this.SafeString(order , MkString("child_order_date") )); _ = timestamp;
  amount := this.SafeNumber(order , MkString("size") ); _ = amount;
  remaining := this.SafeNumber(order , MkString("outstanding_size") ); _ = remaining;
  filled := this.SafeNumber(order , MkString("executed_size") ); _ = filled;
  price := this.SafeNumber(order , MkString("price") ); _ = price;
  status := this.ParseOrderStatus(this.SafeString(order , MkString("child_order_state") )); _ = status;
  type_ := this.SafeStringLower(order , MkString("child_order_type") ); _ = type_;
  side := this.SafeStringLower(order , MkString("side") ); _ = side;
  marketId := this.SafeString(order , MkString("product_code") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(order , MkString("total_commission") ); _ = feeCost;
  ...
  id := this.SafeString(order , MkString("child_order_acceptance_id") ); _ = id;
...
func (this *Bitflyer) FetchOrders(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined() ); _ = symbol;
  since := GoGetArg(goArgs, 1, MkUndefined() ); _ = since;
  limit := GoGetArg(goArgs, 2, MkInteger(100) ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  if IsTrue(OpEq2(symbol , MkUndefined() )) {
  ...
  response := this.Call(MkString("privateGetGetchildorders"), this.Extend(request , params )); _ = response;
  orders := this.ParseOrders(response , market , since , limit ); _ = orders;
...
func (this *Bitflyer) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkInteger(100) ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"child_order_state":MkString("ACTIVE") }); _ = request;
...
func (this *Bitflyer) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkInteger(100) ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"child_order_state":MkString("COMPLETED") }); _ = request;
  return this.FetchOrders(symbol , since , limit , this.Extend(request , params ));
}
...
func (this *Bitflyer) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  orders := this.FetchOrders(symbol ); _ = orders;
  ordersById := this.IndexBy(orders , MkString("id") ); _ = ordersById;
...
func (this *Bitflyer) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetexecutions"), this.Extend(request , params )); _ = response;
  return this.ParseTrades(response , market , since , limit );
}
...
func (this *Bitflyer) FetchPositions(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"product_code":this.MarketIds(symbols )}); _ = request;
...
func (this *Bitflyer) Withdraw(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString(response , MkString("message_id") ); _ = id;
  return MkMap(&VarMap{
        "info":response ,
        "id":id ,
        });
...
func (this *Bitflyer) Sign(goArgs ...*Variant) *Variant{
  ...
  request := OpAdd(MkString("/") , OpAdd(*this.At(MkString("version")) , MkString("/") )); _ = request;
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    auth := (MkArray(&VarArray{
            ...
            method ,
            request ,
            })).Call(MkString("join") , MkString("") ); _ = auth;
    if IsTrue(*(GoGetKeys(params )).At(MkString("length") )) {
