...
func (this *Bitstamp) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = symbol
		symbolId := OpAdd(baseId, OpAdd(MkString("_"), quoteId))
		_ = symbolId
		id := this.SafeString(market, MkString("url_symbol"))
		_ = id
		amountPrecisionString := this.SafeString(market, MkString("base_decimals"))
		_ = amountPrecisionString
		pricePrecisionString := this.SafeString(market, MkString("counter_decimals"))
		...
		_ = precision
		minimumOrder := this.SafeString(market, MkString("minimum_order"))
		_ = minimumOrder
		parts := minimumOrder.Split(MkString(" "))
		_ = parts
		cost := *(parts).At(MkInteger(0))
		_ = cost
		trading := this.SafeString(market, MkString("trading"))
		_ = trading
		active := OpEq2(trading, MkString("Enabled"))
		_ = active
...
func (this *Bitstamp) ConstructCurrencyObject(goArgs ...*Variant) *Variant {
	...
	_ = code
	name := GoGetArg(goArgs, 2, MkUndefined())
	_ = name
	precision := GoGetArg(goArgs, 3, MkUndefined())
	_ = precision
	minCost := GoGetArg(goArgs, 4, MkUndefined())
	_ = minCost
	originalPayload := GoGetArg(goArgs, 5, MkUndefined())
	_ = originalPayload
	currencyType := MkString("crypto")
	_ = currencyType
...
func (this *Bitstamp) FetchMarketsFromCache(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpOr(OpEq2(timestamp, MkUndefined()), OpGt(OpSub(now, timestamp), expires))) {
		response := this.Call(MkString("publicGetTradingPairsInfo"), params)
		_ = response
...
func (this *Bitstamp) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		base = this.SafeCurrencyCode(base)
		quote = this.SafeCurrencyCode(quote)
		description := this.SafeString(market, MkString("description"))
		_ = description
		baseDescription := MkUndefined()
		_ = baseDescription
		quoteDescription := MkUndefined()
		_ = quoteDescription
		...
		if IsTrue(OpNot(OpHasMember(base, result))) {
			baseDecimals := this.SafeInteger(market, MkString("base_decimals"))
			_ = baseDecimals
		...
		if IsTrue(OpNot(OpHasMember(quote, result))) {
			counterDecimals := this.SafeInteger(market, MkString("counter_decimals"))
			_ = counterDecimals
...
func (this *Bitstamp) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = response
	microtimestamp := this.SafeInteger(response, MkString("microtimestamp"))
	_ = microtimestamp
	timestamp := parseInt(OpDiv(microtimestamp, MkInteger(1000)))
	_ = timestamp
...
func (this *Bitstamp) GetCurrencyIdFromTransaction(goArgs ...*Variant) *Variant {
	...
	_ = transaction
	currencyId := this.SafeStringLower(transaction, MkString("currency"))
	_ = currencyId
...
func (this *Bitstamp) GetMarketFromTrade(goArgs ...*Variant) *Variant {
	...
	currencyIds := GoGetKeys(trade)
	_ = currencyIds
	numCurrencyIds := OpCopy(currencyIds.Length)
	_ = numCurrencyIds
	...
	if IsTrue(OpEq2(numCurrencyIds, MkInteger(2))) {
		marketId := OpAdd(*(currencyIds).At(MkInteger(0)), *(currencyIds).At(MkInteger(1)))
		_ = marketId
...
func (this *Bitstamp) GetMarketFromTrades(goArgs ...*Variant) *Variant {
	...
	_ = trades
	tradesBySymbol := this.IndexBy(trades, MkString("symbol"))
	_ = tradesBySymbol
...
func (this *Bitstamp) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = type_
	cost := this.SafeNumber(trade, MkString("cost"))
	_ = cost
	if IsTrue(OpEq2(market, MkUndefined())) {
		keys := GoGetKeys(trade)
		_ = keys
		for i := MkInteger(0); IsTrue(OpLw(i, keys.Length)); OpIncr(&i) {
			if IsTrue(OpGtEq((*(keys).At(i)).Call(MkString("indexOf"), MkString("_")), MkInteger(0))) {
				marketId := (*(keys).At(i)).Call(MkString("replace"), MkString("_"), MkString(""))
				_ = marketId
				if IsTrue(OpHasMember(marketId, *this.At(MkString("markets_by_id")))) {
	...
	timestamp := this.SafeString2(trade, MkString("date"), MkString("datetime"))
	_ = timestamp
...
func (this *Bitstamp) ParseTradingFee(goArgs ...*Variant) *Variant {
	balances := GoGetArg(goArgs, 0, MkUndefined())
	_ = balances
	symbol := GoGetArg(goArgs, 1, MkUndefined())
	_ = symbol
	market := this.Market(symbol)
	_ = market
	tradeFee := this.SafeNumber(balances, OpAdd(*(market).At(MkString("id")), MkString("_fee")))
	_ = tradeFee
	return MkMap(&VarMap{
		"symbol": symbol,
		"maker":  tradeFee,
...
func (this *Bitstamp) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetTransactionsPair"), this.Extend(request, params))
	_ = response
...
func (this *Bitstamp) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetOhlcPair"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	ohlc := this.SafeValue(data, MkString("ohlc"), MkArray(&VarArray{}))
	_ = ohlc
...
func (this *Bitstamp) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	balance := this.Call(MkString("privatePostBalance"), params)
	_ = balance
	result := MkMap(&VarMap{
		"info":      balance,
...
func (this *Bitstamp) FetchTradingFee(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	method := MkString("privatePostBalance")
	_ = method
	...
	balance := this.Call(method, this.Extend(request, params))
	_ = balance
	return MkMap(&VarMap{
		"info":   balance,
		"symbol": symbol,
...
func (this *Bitstamp) PraseTradingFees(goArgs ...*Variant) *Variant {
	...
	_ = balance
	result := MkMap(&VarMap{"info": balance})
	_ = result
	markets := GoGetKeys(*this.At(MkString("markets")))
	_ = markets
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		symbol := *(markets).At(i)
		_ = symbol
		fee := this.ParseTradingFee(balance, symbol)
		_ = fee
...
func (this *Bitstamp) ParseFundingFees(goArgs ...*Variant) *Variant {
	...
	withdraw := MkMap(&VarMap{})
	_ = withdraw
	ids := GoGetKeys(balance)
	_ = ids
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		id := *(ids).At(i)
		_ = id
		if IsTrue(OpGtEq(id.IndexOf(MkString("_withdrawal_fee")), MkInteger(0))) {
			currencyId := *(id.Split(MkString("_"))).At(MkInteger(0))
			_ = currencyId
...
func (this *Bitstamp) FetchFees(goArgs ...*Variant) *Variant {
	...
	balance := this.Call(MkString("privatePostBalance"), params)
	_ = balance
	tradingFees := this.PraseTradingFees(balance)
	_ = tradingFees
	*(tradingFees).At(MkString("info")) = MkUndefined()
	fundingFees := this.ParseFundingFees(balance)
	_ = fundingFees
...
func (this *Bitstamp) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	method := MkString("privatePostCancelAllOrders")
	_ = method
	if IsTrue(OpNotEq2(symbol, MkUndefined())) {
		market = this.Market(symbol)
		*(request).At(MkString("pair")) = *(market).At(MkString("id"))
...
func (this *Bitstamp) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	method := MkString("privatePostUserTransactions")
	_ = method
	market := OpCopy(MkUndefined())
	_ = market
	if IsTrue(OpNotEq2(symbol, MkUndefined())) {
	...
	response := this.Call(method, this.Extend(request, params))
	_ = response
	result := this.FilterBy(response, MkString("type"), MkString("2"))
	_ = result
...
func (this *Bitstamp) FetchTransactions(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostUserTransactions"), this.Extend(request, params))
	_ = response
...
func (this *Bitstamp) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostWithdrawalRequests"), this.Extend(request, params))
	_ = response
...
func (this *Bitstamp) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = currency
	timestamp := this.Parse8601(this.SafeString(transaction, MkString("datetime")))
	_ = timestamp
	...
	if IsTrue(OpHasMember(MkString("type"), transaction)) {
		rawType := this.SafeString(transaction, MkString("type"))
		_ = rawType
	...
	if IsTrue(OpNotEq2(address, MkUndefined())) {
		addressParts := address.Split(MkString("?dt="))
		_ = addressParts
		numParts := OpCopy(addressParts.Length)
		_ = numParts
...
func (this *Bitstamp) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	marketId := this.SafeStringLower(order, MkString("currency_pair"))
	_ = marketId
	...
	_ = amount
	transactions := this.SafeValue(order, MkString("transactions"), MkArray(&VarArray{}))
	_ = transactions
	trades := this.ParseTrades(transactions, market)
	_ = trades
...
func (this *Bitstamp) ParseLedgerEntry(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(type_, MkString("trade"))) {
		parsedTrade := this.ParseTrade(item)
		_ = parsedTrade
		market := OpCopy(MkUndefined())
		_ = market
		keys := GoGetKeys(item)
		...
		direction := OpTrinary(OpEq2(*(parsedTrade).At(MkString("side")), MkString("buy")), MkString("in"), MkString("out"))
		_ = direction
		return MkMap(&VarMap{
		...
		parsedTransaction := this.ParseTransaction(item, currency)
		_ = parsedTransaction
		...
		if IsTrue(OpHasMember(MkString("amount"), item)) {
			...
			if IsTrue(OpAnd(OpHasMember(MkString("currency"), parsedTransaction), OpNotEq2(*(parsedTransaction).At(MkString("currency")), MkUndefined()))) {
				code := *(parsedTransaction).At(MkString("currency"))
				_ = code
				currencyId := this.SafeString(*this.At(MkString("currencies_by_id")), code, code)
				_ = currencyId
				amount := this.SafeNumber(item, currencyId)
				_ = amount
				direction = OpTrinary(OpGt(amount, MkInteger(0)), MkString("in"), MkString("out"))
			}
		}
...
func (this *Bitstamp) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostOpenOrdersAll"), params)
	_ = response
...
func (this *Bitstamp) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = name
	method := OpAdd(MkString("privatePost"), OpAdd(this.Capitalize(name), MkString("Address")))
	_ = method
	response := this.Call(method, params)
	_ = response
	address := this.SafeString(response, MkString("address"))
	_ = address
	tag := this.SafeString2(response, MkString("memo_id"), MkString("destination_tag"))
	_ = tag
...
func (this *Bitstamp) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		xAuth := OpAdd(MkString("BITSTAMP "), *this.At(MkString("apiKey")))
		_ = xAuth
		xAuthNonce := this.Uuid()
		_ = xAuthNonce
		xAuthTimestamp := (this.Milliseconds()).Call(MkString("toString"))
		_ = xAuthTimestamp
		xAuthVersion := MkString("v2")
		_ = xAuthVersion
		contentType := MkString("")
		_ = contentType
		...
		authBody := OpTrinary(body, body, MkString(""))
		_ = authBody
		auth := OpAdd(xAuth, OpAdd(method, OpAdd(url.Replace(MkString("https://"), MkString("")), OpAdd(contentType, OpAdd(xAuthNonce, OpAdd(xAuthTimestamp, OpAdd(xAuthVersion, authBody)))))))
		_ = auth
...
func (this *Bitstamp) HandleErrors(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpOr(OpEq2(status, MkString("error")), OpNotEq2(error, MkUndefined()))) {
		...
		reason := this.SafeValue(response, MkString("reason"), MkMap(&VarMap{}))
		_ = reason
		if IsTrue(OpEq2(OpType(reason), MkString("string"))) {
			errors.Push(reason)
		} else {
			all := this.SafeValue(reason, MkString("__all__"), MkArray(&VarArray{}))
			_ = all
