...
func (this *Gopax) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    minimums := this.SafeValue(market , MkString("restApiOrderAmountMin") , MkMap(&VarMap{})); _ = minimums;
    marketAsk := this.SafeValue(minimums , MkString("marketAsk") , MkMap(&VarMap{})); _ = marketAsk;
    marketBid := this.SafeValue(minimums , MkString("marketBid") , MkMap(&VarMap{})); _ = marketBid;
    takerFeePercentString := this.SafeString(market , MkString("takerFeePercent") ); _ = takerFeePercentString;
    makerFeePercentString := this.SafeString(market , MkString("makerFeePercent") ); _ = makerFeePercentString;
    taker := this.ParseNumber(Precise.StringDiv(takerFeePercentString , MkString("100") )); _ = taker;
    maker := this.ParseNumber(Precise.StringDiv(makerFeePercentString , MkString("100") )); _ = maker;
...
func (this *Gopax) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    fee := this.SafeNumber(currency , MkString("withdrawalFee") ); _ = fee;
    precision := this.SafeNumber(currency , MkString("scale") ); _ = precision;
...
func (this *Gopax) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"tradingPair":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetTradingPairsTradingPairBook"), this.Extend(request , params )); _ = response;
  nonce := this.SafeInteger(response , MkString("sequence") ); _ = nonce;
  result := this.ParseOrderBook(response , symbol , MkUndefined() , MkString("bid") , MkString("ask") , MkInteger(1) , MkInteger(2) ); _ = result;
...
func (this *Gopax) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  last := this.SafeNumber2(ticker , MkString("price") , MkString("close") ); _ = last;
...
func (this *Gopax) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"tradingPair":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetTradingPairsTradingPairTicker"), this.Extend(request , params )); _ = response;
...
func (this *Gopax) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicGetTradingPairsStats"), params ); _ = response;
...
func (this *Gopax) ParsePrivateTrade(goArgs ...*Variant) *Variant{
  ...
  symbol := (this.SafeString(trade , MkString("tradingPairName") )).Call(MkString("replace") , MkString("-") , MkString("/") ); _ = symbol;
  side := this.SafeString(trade , MkString("side") ); _ = side;
  price := this.SafeNumber(trade , MkString("price") ); _ = price;
  amount := this.SafeNumber(trade , MkString("baseAmount") ); _ = amount;
  feeCurrency := symbol.Slice(MkInteger(0) , MkInteger(3) ); _ = feeCurrency;
...
func (this *Gopax) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  orderId := this.SafeInteger(trade , MkString("orderId") ); _ = orderId;
  timestamp := this.Parse8601(this.SafeString2(trade , MkString("time") , MkString("timestamp") )); _ = timestamp;
  timestamp = this.SafeTimestamp(trade , MkString("occuredAt") , timestamp );
  marketId := this.SafeString(trade , MkString("tradingPairName") ); _ = marketId;
  ...
  amountString := this.SafeString2(trade , MkString("amount") , MkString("baseAmount") ); _ = amountString;
  price := this.ParseNumber(priceString ); _ = price;
  amount := this.ParseNumber(amountString ); _ = amount;
  cost := this.SafeNumber(trade , MkString("quoteAmount") ); _ = cost;
  ...
  takerOrMaker := this.SafeString(trade , MkString("position") ); _ = takerOrMaker;
  return MkMap(&VarMap{
        "info":trade ,
        "id":id ,
        "timestamp":timestamp ,
...
func (this *Gopax) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTradingPairsTradingPairTrades"), this.Extend(request , params )); _ = response;
...
func (this *Gopax) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTradingPairsTradingPairCandles"), this.Extend(request , params )); _ = response;
...
func (this *Gopax) ParseBalanceResponse(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    currencyId := this.SafeString2(balance , MkString("asset") , MkString("isoAlpha3") ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    hold := this.SafeString(balance , MkString("hold") ); _ = hold;
    pendingWithdrawal := this.SafeString(balance , MkString("pendingWithdrawal") ); _ = pendingWithdrawal;
...
func (this *Gopax) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  timeInForce := this.SafeStringUpper(order , MkString("timeInForce") ); _ = timeInForce;
  ...
  remaining := this.SafeNumber(order , MkString("remaining") ); _ = remaining;
  marketId := this.SafeString(order , MkString("tradingPairName") ); _ = marketId;
  market = this.SafeMarket(marketId , market , MkString("-") );
  status := this.ParseOrderStatus(this.SafeString(order , MkString("status") )); _ = status;
  balanceChange := this.SafeValue(order , MkString("balanceChange") , MkMap(&VarMap{})); _ = balanceChange;
  filled := this.SafeNumber(balanceChange , MkString("baseNet") ); _ = filled;
  cost := this.SafeNumber(balanceChange , MkString("quoteNet") ); _ = cost;
  ...
  if IsTrue(OpEq2(side , MkString("buy") )) {
    baseFee := this.SafeValue(balanceChange , MkString("baseFee") , MkMap(&VarMap{})); _ = baseFee;
    taking := this.SafeNumber(baseFee , MkString("taking") ); _ = taking;
    making := this.SafeNumber(baseFee , MkString("making") ); _ = making;
    fee = MkMap(&VarMap{
        "currency":*(market ).At(MkString("base") ),
        "cost":this.Sum(taking , making ),
        });
  } else {
    quoteFee := this.SafeValue(balanceChange , MkString("quoteFee") , MkMap(&VarMap{})); _ = quoteFee;
    taking := this.SafeNumber(quoteFee , MkString("taking") ); _ = taking;
    making := this.SafeNumber(quoteFee , MkString("making") ); _ = making;
    fee = MkMap(&VarMap{
...
func (this *Gopax) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"includePast":MkString("true") }); _ = request;
...
func (this *Gopax) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"includePast":MkString("false") }); _ = request;
...
func (this *Gopax) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  timeInForce := this.SafeStringLower(params , MkString("timeInForce") ); _ = timeInForce;
  if IsTrue(OpNotEq2(timeInForce , MkUndefined() )) {
...
func (this *Gopax) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"deepSearch":MkString("true") }); _ = request;
  if IsTrue(OpNotEq2(since , MkUndefined() )) {
    *(request ).At(MkString("after") )= parseInt(OpDiv(since , MkInteger(1000) ));
  }
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
...
func (this *Gopax) ParseDepositAddress(goArgs ...*Variant) *Variant{
  ...
  tag := this.SafeString(depositAddress , MkString("memoId") ); _ = tag;
  currencyId := this.SafeString(depositAddress , MkString("asset") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId ); _ = code;
...
func (this *Gopax) FetchDepositAddresses(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetCryptoDepositAddresses"), params ); _ = response;
...
func (this *Gopax) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.FetchDepositAddresses(MkUndefined() , params ); _ = response;
  address := this.SafeValue(response , code ); _ = address;
...
func (this *Gopax) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  amount := this.SafeNumber(transaction , MkString("netAmount") ); _ = amount;
  ...
  timestamp := this.SafeTimestamp(transaction , MkString("reviewStartedAt") ); _ = timestamp;
  updated := this.SafeTimestamp(transaction , MkString("completedAt") ); _ = updated;
  addressFrom := this.SafeString(transaction , MkString("sourceAddress") ); _ = addressFrom;
  addressTo := this.SafeString(transaction , MkString("destinationAddress") ); _ = addressTo;
  tagFrom := this.SafeString(transaction , MkString("sourceMemoId") ); _ = tagFrom;
  tagTo := this.SafeString(transaction , MkString("destinationMemoId") ); _ = tagTo;
...
func (this *Gopax) FetchTransactions(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetDepositWithdrawalStatus"), this.Extend(request , params )); _ = response;
...
func (this *Gopax) Sign(goArgs ...*Variant) *Variant{
  ...
  endpoint := OpAdd(MkString("/") , this.ImplodeParams(path , params )); _ = endpoint;
  url := OpAdd(this.ImplodeHostname(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(api )), endpoint ); _ = url;
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    if IsTrue(OpEq2(api , MkString("private") )) {
      ...
      auth := OpAdd(MkString("t") , OpAdd(timestamp , OpAdd(method , endpoint ))); _ = auth;
      ...
      if IsTrue(OpEq2(method , MkString("POST") )) {
        *(headers ).At(MkString("Content-Type") )= MkString("application/json") ;
        ...
        if IsTrue(OpEq2(endpoint , MkString("/orders") )) {
          if IsTrue(*(GoGetKeys(query )).At(MkString("length") )) {
            urlQuery := OpAdd(MkString("?") , this.Urlencode(query )); _ = urlQuery;
      ...
      rawSecret := this.Base64ToBinary(*this.At(MkString("secret")) ); _ = rawSecret;
      signature := this.Hmac(this.Encode(auth ), rawSecret , MkString("sha512") , MkString("base64") ); _ = signature;
...
func (this *Gopax) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNot(GoIsArray(response ))) {
    ...
    errorMessage := this.SafeString(response , MkString("errorMessage") ); _ = errorMessage;
