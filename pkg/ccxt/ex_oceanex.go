...
func (this *Oceanex) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	request := MkMap(&VarMap{"show_details": MkBool(true)})
	_ = request
	response := this.Call(MkString("publicGetMarkets"), this.Extend(request, params))
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		market := *(markets).At(i)
		_ = market
		id := this.SafeValue(market, MkString("id"))
		_ = id
		name := this.SafeValue(market, MkString("name"))
		_ = name
		baseId := MkUndefined()
		_ = baseId
		quoteId := MkUndefined()
...
func (this *Oceanex) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetTickersPair"), this.Extend(request, params))
	_ = response
...
func (this *Oceanex) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = marketIds
	request := MkMap(&VarMap{"markets": marketIds})
	_ = request
	response := this.Call(MkString("publicGetTickersMulti"), this.Extend(request, params))
	_ = response
...
func (this *Oceanex) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	ticker := this.SafeValue(data, MkString("ticker"), MkMap(&VarMap{}))
	_ = ticker
	timestamp := this.SafeTimestamp(data, MkString("at"))
	_ = timestamp
	return MkMap(&VarMap{
		"symbol":        *(market).At(MkString("symbol")),
		"timestamp":     timestamp,
...
func (this *Oceanex) FetchOrderBooks(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetOrderBookMulti"), this.Extend(request, params))
	_ = response
...
func (this *Oceanex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	side := this.SafeValue(trade, MkString("side"))
	_ = side
	...
	marketId := this.SafeValue(trade, MkString("market"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market)
	_ = symbol
	timestamp := this.SafeTimestamp(trade, MkString("created_on"))
	_ = timestamp
...
func (this *Oceanex) FetchAllTradingFees(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("publicGetFeesTrading"), params)
	_ = response
	data := this.SafeValue(response, MkString("data"))
	_ = data
	result := MkMap(&VarMap{})
	...
	for i := MkInteger(0); IsTrue(OpLw(i, data.Length)); OpIncr(&i) {
		group := *(data).At(i)
		_ = group
		maker := this.SafeValue(group, MkString("ask_fee"), MkMap(&VarMap{}))
		_ = maker
		taker := this.SafeValue(group, MkString("bid_fee"), MkMap(&VarMap{}))
		_ = taker
		marketId := this.SafeString(group, MkString("market"))
		_ = marketId
		symbol := this.SafeSymbol(marketId)
		_ = symbol
...
func (this *Oceanex) FetchKey(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("privateGetKey"), params)
	_ = response
...
func (this *Oceanex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	data := this.SafeValue(response, MkString("data"))
	_ = data
	balances := this.SafeValue(data, MkString("accounts"))
	_ = balances
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = balance
		currencyId := this.SafeValue(balance, MkString("currency"))
		_ = currencyId
...
func (this *Oceanex) FetchOrders(goArgs ...*Variant) *Variant {
	...
	_ = states
	query := this.Omit(params, MkString("states"))
	_ = query
	request := MkMap(&VarMap{
		"market":     *(market).At(MkString("id")),
		"states":     states,
	...
	response := this.Call(MkString("privateGetOrdersFilter"), this.Extend(request, query))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
	result := MkArray(&VarArray{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, data.Length)); OpIncr(&i) {
		orders := this.SafeValue(*(data).At(i), MkString("orders"), MkArray(&VarArray{}))
		_ = orders
		status := this.ParseOrderStatus(this.SafeValue(*(data).At(i), MkString("state")))
		_ = status
		parsedOrders := this.ParseOrders(orders, market, since, limit, MkMap(&VarMap{"status": status}))
		_ = parsedOrders
...
func (this *Oceanex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	status := this.ParseOrderStatus(this.SafeValue(order, MkString("state")))
	_ = status
	marketId := this.SafeString2(order, MkString("market"), MkString("market_id"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market)
	_ = symbol
	timestamp := this.SafeTimestamp(order, MkString("created_on"))
	_ = timestamp
	if IsTrue(OpEq2(timestamp, MkUndefined())) {
...
func (this *Oceanex) CreateOrders(goArgs ...*Variant) *Variant {
	symbol := GoGetArg(goArgs, 0, MkUndefined())
	_ = symbol
	orders := GoGetArg(goArgs, 1, MkUndefined())
	_ = orders
	params := GoGetArg(goArgs, 2, MkMap(&VarMap{}))
	_ = params
	this.LoadMarkets()
	...
	_ = request
	response := this.Call(MkString("privatePostOrdersMulti"), this.Extend(request, params))
	_ = response
...
func (this *Oceanex) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privatePostOrderDelete"), this.Extend(MkMap(&VarMap{"id": id}), params))
	_ = response
	data := this.SafeValue(response, MkString("data"))
	_ = data
...
func (this *Oceanex) CancelOrders(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privatePostOrderDeleteMulti"), this.Extend(MkMap(&VarMap{"ids": ids}), params))
	_ = response
	data := this.SafeValue(response, MkString("data"))
	_ = data
	return this.ParseOrders(data)
...
func (this *Oceanex) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privatePostOrdersClear"), params)
	_ = response
	data := this.SafeValue(response, MkString("data"))
	_ = data
	return this.ParseOrders(data)
...
func (this *Oceanex) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		if IsTrue(OpOr(OpEq2(path, MkString("tickers_multi")), OpEq2(path, MkString("order_book/multi")))) {
			request := MkString("?")
			_ = request
			markets := this.SafeValue(params, MkString("markets"))
			_ = markets
			for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
				OpAddAssign(&request, OpAdd(MkString("markets[]="), OpAdd(*(markets).At(i), MkString("&"))))
			}
			limit := this.SafeValue(params, MkString("limit"))
			_ = limit
			if IsTrue(OpNotEq2(limit, MkUndefined())) {
		...
		if IsTrue(OpEq2(api, MkString("private"))) {
			...
			_ = request
			jwt_token := this.Jwt(request, this.Encode(*this.At(MkString("secret"))), MkString("RS256"))
			_ = jwt_token
