...
func (this *Ripio) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetPair"), params ); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  results := this.SafeValue(response , MkString("results") , MkArray(&VarArray{})); _ = results;
  for i := MkInteger(0) ; IsTrue(OpLw(i , results.Length )); OpIncr(& i ){
    market := *(results ).At(i ); _ = market;
    ...
    active := this.SafeValue(market , MkString("enabled") , MkBool(true) ); _ = active;
    fees := this.SafeValue(market , MkString("fees") , MkArray(&VarArray{})); _ = fees;
    firstFee := this.SafeValue(fees , MkInteger(0) , MkMap(&VarMap{})); _ = firstFee;
    maker := this.SafeNumber(firstFee , MkString("maker_fee") , MkNumber(0.0) ); _ = maker;
    taker := this.SafeNumber(firstFee , MkString("taker_fee") , MkNumber(0.0) ); _ = taker;
...
func (this *Ripio) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , results.Length )); OpIncr(& i ){
    currency := *(results ).At(i ); _ = currency;
    ...
    active := this.SafeValue(currency , MkString("enabled") , MkBool(true) ); _ = active;
    precision := this.SafeInteger(currency , MkString("decimal_places") ); _ = precision;
...
func (this *Ripio) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  average := this.SafeNumber(ticker , MkString("avg") ); _ = average;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Ripio) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetRatePair"), this.Extend(request , params )); _ = response;
...
func (this *Ripio) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicGetRateAll"), params ); _ = response;
  result := MkMap(&VarMap{}); _ = result;
...
func (this *Ripio) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  orderbook := this.ParseOrderBook(response , symbol , MkUndefined() , MkString("buy") , MkString("sell") , MkString("price") , MkString("amount") ); _ = orderbook;
...
func (this *Ripio) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  takerOrMaker := OpTrinary(OpEq2(takerSide , side ), MkString("taker") , MkString("maker") ); _ = takerOrMaker;
  if IsTrue(OpNotEq2(side , MkUndefined() )) {
    side = side.ToLowerCase();
  }
  priceString := this.SafeString2(trade , MkString("price") , MkString("match_price") ); _ = priceString;
  amountString := this.SafeString2(trade , MkString("amount") , MkString("exchanged") ); _ = amountString;
  ...
  feeCost := this.SafeNumber(trade , OpAdd(takerOrMaker , MkString("_fee") )); _ = feeCost;
  orderId := this.SafeString(trade , takerOrMaker ); _ = orderId;
...
func (this *Ripio) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetBalancesExchangeBalances"), params ); _ = response;
...
func (this *Ripio) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrderPair"), this.Extend(request , params )); _ = response;
...
func (this *Ripio) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrderPairOrderIdCancel"), this.Extend(request , params )); _ = response;
  return this.ParseOrder(response , market );
}
...
func (this *Ripio) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrderPairOrderId"), this.Extend(request , params )); _ = response;
  return this.ParseOrder(response , market );
}
...
func (this *Ripio) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrderPair"), this.Extend(request , params )); _ = response;
  results := this.SafeValue(response , MkString("results") , MkMap(&VarMap{})); _ = results;
  data := this.SafeValue(results , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Ripio) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"status":MkString("OPEN,PART") }); _ = request;
...
func (this *Ripio) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"status":MkString("CLOS,CANC,COMP") }); _ = request;
  return this.FetchOrders(symbol , since , limit , this.Extend(request , params ));
}
...
func (this *Ripio) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  cost := this.SafeNumber(order , MkString("notional") ); _ = cost;
  type_ := this.SafeStringLower(order , MkString("order_type") ); _ = type_;
  priceField := OpTrinary(OpEq2(type_ , MkString("market") ), MkString("fill_price") , MkString("limit_price") ); _ = priceField;
  price := this.SafeNumber(order , priceField ); _ = price;
  ...
  average := this.SafeValue(order , MkString("fill_price") ); _ = average;
  ...
  if IsTrue(OpNotEq2(fills , MkUndefined() )) {
    numFills := OpCopy(fills.Length ); _ = numFills;
...
func (this *Ripio) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetTradePair"), this.Extend(request , params )); _ = response;
  results := this.SafeValue(response , MkString("results") , MkMap(&VarMap{})); _ = results;
  data := this.SafeValue(results , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Ripio) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  detail := this.SafeString(response , MkString("detail") ); _ = detail;
  ...
  if IsTrue(OpNotEq2(errors , MkUndefined() )) {
    feedback := OpAdd(*this.At(MkString("id")) , OpAdd(MkString(" ") , body )); _ = feedback;
    keys := GoGetKeys(errors ); _ = keys;
    for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
      key := *(keys ).At(i ); _ = key;
      error := this.SafeValue(errors , key , MkArray(&VarArray{})); _ = error;
      message := this.SafeString(error , MkInteger(0) ); _ = message;
