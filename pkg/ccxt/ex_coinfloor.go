...
func (this *Coinfloor) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  symbol := this.SafeString(params , MkString("symbol") ); _ = symbol;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(*(params ).At(MkString("symbol") ));
    query = this.Omit(params , MkString("symbol") );
  }
  marketId := this.SafeString(params , MkString("id") ); _ = marketId;
  ...
  response := this.Call(MkString("privatePostIdBalance"), this.Extend(request , query )); _ = response;
  result := MkMap(&VarMap{"info":response }); _ = result;
  base := *(market ).At(MkString("base") ); _ = base;
  quote := *(market ).At(MkString("quote") ); _ = quote;
  baseIdLower := this.SafeStringLower(market , MkString("baseId") ); _ = baseIdLower;
  quoteIdLower := this.SafeStringLower(market , MkString("quoteId") ); _ = quoteIdLower;
...
func (this *Coinfloor) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetIdOrderBook"), this.Extend(request , params )); _ = response;
...
func (this *Coinfloor) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"id":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetIdTicker"), this.Extend(request , params )); _ = response;
...
func (this *Coinfloor) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"id":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetIdTransactions"), this.Extend(request , params )); _ = response;
...
func (this *Coinfloor) FetchLedger(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostIdUserTransactions"), this.Extend(request , params )); _ = response;
...
func (this *Coinfloor) ParseLedgerEntry(goArgs ...*Variant) *Variant{
  ...
  baseAmount := OpCopy(MkUndefined() ); _ = baseAmount;
  quoteAmount := OpCopy(MkUndefined() ); _ = quoteAmount;
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    key := *(keys ).At(i ); _ = key;
    if IsTrue(OpGt(key.IndexOf(MkString("_") ), MkInteger(0) )) {
      parts := key.Split(MkString("_") ); _ = parts;
      numParts := OpCopy(parts.Length ); _ = numParts;
      if IsTrue(OpEq2(numParts , MkInteger(2) )) {
        tmpBaseAmount := this.SafeNumber(item , *(parts ).At(MkInteger(0) )); _ = tmpBaseAmount;
        tmpQuoteAmount := this.SafeNumber(item , *(parts ).At(MkInteger(1) )); _ = tmpQuoteAmount;
  ...
  referenceId := this.SafeString(item , MkString("id") ); _ = referenceId;
  timestamp := this.Parse8601(this.SafeString(item , MkString("datetime") )); _ = timestamp;
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(item , MkString("fee") ); _ = feeCost;
  result := MkMap(&VarMap{
        "id":MkUndefined() ,
        "timestamp":timestamp ,
        "datetime":this.Iso8601(timestamp ),
  ...
  if IsTrue(OpEq2(type_ , MkString("trade") )) {
    ...
    amount := OpTrinary(OpEq2(baseAmount , MkInteger(0) ), quoteAmount , baseAmount ); _ = amount;
    code := OpTrinary(OpEq2(baseAmount , MkInteger(0) ), quote , base ); _ = code;
    direction := OpTrinary(OpGt(amount , MkInteger(0) ), MkString("in") , MkString("out") ); _ = direction;
    if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
      fee = MkMap(&VarMap{
          "cost":feeCost ,
          "currency":code ,
...
func (this *Coinfloor) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"id":this.MarketId(symbol )}); _ = request;
  method := OpAdd(MkString("privatePostId") , this.Capitalize(side )); _ = method;
  ...
  timestamp := this.Parse8601(this.SafeString(response , MkString("datetime") )); _ = timestamp;
  return MkMap(&VarMap{
...
func (this *Coinfloor) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostSymbolCancelOrder"), request ); _ = response;
...
func (this *Coinfloor) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  rawType := this.SafeString(order , MkString("type") ); _ = rawType;
...
func (this *Coinfloor) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"id":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostIdOpenOrders"), this.Extend(request , params )); _ = response;
...
func (this *Coinfloor) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  message := this.SafeString(response , MkString("error_msg") ); _ = message;
...
func (this *Coinfloor) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    auth := OpAdd(*this.At(MkString("uid")) , OpAdd(MkString("/") , OpAdd(*this.At(MkString("apiKey")) , OpAdd(MkString(":") , *this.At(MkString("password")) )))); _ = auth;
    signature := this.Decode(this.StringToBase64(auth )); _ = signature;
