...
func (this *Latoken) FetchTime(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetExchangeInfoTime"), params ); _ = response;
  return this.SafeInteger(response , MkString("unixTimeMiliseconds") );
}

func (this *Latoken) FetchMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetExchangeInfoPairs"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    quoteId := this.SafeString(market , MkString("quotedCurrency") ); _ = quoteId;
    numericId := this.SafeInteger(market , MkString("pairId") ); _ = numericId;
    ...
    pricePrecisionString := this.SafeString(market , MkString("pricePrecision") ); _ = pricePrecisionString;
...
func (this *Latoken) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetExchangeInfoCurrencies"), params ); _ = response;
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    currency := *(response ).At(i ); _ = currency;
    id := this.SafeString(currency , MkString("symbol") ); _ = id;
    numericId := this.SafeInteger(currency , MkString("currencyId") ); _ = numericId;
    code := this.SafeCurrencyCode(id ); _ = code;
    precision := this.SafeInteger(currency , MkString("precission") ); _ = precision;
    fee := this.SafeNumber(currency , MkString("fee") ); _ = fee;
...
func (this *Latoken) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    frozen := this.SafeString(balance , MkString("frozen") ); _ = frozen;
    pending := this.SafeString(balance , MkString("pending") ); _ = pending;
...
func (this *Latoken) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketDataOrderBookSymbolLimit"), this.Extend(request , params )); _ = response;
...
func (this *Latoken) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  percentage := this.SafeNumber(ticker , MkString("priceChange") ); _ = percentage;
...
func (this *Latoken) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketDataTickerSymbol"), this.Extend(request , params )); _ = response;
...
func (this *Latoken) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicGetMarketDataTickers"), params ); _ = response;
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
...
func (this *Latoken) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger2(trade , MkString("timestamp") , MkString("time") ); _ = timestamp;
...
func (this *Latoken) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketDataTradesSymbol"), this.Extend(request , params )); _ = response;
...
func (this *Latoken) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetOrderTrades"), this.Extend(request , params )); _ = response;
  trades := this.SafeValue(response , MkString("trades") , MkArray(&VarArray{})); _ = trades;
  return this.ParseTrades(trades , market , since , limit );
}
...
func (this *Latoken) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeTimestamp(order , MkString("timeCreated") ); _ = timestamp;
  ...
  filled := this.SafeNumber(order , MkString("executedAmount") ); _ = filled;
  status := this.ParseOrderStatus(this.SafeString(order , MkString("orderStatus") )); _ = status;
  timeFilled := this.SafeTimestamp(order , MkString("timeFilled") ); _ = timeFilled;
  ...
  clientOrderId := this.SafeString(order , MkString("cliOrdId") ); _ = clientOrderId;
...
func (this *Latoken) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrderGetOrder"), this.Extend(request , params )); _ = response;
...
func (this *Latoken) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  method := this.SafeString(*this.At(MkString("options")) , MkString("createOrderMethod") , MkString("private_post_order_new") ); _ = method;
  response := this.Call(method , this.Extend(request , params )); _ = response;
  return this.ParseOrder(response );
}
...
func (this *Latoken) CancelAllOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrderCancelAll"), this.Extend(request , params )); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  canceledOrders := this.SafeValue(response , MkString("cancelledOrders") , MkArray(&VarArray{})); _ = canceledOrders;
...
func (this *Latoken) Sign(goArgs ...*Variant) *Variant{
  ...
  urlencodedQuery := this.Urlencode(query ); _ = urlencodedQuery;
...
func (this *Latoken) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  errorMessage := this.SafeString(error , MkString("message") ); _ = errorMessage;
