...
func (this *Bitvavo) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  currencies := this.FetchCurrenciesFromCache(params ); _ = currencies;
  currenciesById := this.IndexBy(currencies , MkString("symbol") ); _ = currenciesById;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    baseCurrency := this.SafeValue(currenciesById , baseId ); _ = baseCurrency;
...
func (this *Bitvavo) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    depositStatus := this.SafeValue(currency , MkString("depositStatus") ); _ = depositStatus;
    deposit := OpEq2(depositStatus , MkString("OK") ); _ = deposit;
    withdrawalStatus := this.SafeValue(currency , MkString("withdrawalStatus") ); _ = withdrawalStatus;
    withdrawal := OpEq2(withdrawalStatus , MkString("OK") ); _ = withdrawal;
    active := OpAnd(deposit , withdrawal ); _ = active;
    name := this.SafeString(currency , MkString("name") ); _ = name;
    precision := this.SafeInteger(currency , MkString("decimals") , MkInteger(8) ); _ = precision;
...
func (this *Bitvavo) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTicker24h"), this.Extend(request , params )); _ = response;
...
func (this *Bitvavo) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicGetTicker24h"), params ); _ = response;
...
func (this *Bitvavo) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString2(trade , MkString("id") , MkString("fillId") ); _ = id;
  marketId := this.SafeInteger(trade , MkString("market") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("-") ); _ = symbol;
  taker := this.SafeValue(trade , MkString("taker") ); _ = taker;
...
func (this *Bitvavo) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketBook"), this.Extend(request , params )); _ = response;
...
func (this *Bitvavo) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    currencyId := this.SafeString(balance , MkString("symbol") ); _ = currencyId;
...
func (this *Bitvavo) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetDeposit"), this.Extend(request , params )); _ = response;
...
func (this *Bitvavo) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  isStopLimit := OpOr(OpEq2(type_ , MkString("stopLossLimit") ), OpEq2(type_ , MkString("takeProfitLimit") )); _ = isStopLimit;
  isStopMarket := OpOr(OpEq2(type_ , MkString("stopLoss") ), OpEq2(type_ , MkString("takeProfit") )); _ = isStopMarket;
    ...
    if IsTrue(OpEq2(type_ , MkString("limit") )) {
      ...
      *(request ).At(MkString("amount") )= this.AmountToPrecision(symbol , amount );
      ...
      if IsTrue(OpOr(isStopMarket , isStopLimit )) {
        stopPrice := this.SafeNumber2(params , MkString("stopPrice") , MkString("triggerAmount") ); _ = stopPrice;
...
func (this *Bitvavo) EditOrder(goArgs ...*Variant) *Variant{
  ...
  amountRemaining := this.SafeNumber(params , MkString("amountRemaining") ); _ = amountRemaining;
...
func (this *Bitvavo) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(order , MkString("created") ); _ = timestamp;
  ...
  type_ := this.SafeString(order , MkString("orderType") ); _ = type_;
  price := this.SafeNumber(order , MkString("price") ); _ = price;
  amount := this.SafeNumber(order , MkString("amount") ); _ = amount;
  remaining := this.SafeNumber(order , MkString("amountRemaining") ); _ = remaining;
  filled := this.SafeNumber(order , MkString("filledAmount") ); _ = filled;
  cost := this.SafeNumber(order , MkString("filledAmountQuote") ); _ = cost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(order , MkString("feePaid") ); _ = feeCost;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
  ...
  rawTrades := this.SafeValue(order , MkString("fills") , MkArray(&VarArray{})); _ = rawTrades;
...
func (this *Bitvavo) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetWithdrawalHistory"), this.Extend(request , params )); _ = response;
...
func (this *Bitvavo) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetDepositHistory"), this.Extend(request , params )); _ = response;
  return this.ParseTransactions(response , currency , since , limit );
}
...
func (this *Bitvavo) Sign(goArgs ...*Variant) *Variant{
  ...
  url := OpAdd(MkString("/") , OpAdd(*this.At(MkString("version")) , OpAdd(MkString("/") , this.ImplodeParams(path , params )))); _ = url;
  getOrDelete := OpOr(OpEq2(method , MkString("GET") ), OpEq2(method , MkString("DELETE") )); _ = getOrDelete;
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    auth := OpAdd(timestamp , OpAdd(method , OpAdd(url , payload ))); _ = auth;
    signature := this.Hmac(this.Encode(auth ), this.Encode(*this.At(MkString("secret")) )); _ = signature;
    accessWindow := this.SafeString(*this.At(MkString("options")) , MkString("BITVAVO-ACCESS-WINDOW") , MkString("10000") ); _ = accessWindow;
