...
func (this *Zaif) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  markets := this.Call(MkString("publicGetCurrencyPairsAll"), params ); _ = markets;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    market := *(markets ).At(i ); _ = market;
    id := this.SafeString(market , MkString("currency_pair") ); _ = id;
    ...
    fees := this.SafeValue(*(*this.At(MkString("options")) ).At(MkString("fees") ), symbol , *(*this.At(MkString("fees")) ).At(MkString("trading") )); _ = fees;
    taker := *(fees ).At(MkString("taker") ); _ = taker;
    maker := *(fees ).At(MkString("maker") ); _ = maker;
...
func (this *Zaif) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  deposit := this.SafeValue(balances , MkString("deposit") ); _ = deposit;
  result := MkMap(&VarMap{
        "info":response ,
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencyIds.Length )); OpIncr(& i ){
    ...
    balance := this.SafeString(funds , currencyId ); _ = balance;
...
func (this *Zaif) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  numTrades := OpCopy(response.Length ); _ = numTrades;
  if IsTrue(OpEq2(numTrades , MkInteger(1) )) {
    firstTrade := *(response ).At(MkInteger(0) ); _ = firstTrade;
...
func (this *Zaif) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  side := this.SafeString(order , MkString("action") ); _ = side;
  side = OpTrinary(OpEq2(side , MkString("bid") ), MkString("buy") , MkString("sell") );
...
func (this *Zaif) Withdraw(goArgs ...*Variant) *Variant{
  ...
  result := this.Call(MkString("privatePostWithdraw"), this.Extend(request , params )); _ = result;
  return MkMap(&VarMap{
        "info":result ,
...
func (this *Zaif) Nonce(goArgs ...*Variant) *Variant{
  nonce := parseFloat(OpDiv(this.Milliseconds(), MkInteger(1000) )); _ = nonce;
