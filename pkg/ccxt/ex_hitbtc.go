...
func (this *Hitbtc) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("publicGetSymbol"), params)
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		lotString := this.SafeString(market, MkString("quantityIncrement"))
		_ = lotString
		stepString := this.SafeString(market, MkString("tickSize"))
		_ = stepString
		lot := this.ParseNumber(lotString)
		_ = lot
		step := this.ParseNumber(stepString)
		_ = step
		...
		_ = precision
		taker := this.SafeNumber(market, MkString("takeLiquidityRate"))
		_ = taker
		maker := this.SafeNumber(market, MkString("provideLiquidityRate"))
		_ = maker
		feeCurrencyId := this.SafeString(market, MkString("feeCurrency"))
		_ = feeCurrencyId
...
func (this *Hitbtc) Transfer(goArgs ...*Variant) *Variant {
	...
	_ = currency
	requestAmount := this.CurrencyToPrecision(code, amount)
	_ = requestAmount
	...
	*(request).At(MkString("type")) = OpCopy(type_)
	response := this.Call(MkString("privatePostAccountTransfer"), this.Extend(request, params))
	_ = response
...
func (this *Hitbtc) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = id
		decimals := this.SafeInteger(currency, MkString("precisionTransfer"), MkInteger(8))
		_ = decimals
		precision := OpDiv(MkInteger(1), Math.Pow(MkInteger(10), decimals))
		_ = precision
		code := this.SafeCurrencyCode(id)
		_ = code
		payin := this.SafeValue(currency, MkString("payinEnabled"))
		_ = payin
		payout := this.SafeValue(currency, MkString("payoutEnabled"))
		_ = payout
		transfer := this.SafeValue(currency, MkString("transferEnabled"))
		_ = transfer
		active := OpAnd(payin, OpAnd(payout, transfer))
		_ = active
		...
		name := this.SafeString(currency, MkString("fullName"))
		_ = name
...
func (this *Hitbtc) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	type_ := this.SafeString(params, MkString("type"), MkString("trading"))
	_ = type_
	fetchBalanceAccounts := this.SafeValue(*this.At(MkString("options")), MkString("fetchBalanceMethod"), MkMap(&VarMap{}))
	_ = fetchBalanceAccounts
	typeId := this.SafeString(fetchBalanceAccounts, type_)
	_ = typeId
	...
	method := OpAdd(MkString("privateGet"), OpAdd(this.Capitalize(typeId), MkString("Balance")))
	_ = method
...
func (this *Hitbtc) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetCandlesSymbol"), this.Extend(request, params))
	_ = response
...
func (this *Hitbtc) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetOrderbookSymbol"), this.Extend(request, params))
	_ = response
...
func (this *Hitbtc) ParseTicker(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.Parse8601(*(ticker).At(MkString("timestamp")))
	_ = timestamp
...
func (this *Hitbtc) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetTickerSymbol"), this.Extend(request, params))
	_ = response
...
func (this *Hitbtc) ParseTrade(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.Parse8601(*(trade).At(MkString("timestamp")))
	_ = timestamp
	...
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCurrencyCode := OpTrinary(market, *(market).At(MkString("feeCurrency")), MkUndefined())
		_ = feeCurrencyCode
		fee = MkMap(&VarMap{
			"cost":     feeCost,
	...
	orderId := this.SafeString(trade, MkString("clientOrderId"))
	_ = orderId
...
func (this *Hitbtc) FetchTransactions(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetAccountTransactions"), this.Extend(request, params))
	_ = response
...
func (this *Hitbtc) CreateOrder(goArgs ...*Variant) *Variant {
	...
	uuid := this.Uuid()
	_ = uuid
	parts := uuid.Split(MkString("-"))
	_ = parts
	clientOrderId := parts.Join(MkString(""))
	_ = clientOrderId
...
func (this *Hitbtc) EditOrder(goArgs ...*Variant) *Variant {
	...
	parts := uuid.Split(MkString("-"))
	_ = parts
	requestClientId := parts.Join(MkString(""))
	_ = requestClientId
	...
	response := this.Call(MkString("privatePatchOrderClientOrderId"), this.Extend(request, params))
	_ = response
...
func (this *Hitbtc) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"clientOrderId": id})
	_ = request
	response := this.Call(MkString("privateDeleteOrderClientOrderId"), this.Extend(request, params))
	_ = response
	return this.ParseOrder(response)
}
...
func (this *Hitbtc) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	created := this.Parse8601(this.SafeString(order, MkString("createdAt")))
	_ = created
	...
	_ = amount
	filled := this.SafeNumber(order, MkString("cumQuantity"))
	_ = filled
	status := this.ParseOrderStatus(this.SafeString(order, MkString("status")))
	_ = status
	id := this.SafeString(order, MkString("clientOrderId"))
	_ = id
	clientOrderId := OpCopy(id)
	_ = clientOrderId
	...
	_ = side
	trades := this.SafeValue(order, MkString("tradesReport"))
	_ = trades
	fee := OpCopy(MkUndefined())
	_ = fee
...
func (this *Hitbtc) FetchOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"clientOrderId": id})
	_ = request
	response := this.Call(MkString("privateGetHistoryOrder"), this.Extend(request, params))
	_ = response
...
func (this *Hitbtc) FetchOpenOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"clientOrderId": id})
	_ = request
	response := this.Call(MkString("privateGetOrderClientOrderId"), this.Extend(request, params))
	_ = response
	return this.ParseOrder(response)
}
...
func (this *Hitbtc) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetHistoryTrades"), this.Extend(request, params))
	_ = response
	return this.ParseTrades(response, market, since, limit)
}
...
func (this *Hitbtc) FetchOrderTrades(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetHistoryOrderOrderIdTrades"), this.Extend(request, params))
	_ = response
	numOrders := OpCopy(response.Length)
	_ = numOrders
	if IsTrue(OpGt(numOrders, MkInteger(0))) {
...
func (this *Hitbtc) CreateDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostAccountCryptoAddressCurrency"), this.Extend(request, params))
	_ = response
...
func (this *Hitbtc) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"currency": *(currency).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("privateGetAccountCryptoAddressCurrency"), this.Extend(request, params))
	_ = response
	address := this.SafeString(response, MkString("address"))
	_ = address
	this.CheckAddress(address)
...
func (this *Hitbtc) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostAccountCryptoWithdraw"), this.Extend(request, params))
	_ = response
	return MkMap(&VarMap{
		"info": response,
...
func (this *Hitbtc) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	url := OpAdd(MkString("/api/"), OpAdd(*this.At(MkString("version")), MkString("/")))
	_ = url
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		payload := this.Encode(OpAdd(*this.At(MkString("apiKey")), OpAdd(MkString(":"), *this.At(MkString("secret")))))
		_ = payload
		auth := this.StringToBase64(payload)
		_ = auth
	...
	if IsTrue(OpGtEq(code, MkInteger(400))) {
		...
		if IsTrue(OpEq2(*(body).At(MkInteger(0)), MkString("{"))) {
			if IsTrue(OpHasMember(MkString("error"), response)) {
				errorCode := this.SafeString(*(response).At(MkString("error")), MkString("code"))
				_ = errorCode
				this.ThrowExactlyMatchedException(*this.At(MkString("exceptions")), errorCode, feedback)
				message := this.SafeString(*(response).At(MkString("error")), MkString("message"))
				_ = message
