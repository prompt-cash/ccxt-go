  ...
  if IsTrue(OpEq2(since , MkUndefined() )) {
    ...
    *(request ).At(MkString("start") )= parseInt(OpDiv(since , MkInteger(1000) ));
    if IsTrue(OpNotEq2(limit , MkUndefined() )) {
      end := this.Sum(*(request ).At(MkString("start") ), OpMulti(limit , this.ParseTimeframe(timeframe ))); _ = end;
      *(request ).At(MkString("end") )= OpCopy(end );
    }
  }
  response := this.Call(MkString("publicGetReturnChartData"), this.Extend(request , params )); _ = response;
...
func (this *Poloniex) LoadMarkets(goArgs ...*Variant) *Variant{
  ...
  currenciesByNumericId := this.SafeValue(*this.At(MkString("options")) , MkString("currenciesByNumericId") ); _ = currenciesByNumericId;
...
func (this *Poloniex) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  markets := this.Call(MkString("publicGetReturnTicker"), params ); _ = markets;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    ...
    limits := this.Extend(*this.At(MkString("limits")) , MkMap(&VarMap{"cost":MkMap(&VarMap{"min":this.SafeValue(*(*(*(*this.At(MkString("options")) ).At(MkString("limits") )).At(MkString("cost") )).At(MkString("min") ), quote )})})); _ = limits;
    isFrozen := this.SafeString(market , MkString("isFrozen") ); _ = isFrozen;
    active := OpNotEq2(isFrozen , MkString("1") ); _ = active;
...
func (this *Poloniex) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"account":MkString("all") }); _ = request;
  response := this.Call(MkString("privatePostReturnCompleteBalances"), this.Extend(request , params )); _ = response;
  result := MkMap(&VarMap{
        "info":response ,
        "timestamp":MkUndefined() ,
        "datetime":MkUndefined() ,
        }); _ = result;
  currencyIds := GoGetKeys(response ); _ = currencyIds;
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencyIds.Length )); OpIncr(& i ){
    currencyId := *(currencyIds ).At(i ); _ = currencyId;
    balance := this.SafeValue(response , currencyId , MkMap(&VarMap{})); _ = balance;
...
func (this *Poloniex) FetchTradingFees(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  fees := this.Call(MkString("privatePostReturnFeeInfo"), params ); _ = fees;
  return MkMap(&VarMap{
        "info":fees ,
...
func (this *Poloniex) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetReturnOrderBook"), this.Extend(request , params )); _ = response;
...
func (this *Poloniex) FetchOrderBooks(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"currencyPair":MkString("all") }); _ = request;
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
    *(request ).At(MkString("depth") )= OpCopy(limit );
  }
  response := this.Call(MkString("publicGetReturnOrderBook"), this.Extend(request , params )); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , marketIds.Length )); OpIncr(& i ){
    ...
    orderbook := this.ParseOrderBook(*(response ).At(marketId ), symbol ); _ = orderbook;
...
func (this *Poloniex) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  relativeChange := this.SafeNumber(ticker , MkString("percentChange") ); _ = relativeChange;
...
func (this *Poloniex) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicGetReturnTicker"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    ...
    ticker := *(response ).At(id ); _ = ticker;
...
func (this *Poloniex) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetReturnCurrencies"), params ); _ = response;
  ids := GoGetKeys(response ); _ = ids;
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    id := *(ids ).At(i ); _ = id;
    ...
    amountLimit := MkString("1e-8") ; _ = amountLimit;
    code := this.SafeCurrencyCode(id ); _ = code;
    active := OpAnd(OpEq2(*(currency ).At(MkString("delisted") ), MkInteger(0) ), OpNot(*(currency ).At(MkString("disabled") ))); _ = active;
...
func (this *Poloniex) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  response := this.Call(MkString("publicGetReturnTicker"), params ); _ = response;
  ticker := *(response ).At(*(market ).At(MkString("id") )); _ = ticker;
...
func (this *Poloniex) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  id := this.SafeString2(trade , MkString("globalTradeID") , MkString("tradeID") ); _ = id;
  orderId := this.SafeString(trade , MkString("orderNumber") ); _ = orderId;
  ...
  costString := this.SafeString(trade , MkString("total") ); _ = costString;
  ...
  feeDisplay := this.SafeString(trade , MkString("feeDisplay") ); _ = feeDisplay;
  if IsTrue(OpNotEq2(feeDisplay , MkUndefined() )) {
    parts := feeDisplay.Split(MkString(" ") ); _ = parts;
    feeCost := this.SafeNumber(parts , MkInteger(0) ); _ = feeCost;
    if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
      feeCurrencyId := this.SafeString(parts , MkInteger(1) ); _ = feeCurrencyId;
      feeCurrencyCode := this.SafeCurrencyCode(feeCurrencyId ); _ = feeCurrencyCode;
      feeRate := this.SafeString(parts , MkInteger(2) ); _ = feeRate;
      if IsTrue(OpNotEq2(feeRate , MkUndefined() )) {
        feeRate = feeRate.Replace(MkString("(") , MkString("") );
        feeRateParts := feeRate.Split(MkString("%") ); _ = feeRateParts;
    ...
    if IsTrue(OpAnd(OpNotEq2(feeCostString , MkUndefined() ), OpNotEq2(market , MkUndefined() ))) {
      feeCurrencyCode := OpTrinary(OpEq2(side , MkString("buy") ), *(market ).At(MkString("base") ), *(market ).At(MkString("quote") )); _ = feeCurrencyCode;
      feeBase := OpTrinary(OpEq2(side , MkString("buy") ), amountString , costString ); _ = feeBase;
      feeRateString := Precise.StringDiv(feeCostString , feeBase ); _ = feeRateString;
      fee = MkMap(&VarMap{
  ...
  takerAdjustment := this.SafeNumber(trade , MkString("takerAdjustment") ); _ = takerAdjustment;
...
func (this *Poloniex) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  trades := this.Call(MkString("publicGetReturnTradeHistory"), this.Extend(request , params )); _ = trades;
...
func (this *Poloniex) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"currencyPair":pair }); _ = request;
  if IsTrue(OpNotEq2(since , MkUndefined() )) {
    *(request ).At(MkString("start") )= parseInt(OpDiv(since , MkInteger(1000) ));
  ...
  response := this.Call(MkString("privatePostReturnTradeHistory"), this.Extend(request , params )); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  if IsTrue(OpNotEq2(market , MkUndefined() )) {
    ...
    if IsTrue(response ) {
      ...
      for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
        ...
        if IsTrue(OpHasMember(id , *this.At(MkString("markets_by_id")) )) {
          market = *(*this.At(MkString("markets_by_id")) ).At(id );
          trades := this.ParseTrades(*(response ).At(id ), market ); _ = trades;
          ...
          quote := this.SafeCurrencyCode(quoteId ); _ = quote;
          symbol := OpAdd(base , OpAdd(MkString("/") , quote )); _ = symbol;
          trades := *(response ).At(id ); _ = trades;
          for j := MkInteger(0) ; IsTrue(OpLw(j , trades.Length )); OpIncr(& j ){
            market := MkMap(&VarMap{
                  "symbol":symbol ,
                  "base":base ,
...
func (this *Poloniex) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  resultingTrades := this.SafeValue(order , MkString("resultingTrades") ); _ = resultingTrades;
  ...
  price := this.SafeNumber2(order , MkString("price") , MkString("rate") ); _ = price;
  remaining := this.SafeNumber(order , MkString("amount") ); _ = remaining;
  amount := this.SafeNumber(order , MkString("startingAmount") ); _ = amount;
  filled := OpCopy(MkUndefined() ); _ = filled;
  cost := MkInteger(0) ; _ = cost;
  ...
  if IsTrue(OpEq2(filled , MkUndefined() )) {
    if IsTrue(OpNotEq2(trades , MkUndefined() )) {
      filled = MkInteger(0) ;
      cost = MkInteger(0) ;
      tradesLength := OpCopy(trades.Length ); _ = tradesLength;
      if IsTrue(OpGt(tradesLength , MkInteger(0) )) {
        ...
        for i := MkInteger(0) ; IsTrue(OpLw(i , tradesLength )); OpIncr(& i ){
          ...
          tradeAmount := *(trade ).At(MkString("amount") ); _ = tradeAmount;
          tradePrice := *(trade ).At(MkString("price") ); _ = tradePrice;
  ...
  side := this.SafeString(order , MkString("side") , type_ ); _ = side;
  if IsTrue(OpEq2(type_ , side )) {
    type_ = OpCopy(MkUndefined() );
  }
  id := this.SafeString(order , MkString("orderNumber") ); _ = id;
  fee := OpCopy(MkUndefined() ); _ = fee;
...
func (this *Poloniex) ParseOpenOrders(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined()); _ = market;
  result := GoGetArg(goArgs, 2, MkUndefined()); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , orders.Length )); OpIncr(& i ){
    order := *(orders ).At(i ); _ = order;
...
func (this *Poloniex) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  pair := OpTrinary(market , *(market ).At(MkString("id") ), MkString("all") ); _ = pair;
  request := MkMap(&VarMap{"currencyPair":pair }); _ = request;
  response := this.Call(MkString("privatePostReturnOpenOrders"), this.Extend(request , params )); _ = response;
  extension := MkMap(&VarMap{"status":MkString("open") }); _ = extension;
  if IsTrue(OpEq2(market , MkUndefined() )) {
    marketIds := GoGetKeys(response ); _ = marketIds;
    openOrders := MkArray(&VarArray{}); _ = openOrders;
    for i := MkInteger(0) ; IsTrue(OpLw(i , marketIds.Length )); OpIncr(& i ){
      marketId := *(marketIds ).At(i ); _ = marketId;
      orders := *(response ).At(marketId ); _ = orders;
      m := *(*this.At(MkString("markets_by_id")) ).At(marketId ); _ = m;
...
func (this *Poloniex) EditOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostMoveOrder"), this.Extend(request , params )); _ = response;
...
func (this *Poloniex) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  clientOrderId := this.SafeValue(params , MkString("clientOrderId") ); _ = clientOrderId;
...
func (this *Poloniex) FetchOpenOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  id = id.ToString();
  request := MkMap(&VarMap{"orderNumber":id }); _ = request;
  response := this.Call(MkString("privatePostReturnOrderStatus"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(*(response ).At(MkString("result") ), id ); _ = result;
...
func (this *Poloniex) FetchClosedOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"orderNumber":id }); _ = request;
  response := this.Call(MkString("privatePostReturnOrderTrades"), this.Extend(request , params )); _ = response;
  trades := this.ParseTrades(response ); _ = trades;
  firstTrade := this.SafeValue(trades , MkInteger(0) ); _ = firstTrade;
  if IsTrue(OpEq2(firstTrade , MkUndefined() )) {
    panic(NewOrderNotFound(OpAdd(*this.At(MkString("id")) , OpAdd(MkString(" order id ") , OpAdd(id , MkString(" not found") )))));
  }
  symbol = this.SafeString(firstTrade , MkString("symbol") , symbol );
  side := this.SafeString(firstTrade , MkString("side") ); _ = side;
  timestamp := this.SafeNumber(firstTrade , MkString("timestamp") ); _ = timestamp;
...
func (this *Poloniex) FetchOrderStatus(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  orders := this.FetchOpenOrders(symbol , MkUndefined() , MkUndefined() , params ); _ = orders;
  indexed := this.IndexBy(orders , MkString("id") ); _ = indexed;
...
func (this *Poloniex) FetchOrderTrades(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"orderNumber":id }); _ = request;
  trades := this.Call(MkString("privatePostReturnOrderTrades"), this.Extend(request , params )); _ = trades;
...
func (this *Poloniex) CreateDepositAddress(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"currency":currencyId }); _ = request;
  response := this.Call(MkString("privatePostGenerateNewAddress"), this.Extend(request , params )); _ = response;
  ...
  if IsTrue(OpNotEq2(currency , MkUndefined() )) {
    depositAddress := this.SafeString(*(currency ).At(MkString("info") ), MkString("depositAddress") ); _ = depositAddress;
...
func (this *Poloniex) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostReturnDepositAddresses"), params ); _ = response;
  currencyId := OpCopy(MkUndefined() ); _ = currencyId;
  currency := OpCopy(MkUndefined() ); _ = currency;
  if IsTrue(OpHasMember(code , *this.At(MkString("currencies")) )) {
    currency = this.Currency(code );
  ...
  address := this.SafeString(response , currencyId ); _ = address;
  tag := OpCopy(MkUndefined() ); _ = tag;
  this.CheckAddress(address );
  if IsTrue(OpNotEq2(currency , MkUndefined() )) {
    depositAddress := this.SafeString(*(currency ).At(MkString("info") ), MkString("depositAddress") ); _ = depositAddress;
...
func (this *Poloniex) FetchTransactionsHelper(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  year := MkInteger(31104000) ; _ = year;
  now := this.Seconds(); _ = now;
  start := OpTrinary(OpNotEq2(since , MkUndefined() ), parseInt(OpDiv(since , MkInteger(1000) )), OpSub(now , OpMulti(MkInteger(10) , year ))); _ = start;
  request := MkMap(&VarMap{
        "start":start ,
        "end":now ,
        }); _ = request;
  ...
  response := this.Call(MkString("privatePostReturnDepositsWithdrawals"), this.Extend(request , params )); _ = response;
  return response ;
}
...
func (this *Poloniex) FetchTransactions(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.FetchTransactionsHelper(code , since , limit , params ); _ = response;
  currency := OpCopy(MkUndefined() ); _ = currency;
  if IsTrue(OpNotEq2(code , MkUndefined() )) {
    currency = this.Currency(code );
  }
  withdrawals := this.SafeValue(response , MkString("withdrawals") , MkArray(&VarArray{})); _ = withdrawals;
  deposits := this.SafeValue(response , MkString("deposits") , MkArray(&VarArray{})); _ = deposits;
  withdrawalTransactions := this.ParseTransactions(withdrawals , currency , since , limit ); _ = withdrawalTransactions;
  depositTransactions := this.ParseTransactions(deposits , currency , since , limit ); _ = depositTransactions;
  transactions := this.ArrayConcat(depositTransactions , withdrawalTransactions ); _ = transactions;
...
func (this *Poloniex) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  withdrawals := this.SafeValue(response , MkString("withdrawals") , MkArray(&VarArray{})); _ = withdrawals;
  transactions := this.ParseTransactions(withdrawals , currency , since , limit ); _ = transactions;
...
func (this *Poloniex) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  deposits := this.SafeValue(response , MkString("deposits") , MkArray(&VarArray{})); _ = deposits;
  transactions := this.ParseTransactions(deposits , currency , since , limit ); _ = transactions;
  return this.FilterByCurrencySinceLimit(transactions , code , since , limit );
}
...
func (this *Poloniex) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeTimestamp(transaction , MkString("timestamp") ); _ = timestamp;
  currencyId := this.SafeString(transaction , MkString("currency") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId ); _ = code;
  status := this.SafeString(transaction , MkString("status") , MkString("pending") ); _ = status;
  ...
  defaultType := OpTrinary(OpHasMember(MkString("withdrawalNumber") , transaction ), MkString("withdrawal") , MkString("deposit") ); _ = defaultType;
  type_ := this.SafeString(transaction , MkString("type") , defaultType ); _ = type_;
  id := this.SafeString2(transaction , MkString("withdrawalNumber") , MkString("depositNumber") ); _ = id;
  ...
  tag := this.SafeString(transaction , MkString("paymentID") ); _ = tag;
  feeCost := this.SafeNumber(transaction , MkString("fee") , MkInteger(0) ); _ = feeCost;
...
func (this *Poloniex) FetchPosition(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"currencyPair":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostGetMarginPosition"), this.Extend(request , params )); _ = response;
  return response ;
}
...
func (this *Poloniex) Sign(goArgs ...*Variant) *Variant{
  ...
  query := this.Extend(MkMap(&VarMap{"command":path }), params ); _ = query;
...
func (this *Poloniex) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("error") , response )) {
    message := *(response ).At(MkString("error") ); _ = message;
