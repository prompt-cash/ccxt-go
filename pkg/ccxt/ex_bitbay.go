...
func (this *Bitbay) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("v1_01PublicGetTradingTicker"), params)
	_ = response
	fiatCurrencies := this.SafeValue(*this.At(MkString("options")), MkString("fiatCurrencies"), MkArray(&VarArray{}))
	_ = fiatCurrencies
	result := MkArray(&VarArray{})
	_ = result
	items := this.SafeValue(response, MkString("items"))
	_ = items
	keys := GoGetKeys(items)
	_ = keys
	for i := MkInteger(0); IsTrue(OpLw(i, keys.Length)); OpIncr(&i) {
		...
		item := *(items).At(key)
		_ = item
		market := this.SafeValue(item, MkString("market"), MkMap(&VarMap{}))
		_ = market
		first := this.SafeValue(market, MkString("first"), MkMap(&VarMap{}))
		_ = first
		second := this.SafeValue(market, MkString("second"), MkMap(&VarMap{}))
		_ = second
		baseId := this.SafeString(first, MkString("currency"))
		_ = baseId
		quoteId := this.SafeString(second, MkString("currency"))
		_ = quoteId
		...
		_ = precision
		fees := this.SafeValue(*this.At(MkString("fees")), MkString("trading"), MkMap(&VarMap{}))
		_ = fees
		...
		maker := this.SafeNumber(fees, MkString("maker"))
		_ = maker
		taker := this.SafeNumber(fees, MkString("taker"))
		_ = taker
...
func (this *Bitbay) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("v1_01PrivateGetTradingOffer"), this.Extend(request, params))
	_ = response
	items := this.SafeValue(response, MkString("items"), MkArray(&VarArray{}))
	_ = items
...
func (this *Bitbay) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	amount := this.SafeNumber(order, MkString("startAmount"))
	_ = amount
	remaining := this.SafeNumber(order, MkString("currentAmount"))
	_ = remaining
...
func (this *Bitbay) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	query := MkMap(&VarMap{"query": this.Json(this.Extend(request, params))})
	_ = query
	response := this.Call(MkString("v1_01PrivateGetTradingHistoryTransactions"), query)
	_ = response
	items := this.SafeValue(response, MkString("items"))
	_ = items
	result := this.ParseTrades(items, MkUndefined(), since, limit)
	_ = result
...
func (this *Bitbay) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("v1_01PrivateGetBalancesBITBAYBalance"), params)
	_ = response
...
func (this *Bitbay) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = request
	orderbook := this.Call(MkString("publicGetIdOrderbook"), this.Extend(request, params))
	_ = orderbook
...
func (this *Bitbay) FetchTicker(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"id": this.MarketId(symbol)})
	_ = request
	ticker := this.Call(MkString("publicGetIdTicker"), this.Extend(request, params))
	_ = ticker
...
func (this *Bitbay) FetchLedger(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	balanceCurrencies := MkArray(&VarArray{})
	_ = balanceCurrencies
	...
	request := MkMap(&VarMap{"balanceCurrencies": balanceCurrencies})
	_ = request
	...
	response := this.Call(MkString("v1_01PrivateGetBalancesBITBAYHistory"), MkMap(&VarMap{"query": this.Json(request)}))
	_ = response
	items := *(response).At(MkString("items"))
	_ = items
...
func (this *Bitbay) ParseLedgerEntry(goArgs ...*Variant) *Variant {
	...
	_ = currency
	timestamp := this.SafeInteger(item, MkString("time"))
	_ = timestamp
	balance := this.SafeValue(item, MkString("balance"), MkMap(&VarMap{}))
	_ = balance
	currencyId := this.SafeString(balance, MkString("currency"))
	_ = currencyId
	code := this.SafeCurrencyCode(currencyId)
	_ = code
	change := this.SafeValue(item, MkString("change"), MkMap(&VarMap{}))
	_ = change
	amount := this.SafeNumber(change, MkString("total"))
	_ = amount
	...
	id := this.SafeString(item, MkString("historyId"))
	_ = id
	referenceId := this.SafeString(item, MkString("detailId"))
	_ = referenceId
	type_ := this.ParseLedgerEntryType(this.SafeString(item, MkString("type")))
	_ = type_
	fundsBefore := this.SafeValue(item, MkString("fundsBefore"), MkMap(&VarMap{}))
	_ = fundsBefore
	before := this.SafeNumber(fundsBefore, MkString("total"))
	_ = before
	fundsAfter := this.SafeValue(item, MkString("fundsAfter"), MkMap(&VarMap{}))
	_ = fundsAfter
	after := this.SafeNumber(fundsAfter, MkString("total"))
	_ = after
	return MkMap(&VarMap{
		"info":             item,
		"id":               id,
...
func (this *Bitbay) ParseOHLCV(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	first := this.SafeValue(ohlcv, MkInteger(1), MkMap(&VarMap{}))
	_ = first
...
func (this *Bitbay) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	_ = market
	tradingSymbol := OpAdd(*(market).At(MkString("baseId")), OpAdd(MkString("-"), *(market).At(MkString("quoteId"))))
	_ = tradingSymbol
	request := MkMap(&VarMap{
	...
	_ = duration
	timerange := OpMulti(limit, OpMulti(duration, MkInteger(1000)))
	_ = timerange
	...
	response := this.Call(MkString("v1_01PublicGetTradingCandleHistorySymbolResolution"), this.Extend(request, params))
	_ = response
	items := this.SafeValue(response, MkString("items"), MkArray(&VarArray{}))
	_ = items
...
func (this *Bitbay) ParseTrade(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.SafeInteger2(trade, MkString("time"), MkString("t"))
	_ = timestamp
	side := this.SafeStringLower2(trade, MkString("userAction"), MkString("ty"))
	_ = side
	wasTaker := this.SafeValue(trade, MkString("wasTaker"))
	_ = wasTaker
	...
	priceString := this.SafeString2(trade, MkString("rate"), MkString("r"))
	_ = priceString
	amountString := this.SafeString2(trade, MkString("amount"), MkString("a"))
	_ = amountString
	...
	_ = cost
	feeCost := this.SafeNumber(trade, MkString("commissionValue"))
	_ = feeCost
	...
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCcy := OpTrinary(OpEq2(side, MkString("buy")), *(market).At(MkString("base")), *(market).At(MkString("quote")))
		_ = feeCcy
		fee = MkMap(&VarMap{
			"currency": feeCcy,
			"cost":     feeCost,
	...
	order := this.SafeString(trade, MkString("offerId"))
	_ = order
...
func (this *Bitbay) FetchTrades(goArgs ...*Variant) *Variant {
	...
	tradingSymbol := OpAdd(*(market).At(MkString("baseId")), OpAdd(MkString("-"), *(market).At(MkString("quoteId"))))
	_ = tradingSymbol
	request := MkMap(&VarMap{"symbol": tradingSymbol})
	_ = request
	if IsTrue(OpNotEq2(since, MkUndefined())) {
	...
	response := this.Call(MkString("v1_01PublicGetTradingTransactionsSymbol"), this.Extend(request, params))
	_ = response
	items := this.SafeValue(response, MkString("items"))
	_ = items
...
func (this *Bitbay) CreateOrder(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("v1_01PrivatePostTradingOfferSymbol"), this.Extend(request, params))
	_ = response
	timestamp := this.Milliseconds()
	_ = timestamp
	id := this.SafeString(response, MkString("offerId"))
	_ = id
	completed := this.SafeValue(response, MkString("completed"), MkBool(false))
	_ = completed
	status := OpTrinary(completed, MkString("closed"), MkString("open"))
	_ = status
	filled := MkInteger(0)
	_ = filled
	cost := OpCopy(MkUndefined())
	_ = cost
	transactions := this.SafeValue(response, MkString("transactions"))
	_ = transactions
...
func (this *Bitbay) CancelOrder(goArgs ...*Variant) *Variant {
	...
	price := this.SafeValue(params, MkString("price"))
	_ = price
...
func (this *Bitbay) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	url := this.ImplodeHostname(*(*(*this.At(MkString("urls"))).At(MkString("api"))).At(api))
	_ = url
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		if IsTrue(OpEq2(api, MkString("v1_01Public"))) {
			...
			if IsTrue(OpEq2(api, MkString("v1_01Private"))) {
				...
				_ = query
				OpAddAssign(&url, OpAdd(MkString("/"), this.ImplodeParams(path, params)))
				nonce := (this.Milliseconds()).Call(MkString("toString"))
				_ = nonce
