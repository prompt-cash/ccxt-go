...
func (this *Idex) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = response
	response2 := this.Call(MkString("publicGetExchange"))
	_ = response2
	maker := this.SafeNumber(response2, MkString("makerFeeRate"))
	_ = maker
	taker := this.SafeNumber(response2, MkString("takerFeeRate"))
	_ = taker
	makerMin := this.SafeNumber(response2, MkString("makerTradeMinimum"))
	_ = makerMin
	takerMin := this.SafeNumber(response2, MkString("takerTradeMinimum"))
	_ = takerMin
	minCostETH := Math.Min(makerMin, takerMin)
	_ = minCostETH
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		entry := *(response).At(i)
		_ = entry
		marketId := this.SafeString(entry, MkString("market"))
		_ = marketId
		baseId := this.SafeString(entry, MkString("baseAsset"))
		_ = baseId
		quoteId := this.SafeString(entry, MkString("quoteAsset"))
		_ = quoteId
		...
		_ = symbol
		basePrecisionString := this.SafeString(entry, MkString("baseAssetPrecision"))
		_ = basePrecisionString
		quotePrecisionString := this.SafeString(entry, MkString("quoteAssetPrecision"))
		_ = quotePrecisionString
		basePrecision := this.ParsePrecision(basePrecisionString)
		_ = basePrecision
		quotePrecision := this.ParsePrecision(quotePrecisionString)
		_ = quotePrecision
...
func (this *Idex) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetCandles"), this.Extend(request, params))
	_ = response
...
func (this *Idex) ParseOHLCV(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.SafeInteger(ohlcv, MkString("start"))
	_ = timestamp
	open := this.SafeNumber(ohlcv, MkString("open"))
	_ = open
	high := this.SafeNumber(ohlcv, MkString("high"))
	_ = high
	low := this.SafeNumber(ohlcv, MkString("low"))
	_ = low
	close := this.SafeNumber(ohlcv, MkString("close"))
	_ = close
	volume := this.SafeNumber(ohlcv, MkString("volume"))
	_ = volume
	return MkArray(&VarArray{
		timestamp,
		open,
...
func (this *Idex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	id := this.SafeString(trade, MkString("fillId"))
	_ = id
	...
	_ = amount
	cost := this.SafeNumber(trade, MkString("quoteQuantity"))
	_ = cost
	...
	symbol := this.SafeSymbol(marketId, market, MkString("-"))
	_ = symbol
	makerSide := this.SafeString(trade, MkString("makerSide"))
	_ = makerSide
	oppositeSide := OpTrinary(OpEq2(makerSide, MkString("buy")), MkString("sell"), MkString("buy"))
	_ = oppositeSide
	side := this.SafeString(trade, MkString("side"), oppositeSide)
	_ = side
	takerOrMaker := this.SafeString(trade, MkString("liquidity"), MkString("taker"))
	_ = takerOrMaker
	...
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCurrencyId := this.SafeString(trade, MkString("feeAsset"))
		_ = feeCurrencyId
		fee = MkMap(&VarMap{
			"cost":     feeCost,
...
func (this *Idex) ParseSide(goArgs ...*Variant) *Variant {
	book := GoGetArg(goArgs, 0, MkUndefined())
	_ = book
	side := GoGetArg(goArgs, 1, MkUndefined())
	_ = side
	bookSide := this.SafeValue(book, side, MkArray(&VarArray{}))
	_ = bookSide
	result := MkArray(&VarArray{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, bookSide.Length)); OpIncr(&i) {
		order := *(bookSide).At(i)
		_ = order
		price := this.SafeNumber(order, MkInteger(0))
		_ = price
		amount := this.SafeNumber(order, MkInteger(1))
		_ = amount
		orderCount := this.SafeInteger(order, MkInteger(2))
		_ = orderCount
	...
	descending := OpEq2(side, MkString("bids"))
	_ = descending
...
func (this *Idex) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = name
		currencyId := this.SafeString(entry, MkString("symbol"))
		_ = currencyId
		precisionString := this.SafeString(entry, MkString("exchangeDecimals"))
		_ = precisionString
		code := this.SafeCurrencyCode(currencyId)
		_ = code
		precision := this.ParsePrecision(precisionString)
		_ = precision
		lot := this.ParseNumber(precision)
		_ = lot
				...
				if e := recover().(*Variant); e != nil {
					ret_ = func(this *Idex) *Variant {
						...
						if IsTrue(OpIsType(e, InvalidAddress)) {
							walletAddress := *(extendedRequest).At(MkString("wallet"))
							_ = walletAddress
...
func (this *Idex) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"closed": MkBool(false)})
	_ = request
...
func (this *Idex) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"closed": MkBool(true)})
	_ = request
	return this.FetchOrdersHelper(symbol, since, limit, this.Extend(request, params))
}
...
func (this *Idex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = type_
	amount := this.SafeNumber(order, MkString("originalQuantity"))
	_ = amount
	filled := this.SafeNumber(order, MkString("executedQuantity"))
	_ = filled
	average := this.SafeNumber(order, MkString("avgExecutionPrice"))
	_ = average
...
func (this *Idex) AssociateWallet(goArgs ...*Variant) *Variant {
	walletAddress := GoGetArg(goArgs, 0, MkUndefined())
	_ = walletAddress
	params := GoGetArg(goArgs, 1, MkMap(&VarMap{}))
	_ = params
	nonce := this.Uuidv1()
	_ = nonce
	noPrefix := this.Remove0xPrefix(walletAddress)
	_ = noPrefix
	...
	_ = binary
	hash := this.Hash(binary, MkString("keccak"), MkString("hex"))
	_ = hash
	signature := this.SignMessageString(hash, *this.At(MkString("privateKey")))
	_ = signature
	request := MkMap(&VarMap{
	...
	_ = request
	result := this.Call(MkString("privatePostWallets"), request)
	_ = result
	return result
}
...
func (this *Idex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = stopLossTypeEnums
	stopPriceString := OpCopy(MkUndefined())
	_ = stopPriceString
	...
	_ = priceString
	typeLower := type_.ToLowerCase()
	_ = typeLower
	limitOrder := OpGt(typeLower.IndexOf(MkString("limit")), OpNeg(MkInteger(1)))
	_ = limitOrder
	...
	sideEnum := OpTrinary(OpEq2(side, MkString("buy")), MkInteger(0), MkInteger(1))
	_ = sideEnum
	walletBytes := this.Remove0xPrefix(*this.At(MkString("walletAddress")))
	_ = walletBytes
	network := this.SafeString(*this.At(MkString("options")), MkString("network"), MkString("ETH"))
	_ = network
	orderVersion := OpTrinary(OpEq2(network, MkString("ETH")), MkInteger(1), MkInteger(2))
	_ = orderVersion
	...
	_ = timeInForceEnums
	defaultTimeInForce := this.SafeString(*this.At(MkString("options")), MkString("defaultTimeInForce"), MkString("gtc"))
	_ = defaultTimeInForce
	timeInForce := this.SafeString(params, MkString("timeInForce"), defaultTimeInForce)
	_ = timeInForce
	timeInForceEnum := OpCopy(MkUndefined())
	_ = timeInForceEnum
	if IsTrue(OpHasMember(timeInForce, timeInForceEnums)) {
		timeInForceEnum = *(timeInForceEnums).At(timeInForce)
	} else {
		allOptions := GoGetKeys(timeInForceEnums)
		_ = allOptions
		asString := allOptions.Join(MkString(", "))
		_ = asString
	...
	defaultSelfTradePrevention := this.SafeString(*this.At(MkString("options")), MkString("defaultSelfTradePrevention"), MkString("cn"))
	_ = defaultSelfTradePrevention
	selfTradePrevention := this.SafeString(params, MkString("selfTradePrevention"), defaultSelfTradePrevention)
	_ = selfTradePrevention
	selfTradePreventionEnum := OpCopy(MkUndefined())
	_ = selfTradePreventionEnum
	if IsTrue(OpHasMember(selfTradePrevention, selfTradePreventionEnums)) {
		...
		allOptions := GoGetKeys(selfTradePreventionEnums)
		_ = allOptions
		asString := allOptions.Join(MkString(", "))
		_ = asString
	...
	if IsTrue(limitOrder) {
		encodedPrice := this.StringToBinary(this.Encode(priceString))
		_ = encodedPrice
		byteArray.Push(encodedPrice)
	}
	if IsTrue(OpHasMember(type_, stopLossTypeEnums)) {
		encodedPrice := this.StringToBinary(this.Encode(OpOr(stopPriceString, priceString)))
		_ = encodedPrice
		byteArray.Push(encodedPrice)
	}
	...
	_ = after
	allBytes := this.ArrayConcat(byteArray, after)
	_ = allBytes
	binary := this.BinaryConcatArray(allBytes)
	_ = binary
	hash := this.Hash(binary, MkString("keccak"), MkString("hex"))
	_ = hash
	signature := this.SignMessageString(hash, *this.At(MkString("privateKey")))
	...
	response := this.Call(MkString("privatePostOrders"), request)
	_ = response
	return this.ParseOrder(response, market)
}
...
func (this *Idex) Withdraw(goArgs ...*Variant) *Variant {
	...
	nonce := this.Uuidv1()
	_ = nonce
	amountString := this.CurrencyToPrecision(code, amount)
	_ = amountString
	...
	_ = request
	response := this.Call(MkString("privatePostWithdrawals"), request)
	_ = response
	id := this.SafeString(response, MkString("withdrawalId"))
	_ = id
	return MkMap(&VarMap{
		"info": response,
		"id":   id,
...
func (this *Idex) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = response
	canceledOrder := this.SafeValue(response, MkInteger(0))
	_ = canceledOrder
...
func (this *Idex) HandleErrors(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpHasMember(errorCode, *this.At(MkString("exceptions")))) {
		Exception := *(*this.At(MkString("exceptions"))).At(errorCode)
		_ = Exception
...
func (this *Idex) FetchTransactionsHelper(goArgs ...*Variant) *Variant {
	...
	method := *(params).At(MkString("method"))
	_ = method
...
func (this *Idex) ParseTransactionStatus(goArgs ...*Variant) *Variant {
	status := GoGetArg(goArgs, 0, MkUndefined())
	_ = status
	statuses := MkMap(&VarMap{"mined": MkString("ok")})
	_ = statuses
	return this.SafeString(statuses, status, status)
}
...
func (this *Idex) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	id := this.SafeString2(transaction, MkString("depositId"), MkString("withdrawId"))
	_ = id
	code := this.SafeCurrencyCode(this.SafeString(transaction, MkString("asset")), currency)
	_ = code
	amount := this.SafeNumber(transaction, MkString("quantity"))
	_ = amount
	txid := this.SafeString(transaction, MkString("txId"))
	_ = txid
	timestamp := this.SafeInteger(transaction, MkString("txTime"))
	_ = timestamp
	fee := OpCopy(MkUndefined())
	_ = fee
	...
	rawStatus := this.SafeString(transaction, MkString("txStatus"))
	_ = rawStatus
	status := this.ParseTransactionStatus(rawStatus)
	_ = status
	updated := this.SafeInteger(transaction, MkString("confirmationTime"))
	_ = updated
	return MkMap(&VarMap{
		"info":      transaction,
		"id":        id,
...
func (this *Idex) Sign(goArgs ...*Variant) *Variant {
	...
	network := this.SafeString(*this.At(MkString("options")), MkString("network"), MkString("ETH"))
	_ = network
	version := this.SafeString(*this.At(MkString("options")), MkString("version"), MkString("v1"))
	_ = version
	url := OpAdd(*(*(*this.At(MkString("urls"))).At(MkString("api"))).At(network), OpAdd(MkString("/"), OpAdd(version, OpAdd(MkString("/"), path))))
	_ = url
	keys := GoGetKeys(params)
	_ = keys
	length := OpCopy(keys.Length)
	_ = length
