...
func (this *Bybit) FetchTime(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("v2PublicGetTime"), params ); _ = response;
...
func (this *Bybit) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v2PublicGetSymbols"), params ); _ = response;
  ...
  linearQuoteCurrencies := this.SafeValue(options , MkString("linear") , MkMap(&VarMap{"USDT":MkBool(true) })); _ = linearQuoteCurrencies;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    market := *(markets ).At(i ); _ = market;
    id := this.SafeString2(market , MkString("name") , MkString("symbol") ); _ = id;
    ...
    linear := OpHasMember(quote , linearQuoteCurrencies ); _ = linear;
    inverse := OpNot(linear ); _ = inverse;
    symbol := OpAdd(base , OpAdd(MkString("/") , quote )); _ = symbol;
    baseQuote := OpAdd(base , quote ); _ = baseQuote;
    type_ := MkString("swap") ; _ = type_;
    if IsTrue(OpNotEq2(baseQuote , id )) {
      symbol = OpCopy(id );
      type_ = MkString("futures") ;
    }
    lotSizeFilter := this.SafeValue(market , MkString("lot_size_filter") , MkMap(&VarMap{})); _ = lotSizeFilter;
    priceFilter := this.SafeValue(market , MkString("price_filter") , MkMap(&VarMap{})); _ = priceFilter;
...
func (this *Bybit) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  open := this.SafeNumber(ticker , MkString("prev_price_24h") ); _ = open;
  percentage := this.SafeNumber(ticker , MkString("price_24h_pcnt") ); _ = percentage;
  ...
  baseVolume := this.SafeNumber(ticker , MkString("turnover_24h") ); _ = baseVolume;
  quoteVolume := this.SafeNumber(ticker , MkString("volume_24h") ); _ = quoteVolume;
...
func (this *Bybit) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v2PublicGetTickers"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkArray(&VarArray{})); _ = result;
  first := this.SafeValue(result , MkInteger(0) ); _ = first;
  timestamp := this.SafeTimestamp(response , MkString("time_now") ); _ = timestamp;
  ticker := this.ParseTicker(first , market ); _ = ticker;
...
func (this *Bybit) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("v2PublicGetTickers"), params ); _ = response;
  result := this.SafeValue(response , MkString("result") , MkArray(&VarArray{})); _ = result;
  tickers := MkMap(&VarMap{}); _ = tickers;
  for i := MkInteger(0) ; IsTrue(OpLw(i , result.Length )); OpIncr(& i ){
    ticker := this.ParseTicker(*(result ).At(i )); _ = ticker;
...
func (this *Bybit) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  method := OpTrinary(*(market ).At(MkString("linear") ), MkString("publicLinearGetKline") , MkString("v2PublicGetKlineList") ); _ = method;
...
func (this *Bybit) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  id := this.SafeString2(trade , MkString("id") , MkString("exec_id") ); _ = id;
  marketId := this.SafeString(trade , MkString("symbol") ); _ = marketId;
  market = this.SafeMarket(marketId , market );
  symbol := *(market ).At(MkString("symbol") ); _ = symbol;
  amountString := this.SafeString2(trade , MkString("qty") , MkString("exec_qty") ); _ = amountString;
  priceString := this.SafeString2(trade , MkString("exec_price") , MkString("price") ); _ = priceString;
  cost := this.SafeNumber(trade , MkString("exec_value") ); _ = cost;
  ...
  lastLiquidityInd := this.SafeString(trade , MkString("last_liquidity_ind") ); _ = lastLiquidityInd;
  takerOrMaker := OpTrinary(OpEq2(lastLiquidityInd , MkString("AddedLiquidity") ), MkString("maker") , MkString("taker") ); _ = takerOrMaker;
  feeCost := this.SafeNumber(trade , MkString("exec_fee") ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyCode := OpTrinary(*(market ).At(MkString("inverse") ), *(market ).At(MkString("base") ), *(market ).At(MkString("quote") )); _ = feeCurrencyCode;
    fee = MkMap(&VarMap{
        "cost":feeCost ,
...
func (this *Bybit) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  method := OpTrinary(*(market ).At(MkString("linear") ), MkString("publicLinearGetRecentTradingRecords") , MkString("v2PublicGetTradingRecords") ); _ = method;
  response := this.Call(method , this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
...
func (this *Bybit) ParseOrderBook(goArgs ...*Variant) *Variant{
  ...
  bidsKey := GoGetArg(goArgs, 3, MkString("Buy") ); _ = bidsKey;
  asksKey := GoGetArg(goArgs, 4, MkString("Sell") ); _ = asksKey;
  priceKey := GoGetArg(goArgs, 5, MkString("price") ); _ = priceKey;
  amountKey := GoGetArg(goArgs, 6, MkString("size") ); _ = amountKey;
  bids := MkArray(&VarArray{}); _ = bids;
  asks := MkArray(&VarArray{}); _ = asks;
  for i := MkInteger(0) ; IsTrue(OpLw(i , orderbook.Length )); OpIncr(& i ){
    bidask := *(orderbook ).At(i ); _ = bidask;
    side := this.SafeString(bidask , MkString("side") ); _ = side;
...
func (this *Bybit) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("v2PublicGetOrderBookL2"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkArray(&VarArray{})); _ = result;
  timestamp := this.SafeTimestamp(response , MkString("time_now") ); _ = timestamp;
...
func (this *Bybit) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  coin := this.SafeString(params , MkString("coin") ); _ = coin;
  ...
  response := this.Call(MkString("v2PrivateGetWalletBalance"), this.Extend(request , params )); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencyIds.Length )); OpIncr(& i ){
    ...
    balance := *(balances ).At(currencyId ); _ = balance;
...
func (this *Bybit) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString2(order , MkString("order_id") , MkString("stop_order_id") ); _ = id;
  type_ := this.SafeStringLower(order , MkString("order_type") ); _ = type_;
  ...
  amount := this.SafeNumber(order , MkString("qty") ); _ = amount;
  cost := this.SafeNumber(order , MkString("cum_exec_value") ); _ = cost;
  filled := this.SafeNumber(order , MkString("cum_exec_qty") ); _ = filled;
  remaining := this.SafeNumber(order , MkString("leaves_qty") ); _ = remaining;
  marketTypes := this.SafeValue(*this.At(MkString("options")) , MkString("marketTypes") , MkMap(&VarMap{})); _ = marketTypes;
  marketType := this.SafeString(marketTypes , symbol ); _ = marketType;
  ...
  lastTradeTimestamp := this.SafeTimestamp(order , MkString("last_exec_time") ); _ = lastTradeTimestamp;
  if IsTrue(OpEq2(lastTradeTimestamp , MkInteger(0) )) {
    lastTradeTimestamp = OpCopy(MkUndefined() );
  }
  status := this.ParseOrderStatus(this.SafeString2(order , MkString("order_status") , MkString("stop_order_status") )); _ = status;
  side := this.SafeStringLower(order , MkString("side") ); _ = side;
  feeCost := this.SafeNumber(order , MkString("cum_exec_fee") ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
  ...
  clientOrderId := this.SafeString(order , MkString("order_link_id") ); _ = clientOrderId;
  if IsTrue(OpAnd(OpNotEq2(clientOrderId , MkUndefined() ), OpLw(clientOrderId.Length , MkInteger(1) ))) {
    clientOrderId = OpCopy(MkUndefined() );
  }
  timeInForce := this.ParseTimeInForce(this.SafeString(order , MkString("time_in_force") )); _ = timeInForce;
  stopPrice := this.SafeNumber2(order , MkString("trigger_price") , MkString("stop_px") ); _ = stopPrice;
...
func (this *Bybit) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  stopOrderId := this.SafeString(params , MkString("stop_order_id") ); _ = stopOrderId;
  if IsTrue(OpEq2(stopOrderId , MkUndefined() )) {
    orderLinkId := this.SafeString(params , MkString("order_link_id") ); _ = orderLinkId;
...
func (this *Bybit) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  qty := this.AmountToPrecision(symbol , amount ); _ = qty;
  if IsTrue(*(market ).At(MkString("inverse") )) {
    qty = parseInt(qty );
  } else {
    qty = parseFloat(qty );
  ...
  clientOrderId := this.SafeString2(params , MkString("order_link_id") , MkString("clientOrderId") ); _ = clientOrderId;
  ...
  stopPx := this.SafeValue2(params , MkString("stop_px") , MkString("stopPrice") ); _ = stopPx;
  basePrice := this.SafeValue(params , MkString("base_price") ); _ = basePrice;
  method := OpCopy(MkUndefined() ); _ = method;
  if IsTrue(*(market ).At(MkString("swap") )) {
    if IsTrue(*(market ).At(MkString("linear") )) {
...
func (this *Bybit) CancelAllOrders(goArgs ...*Variant) *Variant{
  ...
  method := this.SafeString(options , MkString("method") , defaultMethod ); _ = method;
  response := this.Call(method , this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkArray(&VarArray{})); _ = result;
...
func (this *Bybit) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  defaultType := this.SafeString(*this.At(MkString("options")) , MkString("defaultType") , MkString("linear") ); _ = defaultType;
  marketTypes := this.SafeValue(*this.At(MkString("options")) , MkString("marketTypes") , MkMap(&VarMap{})); _ = marketTypes;
  marketType := this.SafeString(marketTypes , symbol , defaultType ); _ = marketType;
  defaultMethod := OpCopy(MkUndefined() ); _ = defaultMethod;
  marketDefined := OpNotEq2(market , MkUndefined() ); _ = marketDefined;
  linear := OpOr(OpAnd(marketDefined , *(market ).At(MkString("linear") )), OpEq2(marketType , MkString("linear") )); _ = linear;
  inverse := OpOr(OpAnd(marketDefined , OpAnd(*(market ).At(MkString("swap") ), *(market ).At(MkString("inverse") ))), OpEq2(marketType , MkString("inverse") )); _ = inverse;
  futures := OpOr(OpAnd(marketDefined , *(market ).At(MkString("futures") )), OpEq2(marketType , MkString("futures") )); _ = futures;
  if IsTrue(linear ) {
  ...
  if IsTrue(OpOr(OpHasMember(MkString("stop_order_id") , params ), OpHasMember(MkString("stop_order_status") , params ))) {
    stopOrderStatus := this.SafeValue(params , MkString("stop_order_status") ); _ = stopOrderStatus;
...
func (this *Bybit) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  status := this.SafeValue(options , MkString("order_status") , defaultStatuses ); _ = status;
...
func (this *Bybit) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  market := OpCopy(MkUndefined() ); _ = market;
  if IsTrue(OpEq2(symbol , MkUndefined() )) {
    orderId := this.SafeString(params , MkString("order_id") ); _ = orderId;
  ...
  response := this.Call(method , this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  trades := this.SafeValue2(result , MkString("trade_list") , MkString("data") , MkArray(&VarArray{})); _ = trades;
...
func (this *Bybit) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"wallet_fund_type":MkString("Deposit") }); _ = request;
  ...
  response := this.Call(MkString("v2PrivateGetWalletFundRecords"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  data := this.SafeValue(result , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Bybit) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("v2PrivateGetWalletWithdrawList"), this.Extend(request , params )); _ = response;
  result := this.SafeValue(response , MkString("result") , MkMap(&VarMap{})); _ = result;
  data := this.SafeValue(result , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Bybit) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.Parse8601(this.SafeString2(transaction , MkString("submited_at") , MkString("exec_time") )); _ = timestamp;
...
func (this *Bybit) ParseLedgerEntry(goArgs ...*Variant) *Variant{
  ...
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  currencyId := this.SafeString(item , MkString("coin") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  amount := this.SafeNumber(item , MkString("amount") ); _ = amount;
  after := this.SafeNumber(item , MkString("wallet_balance") ); _ = after;
  direction := OpTrinary(OpLw(amount , MkInteger(0) ), MkString("out") , MkString("in") ); _ = direction;
  ...
  timestamp := this.Parse8601(this.SafeString(item , MkString("exec_time") )); _ = timestamp;
  type_ := this.ParseLedgerEntryType(this.SafeString(item , MkString("type") )); _ = type_;
  id := this.SafeString(item , MkString("id") ); _ = id;
  referenceId := this.SafeString(item , MkString("tx_id") ); _ = referenceId;
  return MkMap(&VarMap{
        "id":id ,
        "currency":code ,
...
func (this *Bybit) FetchPositions(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(GoIsArray(symbols )) {
    length := OpCopy(symbols.Length ); _ = length;
...
func (this *Bybit) Sign(goArgs ...*Variant) *Variant{
  ...
  type_ := this.SafeString(api , MkInteger(0) ); _ = type_;
  section := this.SafeString(api , MkInteger(1) ); _ = section;
  url := this.ImplodeHostname(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(type_ )); _ = url;
  request := OpAdd(MkString("/") , OpAdd(type_ , OpAdd(MkString("/") , OpAdd(section , OpAdd(MkString("/") , path ))))); _ = request;
  if IsTrue(OpEq2(section , MkString("public") )) {
    ...
    if IsTrue(OpEq2(type_ , MkString("public") )) {
      ...
      timestamp := this.Nonce(); _ = timestamp;
      ...
      sortedQuery := this.Keysort(query ); _ = sortedQuery;
      auth := this.Rawencode(sortedQuery ); _ = auth;
...
func (this *Bybit) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  errorCode := this.SafeString(response , MkString("ret_code") ); _ = errorCode;
