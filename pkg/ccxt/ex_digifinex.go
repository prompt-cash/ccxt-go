...
func (this *Digifinex) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, data.Length)); OpIncr(&i) {
		...
		_ = code
		depositStatus := this.SafeValue(currency, MkString("deposit_status"), MkInteger(1))
		_ = depositStatus
		withdrawStatus := this.SafeValue(currency, MkString("withdraw_status"), MkInteger(1))
		_ = withdrawStatus
		active := OpAnd(depositStatus, withdrawStatus)
		_ = active
		fee := this.SafeNumber(currency, MkString("withdraw_fee_rate"))
		_ = fee
...
func (this *Digifinex) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = options
	method := this.SafeString(options, MkString("method"), MkString("fetch_markets_v2"))
	_ = method
...
func (this *Digifinex) FetchMarketsV2(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("publicGetTradesSymbols"), params)
	_ = response
	markets := this.SafeValue(response, MkString("symbol_list"), MkArray(&VarArray{}))
	_ = markets
	...
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		...
		_ = id
		baseId := this.SafeString(market, MkString("base_asset"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("quote_asset"))
		_ = quoteId
		...
		_ = limits
		isAllowed := this.SafeInteger(market, MkString("is_allow"), MkInteger(1))
		_ = isAllowed
		active := OpTrinary(isAllowed, MkBool(true), MkBool(false))
		_ = active
		type_ := MkString("spot")
		_ = type_
		spot := OpEq2(type_, MkString("spot"))
		_ = spot
		margin := OpEq2(type_, MkString("margin"))
		_ = margin
...
func (this *Digifinex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	method := OpAdd(MkString("privateGet"), OpAdd(this.Capitalize(type_), MkString("Assets")))
	_ = method
	response := this.Call(method, params)
	_ = response
	balances := this.SafeValue(response, MkString("list"), MkArray(&VarArray{}))
	_ = balances
...
func (this *Digifinex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = response
	timestamp := this.SafeTimestamp(response, MkString("date"))
	_ = timestamp
...
func (this *Digifinex) FetchTickers(goArgs ...*Variant) *Variant {
	...
	result := MkMap(&VarMap{})
	_ = result
	tickers := this.SafeValue(response, MkString("ticker"), MkArray(&VarArray{}))
	_ = tickers
	date := this.SafeInteger(response, MkString("date"))
	_ = date
	for i := MkInteger(0); IsTrue(OpLw(i, tickers.Length)); OpIncr(&i) {
		rawTicker := this.Extend(MkMap(&VarMap{"date": date}), *(tickers).At(i))
		_ = rawTicker
		ticker := this.ParseTicker(rawTicker)
		_ = ticker
...
func (this *Digifinex) FetchTicker(goArgs ...*Variant) *Variant {
	...
	tickers := this.SafeValue(response, MkString("ticker"), MkArray(&VarArray{}))
	_ = tickers
	firstTicker := this.SafeValue(tickers, MkInteger(0), MkMap(&VarMap{}))
	_ = firstTicker
	result := this.Extend(MkMap(&VarMap{"date": date}), firstTicker)
	_ = result
...
func (this *Digifinex) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	marketId := this.SafeStringUpper(ticker, MkString("symbol"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("_"))
	_ = symbol
	timestamp := this.SafeTimestamp(ticker, MkString("date"))
	_ = timestamp
...
func (this *Digifinex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = orderId
	timestamp := this.SafeTimestamp2(trade, MkString("date"), MkString("timestamp"))
	_ = timestamp
	side := this.SafeString2(trade, MkString("type"), MkString("side"))
	_ = side
	...
	symbol := this.SafeSymbol(marketId, market, MkString("_"))
	_ = symbol
	takerOrMaker := this.SafeValue(trade, MkString("is_maker"))
	_ = takerOrMaker
...
func (this *Digifinex) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpNotEq2(since, MkUndefined())) {
		startTime := parseInt(OpDiv(since, MkInteger(1000)))
		_ = startTime
...
func (this *Digifinex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	*(request).At(MkString("type")) = OpAdd(side, suffix)
	response := this.Call(MkString("privatePostMarketOrderNew"), this.Extend(request, params))
	_ = response
...
func (this *Digifinex) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostMarketOrderCancel"), this.Extend(request, params))
	_ = response
	canceledOrders := this.SafeValue(response, MkString("success"), MkArray(&VarArray{}))
	_ = canceledOrders
	numCanceledOrders := OpCopy(canceledOrders.Length)
	_ = numCanceledOrders
...
func (this *Digifinex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = id
	timestamp := this.SafeTimestamp(order, MkString("created_date"))
	_ = timestamp
	lastTradeTimestamp := this.SafeTimestamp(order, MkString("finished_date"))
...
func (this *Digifinex) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"market": orderType})
	_ = request
	if IsTrue(OpNotEq2(symbol, MkUndefined())) {
		market = this.Market(symbol)
		*(request).At(MkString("symbol")) = *(market).At(MkString("id"))
	}
	response := this.Call(MkString("privateGetMarketOrderCurrent"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
...
func (this *Digifinex) FetchOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetMarketOrderHistory"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
	return this.ParseOrders(data, market, since, limit)
...
func (this *Digifinex) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetMarketOrder"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
...
func (this *Digifinex) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetMarketMytrades"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("list"), MkArray(&VarArray{}))
	_ = data
	return this.ParseTrades(data, market, since, limit)
}
...
func (this *Digifinex) ParseLedgerEntry(goArgs ...*Variant) *Variant {
	...
	_ = currency
	id := this.SafeString(item, MkString("num"))
	_ = id
	account := OpCopy(MkUndefined())
	_ = account
	type_ := this.ParseLedgerEntryType(this.SafeString(item, MkString("type")))
	_ = type_
	code := this.SafeCurrencyCode(this.SafeString(item, MkString("currency_mark")), currency)
	_ = code
	timestamp := this.SafeTimestamp(item, MkString("time"))
	_ = timestamp
...
func (this *Digifinex) FetchLedger(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetMarketFinancelog"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	items := this.SafeValue(data, MkString("finance"), MkArray(&VarArray{}))
	_ = items
...
func (this *Digifinex) ParseDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = tag
	currencyId := this.SafeStringUpper(depositAddress, MkString("currency"))
	_ = currencyId
	code := this.SafeCurrencyCode(currencyId)
	_ = code
	return MkMap(&VarMap{
...
func (this *Digifinex) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
	addresses := this.ParseDepositAddresses(data)
	_ = addresses
	address := this.SafeValue(addresses, code)
	_ = address
...
func (this *Digifinex) FetchTransactionsByType(goArgs ...*Variant) *Variant {
	...
	method := OpTrinary(OpEq2(type_, MkString("deposit")), MkString("privateGetDepositHistory"), MkString("privateGetWithdrawHistory"))
	_ = method
...
func (this *Digifinex) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	currency := GoGetArg(goArgs, 1, MkUndefined())
	_ = currency
	id := this.SafeString2(transaction, MkString("id"), MkString("withdraw_id"))
	_ = id
	...
	_ = txid
	currencyId := this.SafeStringUpper(transaction, MkString("currency"))
	_ = currencyId
	code := this.SafeCurrencyCode(currencyId, currency)
	_ = code
	timestamp := this.Parse8601(this.SafeString(transaction, MkString("created_date")))
	_ = timestamp
	updated := this.Parse8601(this.SafeString(transaction, MkString("finished_date")))
	_ = updated
...
func (this *Digifinex) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostWithdrawNew"), this.Extend(request, params))
	_ = response
...
func (this *Digifinex) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	version := OpCopy(*this.At(MkString("version")))
	_ = version
	url := OpAdd(*(*this.At(MkString("urls"))).At(MkString("api")), OpAdd(MkString("/"), OpAdd(version, OpAdd(MkString("/"), this.ImplodeParams(path, params)))))
	_ = url
...
func (this *Digifinex) HandleErrors(goArgs ...*Variant) *Variant {
	...
	feedback := OpAdd(*this.At(MkString("id")), OpAdd(MkString(" "), responseBody))
	_ = feedback
