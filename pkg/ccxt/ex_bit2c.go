...
func (this *Bit2c) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  balance := this.Call(MkString("privateGetAccountBalanceV2"), params ); _ = balance;
  result := MkMap(&VarMap{
        "info":balance ,
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , codes.Length )); OpIncr(& i ){
    ...
    uppercase := (*(currency ).At(MkString("id") )).Call(MkString("toUpperCase") ); _ = uppercase;
...
func (this *Bit2c) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  orderbook := this.Call(MkString("publicGetExchangesPairOrderbook"), this.Extend(request , params )); _ = orderbook;
...
func (this *Bit2c) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"pair":this.MarketId(symbol )}); _ = request;
  ticker := this.Call(MkString("publicGetExchangesPairTicker"), this.Extend(request , params )); _ = ticker;
  timestamp := this.Milliseconds(); _ = timestamp;
  averagePrice := this.SafeNumber(ticker , MkString("av") ); _ = averagePrice;
  baseVolume := this.SafeNumber(ticker , MkString("a") ); _ = baseVolume;
  ...
  last := this.SafeNumber(ticker , MkString("ll") ); _ = last;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Bit2c) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  method := *(*this.At(MkString("options")) ).At(MkString("fetchTradesMethod") ); _ = method;
...
func (this *Bit2c) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  method := MkString("privatePostOrderAddOrder") ; _ = method;
...
func (this *Bit2c) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"pair":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetOrderMyOrders"), this.Extend(request , params )); _ = response;
  orders := this.SafeValue(response , *(market ).At(MkString("id") ), MkMap(&VarMap{})); _ = orders;
  asks := this.SafeValue(orders , MkString("ask") , MkArray(&VarArray{})); _ = asks;
  bids := this.SafeValue(orders , MkString("bid") , MkArray(&VarArray{})); _ = bids;
...
func (this *Bit2c) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  side := this.SafeValue(order , MkString("type") ); _ = side;
...
func (this *Bit2c) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  *(request ).At(MkString("take") )= OpCopy(limit );
  ...
  response := this.Call(MkString("privateGetOrderOrderHistory"), this.Extend(request , params )); _ = response;
  return this.ParseTrades(response , market , since , limit );
}
...
func (this *Bit2c) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  reference := this.SafeString(trade , MkString("reference") ); _ = reference;
  if IsTrue(OpNotEq2(reference , MkUndefined() )) {
    ...
    reference_parts := reference.Split(MkString("|") ); _ = reference_parts;
...
func (this *Bit2c) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    query := this.Extend(MkMap(&VarMap{"nonce":nonce }), params ); _ = query;
    ...
    signature := this.Hmac(this.Encode(auth ), this.Encode(*this.At(MkString("secret")) ), MkString("sha512") , MkString("base64") ); _ = signature;
