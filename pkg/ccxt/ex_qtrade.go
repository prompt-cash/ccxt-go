...
func (this *Qtrade) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = data
	markets := this.SafeValue(data, MkString("markets"), MkArray(&VarArray{}))
	_ = markets
	...
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		market := *(markets).At(i)
		_ = market
		marketId := this.SafeString(market, MkString("market_string"))
		_ = marketId
		numericId := this.SafeInteger(market, MkString("id"))
		_ = numericId
		baseId := this.SafeString(market, MkString("market_currency"))
		_ = baseId
		...
		_ = precision
		canView := this.SafeValue(market, MkString("can_view"), MkBool(false))
		_ = canView
		canTrade := this.SafeValue(market, MkString("can_trade"), MkBool(false))
		_ = canTrade
		active := OpAnd(canTrade, canView)
		_ = active
...
func (this *Qtrade) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, currencies.Length)); OpIncr(&i) {
		...
		_ = code
		name := this.SafeString(currency, MkString("long_name"))
		_ = name
		type_ := this.SafeString(currency, MkString("type"))
		_ = type_
		canWithdraw := this.SafeValue(currency, MkString("can_withdraw"), MkBool(true))
		_ = canWithdraw
		depositDisabled := this.SafeValue(currency, MkString("deposit_disabled"), MkBool(false))
		_ = depositDisabled
		config := this.SafeValue(currency, MkString("config"), MkMap(&VarMap{}))
		_ = config
		status := this.SafeString(currency, MkString("status"))
		_ = status
		active := OpAnd(canWithdraw, OpAnd(OpEq2(status, MkString("ok")), OpNot(depositDisabled)))
		_ = active
...
func (this *Qtrade) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetMarketMarketStringOhlcvInterval"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	ohlcvs := this.SafeValue(data, MkString("slices"), MkArray(&VarArray{}))
	_ = ohlcvs
...
func (this *Qtrade) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = marketId
	request := MkMap(&VarMap{"market_string": marketId})
	_ = request
	response := this.Call(MkString("publicGetOrderbookMarketString"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	...
	for i := MkInteger(0); IsTrue(OpLw(i, keys.Length)); OpIncr(&i) {
		...
		side := *(sides).At(key)
		_ = side
		bidasks := this.SafeValue(data, key, MkMap(&VarMap{}))
		_ = bidasks
		prices := GoGetKeys(bidasks)
		_ = prices
		result := MkArray(&VarArray{})
		_ = result
		for j := MkInteger(0); IsTrue(OpLw(j, prices.Length)); OpIncr(&j) {
			priceAsString := *(prices).At(j)
			_ = priceAsString
			price := this.SafeNumber(prices, j)
			_ = price
			amount := this.SafeNumber(bidasks, priceAsString)
			_ = amount
	...
	timestamp := this.SafeIntegerProduct(data, MkString("last_change"), MkNumber(0.001))
	_ = timestamp
...
func (this *Qtrade) ParseTicker(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	marketId := this.SafeString(ticker, MkString("id_hr"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("_"))
	_ = symbol
	timestamp := this.SafeIntegerProduct(ticker, MkString("last_change"), MkNumber(0.001))
	_ = timestamp
	previous := this.SafeNumber(ticker, MkString("day_open"))
	_ = previous
	last := this.SafeNumber(ticker, MkString("last"))
	_ = last
	day_change := this.SafeNumber(ticker, MkString("day_change"))
	_ = day_change
	percentage := OpCopy(MkUndefined())
	_ = percentage
	change := OpCopy(MkUndefined())
	_ = change
	average := this.SafeNumber(ticker, MkString("day_avg_price"))
	_ = average
	...
	baseVolume := this.SafeNumber(ticker, MkString("day_volume_market"))
	_ = baseVolume
	quoteVolume := this.SafeNumber(ticker, MkString("day_volume_base"))
	_ = quoteVolume
...
func (this *Qtrade) FetchTickers(goArgs ...*Variant) *Variant {
	...
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	tickers := this.SafeValue(data, MkString("markets"), MkArray(&VarArray{}))
	_ = tickers
	result := MkMap(&VarMap{})
	_ = result
...
func (this *Qtrade) FetchTicker(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	request := MkMap(&VarMap{"market_string": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetTickerMarketString"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
...
func (this *Qtrade) FetchTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"market_string": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetMarketMarketStringTrades"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
...
func (this *Qtrade) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"desc": MkBool(true)})
	_ = request
	market := OpCopy(MkUndefined())
	_ = market
	numericId := this.SafeValue(params, MkString("market_id"))
	_ = numericId
...
func (this *Qtrade) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = id
	timestamp := this.SafeIntegerProduct(trade, MkString("created_at_ts"), MkNumber(0.001))
	_ = timestamp
	...
	_ = side
	marketId := this.SafeString(trade, MkString("market_string"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("_"))
	_ = symbol
	cost := this.SafeNumber2(trade, MkString("base_volume"), MkString("base_amount"))
	_ = cost
	priceString := this.SafeString(trade, MkString("price"))
	_ = priceString
	amountString := this.SafeString2(trade, MkString("market_amount"), MkString("amount"))
	_ = amountString
	...
	_ = fee
	feeCost := this.SafeNumber(trade, MkString("base_fee"))
	_ = feeCost
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCurrencyCode := OpTrinary(OpEq2(market, MkUndefined()), MkUndefined(), *(market).At(MkString("quote")))
		_ = feeCurrencyCode
		fee = MkMap(&VarMap{
	...
	taker := this.SafeValue(trade, MkString("taker"), MkBool(true))
	_ = taker
...
func (this *Qtrade) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetBalancesAll"), params)
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	balances := this.SafeValue(data, MkString("balances"), MkArray(&VarArray{}))
	_ = balances
	result := MkMap(&VarMap{
		"info":      response,
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = code
		account := OpTrinary(OpHasMember(code, result), *(result).At(code), this.Account())
		_ = account
...
func (this *Qtrade) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	method := OpTrinary(OpEq2(side, MkString("sell")), MkString("privatePostSellLimit"), MkString("privatePostBuyLimit"))
	_ = method
	response := this.Call(method, this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	order := this.SafeValue(data, MkString("order"), MkMap(&VarMap{}))
	_ = order
...
func (this *Qtrade) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	sideType := this.SafeString(order, MkString("order_type"))
	_ = sideType
	orderType := OpCopy(MkUndefined())
	_ = orderType
	side := OpCopy(MkUndefined())
	_ = side
	if IsTrue(OpNotEq2(sideType, MkUndefined())) {
		parts := sideType.Split(MkString("_"))
		_ = parts
	...
	_ = price
	amount := this.SafeNumber(order, MkString("market_amount"))
	_ = amount
	remaining := this.SafeNumber(order, MkString("market_amount_remaining"))
	_ = remaining
	open := this.SafeValue(order, MkString("open"), MkBool(false))
	_ = open
	closeReason := this.SafeString(order, MkString("close_reason"))
	_ = closeReason
	...
	marketId := this.SafeString(order, MkString("market_string"))
	_ = marketId
...
func (this *Qtrade) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetOrderOrderId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	order := this.SafeValue(data, MkString("order"), MkMap(&VarMap{}))
...
func (this *Qtrade) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"open": MkBool(true)})
	_ = request
...
func (this *Qtrade) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"open": MkBool(false)})
	_ = request
	return this.FetchOrders(symbol, since, limit, this.Extend(request, params))
}
...
func (this *Qtrade) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostDepositAddressCurrency"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
...
func (this *Qtrade) FetchDeposit(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"deposit_id": id})
	_ = request
	response := this.Call(MkString("privateGetDepositDepositId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	deposit := this.SafeValue(data, MkString("deposit"), MkMap(&VarMap{}))
	_ = deposit
...
func (this *Qtrade) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetDeposits"), params)
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	deposits := this.SafeValue(data, MkString("deposits"), MkArray(&VarArray{}))
	_ = deposits
...
func (this *Qtrade) FetchWithdrawal(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"withdraw_id": id})
	_ = request
	response := this.Call(MkString("privateGetWithdrawWithdrawId"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	withdrawal := this.SafeValue(data, MkString("withdraw"), MkMap(&VarMap{}))
	_ = withdrawal
...
func (this *Qtrade) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetWithdraws"), params)
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	withdrawals := this.SafeValue(data, MkString("withdraws"), MkArray(&VarArray{}))
	_ = withdrawals
...
func (this *Qtrade) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = id
	networkData := this.SafeValue(transaction, MkString("network_data"), MkMap(&VarMap{}))
	_ = networkData
	unsignedTx := this.SafeValue(networkData, MkString("unsigned_tx"), MkMap(&VarMap{}))
	_ = unsignedTx
	addressFrom := this.SafeString(unsignedTx, MkString("from"))
	_ = addressFrom
	txid := this.SafeString(networkData, MkString("txid"))
	_ = txid
	...
	tagTo := OpCopy(tag)
	_ = tagTo
	cancelRequested := this.SafeValue(transaction, MkString("cancel_requested"))
	_ = cancelRequested
	type_ := OpTrinary(OpEq2(cancelRequested, MkUndefined()), MkString("deposit"), MkString("withdrawal"))
	_ = type_
	...
	_ = status
	statusCode := this.SafeString(transaction, MkString("code"))
	_ = statusCode
...
func (this *Qtrade) Withdraw(goArgs ...*Variant) *Variant {
	...
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	result := this.ParseTransaction(data)
	_ = result
...
func (this *Qtrade) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("private"))) {
		...
		_ = timestamp
		bodyAsString := OpTrinary(OpEq2(method, MkString("POST")), body, MkString(""))
		_ = bodyAsString
		...
		_ = auth
		hash := this.Hash(this.Encode(auth), MkString("sha256"), MkString("base64"))
		_ = hash
		key := OpCopy(*this.At(MkString("apiKey")))
		_ = key
		if IsTrue(OpNotEq2(OpType(key), MkString("string"))) {
			key = key.ToString()
		}
		signature := OpAdd(MkString("HMAC-SHA256 "), OpAdd(key, OpAdd(MkString(":"), hash)))
		_ = signature
