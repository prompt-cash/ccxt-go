...
func (this *Exmo) FetchTradingFees(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(*(*this.At(MkString("options")) ).At(MkString("useWebapiForFetchingFees") )) {
    response := this.Call(MkString("webGetEnDocsFees"), params ); _ = response;
    parts := response.Split(MkString("<td class=\"th_fees_2\" colspan=\"2\">") ); _ = parts;
    ...
    rest := *(parts ).At(MkInteger(1) ); _ = rest;
    parts = rest.Split(MkString("</td>") );
    numParts = OpCopy(parts.Length );
    if IsTrue(OpLw(numParts , MkInteger(2) )) {
      panic(NewNotSupported(OpAdd(*this.At(MkString("id")) , MkString(" fetchTradingFees format has changed") )));
    }
    fee := OpMulti(parseFloat((*(parts ).At(MkInteger(0) )).Call(MkString("replace") , MkString("%") , MkString("") )), MkNumber(0.01) ); _ = fee;
    taker := OpCopy(fee ); _ = taker;
    maker := OpCopy(fee ); _ = maker;
    return MkMap(&VarMap{
          "maker":maker ,
          "taker":taker ,
          });
...
func (this *Exmo) ParseFixedFloatValue(goArgs ...*Variant) *Variant{
  input := GoGetArg(goArgs, 0, MkUndefined()); _ = input;
  ...
  isPercentage := OpGtEq(input.IndexOf(MkString("%") ), MkInteger(0) ); _ = isPercentage;
  parts := input.Split(MkString(" ") ); _ = parts;
  value := (*(parts ).At(MkInteger(0) )).Call(MkString("replace") , MkString("%") , MkString("") ); _ = value;
  result := parseFloat(value ); _ = result;
...
func (this *Exmo) FetchFundingFees(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  currencyList := this.Call(MkString("publicGetCurrencyListExtended"), params ); _ = currencyList;
  cryptoList := this.Call(MkString("publicGetPaymentsProvidersCryptoList"), params ); _ = cryptoList;
  result := MkMap(&VarMap{
        "info":cryptoList ,
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencyList.Length )); OpIncr(& i ){
    currency := *(currencyList ).At(i ); _ = currency;
    currencyId := this.SafeString(currency , MkString("name") ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    providers := this.SafeValue(cryptoList , currencyId , MkArray(&VarArray{})); _ = providers;
    for j := MkInteger(0) ; IsTrue(OpLw(j , providers.Length )); OpIncr(& j ){
      provider := *(providers ).At(j ); _ = provider;
      type_ := this.SafeString(provider , MkString("type") ); _ = type_;
      commissionDesc := this.SafeString(provider , MkString("commission_desc") ); _ = commissionDesc;
      newFee := this.ParseFixedFloatValue(commissionDesc ); _ = newFee;
      previousFee := this.SafeNumber(*(result ).At(type_ ), code ); _ = previousFee;
...
func (this *Exmo) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencyList.Length )); OpIncr(& i ){
    currency := *(currencyList ).At(i ); _ = currency;
    currencyId := this.SafeString(currency , MkString("name") ); _ = currencyId;
    name := this.SafeString(currency , MkString("description") ); _ = name;
    providers := this.SafeValue(cryptoList , currencyId ); _ = providers;
    active := OpCopy(MkBool(false) ); _ = active;
    type_ := MkString("crypto") ; _ = type_;
    limits := MkMap(&VarMap{
          "deposit":MkMap(&VarMap{
              "min":MkUndefined() ,
              "max":MkUndefined() ,
    ...
    if IsTrue(OpEq2(providers , MkUndefined() )) {
      ...
      for j := MkInteger(0) ; IsTrue(OpLw(j , providers.Length )); OpIncr(& j ){
        provider := *(providers ).At(j ); _ = provider;
        type_ := this.SafeString(provider , MkString("type") ); _ = type_;
        minValue := this.SafeNumber(provider , MkString("min") ); _ = minValue;
        maxValue := this.SafeNumber(provider , MkString("max") ); _ = maxValue;
        if IsTrue(OpEq2(maxValue , MkNumber(0.0) )) {
          maxValue = OpCopy(MkUndefined() );
        }
        activeProvider := this.SafeValue(provider , MkString("enabled") ); _ = activeProvider;
...
func (this *Exmo) FetchMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetPairSettings"), params ); _ = response;
  keys := GoGetKeys(response ); _ = keys;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    id := *(keys ).At(i ); _ = id;
    market := *(response ).At(id ); _ = market;
    symbol := id.Replace(MkString("_") , MkString("/") ); _ = symbol;
    ...
    takerString := this.SafeString(market , MkString("commission_taker_percent") ); _ = takerString;
    makerString := this.SafeString(market , MkString("commission_maker_percent") ); _ = makerString;
    taker := this.ParseNumber(Precise.StringDiv(takerString , MkString("100") )); _ = taker;
    maker := this.ParseNumber(Precise.StringDiv(makerString , MkString("100") )); _ = maker;
...
func (this *Exmo) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  options := this.SafeValue(*this.At(MkString("options")) , MkString("fetchOHLCV") ); _ = options;
  maxLimit := this.SafeInteger(options , MkString("maxLimit") , MkInteger(3000) ); _ = maxLimit;
    ...
    *(request ).At(MkString("from") )= OpSub(parseInt(OpDiv(since , MkInteger(1000) )), MkInteger(1) );
    if IsTrue(OpEq2(limit , MkUndefined() )) {
      *(request ).At(MkString("to") )= parseInt(OpDiv(now , MkInteger(1000) ));
      ...
      to := this.Sum(since , OpMulti(limit , OpMulti(duration , MkInteger(1000) ))); _ = to;
      *(request ).At(MkString("to") )= parseInt(OpDiv(to , MkInteger(1000) ));
    }
  }
  response := this.Call(MkString("publicGetCandlesHistory"), this.Extend(request , params )); _ = response;
...
func (this *Exmo) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostUserInfo"), params ); _ = response;
  result := MkMap(&VarMap{"info":response }); _ = result;
  free := this.SafeValue(response , MkString("balances") , MkMap(&VarMap{})); _ = free;
  used := this.SafeValue(response , MkString("reserved") , MkMap(&VarMap{})); _ = used;
...
func (this *Exmo) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  result := this.SafeValue(response , *(market ).At(MkString("id") )); _ = result;
...
func (this *Exmo) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  last := this.SafeNumber(ticker , MkString("last_trade") ); _ = last;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Exmo) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  amount := this.SafeNumber(trade , MkString("quantity") ); _ = amount;
  cost := this.SafeNumber(trade , MkString("amount") ); _ = cost;
  ...
  takerOrMaker := this.SafeString(trade , MkString("exec_type") ); _ = takerOrMaker;
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(trade , MkString("commission_amount") ); _ = feeCost;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyId := this.SafeString(trade , MkString("commission_currency") ); _ = feeCurrencyId;
    feeCurrencyCode := this.SafeCurrencyCode(feeCurrencyId ); _ = feeCurrencyCode;
    feeRate := this.SafeNumber(trade , MkString("commission_percent") ); _ = feeRate;
...
func (this *Exmo) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  data := this.SafeValue(response , *(market ).At(MkString("id") ), MkArray(&VarArray{})); _ = data;
...
func (this *Exmo) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  pair := OpCopy(MkUndefined() ); _ = pair;
  market := OpCopy(MkUndefined() ); _ = market;
  if IsTrue(GoIsArray(symbol )) {
    numSymbols := OpCopy(symbol.Length ); _ = numSymbols;
    ...
    marketIds := this.MarketIds(symbol ); _ = marketIds;
    pair = marketIds.Join(MkString(",") );
  } else {
    market = this.Market(symbol );
    pair = *(market ).At(MkString("id") );
  }
  request := MkMap(&VarMap{"pair":pair }); _ = request;
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
    *(request ).At(MkString("limit") )= OpCopy(limit );
  }
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , marketIds.Length )); OpIncr(& i ){
    ...
    items := *(response ).At(marketId ); _ = items;
    trades := this.ParseTrades(items , market , since , limit , MkMap(&VarMap{"symbol":symbol })); _ = trades;
...
func (this *Exmo) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  prefix := OpTrinary(OpEq2(type_ , MkString("market") ), OpAdd(type_ , MkString("_") ), MkString("") ); _ = prefix;
  orderType := OpAdd(prefix , side ); _ = orderType;
  orderPrice := OpCopy(price ); _ = orderPrice;
  ...
  method := MkString("privatePostOrderCreate") ; _ = method;
  clientOrderId := this.SafeValue2(params , MkString("client_id") , MkString("clientOrderId") ); _ = clientOrderId;
  ...
  id := this.SafeString(response , MkString("order_id") ); _ = id;
...
func (this *Exmo) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"order_id":id.ToString()}); _ = request;
  response := this.Call(MkString("privatePostOrderTrades"), this.Extend(request , params )); _ = response;
...
func (this *Exmo) FetchOrderTrades(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"order_id":id.ToString()}); _ = request;
  response := this.Call(MkString("privatePostOrderTrades"), this.Extend(request , params )); _ = response;
  trades := this.SafeValue(response , MkString("trades") ); _ = trades;
...
func (this *Exmo) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostUserOpenOrders"), params ); _ = response;
  marketIds := GoGetKeys(response ); _ = marketIds;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , marketIds.Length )); OpIncr(& i ){
    ...
    parsedOrders := this.ParseOrders(*(response ).At(marketId ), market ); _ = parsedOrders;
...
func (this *Exmo) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeTimestamp(order , MkString("created") ); _ = timestamp;
  symbol := OpCopy(MkUndefined() ); _ = symbol;
  ...
  if IsTrue(OpEq2(amount , MkUndefined() )) {
    amountField := OpTrinary(OpEq2(side , MkString("buy") ), MkString("in_amount") , MkString("out_amount") ); _ = amountField;
    amount = this.SafeNumber(order , amountField );
  }
  price := this.SafeNumber(order , MkString("price") ); _ = price;
  cost := this.SafeNumber(order , MkString("amount") ); _ = cost;
  filled := MkNumber(0.0) ; _ = filled;
  trades := MkArray(&VarArray{}); _ = trades;
  transactions := this.SafeValue(order , MkString("trades") , MkArray(&VarArray{})); _ = transactions;
  ...
  numTransactions := OpCopy(transactions.Length ); _ = numTransactions;
  if IsTrue(OpGt(numTransactions , MkInteger(0) )) {
    feeCost = MkInteger(0) ;
    for i := MkInteger(0) ; IsTrue(OpLw(i , numTransactions )); OpIncr(& i ){
      trade := this.ParseTrade(*(transactions ).At(i ), market ); _ = trade;
  ...
  clientOrderId := this.SafeInteger(order , MkString("client_id") ); _ = clientOrderId;
  return MkMap(&VarMap{
        "id":id ,
        "clientOrderId":clientOrderId ,
        "datetime":this.Iso8601(timestamp ),
...
func (this *Exmo) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostDepositAddress"), params ); _ = response;
  depositAddress := this.SafeString(response , code ); _ = depositAddress;
  address := OpCopy(MkUndefined() ); _ = address;
  tag := OpCopy(MkUndefined() ); _ = tag;
  if IsTrue(depositAddress ) {
    addressAndTag := depositAddress.Split(MkString(",") ); _ = addressAndTag;
    address = *(addressAndTag ).At(MkInteger(0) );
    numParts := OpCopy(addressAndTag.Length ); _ = numParts;
...
func (this *Exmo) GetMarketFromTrades(goArgs ...*Variant) *Variant{
  ...
  tradesBySymbol := this.IndexBy(trades , MkString("pair") ); _ = tradesBySymbol;
...
func (this *Exmo) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostWithdrawCrypt"), this.Extend(request , params )); _ = response;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Exmo) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString(transaction , MkString("order_id") ); _ = id;
  timestamp := this.SafeTimestamp2(transaction , MkString("dt") , MkString("created") ); _ = timestamp;
  updated := this.SafeTimestamp(transaction , MkString("updated") ); _ = updated;
  ...
  if IsTrue(OpEq2(txid , MkUndefined() )) {
    extra := this.SafeValue(transaction , MkString("extra") , MkMap(&VarMap{})); _ = extra;
    extraTxid := this.SafeString(extra , MkString("txid") ); _ = extraTxid;
    if IsTrue(OpNotEq2(extraTxid , MkString("") )) {
      txid = OpCopy(extraTxid );
    }
  }
  type_ := this.SafeString(transaction , MkString("type") ); _ = type_;
  currencyId := this.SafeString2(transaction , MkString("curr") , MkString("currency") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  address := OpCopy(MkUndefined() ); _ = address;
  tag := OpCopy(MkUndefined() ); _ = tag;
  comment := OpCopy(MkUndefined() ); _ = comment;
  account := this.SafeString(transaction , MkString("account") ); _ = account;
  ...
  if IsTrue(OpNot(*(*(*this.At(MkString("fees")) ).At(MkString("funding") )).At(MkString("percentage") ))) {
    key := OpTrinary(OpEq2(type_ , MkString("withdrawal") ), MkString("withdraw") , MkString("deposit") ); _ = key;
    ...
    provider := this.SafeString(transaction , MkString("provider") ); _ = provider;
...
func (this *Exmo) FetchTransactions(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostWalletHistory"), this.Extend(request , params )); _ = response;
...
func (this *Exmo) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostWalletOperations"), this.Extend(request , params )); _ = response;
...
func (this *Exmo) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpOr(OpHasMember(MkString("result") , response ), OpHasMember(MkString("errmsg") , response ))) {
    success := this.SafeValue(response , MkString("result") , MkBool(false) ); _ = success;
    ...
    if IsTrue(OpNot(success )) {
      ...
      message := this.SafeString2(response , MkString("error") , MkString("errmsg") ); _ = message;
      errorParts := message.Split(MkString(":") ); _ = errorParts;
      numParts := OpCopy(errorParts.Length ); _ = numParts;
      if IsTrue(OpGt(numParts , MkInteger(1) )) {
        errorSubParts := (*(errorParts ).At(MkInteger(0) )).Call(MkString("split") , MkString(" ") ); _ = errorSubParts;
        numSubParts := OpCopy(errorSubParts.Length ); _ = numSubParts;
