...
func (this *Xena) FetchTime(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("publicGetMarketDataV2ServerTime"), params)
	_ = response
	transactTime := this.SafeInteger(response, MkString("transactTime"))
	_ = transactTime
...
func (this *Xena) FetchMarkets(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("publicGetCommonInstruments"), params)
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = id
		numericId := this.SafeString(market, MkString("id"))
		_ = numericId
		marginType := this.SafeString(market, MkString("marginType"))
		_ = marginType
		...
		_ = swap
		pricePrecision := this.SafeInteger2(market, MkString("tickSize"), MkString("pricePrecision"))
		_ = pricePrecision
		...
		_ = precision
		maxCost := this.SafeNumber(market, MkString("maxOrderQty"))
		_ = maxCost
		minCost := this.SafeNumber(market, MkString("minOrderQuantity"))
		_ = minCost
		limits := MkMap(&VarMap{
			"amount": MkMap(&VarMap{
				"min": MkUndefined(),
		...
		_ = limits
		active := this.SafeValue(market, MkString("enabled"), MkBool(false))
		_ = active
...
func (this *Xena) FetchCurrencies(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("publicGetCommonCurrencies"), params)
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		...
		_ = code
		name := this.SafeString(currency, MkString("title"))
		_ = name
		precision := this.SafeInteger(currency, MkString("precision"))
		_ = precision
		enabled := this.SafeValue(currency, MkString("enabled"))
		_ = enabled
		active := OpEq2(enabled, MkBool(true))
		_ = active
		withdraw := this.SafeValue(currency, MkString("withdraw"), MkMap(&VarMap{}))
		_ = withdraw
...
func (this *Xena) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = symbol
	last := this.SafeNumber(ticker, MkString("lastPx"))
	_ = last
	open := this.SafeNumber(ticker, MkString("firstPx"))
	_ = open
	...
	buyVolume := this.SafeNumber(ticker, MkString("buyVolume"))
	_ = buyVolume
	sellVolume := this.SafeNumber(ticker, MkString("sellVolume"))
	_ = sellVolume
...
func (this *Xena) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	tickers := this.FetchTickers(MkUndefined(), params)
	_ = tickers
...
func (this *Xena) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	tickers := this.Call(MkString("publicGetMarketDataMarketWatch"), params)
	_ = tickers
	result := MkMap(&VarMap{})
	_ = result
...
func (this *Xena) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetMarketDataV2DomSymbol"), this.Extend(request, params))
	_ = response
	mdEntry := this.SafeValue(response, MkString("mdEntry"), MkArray(&VarArray{}))
	_ = mdEntry
	mdEntriesByType := this.GroupBy(mdEntry, MkString("mdEntryType"))
	_ = mdEntriesByType
	lastUpdateTime := this.SafeInteger(response, MkString("lastUpdateTime"))
	_ = lastUpdateTime
	timestamp := parseInt(OpDiv(lastUpdateTime, MkInteger(1000000)))
	_ = timestamp
...
func (this *Xena) FetchAccounts(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("privateGetTradingAccounts"), params)
	_ = response
	accounts := this.SafeValue(response, MkString("accounts"))
	_ = accounts
	result := MkArray(&VarArray{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, accounts.Length)); OpIncr(&i) {
		...
		_ = code
		type_ := this.SafeStringLower(account, MkString("kind"))
		_ = type_
		result.Push(MkMap(&VarMap{
			"id":       accountId,
			"type":     type_,
...
func (this *Xena) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetTradingAccountsAccountIdBalance"), this.Extend(request, params))
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = balance
		lastUpdateTime := this.SafeString(balance, MkString("lastUpdateTime"))
		_ = lastUpdateTime
		lastUpdated := lastUpdateTime.Slice(MkInteger(0), MkInteger(13))
		_ = lastUpdated
		currentTimestamp := parseInt(lastUpdated)
		_ = currentTimestamp
...
func (this *Xena) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = id
	timestamp := this.SafeInteger(trade, MkString("transactTime"))
	_ = timestamp
	if IsTrue(OpNotEq2(timestamp, MkUndefined())) {
		timestamp = parseInt(OpDiv(timestamp, MkInteger(1000000)))
	}
	side := this.SafeStringLower2(trade, MkString("side"), MkString("aggressorSide"))
	_ = side
	...
	symbol := this.SafeSymbol(marketId, market)
	_ = symbol
	priceString := this.SafeString2(trade, MkString("lastPx"), MkString("mdEntryPx"))
	_ = priceString
	amountString := this.SafeString2(trade, MkString("lastQty"), MkString("mdEntrySize"))
	_ = amountString
	...
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		...
		_ = feeCurrencyCode
		feeRate := this.SafeNumber(trade, MkString("commRate"))
		_ = feeRate
		fee = MkMap(&VarMap{
			"cost":     feeCost,
			"rate":     feeRate,
...
func (this *Xena) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetTradingAccountsAccountIdTradeHistory"), this.Extend(request, params))
	_ = response
...
func (this *Xena) ParseOHLCV(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	transactTime := this.SafeInteger(ohlcv, MkString("transactTime"))
	_ = transactTime
	timestamp := parseInt(OpDiv(transactTime, MkInteger(1000000)))
	_ = timestamp
	buyVolume := this.SafeNumber(ohlcv, MkString("buyVolume"))
	_ = buyVolume
	sellVolume := this.SafeNumber(ohlcv, MkString("sellVolume"))
	_ = sellVolume
...
func (this *Xena) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetMarketDataV2CandlesSymbolTimeframe"), this.Extend(request, params))
	_ = response
	mdEntry := this.SafeValue(response, MkString("mdEntry"), MkArray(&VarArray{}))
	_ = mdEntry
...
func (this *Xena) FetchTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetMarketDataV2TradesSymbol"), this.Extend(request, params))
	_ = response
	mdEntry := this.SafeValue(response, MkString("mdEntry"), MkArray(&VarArray{}))
	_ = mdEntry
...
func (this *Xena) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = clientOrderId
	transactTime := this.SafeInteger(order, MkString("transactTime"))
	_ = transactTime
	timestamp := parseInt(OpDiv(transactTime, MkInteger(1000000)))
	_ = timestamp
	...
	type_ := this.SafeStringLower(order, MkString("ordType"))
	_ = type_
...
func (this *Xena) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = orderTypes
	orderType := this.SafeString(orderTypes, type_)
	_ = orderType
	...
	_ = orderSides
	orderSide := this.SafeString(orderSides, side)
	_ = orderSide
	...
	if IsTrue(OpOr(OpEq2(type_, MkString("stop")), OpEq2(type_, MkString("stop-limit")))) {
		stopPx := this.SafeNumber(params, MkString("stopPx"))
		_ = stopPx
	...
	clientOrderId := this.SafeString2(params, MkString("clientOrderId"), MkString("clOrdId"), this.Uuid())
	_ = clientOrderId
	...
	response := this.Call(MkString("privatePostTradingOrderNew"), this.Extend(request, params))
	_ = response
...
func (this *Xena) EditOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	clientOrderId := this.SafeString2(params, MkString("clientOrderId"), MkString("origClOrdId"))
	_ = clientOrderId
	if IsTrue(OpNotEq2(clientOrderId, MkUndefined())) {
	...
	capPrice := this.SafeNumber(params, MkString("capPrice"))
	_ = capPrice
	...
	response := this.Call(MkString("privatePostTradingOrderReplace"), this.Extend(request, params))
	_ = response
	return this.ParseOrder(response, market)
}
...
func (this *Xena) CancelOrder(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostTradingOrderCancel"), this.Extend(request, params))
	_ = response
	return this.ParseOrder(response, market)
}
...
func (this *Xena) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostTradingOrderMassCancel"), this.Extend(request, params))
	_ = response
	return response
}
...
func (this *Xena) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetTradingAccountsAccountIdActiveOrders"), this.Extend(request, params))
	_ = response
...
func (this *Xena) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetTradingAccountsAccountIdLastOrderStatuses"), this.Extend(request, params))
	_ = response
	return this.ParseOrders(response, market, since, limit)
}
...
func (this *Xena) CreateDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostTransfersAccountsAccountIdDepositAddressCurrency"), this.Extend(request, params))
	_ = response
...
func (this *Xena) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetTransfersAccountsAccountIdDepositAddressCurrency"), this.Extend(request, params))
	_ = response
	address := this.SafeValue(response, MkString("address"))
	_ = address
	tag := OpCopy(MkUndefined())
...
func (this *Xena) FetchTransactionsByType(goArgs ...*Variant) *Variant {
	...
	method := OpAdd(MkString("privateGetTransfersAccountsAccountId"), this.Capitalize(type_))
	_ = method
	response := this.Call(method, this.Extend(request, params))
	_ = response
	transactions := this.SafeValue(response, type_, MkArray(&VarArray{}))
	_ = transactions
...
func (this *Xena) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = currency
	id := this.SafeString(transaction, MkString("withdrawalRequestId"))
	_ = id
	type_ := OpTrinary(OpEq2(id, MkUndefined()), MkString("deposit"), MkString("withdrawal"))
	_ = type_
	updated := this.SafeInteger(transaction, MkString("lastUpdated"))
	_ = updated
...
func (this *Xena) Withdraw(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostTransfersAccountsAccountIdWithdrawals"), this.Extend(request, params))
	_ = response
...
func (this *Xena) ParseLedgerEntry(goArgs ...*Variant) *Variant {
	...
	_ = referenceAccount
	type_ := this.ParseLedgerEntryType(this.SafeString(item, MkString("kind")))
	_ = type_
...
func (this *Xena) FetchLedger(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetTransfersAccountsAccountIdBalanceHistory"), this.Extend(request, params))
	_ = response
...
func (this *Xena) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		if IsTrue(OpEq2(api, MkString("private"))) {
			...
			payload := OpAdd(MkString("AUTH"), nonce)
			_ = payload
			secret := (*((this).At(MkString("secret")))).Call(MkString("slice"), MkInteger(14), MkInteger(78))
			_ = secret
			ecdsa := this.Ecdsa(payload, secret, MkString("p256"), MkString("sha256"))
			_ = ecdsa
			signature := OpAdd(*(ecdsa).At(MkString("r")), *(ecdsa).At(MkString("s")))
			_ = signature
...
func (this *Xena) HandleErrors(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpGtEq(code, MkInteger(400))) {
		...
		_ = message
		exact := *(*this.At(MkString("exceptions"))).At(MkString("exact"))
		_ = exact
		if IsTrue(OpHasMember(message, exact)) {
			panic(exact.Call(message, feedback))
		}
		broad := *(*this.At(MkString("exceptions"))).At(MkString("broad"))
		_ = broad
