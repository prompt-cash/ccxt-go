...
func (this *Kuna) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    ...
    for j := MkInteger(0) ; IsTrue(OpLw(j , quotes.Length )); OpIncr(& j ){
      quoteId := *(quotes ).At(j ); _ = quoteId;
      slicedId := id.Slice(MkInteger(1) ); _ = slicedId;
      index := slicedId.IndexOf(quoteId ); _ = index;
      slice := slicedId.Slice(index ); _ = slice;
      if IsTrue(OpAnd(OpGt(index , MkInteger(0) ), OpEq2(slice , quoteId ))) {
        baseId := OpAdd(*(id ).At(MkInteger(0) ), slicedId.Replace(quoteId , MkString("") )); _ = baseId;
...
func (this *Kuna) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  orderbook := this.Call(MkString("publicGetDepth"), this.Extend(request , params )); _ = orderbook;
...
func (this *Kuna) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    ...
    if IsTrue(OpHasMember(id , *this.At(MkString("markets_by_id")) )) {
      ...
      base := id.Slice(MkInteger(0) , MkInteger(3) ); _ = base;
      quote := id.Slice(MkInteger(3) , MkInteger(6) ); _ = quote;
...
func (this *Kuna) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  side := this.SafeString2(trade , MkString("side") , MkString("trend") ); _ = side;
...
func (this *Kuna) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  trades := this.FetchTrades(symbol , since , limit , params ); _ = trades;
  ohlcvc := this.BuildOHLCVC(trades , timeframe , since , limit ); _ = ohlcvc;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , ohlcvc.Length )); OpIncr(& i ){
    ohlcv := *(ohlcvc ).At(i ); _ = ohlcv;
...
func (this *Kuna) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeValue(response , MkString("market") ); _ = marketId;
...
func (this *Kuna) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrderDelete"), this.Extend(request , params )); _ = response;
...
func (this *Kuna) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"market":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetTradesMy"), this.Extend(request , params )); _ = response;
  return this.ParseTrades(response , market , since , limit );
}
...
func (this *Kuna) EncodeParams(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("orders") , params )) {
    orders := *(params ).At(MkString("orders") ); _ = orders;
    query := this.Urlencode(this.Keysort(this.Omit(params , MkString("orders") ))); _ = query;
    for i := MkInteger(0) ; IsTrue(OpLw(i , orders.Length )); OpIncr(& i ){
      order := *(orders ).At(i ); _ = order;
      keys := GoGetKeys(order ); _ = keys;
      for k := MkInteger(0) ; IsTrue(OpLw(k , keys.Length )); OpIncr(& k ){
        key := *(keys ).At(k ); _ = key;
        value := *(order ).At(key ); _ = value;
...
func (this *Kuna) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    auth := OpAdd(method , OpAdd(MkString("|") , OpAdd(request , OpAdd(MkString("|") , query )))); _ = auth;
    signed := this.Hmac(this.Encode(auth ), this.Encode(*this.At(MkString("secret")) )); _ = signed;
    suffix := OpAdd(query , OpAdd(MkString("&signature=") , signed )); _ = suffix;
