...
func (this *Indodax) FetchTime(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("publicGetServerTime"), params)
	_ = response
...
func (this *Indodax) FetchMarkets(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("publicGetPairs"), params)
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = market
		id := this.SafeString(market, MkString("ticker_id"))
		_ = id
		baseId := this.SafeString(market, MkString("traded_currency"))
		_ = baseId
		...
		_ = symbol
		taker := this.SafeNumber(market, MkString("trade_fee_percent"))
		_ = taker
		isMaintenance := this.SafeInteger(market, MkString("is_maintenance"))
		_ = isMaintenance
		active := OpTrinary(isMaintenance, MkBool(false), MkBool(true))
		_ = active
		pricePrecision := this.SafeInteger(market, MkString("price_round"))
		_ = pricePrecision
...
func (this *Indodax) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = balances
	free := this.SafeValue(balances, MkString("balance"), MkMap(&VarMap{}))
	_ = free
	used := this.SafeValue(balances, MkString("balance_hold"), MkMap(&VarMap{}))
	_ = used
...
func (this *Indodax) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = request
	orderbook := this.Call(MkString("publicGetPairDepth"), this.Extend(request, params))
	_ = orderbook
...
func (this *Indodax) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = response
	ticker := *(response).At(MkString("ticker"))
	_ = ticker
	timestamp := this.SafeTimestamp(ticker, MkString("server_time"))
	_ = timestamp
	baseVolume := OpAdd(MkString("vol_"), (*(market).At(MkString("baseId"))).Call(MkString("toLowerCase")))
	_ = baseVolume
	quoteVolume := OpAdd(MkString("vol_"), (*(market).At(MkString("quoteId"))).Call(MkString("toLowerCase")))
	_ = quoteVolume
...
func (this *Indodax) FetchTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"pair": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetPairTrades"), this.Extend(request, params))
	_ = response
...
func (this *Indodax) ParseOrder(goArgs ...*Variant) *Variant {
	...
	status := this.ParseOrderStatus(this.SafeString(order, MkString("status"), MkString("open")))
	_ = status
	symbol := OpCopy(MkUndefined())
	_ = symbol
	...
	timestamp := this.SafeInteger(order, MkString("submit_time"))
	_ = timestamp
...
func (this *Indodax) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = response
	orders := *(response).At(MkString("return"))
	_ = orders
	order := this.ParseOrder(this.Extend(MkMap(&VarMap{"id": id}), *(orders).At(MkString("order"))), market)
	_ = order
...
func (this *Indodax) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	_ = response
	rawOrders := *(*(response).At(MkString("return"))).At(MkString("orders"))
	_ = rawOrders
	...
	marketIds := GoGetKeys(rawOrders)
	_ = marketIds
	exchangeOrders := MkArray(&VarArray{})
	_ = exchangeOrders
	for i := MkInteger(0); IsTrue(OpLw(i, marketIds.Length)); OpIncr(&i) {
		marketId := *(marketIds).At(i)
		_ = marketId
		marketOrders := *(rawOrders).At(marketId)
		_ = marketOrders
...
func (this *Indodax) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	_ = response
	orders := this.ParseOrders(*(*(response).At(MkString("return"))).At(MkString("orders")), market)
	_ = orders
...
func (this *Indodax) CreateOrder(goArgs ...*Variant) *Variant {
	...
	*(request).At(currency) = OpCopy(amount)
	result := this.Call(MkString("privatePostTrade"), this.Extend(request, params))
	_ = result
	data := this.SafeValue(result, MkString("return"), MkMap(&VarMap{}))
	_ = data
	id := this.SafeString(data, MkString("order_id"))
	_ = id
	return MkMap(&VarMap{
		"info": result,
		"id":   id,
...
func (this *Indodax) CancelOrder(goArgs ...*Variant) *Variant {
	...
	side := this.SafeValue(params, MkString("side"))
	_ = side
...
func (this *Indodax) HandleErrors(goArgs ...*Variant) *Variant {
	...
	error := this.SafeValue(response, MkString("error"), MkString(""))
	_ = error
