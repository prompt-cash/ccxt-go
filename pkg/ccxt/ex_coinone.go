...
func (this *Coinone) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTicker"), request ); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  quoteId := MkString("krw") ; _ = quoteId;
  quote := this.SafeCurrencyCode(quoteId ); _ = quote;
  baseIds := GoGetKeys(response ); _ = baseIds;
  for i := MkInteger(0) ; IsTrue(OpLw(i , baseIds.Length )); OpIncr(& i ){
    baseId := *(baseIds ).At(i ); _ = baseId;
    ticker := this.SafeValue(response , baseId , MkMap(&VarMap{})); _ = ticker;
    currency := this.SafeValue(ticker , MkString("currency") ); _ = currency;
...
func (this *Coinone) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostAccountBalance"), params ); _ = response;
...
func (this *Coinone) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  first := this.SafeNumber(ticker , MkString("first") ); _ = first;
  ...
  previousClose := this.SafeNumber(ticker , MkString("yesterday_last") ); _ = previousClose;
  ...
  symbol := OpTrinary(OpNotEq2(market , MkUndefined() ), *(market ).At(MkString("symbol") ), MkUndefined() ); _ = symbol;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Coinone) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  symbol := OpTrinary(OpNotEq2(market , MkUndefined() ), *(market ).At(MkString("symbol") ), MkUndefined() ); _ = symbol;
  is_ask := this.SafeString(trade , MkString("is_ask") ); _ = is_ask;
  ...
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    ...
    feeRate := this.SafeNumber(trade , MkString("feeRate") ); _ = feeRate;
...
func (this *Coinone) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  completeOrders := this.SafeValue(response , MkString("completeOrders") , MkArray(&VarArray{})); _ = completeOrders;
...
func (this *Coinone) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  method := OpAdd(MkString("privatePostOrder") , OpAdd(this.Capitalize(type_ ), this.Capitalize(side ))); _ = method;
...
func (this *Coinone) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostOrderOrderInfo"), this.Extend(request , params )); _ = response;
...
func (this *Coinone) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  remaining := this.SafeNumber(order , MkString("remainQty") ); _ = remaining;
  ...
  marketId := this.SafeStringLower(order , MkString("currency") ); _ = marketId;
  ...
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyCode := OpTrinary(OpEq2(side , MkString("sell") ), quote , base ); _ = feeCurrencyCode;
    fee = MkMap(&VarMap{
        "cost":feeCost ,
...
func (this *Coinone) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"currency":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostOrderLimitOrders"), this.Extend(request , params )); _ = response;
  limitOrders := this.SafeValue(response , MkString("limitOrders") , MkArray(&VarArray{})); _ = limitOrders;
...
func (this *Coinone) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"currency":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostOrderCompleteOrders"), this.Extend(request , params )); _ = response;
  completeOrders := this.SafeValue(response , MkString("completeOrders") , MkArray(&VarArray{})); _ = completeOrders;
  return this.ParseTrades(completeOrders , market , since , limit );
}
...
func (this *Coinone) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  price := this.SafeNumber(params , MkString("price") ); _ = price;
  qty := this.SafeNumber(params , MkString("qty") ); _ = qty;
  isAsk := this.SafeInteger(params , MkString("is_ask") ); _ = isAsk;
...
func (this *Coinone) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    payload := this.StringToBase64(json ); _ = payload;
    body = this.Decode(payload );
    secret := (*((this).At(MkString("secret")))).Call(MkString("toUpperCase")); _ = secret;
    signature := this.Hmac(payload , this.Encode(secret ), MkString("sha512") ); _ = signature;
