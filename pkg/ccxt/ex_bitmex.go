...
func (this *Bitmex) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("publicGetInstrumentActiveAndIndices"), params)
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = market
		active := OpNotEq2(*(market).At(MkString("state")), MkString("Unlisted"))
		_ = active
		id := *(market).At(MkString("symbol"))
		_ = id
		baseId := *(market).At(MkString("underlying"))
		_ = baseId
		quoteId := *(market).At(MkString("quoteCurrency"))
		_ = quoteId
		basequote := OpAdd(baseId, quoteId)
		_ = basequote
		base := this.SafeCurrencyCode(baseId)
		_ = base
		quote := this.SafeCurrencyCode(quoteId)
		_ = quote
		swap := OpEq2(id, basequote)
		_ = swap
		positionId := this.SafeString2(market, MkString("positionCurrency"), MkString("quoteCurrency"))
		_ = positionId
		type_ := OpCopy(MkUndefined())
		_ = type_
		future := OpCopy(MkBool(false))
		_ = future
		prediction := OpCopy(MkBool(false))
		_ = prediction
		...
		_ = precision
		lotSize := this.SafeNumber(market, MkString("lotSize"))
		_ = lotSize
		...
		_ = limits
		limitField := OpTrinary(OpEq2(position, quote), MkString("cost"), MkString("amount"))
		_ = limitField
...
func (this *Bitmex) ParseBalanceResponse(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = account
		free := this.SafeString(balance, MkString("availableMargin"))
		_ = free
		total := this.SafeString(balance, MkString("marginBalance"))
		_ = total
...
func (this *Bitmex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetUserMargin"), this.Extend(request, params))
	_ = response
...
func (this *Bitmex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetOrderBookL2"), this.Extend(request, params))
	_ = response
	result := MkMap(&VarMap{
		"symbol":    symbol,
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		order := *(response).At(i)
		_ = order
		side := OpTrinary(OpEq2(*(order).At(MkString("side")), MkString("Sell")), MkString("asks"), MkString("bids"))
		_ = side
...
func (this *Bitmex) FetchOrder(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 2, MkMap(&VarMap{}))
	_ = params
	filter := MkMap(&VarMap{"filter": MkMap(&VarMap{"orderID": id})})
	_ = filter
	response := this.FetchOrders(symbol, MkUndefined(), MkUndefined(), this.DeepExtend(filter, params))
	_ = response
	numResults := OpCopy(response.Length)
	_ = numResults
...
func (this *Bitmex) FetchOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetOrder"), request)
	_ = response
...
func (this *Bitmex) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"filter": MkMap(&VarMap{"open": MkBool(true)})})
	_ = request
...
func (this *Bitmex) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetExecutionTradeHistory"), request)
	_ = response
...
func (this *Bitmex) ParseLedgerEntry(goArgs ...*Variant) *Variant {
	...
	_ = currency
	id := this.SafeString(item, MkString("transactID"))
	_ = id
	account := this.SafeString(item, MkString("account"))
	_ = account
	referenceId := this.SafeString(item, MkString("tx"))
	_ = referenceId
	referenceAccount := OpCopy(MkUndefined())
	_ = referenceAccount
	type_ := this.ParseLedgerEntryType(this.SafeString(item, MkString("transactType")))
	_ = type_
	...
	timestamp := this.Parse8601(this.SafeString(item, MkString("transactTime")))
	_ = timestamp
	if IsTrue(OpEq2(timestamp, MkUndefined())) {
		timestamp = MkInteger(0)
	}
	feeCost := this.SafeNumber(item, MkString("fee"), MkInteger(0))
	_ = feeCost
	...
	_ = fee
	after := this.SafeNumber(item, MkString("walletBalance"))
	_ = after
	if IsTrue(OpNotEq2(after, MkUndefined())) {
		after = OpDiv(after, MkInteger(100000000))
	}
	before := this.Sum(after, OpNeg(amount))
	_ = before
	...
	status := this.ParseTransactionStatus(this.SafeString(item, MkString("transactStatus")))
	_ = status
	return MkMap(&VarMap{
		"id":               id,
		"info":             item,
...
func (this *Bitmex) FetchLedger(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetUserWalletHistory"), this.Extend(request, params))
	_ = response
...
func (this *Bitmex) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	currency := GoGetArg(goArgs, 1, MkUndefined())
	_ = currency
	id := this.SafeString(transaction, MkString("transactID"))
	_ = id
	transactTime := this.Parse8601(this.SafeString(transaction, MkString("transactTime")))
	_ = transactTime
	timestamp := this.Parse8601(this.SafeString(transaction, MkString("timestamp")))
	_ = timestamp
	type_ := this.SafeStringLower(transaction, MkString("transactType"))
	_ = type_
	...
	feeCostString := this.SafeString(transaction, MkString("fee"))
	_ = feeCostString
	...
	_ = fee
	status := this.SafeString(transaction, MkString("transactStatus"))
	_ = status
...
func (this *Bitmex) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	open := this.SafeNumber(ticker, MkString("prevPrice24h"))
	_ = open
...
func (this *Bitmex) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	_ = duration
	fetchOHLCVOpenTimestamp := this.SafeValue(*this.At(MkString("options")), MkString("fetchOHLCVOpenTimestamp"), MkBool(true))
	_ = fetchOHLCVOpenTimestamp
	if IsTrue(OpNotEq2(since, MkUndefined())) {
		timestamp := OpCopy(since)
		_ = timestamp
	...
	response := this.Call(MkString("publicGetTradeBucketed"), this.Extend(request, params))
	_ = response
	result := this.ParseOHLCVs(response, market, timeframe, since, limit)
	_ = result
	if IsTrue(fetchOHLCVOpenTimestamp) {
...
func (this *Bitmex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	priceString := this.SafeString2(trade, MkString("avgPx"), MkString("price"))
	_ = priceString
	amountString := this.SafeString2(trade, MkString("size"), MkString("lastQty"))
	_ = amountString
	id := this.SafeString(trade, MkString("trdMatchID"))
	_ = id
	order := this.SafeString(trade, MkString("orderID"))
	_ = order
	side := this.SafeStringLower(trade, MkString("side"))
	_ = side
	costString := this.SafeString(trade, MkString("execCost"))
	_ = costString
	costString = Precise.StringDiv(Precise.StringAbs(costString), MkString("1e8"))
	fee := OpCopy(MkUndefined())
	_ = fee
	feeCostString := Precise.StringDiv(this.SafeString(trade, MkString("execComm")), MkString("1e8"))
	_ = feeCostString
	if IsTrue(OpNotEq2(feeCostString, MkUndefined())) {
		currencyId := this.SafeString(trade, MkString("settlCurrency"))
		_ = currencyId
		feeCurrencyCode := this.SafeCurrencyCode(currencyId)
		_ = feeCurrencyCode
		feeRateString := this.SafeString(trade, MkString("commission"))
		_ = feeRateString
		fee = MkMap(&VarMap{
			"cost":     this.ParseNumber(feeCostString),
	...
	execType := this.SafeString(trade, MkString("execType"))
	_ = execType
	...
	_ = symbol
	type_ := this.SafeStringLower(trade, MkString("ordType"))
	_ = type_
	return MkMap(&VarMap{
		"info":         trade,
		"timestamp":    timestamp,
...
func (this *Bitmex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	lastTradeTimestamp := this.Parse8601(this.SafeString(order, MkString("transactTime")))
	_ = lastTradeTimestamp
	price := this.SafeNumber(order, MkString("price"))
	_ = price
	amount := this.SafeNumber(order, MkString("orderQty"))
	_ = amount
	filled := this.SafeNumber(order, MkString("cumQty"), MkNumber(0.0))
	_ = filled
	...
	_ = execInst
	postOnly := OpEq2(execInst, MkString("ParticipateDoNotInitiate"))
	_ = postOnly
...
func (this *Bitmex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	orderType := this.Capitalize(type_)
	_ = orderType
	request := MkMap(&VarMap{
		"symbol":   *(market).At(MkString("id")),
	...
	if IsTrue(OpOr(OpEq2(orderType, MkString("Stop")), OpOr(OpEq2(orderType, MkString("StopLimit")), OpOr(OpEq2(orderType, MkString("MarketIfTouched")), OpEq2(orderType, MkString("LimitIfTouched")))))) {
		stopPrice := this.SafeNumber2(params, MkString("stopPx"), MkString("stopPrice"))
		_ = stopPrice
...
func (this *Bitmex) EditOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	origClOrdID := this.SafeString2(params, MkString("origClOrdID"), MkString("clientOrderId"))
	_ = origClOrdID
	if IsTrue(OpNotEq2(origClOrdID, MkUndefined())) {
		*(request).At(MkString("origClOrdID")) = OpCopy(origClOrdID)
		clientOrderId := this.SafeString(params, MkString("clOrdID"), MkString("clientOrderId"))
		_ = clientOrderId
		if IsTrue(OpNotEq2(clientOrderId, MkUndefined())) {
			*(request).At(MkString("clOrdID")) = OpCopy(clientOrderId)
		}
...
func (this *Bitmex) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = response
	order := this.SafeValue(response, MkInteger(0), MkMap(&VarMap{}))
	_ = order
	error := this.SafeString(order, MkString("error"))
	_ = error
...
func (this *Bitmex) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateDeleteOrderAll"), this.Extend(request, params))
	_ = response
...
func (this *Bitmex) FetchPositions(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetPosition"), params)
	_ = response
	return response
}
...
func (this *Bitmex) Sign(goArgs ...*Variant) *Variant {
	...
	body := GoGetArg(goArgs, 5, MkUndefined())
	_ = body
	query := OpAdd(MkString("/api/"), OpAdd(*this.At(MkString("version")), OpAdd(MkString("/"), path)))
	_ = query
	if IsTrue(OpEq2(method, MkString("GET"))) {
		...
		format := this.SafeString(params, MkString("_format"))
		_ = format
	...
	url := OpAdd(*(*(*this.At(MkString("urls"))).At(MkString("api"))).At(api), query)
	_ = url
	if IsTrue(OpAnd(*this.At(MkString("apiKey")), *this.At(MkString("secret")))) {
		auth := OpAdd(method, query)
		_ = auth
		expires := this.SafeInteger(*this.At(MkString("options")), MkString("api-expires"))
		_ = expires
