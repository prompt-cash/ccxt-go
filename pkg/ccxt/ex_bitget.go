...
func (this *Bitget) FetchTime(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("dataGetCommonTimestamp"), params ); _ = response;
...
func (this *Bitget) ParseMarket(goArgs ...*Variant) *Variant{
  ...
  baseId := this.SafeString2(market , MkString("base_currency") , MkString("coin") ); _ = baseId;
  ...
  symbol := id.ToUpperCase(); _ = symbol;
  if IsTrue(spot ) {
    symbol = OpAdd(base , OpAdd(MkString("/") , quote ));
  }
  tickSize := this.SafeString(market , MkString("tick_size") ); _ = tickSize;
  sizeIncrement := this.SafeString(market , MkString("size_increment") ); _ = sizeIncrement;
  precision := MkMap(&VarMap{
        "amount":this.ParseNumber(this.ParsePrecision(sizeIncrement )),
        "price":this.ParseNumber(this.ParsePrecision(tickSize )),
        }); _ = precision;
  minAmount := this.SafeNumber2(market , MkString("min_size") , MkString("base_min_size") ); _ = minAmount;
...
func (this *Bitget) FetchMarketsByType(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(type_ , MkString("spot") )) {
    response := this.Call(MkString("dataGetCommonSymbols"), params ); _ = response;
    data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
    return this.ParseMarkets(data );
  } else {
    if IsTrue(OpEq2(type_ , MkString("swap") )) {
      response := this.Call(MkString("capiGetMarketContracts"), params ); _ = response;
...
func (this *Bitget) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("dataGetCommonCurrencys"), params ); _ = response;
...
func (this *Bitget) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  data := this.SafeValue(response , MkString("data") , response ); _ = data;
  timestamp := this.SafeInteger2(data , MkString("timestamp") , MkString("ts") ); _ = timestamp;
  nonce := this.SafeInteger(data , MkString("id") ); _ = nonce;
  orderbook := this.ParseOrderBook(data , symbol , timestamp ); _ = orderbook;
...
func (this *Bitget) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger2(ticker , MkString("timestamp") , MkString("id") ); _ = timestamp;
  symbol := OpCopy(MkUndefined() ); _ = symbol;
  marketId := this.SafeString2(ticker , MkString("instrument_id") , MkString("symbol") ); _ = marketId;
  if IsTrue(OpHasMember(marketId , *this.At(MkString("markets_by_id")) )) {
    ...
    if IsTrue(OpNotEq2(marketId , MkUndefined() )) {
      parts := marketId.Split(MkString("_") ); _ = parts;
  ...
  last := this.SafeNumber2(ticker , MkString("last") , MkString("close") ); _ = last;
  ...
  bid := this.SafeValue(ticker , MkString("bid") ); _ = bid;
  ...
  ask := this.SafeValue(ticker , MkString("ask") ); _ = ask;
  ...
  baseVolume := this.SafeNumber2(ticker , MkString("amount") , MkString("volume_24h") ); _ = baseVolume;
...
func (this *Bitget) FetchTickersByType(goArgs ...*Variant) *Variant{
  ...
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    ticker := this.ParseTicker(this.Extend(MkMap(&VarMap{"timestamp":timestamp }), *(data ).At(i ))); _ = ticker;
...
func (this *Bitget) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(trade , MkString("created_at") ); _ = timestamp;
  ...
  amountString := this.SafeString2(trade , MkString("filled_amount") , MkString("order_qty") ); _ = amountString;
  ...
  orderType := this.SafeString(trade , MkString("type") ); _ = orderType;
  ...
  id := this.SafeString2(trade , MkString("trade_id") , MkString("id") ); _ = id;
  return MkMap(&VarMap{
        "info":trade ,
        "timestamp":timestamp ,
        "datetime":this.Iso8601(timestamp ),
...
func (this *Bitget) ParseOHLCV(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timeframe := GoGetArg(goArgs, 2, MkString("1m") ); _ = timeframe;
  options := this.SafeValue(*this.At(MkString("options")) , MkString("parseOHLCV") , MkMap(&VarMap{})); _ = options;
  volume := this.SafeValue(options , MkString("volume") , MkMap(&VarMap{})); _ = volume;
  if IsTrue(GoIsArray(ohlcv )) {
    volumeIndex := this.SafeString(volume , *(market ).At(MkString("type") ), MkString("amount") ); _ = volumeIndex;
    ...
    volumeIndex := this.SafeValue(volume , *(market ).At(MkString("type") ), MkInteger(6) ); _ = volumeIndex;
    return MkArray(&VarArray{
...
func (this *Bitget) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  options := this.SafeValue(*this.At(MkString("options")) , MkString("timeframes") , MkMap(&VarMap{})); _ = options;
  intervals := this.SafeValue(options , type_ , MkMap(&VarMap{})); _ = intervals;
  interval := this.SafeValue(intervals , *(*this.At(MkString("timeframes")) ).At(timeframe )); _ = interval;
  if IsTrue(*(market ).At(MkString("spot") )) {
  ...
  response := this.Call(method , this.Extend(request , params )); _ = response;
  candles := OpCopy(response ); _ = candles;
  if IsTrue(OpNot(GoIsArray(response ))) {
...
func (this *Bitget) ParseSpotBalance(goArgs ...*Variant) *Variant{
  ...
  balances := this.SafeValue(data , MkString("list") ); _ = balances;
  for i := MkInteger(0) ; IsTrue(OpLw(i , balances.Length )); OpIncr(& i ){
    ...
    type_ := this.SafeValue(balance , MkString("type") ); _ = type_;
    if IsTrue(OpEq2(type_ , MkString("trade") )) {
      *(*(result ).At(code )).At(MkString("free") )= this.SafeString(balance , MkString("balance") );
      ...
      if IsTrue(OpOr(OpEq2(type_ , MkString("frozen") ), OpEq2(type_ , MkString("lock") ))) {
        used := this.SafeString(*(result ).At(code ), MkString("used") ); _ = used;
...
func (this *Bitget) ParseSwapBalance(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    marketId := this.SafeString(balance , MkString("symbol") ); _ = marketId;
...
func (this *Bitget) FetchAccounts(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"method":MkString("accounts") }); _ = request;
  response := this.Call(MkString("apiGetAccountAccounts"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    account := *(data ).At(i ); _ = account;
    accountId := this.SafeString(account , MkString("id") ); _ = accountId;
    type_ := this.SafeStringLower(account , MkString("type") ); _ = type_;
...
func (this *Bitget) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger2(order , MkString("created_at") , MkString("createTime") ); _ = timestamp;
  type_ := this.SafeString(order , MkString("type") ); _ = type_;
  side := this.ParseOrderSide(type_ ); _ = side;
  ...
  amount := this.SafeNumber2(order , MkString("amount") , MkString("size") ); _ = amount;
  filled := this.SafeNumber2(order , MkString("filled_amount") , MkString("filled_qty") ); _ = filled;
  cost := this.SafeNumber(order , MkString("filled_cash_amount") ); _ = cost;
  price := this.SafeNumber(order , MkString("price") ); _ = price;
  average := this.SafeNumber(order , MkString("price_avg") ); _ = average;
  status := this.ParseOrderStatus(this.SafeString2(order , MkString("state") , MkString("status") )); _ = status;
  feeCost := this.SafeNumber2(order , MkString("filled_fees") , MkString("fee") ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
...
func (this *Bitget) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  clientOrderId := this.SafeString2(params , MkString("client_oid") , MkString("clientOrderId") , this.Uuid()); _ = clientOrderId;
  ...
  method := OpCopy(MkUndefined() ); _ = method;
  if IsTrue(*(market ).At(MkString("spot") )) {
    accountId := this.GetAccountId(MkMap(&VarMap{"type":*(market ).At(MkString("type") )})); _ = accountId;
    ...
    *(request ).At(MkString("type") )= OpAdd(side , OpAdd(MkString("-") , type_ ));
      ...
      *(request ).At(MkString("price") )= this.PriceToPrecision(symbol , price );
      ...
      if IsTrue(OpEq2(type_ , MkString("market") )) {
        if IsTrue(OpEq2(side , MkString("buy") )) {
          cost := this.SafeNumber(params , MkString("amount") ); _ = cost;
    ...
    if IsTrue(*(market ).At(MkString("swap") )) {
      ...
      *(request ).At(MkString("client_oid") )= OpCopy(clientOrderId );
      orderType := this.SafeString(params , MkString("type") ); _ = orderType;
...
func (this *Bitget) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(symbol , MkUndefined() )) {
    defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("cancelOrder") , MkString("defaultType") ); _ = defaultType;
    type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
    if IsTrue(OpEq2(type_ , MkString("spot") )) {
      if IsTrue(OpEq2(symbol , MkUndefined() )) {
...
func (this *Bitget) CancelOrders(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  type_ := this.SafeString(params , MkString("type") , *(market ).At(MkString("type") )); _ = type_;
  if IsTrue(OpEq2(type_ , MkUndefined() )) {
  ...
  if IsTrue(OpEq2(type_ , MkString("spot") )) {
    ...
    *(request ).At(MkString("method") )= MkString("batchcancel") ;
    jsonIds := this.Json(ids ); _ = jsonIds;
    parts := jsonIds.Split(MkString("\"") ); _ = parts;
...
func (this *Bitget) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("apiGetOrderDepositWithdraw"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Bitget) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  tag := this.SafeString(transaction , MkString("address_tag") ); _ = tag;
  ...
  txid := this.SafeString(transaction , MkString("tx_hash") ); _ = txid;
  timestamp := this.SafeInteger(transaction , MkString("created_at") ); _ = timestamp;
  updated := this.SafeInteger(transaction , MkString("updated_at") ); _ = updated;
...
func (this *Bitget) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(since , MkUndefined() )) {
    *(request ).At(MkString("start_date") )= this.Ymd(since );
    end := this.Sum(since , OpMulti(MkInteger(2) , OpMulti(MkInteger(24) , OpMulti(MkInteger(60) , OpMulti(MkInteger(60) , MkInteger(1000) ))))); _ = end;
    *(request ).At(MkString("end_date") )= this.Ymd(end );
  }
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
    *(request ).At(MkString("size") )= OpCopy(limit );
  }
  response := this.Call(MkString("apiPostOrderMatchresults"), this.Extend(request , query )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Bitget) FetchPosition(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("swapGetPositionSinglePosition"), this.Extend(request , params )); _ = response;
  return response ;
}
...
func (this *Bitget) FetchPositions(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("swapGetPositionAllPosition"), params ); _ = response;
  return response ;
}
...
func (this *Bitget) Sign(goArgs ...*Variant) *Variant{
  ...
  url := OpAdd(this.ImplodeHostname(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(api )), request ); _ = url;
    ...
    if IsTrue(OpEq2(api , MkString("swap") )) {
      ...
      if IsTrue(OpEq2(method , MkString("POST") )) {
        ...
        if IsTrue(*(GoGetKeys(params )).At(MkString("length") )) {
          query := this.Urlencode(this.Keysort(params )); _ = query;
      ...
      if IsTrue(OpEq2(api , MkString("api") )) {
        timestamp := (this.Milliseconds()).Call(MkString("toString") ); _ = timestamp;
        auth := MkString("") ; _ = auth;
        query = this.Keysort(query );
        auth = this.Rawencode(query );
        hash := this.Hash(this.Encode(*this.At(MkString("secret")) ), MkString("sha1") ); _ = hash;
        signed := OpCopy(auth ); _ = signed;
        signature := this.Hmac(this.Encode(auth ), this.Encode(hash ), MkString("md5") ); _ = signature;
...
func (this *Bitget) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  message := this.SafeString(response , MkString("err_msg") ); _ = message;
  errorCode := this.SafeString2(response , MkString("code") , MkString("err_code") ); _ = errorCode;
  ...
  nonZeroErrorCode := OpAnd(OpNotEq2(errorCode , MkUndefined() ), OpNotEq2(errorCode , MkString("00000") )); _ = nonZeroErrorCode;
