...
func (this *Probit) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarket"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    ...
    baseId := this.SafeString(market , MkString("base_currency_id") ); _ = baseId;
    quoteId := this.SafeString(market , MkString("quote_currency_id") ); _ = quoteId;
    ...
    closed := this.SafeValue(market , MkString("closed") , MkBool(false) ); _ = closed;
    active := OpNot(closed ); _ = active;
    amountPrecision := this.SafeString(market , MkString("quantity_precision") ); _ = amountPrecision;
    costPrecision := this.SafeString(market , MkString("cost_precision") ); _ = costPrecision;
    amountTickSize := this.ParsePrecision(amountPrecision ); _ = amountTickSize;
    costTickSize := this.ParsePrecision(costPrecision ); _ = costTickSize;
    ...
    takerFeeRate := this.SafeString(market , MkString("taker_fee_rate") ); _ = takerFeeRate;
    taker := Precise.StringDiv(takerFeeRate , MkString("100") ); _ = taker;
    makerFeeRate := this.SafeString(market , MkString("maker_fee_rate") ); _ = makerFeeRate;
    maker := Precise.StringDiv(makerFeeRate , MkString("100") ); _ = maker;
...
func (this *Probit) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetCurrencyWithPlatform"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    ...
    displayName := this.SafeValue(currency , MkString("display_name") ); _ = displayName;
    name := this.SafeString(displayName , MkString("en-us") ); _ = name;
    platforms := this.SafeValue(currency , MkString("platform") , MkArray(&VarArray{})); _ = platforms;
    platformsByPriority := this.SortBy(platforms , MkString("priority") ); _ = platformsByPriority;
    platform := this.SafeValue(platformsByPriority , MkInteger(0) , MkMap(&VarMap{})); _ = platform;
    precision := this.SafeInteger(platform , MkString("precision") ); _ = precision;
    depositSuspended := this.SafeValue(platform , MkString("deposit_suspended") ); _ = depositSuspended;
    withdrawalSuspended := this.SafeValue(platform , MkString("withdrawal_suspended") ); _ = withdrawalSuspended;
    active := OpNot(OpAnd(depositSuspended , withdrawalSuspended )); _ = active;
    withdrawalFees := this.SafeValue(platform , MkString("withdrawal_fee") , MkMap(&VarMap{})); _ = withdrawalFees;
    fees := MkArray(&VarArray{}); _ = fees;
    for j := MkInteger(0) ; IsTrue(OpLw(j , withdrawalFees.Length )); OpIncr(& j ){
      withdrawalFee := *(withdrawalFees ).At(j ); _ = withdrawalFee;
      amount := this.SafeNumber(withdrawalFee , MkString("amount") ); _ = amount;
      priority := this.SafeInteger(withdrawalFee , MkString("priority") ); _ = priority;
      if IsTrue(OpAnd(OpNotEq2(amount , MkUndefined() ), OpNotEq2(priority , MkUndefined() ))) {
        fees.Push(withdrawalFee );
      }
    }
    withdrawalFeesByPriority := this.SortBy(fees , MkString("priority") ); _ = withdrawalFeesByPriority;
    withdrawalFee := this.SafeValue(withdrawalFeesByPriority , MkInteger(0) , MkMap(&VarMap{})); _ = withdrawalFee;
    fee := this.SafeNumber(withdrawalFee , MkString("amount") ); _ = fee;
...
func (this *Probit) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetBalance"), params ); _ = response;
  result := MkMap(&VarMap{
        "info":response ,
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    ...
    currencyId := this.SafeString(balance , MkString("currency_id") ); _ = currencyId;
...
func (this *Probit) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"market_id":*(market ).At(MkString("id") )}); _ = request;
  ...
  dataBySide := this.GroupBy(data , MkString("side") ); _ = dataBySide;
...
func (this *Probit) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"market_ids":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetTicker"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  ticker := this.SafeValue(data , MkInteger(0) ); _ = ticker;
...
func (this *Probit) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(ticker , MkString("market_id") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("-") ); _ = symbol;
  close := this.SafeNumber(ticker , MkString("last") ); _ = close;
  change := this.SafeNumber(ticker , MkString("change") ); _ = change;
...
func (this *Probit) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetTradeHistory"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Probit) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  feeCost := this.SafeNumber(trade , MkString("fee_amount") ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyId := this.SafeString(trade , MkString("fee_currency_id") ); _ = feeCurrencyId;
...
func (this *Probit) FetchTime(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.Parse8601(this.SafeString(response , MkString("data") )); _ = timestamp;
  return timestamp ;
}

func (this *Probit) NormalizeOHLCVTimestamp(goArgs ...*Variant) *Variant{
  timestamp := GoGetArg(goArgs, 0, MkUndefined()); _ = timestamp;
  timeframe := GoGetArg(goArgs, 1, MkUndefined()); _ = timeframe;
  after := GoGetArg(goArgs, 2, MkBool(false) ); _ = after;
  duration := this.ParseTimeframe(timeframe ); _ = duration;
  if IsTrue(OpEq2(timeframe , MkString("1M") )) {
    iso8601 := this.Iso8601(timestamp ); _ = iso8601;
    parts := iso8601.Split(MkString("-") ); _ = parts;
    year := this.SafeString(parts , MkInteger(0) ); _ = year;
    month := this.SafeInteger(parts , MkInteger(1) ); _ = month;
    if IsTrue(after ) {
    ...
    if IsTrue(OpEq2(timeframe , MkString("1w") )) {
      ...
      firstSunday := MkInteger(259200) ; _ = firstSunday;
      difference := OpSub(timestamp , firstSunday ); _ = difference;
      numWeeks := Math.Floor(OpDiv(difference , duration )); _ = numWeeks;
      previousSunday := this.Sum(firstSunday , OpMulti(numWeeks , duration )); _ = previousSunday;
      if IsTrue(after ) {
...
func (this *Probit) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  interval := *(*this.At(MkString("timeframes")) ).At(timeframe ); _ = interval;
  limit = OpTrinary(OpEq2(limit , MkUndefined() ), MkInteger(100) , limit );
  requestLimit := this.Sum(limit , MkInteger(1) ); _ = requestLimit;
  ...
  duration := this.ParseTimeframe(timeframe ); _ = duration;
  startTime := OpCopy(since ); _ = startTime;
  endTime := OpCopy(now ); _ = endTime;
  ...
  startTimeNormalized := this.NormalizeOHLCVTimestamp(startTime , timeframe ); _ = startTimeNormalized;
  endTimeNormalized := this.NormalizeOHLCVTimestamp(endTime , timeframe , MkBool(true) ); _ = endTimeNormalized;
  *(request ).At(MkString("start_time") )= OpCopy(startTimeNormalized );
  *(request ).At(MkString("end_time") )= OpCopy(endTimeNormalized );
  response := this.Call(MkString("publicGetCandle"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Probit) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOpenOrder"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
...
func (this *Probit) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrderHistory"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  return this.ParseOrders(data , market , since , limit );
}
...
func (this *Probit) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrder"), this.Extend(request , query )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Probit) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(order , MkString("market_id") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("-") ); _ = symbol;
  timestamp := this.Parse8601(this.SafeString(order , MkString("time") )); _ = timestamp;
  price := this.SafeNumber(order , MkString("limit_price") ); _ = price;
  filled := this.SafeNumber(order , MkString("filled_quantity") ); _ = filled;
  remaining := this.SafeNumber(order , MkString("open_quantity") ); _ = remaining;
  canceledAmount := this.SafeNumber(order , MkString("cancelled_quantity") ); _ = canceledAmount;
  if IsTrue(OpNotEq2(canceledAmount , MkUndefined() )) {
    remaining = this.Sum(remaining , canceledAmount );
  }
  amount := this.SafeNumber(order , MkString("quantity") , this.Sum(filled , remaining )); _ = amount;
  cost := this.SafeNumber2(order , MkString("filled_cost") , MkString("cost") ); _ = cost;
...
func (this *Probit) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  options := this.SafeValue(*this.At(MkString("options")) , MkString("timeInForce") ); _ = options;
  defaultTimeInForce := this.SafeValue(options , type_ ); _ = defaultTimeInForce;
  timeInForce := this.SafeString2(params , MkString("timeInForce") , MkString("time_in_force") , defaultTimeInForce ); _ = timeInForce;
  request := MkMap(&VarMap{
        "market_id":*(market ).At(MkString("id") ),
        "type":type_ ,
        "side":side ,
  ...
  costToPrecision := OpCopy(MkUndefined() ); _ = costToPrecision;
  ...
  response := this.Call(MkString("privatePostNewOrder"), this.Extend(request , query )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  order := this.ParseOrder(data , market ); _ = order;
...
func (this *Probit) ParseDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currencyId := this.SafeString(depositAddress , MkString("currency_id") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId ); _ = code;
...
func (this *Probit) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"currency_id":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetDepositAddress"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  firstAddress := this.SafeValue(data , MkInteger(0) ); _ = firstAddress;
...
func (this *Probit) FetchDepositAddresses(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{}); _ = request;
  if IsTrue(codes ) {
    currencyIds := MkArray(&VarArray{}); _ = currencyIds;
    for i := MkInteger(0) ; IsTrue(OpLw(i , codes.Length )); OpIncr(& i ){
      currency := this.Currency(*(codes ).At(i )); _ = currency;
...
func (this *Probit) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostWithdrawal"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
...
func (this *Probit) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  tag := this.SafeString(transaction , MkString("destination_tag") ); _ = tag;
...
func (this *Probit) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("accounts") )) {
    ...
    auth := OpAdd(*this.At(MkString("apiKey")) , OpAdd(MkString(":") , *this.At(MkString("secret")) )); _ = auth;
    ...
    if IsTrue(OpEq2(api , MkString("public") )) {
      ...
      if IsTrue(OpEq2(api , MkString("private") )) {
        now := this.Milliseconds(); _ = now;
        this.CheckRequiredCredentials();
        expires := this.SafeInteger(*this.At(MkString("options")) , MkString("expires") ); _ = expires;
...
func (this *Probit) SignIn(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("accountsPostToken"), this.Extend(request , params )); _ = response;
  expiresIn := this.SafeInteger(response , MkString("expires_in") ); _ = expiresIn;
  accessToken := this.SafeString(response , MkString("access_token") ); _ = accessToken;
...
func (this *Probit) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("errorCode") , response )) {
    errorCode := this.SafeString(response , MkString("errorCode") ); _ = errorCode;
