...
func (this *Novadax) FetchTime(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetCommonTimestamp"), params ); _ = response;
  return this.SafeInteger(response , MkString("data") );
}

func (this *Novadax) FetchMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetCommonSymbols"), params ); _ = response;
...
func (this *Novadax) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  baseVolume := this.SafeNumber(ticker , MkString("baseVolume24h") ); _ = baseVolume;
  quoteVolume := this.SafeNumber(ticker , MkString("quoteVolume24h") ); _ = quoteVolume;
...
func (this *Novadax) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    ticker := this.ParseTicker(*(data ).At(i )); _ = ticker;
...
func (this *Novadax) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  symbol := this.SafeSymbol(marketId , market , MkString("_") ); _ = symbol;
  takerOrMaker := this.SafeStringLower(trade , MkString("role") ); _ = takerOrMaker;
  feeString := this.SafeString(trade , MkString("fee") ); _ = feeString;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeString , MkUndefined() )) {
    parts := feeString.Split(MkString(" ") ); _ = parts;
...
func (this *Novadax) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(since , MkUndefined() )) {
    ...
    *(request ).At(MkString("to") )= OpCopy(now );
    ...
    startFrom := parseInt(OpDiv(since , MkInteger(1000) )); _ = startFrom;
    *(request ).At(MkString("from") )= OpCopy(startFrom );
    *(request ).At(MkString("to") )= this.Sum(startFrom , OpMulti(limit , duration ));
  }
  response := this.Call(MkString("publicGetMarketKlineHistory"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Novadax) ParseOHLCV(goArgs ...*Variant) *Variant{
  ...
  volumeField := this.SafeString(options , MkString("volume") , MkString("amount") ); _ = volumeField;
...
func (this *Novadax) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetAccountGetBalance"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  result := MkMap(&VarMap{
        "info":response ,
...
func (this *Novadax) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(stopPrice , MkUndefined() )) {
    ...
    defaultOperator := OpTrinary(OpEq2(uppercaseSide , MkString("BUY") ), MkString("LTE") , MkString("GTE") ); _ = defaultOperator;
      ...
      if IsTrue(OpEq2(uppercaseSide , MkString("SELL") )) {
        *(request ).At(MkString("amount") )= this.AmountToPrecision(symbol , amount );
        ...
        if IsTrue(OpEq2(uppercaseSide , MkString("BUY") )) {
          value := this.SafeNumber(params , MkString("value") ); _ = value;
  ...
  *(request ).At(MkString("type") )= OpCopy(uppercaseType );
  response := this.Call(MkString("privatePostOrdersCreate"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
...
func (this *Novadax) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"id":id }); _ = request;
  response := this.Call(MkString("privateGetOrdersGet"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  return this.ParseOrder(data );
}
...
func (this *Novadax) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrdersList"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Novadax) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"status":MkString("SUBMITTED,PROCESSING,PARTIAL_FILLED,CANCELING") }); _ = request;
...
func (this *Novadax) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"status":MkString("FILLED,CANCELED,REJECTED") }); _ = request;
  return this.FetchOrders(symbol , since , limit , this.Extend(request , params ));
}
...
func (this *Novadax) FetchOrderTrades(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"id":id }); _ = request;
  response := this.Call(MkString("privateGetOrdersFill"), this.Extend(request , params )); _ = response;
  market := OpCopy(MkUndefined() ); _ = market;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(symbol );
  }
...
func (this *Novadax) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  cost := this.SafeNumber2(order , MkString("filledValue") , MkString("value") ); _ = cost;
  ...
  average := this.SafeNumber(order , MkString("averagePrice") ); _ = average;
  filled := this.SafeNumber(order , MkString("filledAmount") ); _ = filled;
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(order , MkString("filledFee") ); _ = feeCost;
...
func (this *Novadax) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostAccountWithdrawCoin"), this.Extend(request , params )); _ = response;
  return this.ParseTransaction(response , currency );
}

func (this *Novadax) FetchAccounts(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("privateGetAccountSubs"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    account := *(data ).At(i ); _ = account;
    accountId := this.SafeString(account , MkString("subId") ); _ = accountId;
    type_ := this.SafeString(account , MkString("subAccount") ); _ = type_;
    result.Push(MkMap(&VarMap{
            "id":accountId ,
            "type":type_ ,
            "currency":MkUndefined() ,
...
func (this *Novadax) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"type":MkString("coin_in") }); _ = request;
...
func (this *Novadax) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"type":MkString("coin_out") }); _ = request;
  return this.FetchTransactions(code , since , limit , this.Extend(request , params ));
}
...
func (this *Novadax) FetchTransactions(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetWalletQueryDepositWithdraw"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Novadax) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString2(transaction , MkString("id") , MkString("data") ); _ = id;
  ...
  timestamp := this.SafeInteger(transaction , MkString("createdAt") ); _ = timestamp;
...
func (this *Novadax) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrdersFills"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  return this.ParseTrades(data , market , since , limit );
}
...
func (this *Novadax) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    if IsTrue(OpEq2(api , MkString("private") )) {
      ...
      queryString := OpCopy(MkUndefined() ); _ = queryString;
