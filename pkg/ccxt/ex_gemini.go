...
func (this *Gemini) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  method := this.SafeValue(*this.At(MkString("options")) , MkString("fetchMarketsMethod") , MkString("fetch_markets_from_api") ); _ = method;
  return this.Call(method , params );
}

func (this *Gemini) FetchMarketsFromWeb(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("webGetRestApi"), params ); _ = response;
  sections := response.Split(MkString("<h1 id=\"symbols-and-minimums\">Symbols and minimums</h1>") ); _ = sections;
  numSections := OpCopy(sections.Length ); _ = numSections;
  error := OpAdd(*this.At(MkString("id")) , OpAdd(MkString(" the ") , OpAdd(*this.At(MkString("name")) , OpAdd(MkString(" API doc HTML markup has changed, breaking the parser of order limits and precision info for ") , OpAdd(*this.At(MkString("name")) , MkString(" markets.") ))))); _ = error;
  if IsTrue(OpNotEq2(numSections , MkInteger(2) )) {
    panic(NewNotSupported(error ));
  }
  tables := (*(sections ).At(MkInteger(1) )).Call(MkString("split") , MkString("tbody>") ); _ = tables;
  numTables := OpCopy(tables.Length ); _ = numTables;
  if IsTrue(OpLw(numTables , MkInteger(2) )) {
    panic(NewNotSupported(error ));
  }
  rows := (*(tables ).At(MkInteger(1) )).Call(MkString("split") , MkString("\n<tr>\n") ); _ = rows;
  numRows := OpCopy(rows.Length ); _ = numRows;
  ...
  for i := MkInteger(1) ; IsTrue(OpLw(i , numRows )); OpIncr(& i ){
    ...
    numCells := OpCopy(cells.Length ); _ = numCells;
    if IsTrue(OpLw(numCells , MkInteger(5) )) {
      panic(NewNotSupported(error ));
    }
    marketId := (*(cells ).At(MkInteger(0) )).Call(MkString("replace") , MkString("<td>") , MkString("") ); _ = marketId;
    minAmountString := (*(cells ).At(MkInteger(1) )).Call(MkString("replace") , MkString("<td>") , MkString("") ); _ = minAmountString;
    minAmountParts := minAmountString.Split(MkString(" ") ); _ = minAmountParts;
    minAmount := this.SafeNumber(minAmountParts , MkInteger(0) ); _ = minAmount;
    amountPrecisionString := (*(cells ).At(MkInteger(2) )).Call(MkString("replace") , MkString("<td>") , MkString("") ); _ = amountPrecisionString;
    amountPrecisionParts := amountPrecisionString.Split(MkString(" ") ); _ = amountPrecisionParts;
    amountPrecision := this.SafeNumber(amountPrecisionParts , MkInteger(0) ); _ = amountPrecision;
    idLength := OpSub(marketId.Length , MkInteger(0) ); _ = idLength;
    quoteId := marketId.Slice(OpSub(idLength , MkInteger(3) ), idLength ); _ = quoteId;
    quote := this.SafeCurrencyCode(quoteId ); _ = quote;
    pricePrecisionString := (*(cells ).At(MkInteger(3) )).Call(MkString("replace") , MkString("<td>") , MkString("") ); _ = pricePrecisionString;
    pricePrecisionParts := pricePrecisionString.Split(MkString(" ") ); _ = pricePrecisionParts;
    pricePrecision := this.SafeNumber(pricePrecisionParts , MkInteger(0) ); _ = pricePrecision;
    baseId := marketId.Replace(quoteId , MkString("") ); _ = baseId;
...
func (this *Gemini) FetchMarketsFromAPI(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetV1Symbols"), params ); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    marketId := *(response ).At(i ); _ = marketId;
    market := OpCopy(marketId ); _ = market;
    idLength := OpSub(marketId.Length , MkInteger(0) ); _ = idLength;
    baseId := marketId.Slice(MkInteger(0) , OpSub(idLength , MkInteger(3) )); _ = baseId;
    quoteId := marketId.Slice(OpSub(idLength , MkInteger(3) ), idLength ); _ = quoteId;
    base := this.SafeCurrencyCode(baseId ); _ = base;
    quote := this.SafeCurrencyCode(quoteId ); _ = quote;
    symbol := OpAdd(base , OpAdd(MkString("/") , quote )); _ = symbol;
...
func (this *Gemini) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetV1BookSymbol"), this.Extend(request , params )); _ = response;
...
func (this *Gemini) FetchTickerV1(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetV1PubtickerSymbol"), this.Extend(request , params )); _ = response;
...
func (this *Gemini) FetchTickerV2(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetV2TickerSymbol"), this.Extend(request , params )); _ = response;
  return this.ParseTicker(response , market );
}
...
func (this *Gemini) FetchTickerV1AndV2(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  tickerA := this.FetchTickerV1(symbol , params ); _ = tickerA;
  tickerB := this.FetchTickerV2(symbol , params ); _ = tickerB;
...
func (this *Gemini) FetchTicker(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  method := this.SafeValue(*this.At(MkString("options")) , MkString("fetchTickerMethod") , MkString("fetchTickerV1") ); _ = method;
...
func (this *Gemini) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  volume := this.SafeValue(ticker , MkString("volume") , MkMap(&VarMap{})); _ = volume;
  timestamp := this.SafeInteger(volume , MkString("timestamp") ); _ = timestamp;
  symbol := OpCopy(MkUndefined() ); _ = symbol;
  marketId := this.SafeString(ticker , MkString("pair") ); _ = marketId;
  ...
  price := this.SafeNumber(ticker , MkString("price") ); _ = price;
  last := this.SafeNumber2(ticker , MkString("last") , MkString("close") , price ); _ = last;
  percentage := this.SafeNumber(ticker , MkString("percentChange24h") ); _ = percentage;
  ...
  baseVolume := this.SafeNumber(volume , baseId ); _ = baseVolume;
  quoteVolume := this.SafeNumber(volume , quoteId ); _ = quoteVolume;
...
func (this *Gemini) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicGetV1Pricefeed"), params ); _ = response;
...
func (this *Gemini) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeInteger(trade , MkString("timestampms") ); _ = timestamp;
  ...
  side := this.SafeStringLower(trade , MkString("type") ); _ = side;
  symbol := this.SafeSymbol(MkUndefined() , market ); _ = symbol;
  return MkMap(&VarMap{
        "id":id ,
        "order":orderId ,
        "info":trade ,
...
func (this *Gemini) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetV1TradesSymbol"), this.Extend(request , params )); _ = response;
...
func (this *Gemini) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostV1Balances"), params ); _ = response;
...
func (this *Gemini) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeInteger(order , MkString("timestampms") ); _ = timestamp;
  amount := this.SafeNumber(order , MkString("original_amount") ); _ = amount;
  remaining := this.SafeNumber(order , MkString("remaining_amount") ); _ = remaining;
  filled := this.SafeNumber(order , MkString("executed_amount") ); _ = filled;
  status := MkString("closed") ; _ = status;
  ...
  average := this.SafeNumber(order , MkString("avg_execution_price") ); _ = average;
...
func (this *Gemini) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostV1OrderStatus"), this.Extend(request , params )); _ = response;
...
func (this *Gemini) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privatePostV1Orders"), params ); _ = response;
...
func (this *Gemini) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostV1OrderNew"), this.Extend(request , params )); _ = response;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Gemini) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostV1Mytrades"), this.Extend(request , params )); _ = response;
  return this.ParseTrades(response , market , since , limit );
}
...
func (this *Gemini) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostV1WithdrawCurrency"), this.Extend(request , params )); _ = response;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Gemini) FetchTransactions(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostV1Transfers"), this.Extend(request , params )); _ = response;
...
func (this *Gemini) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(transaction , MkString("timestampms") ); _ = timestamp;
  ...
  address := this.SafeString(transaction , MkString("destination") ); _ = address;
  ...
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeAmount := this.SafeNumber(transaction , MkString("feeAmount") ); _ = feeAmount;
...
func (this *Gemini) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    payload := this.Json(request ); _ = payload;
    payload = this.StringToBase64(payload );
    signature := this.Hmac(payload , this.Encode(*this.At(MkString("secret")) ), MkString("sha384") ); _ = signature;
...
func (this *Gemini) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  result := this.SafeString(response , MkString("result") ); _ = result;
  if IsTrue(OpEq2(result , MkString("error") )) {
    reason := this.SafeString(response , MkString("reason") ); _ = reason;
...
func (this *Gemini) CreateDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostV1DepositCurrencyNewAddress"), this.Extend(request , params )); _ = response;
...
func (this *Gemini) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetV2CandlesSymbolTimeframe"), this.Extend(request , params )); _ = response;
