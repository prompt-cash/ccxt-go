...
func (this *Bitpanda) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = currency
		id := this.SafeString(currency, MkString("code"))
		_ = id
...
func (this *Bitpanda) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = market
		baseAsset := this.SafeValue(market, MkString("base"), MkMap(&VarMap{}))
		_ = baseAsset
		quoteAsset := this.SafeValue(market, MkString("quote"), MkMap(&VarMap{}))
		_ = quoteAsset
		baseId := this.SafeString(baseAsset, MkString("code"))
		_ = baseId
		quoteId := this.SafeString(quoteAsset, MkString("code"))
		_ = quoteId
		id := OpAdd(baseId, OpAdd(MkString("_"), quoteId))
		_ = id
...
func (this *Bitpanda) FetchTradingFees(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(method, MkUndefined())) {
		options := this.SafeValue(*this.At(MkString("options")), MkString("fetchTradingFees"), MkMap(&VarMap{}))
		_ = options
...
func (this *Bitpanda) FetchPublicTradingFees(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("publicGetFees"), params)
	_ = response
	feeGroupsById := this.IndexBy(response, MkString("fee_group_id"))
	_ = feeGroupsById
	feeGroupId := this.SafeValue(*this.At(MkString("options")), MkString("fee_group_id"), MkString("default"))
	_ = feeGroupId
	feeGroup := this.SafeValue(feeGroupsById, feeGroupId, MkMap(&VarMap{}))
	_ = feeGroup
	feeTiers := this.SafeValue(feeGroup, MkString("fee_tiers"))
	_ = feeTiers
	result := MkMap(&VarMap{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, (*((this).At(MkString("symbols")))).Length)); OpIncr(&i) {
		...
		for i := MkInteger(0); IsTrue(OpLw(i, feeTiers.Length)); OpIncr(&i) {
			...
			_ = volume
			taker := this.SafeNumber(tier, MkString("taker_fee"))
			_ = taker
			maker := this.SafeNumber(tier, MkString("maker_fee"))
			_ = maker
...
func (this *Bitpanda) FetchPrivateTradingFees(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetAccountFees"), params)
	_ = response
	activeFeeTier := this.SafeValue(response, MkString("active_fee_tier"), MkMap(&VarMap{}))
	_ = activeFeeTier
	result := MkMap(&VarMap{
		"info":       response,
	...
	_ = result
	feeTiers := this.SafeValue(response, MkString("fee_tiers"))
	_ = feeTiers
	takerFees := MkArray(&VarArray{})
	_ = takerFees
	makerFees := MkArray(&VarArray{})
...
func (this *Bitpanda) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	marketId := this.SafeString(ticker, MkString("instrument_code"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("_"))
	_ = symbol
	last := this.SafeNumber(ticker, MkString("last_price"))
	_ = last
	percentage := this.SafeNumber(ticker, MkString("price_change_percentage"))
	_ = percentage
	change := this.SafeNumber(ticker, MkString("price_change"))
	_ = change
...
func (this *Bitpanda) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"instrument_code": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetMarketTickerInstrumentCode"), this.Extend(request, params))
	_ = response
...
func (this *Bitpanda) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("publicGetMarketTicker"), params)
	_ = response
	result := MkMap(&VarMap{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
...
func (this *Bitpanda) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"instrument_code": this.MarketId(symbol)})
	_ = request
	if IsTrue(OpNotEq2(limit, MkUndefined())) {
		*(request).At(MkString("depth")) = OpCopy(limit)
	}
	response := this.Call(MkString("publicGetOrderBookInstrumentCode"), this.Extend(request, params))
	_ = response
	timestamp := this.Parse8601(this.SafeString(response, MkString("time")))
	_ = timestamp
...
func (this *Bitpanda) ParseOHLCV(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	granularity := this.SafeValue(ohlcv, MkString("granularity"))
	_ = granularity
	unit := this.SafeString(granularity, MkString("unit"))
	_ = unit
	period := this.SafeString(granularity, MkString("period"))
	_ = period
	units := MkMap(&VarMap{
		"MINUTES": MkString("m"),
		"HOURS":   MkString("h"),
	...
	_ = units
	lowercaseUnit := this.SafeString(units, unit)
	_ = lowercaseUnit
	timeframe := OpAdd(period, lowercaseUnit)
	_ = timeframe
	durationInSeconds := this.ParseTimeframe(timeframe)
	_ = durationInSeconds
	duration := OpMulti(durationInSeconds, MkInteger(1000))
	_ = duration
	timestamp := this.Parse8601(this.SafeString(ohlcv, MkString("time")))
	_ = timestamp
	alignedTimestamp := OpMulti(duration, parseInt(OpDiv(timestamp, duration)))
	_ = alignedTimestamp
	options := this.SafeValue(*this.At(MkString("options")), MkString("fetchOHLCV"), MkMap(&VarMap{}))
	_ = options
	volumeField := this.SafeString(options, MkString("volume"), MkString("total_amount"))
	_ = volumeField
...
func (this *Bitpanda) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	periodUnit := this.SafeString(*this.At(MkString("timeframes")), timeframe)
	_ = periodUnit
	period := MkUndefined()
	_ = period
	unit := MkUndefined()
	...
	response := this.Call(MkString("publicGetCandlesticksInstrumentCode"), this.Extend(request, params))
	_ = response
...
func (this *Bitpanda) ParseTrade(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	feeInfo := this.SafeValue(trade, MkString("fee"), MkMap(&VarMap{}))
	_ = feeInfo
	trade = this.SafeValue(trade, MkString("trade"), trade)
	timestamp := this.SafeInteger(trade, MkString("trade_timestamp"))
	_ = timestamp
	if IsTrue(OpEq2(timestamp, MkUndefined())) {
		timestamp = this.Parse8601(this.SafeString(trade, MkString("time")))
	}
	side := this.SafeStringLower2(trade, MkString("side"), MkString("taker_side"))
	_ = side
	...
	marketId := this.SafeString(trade, MkString("instrument_code"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("_"))
	_ = symbol
	feeCost := this.SafeNumber(feeInfo, MkString("fee_amount"))
	_ = feeCost
	...
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCurrencyId := this.SafeString(feeInfo, MkString("fee_currency"))
		_ = feeCurrencyId
		feeCurrencyCode := this.SafeCurrencyCode(feeCurrencyId)
		_ = feeCurrencyCode
		feeRate := this.SafeNumber(feeInfo, MkString("fee_percentage"))
		_ = feeRate
		fee = MkMap(&VarMap{
			"cost":     feeCost,
...
func (this *Bitpanda) FetchTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetPriceTicksInstrumentCode"), this.Extend(request, params))
	_ = response
...
func (this *Bitpanda) CreateDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostAccountDepositCrypto"), this.Extend(request, params))
	_ = response
...
func (this *Bitpanda) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	currency := this.Currency(code)
	_ = currency
	request := MkMap(&VarMap{"currency_code": *(currency).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("privateGetAccountDepositCryptoCurrencyCode"), this.Extend(request, params))
	_ = response
	return this.ParseDepositAddress(response, currency)
}
...
func (this *Bitpanda) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpNotEq2(since, MkUndefined())) {
		to := this.SafeString(params, MkString("to"))
		_ = to
	...
	response := this.Call(MkString("privateGetAccountDeposits"), this.Extend(request, params))
	_ = response
	depositHistory := this.SafeValue(response, MkString("deposit_history"), MkArray(&VarArray{}))
	_ = depositHistory
...
func (this *Bitpanda) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetAccountWithdrawals"), this.Extend(request, params))
	_ = response
	withdrawalHistory := this.SafeValue(response, MkString("withdrawal_history"), MkArray(&VarArray{}))
	_ = withdrawalHistory
...
func (this *Bitpanda) Withdraw(goArgs ...*Variant) *Variant {
	...
	_ = request
	options := this.SafeValue(*this.At(MkString("options")), MkString("fiat"), MkArray(&VarArray{}))
	_ = options
	isFiat := this.InArray(code, options)
	_ = isFiat
	method := OpTrinary(isFiat, MkString("privatePostAccountWithdrawFiat"), MkString("privatePostAccountWithdrawCrypto"))
	_ = method
	if IsTrue(isFiat) {
		payoutAccountId := this.SafeString(params, MkString("payout_account_id"))
		_ = payoutAccountId
		...
		recipient := MkMap(&VarMap{"address": address})
		_ = recipient
...
func (this *Bitpanda) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	currency := GoGetArg(goArgs, 1, MkUndefined())
	_ = currency
	id := this.SafeString(transaction, MkString("transaction_id"))
	_ = id
	...
	status := MkString("ok")
	_ = status
	feeCost := this.SafeNumber2(transaction, MkString("fee_amount"), MkString("fee"))
	_ = feeCost
	fee := OpCopy(MkUndefined())
	_ = fee
	addressTo := this.SafeString(transaction, MkString("recipient"))
	_ = addressTo
	tagTo := this.SafeString(transaction, MkString("destination_tag"))
	_ = tagTo
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCurrencyId := this.SafeString(transaction, MkString("fee_currency"), currencyId)
		_ = feeCurrencyId
		feeCurrencyCode := this.SafeCurrencyCode(feeCurrencyId)
		_ = feeCurrencyCode
		fee = MkMap(&VarMap{
...
func (this *Bitpanda) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	rawOrder := this.SafeValue(order, MkString("order"), order)
	_ = rawOrder
	id := this.SafeString(rawOrder, MkString("order_id"))
	_ = id
	clientOrderId := this.SafeString(rawOrder, MkString("client_id"))
	_ = clientOrderId
	timestamp := this.Parse8601(this.SafeString(rawOrder, MkString("time")))
	_ = timestamp
	rawStatus := this.ParseOrderStatus(this.SafeString(rawOrder, MkString("status")))
	_ = rawStatus
	status := this.ParseOrderStatus(rawStatus)
	_ = status
	marketId := this.SafeString(rawOrder, MkString("instrument_code"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("_"))
	_ = symbol
	price := this.SafeNumber(rawOrder, MkString("price"))
	_ = price
	amount := this.SafeNumber(rawOrder, MkString("amount"))
	_ = amount
	filledString := this.SafeString(rawOrder, MkString("filled_amount"))
	_ = filledString
	filled := this.ParseNumber(filledString)
	_ = filled
	side := this.SafeStringLower(rawOrder, MkString("side"))
	_ = side
	type_ := this.SafeStringLower(rawOrder, MkString("type"))
	_ = type_
	timeInForce := this.ParseTimeInForce(this.SafeString(rawOrder, MkString("time_in_force")))
	_ = timeInForce
	stopPrice := this.SafeNumber(rawOrder, MkString("trigger_price"))
	_ = stopPrice
	postOnly := this.SafeValue(rawOrder, MkString("is_post_only"))
	_ = postOnly
	rawTrades := this.SafeValue(order, MkString("trades"), MkArray(&VarArray{}))
	_ = rawTrades
	trades := this.ParseTrades(rawTrades, market, MkUndefined(), MkUndefined(), MkMap(&VarMap{"type": type_}))
	_ = trades
...
func (this *Bitpanda) CreateOrder(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(uppercaseType, MkString("STOP"))) {
		triggerPrice := this.SafeNumber(params, MkString("trigger_price"))
		_ = triggerPrice
	...
	clientOrderId := this.SafeString2(params, MkString("clientOrderId"), MkString("client_id"))
	_ = clientOrderId
	...
	response := this.Call(MkString("privatePostAccountOrders"), this.Extend(request, params))
	_ = response
...
func (this *Bitpanda) CancelOrder(goArgs ...*Variant) *Variant {
	...
	method := MkString("privateDeleteAccountOrdersOrderId")
	_ = method
	request := MkMap(&VarMap{})
	_ = request
	if IsTrue(OpNotEq2(clientOrderId, MkUndefined())) {
...
func (this *Bitpanda) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateDeleteAccountOrders"), this.Extend(request, params))
	_ = response
	return response
}
...
func (this *Bitpanda) CancelOrders(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"ids": ids.Join(MkString(","))})
	_ = request
	response := this.Call(MkString("privateDeleteAccountOrders"), this.Extend(request, params))
	_ = response
	return response
...
func (this *Bitpanda) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetAccountOrdersOrderId"), this.Extend(request, params))
	_ = response
...
func (this *Bitpanda) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetAccountOrders"), this.Extend(request, params))
	_ = response
	orderHistory := this.SafeValue(response, MkString("order_history"), MkArray(&VarArray{}))
	_ = orderHistory
...
func (this *Bitpanda) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"with_cancelled_and_rejected": MkBool(true)})
	_ = request
...
func (this *Bitpanda) FetchOrderTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetAccountOrdersOrderIdTrades"), this.Extend(request, params))
	_ = response
	tradeHistory := this.SafeValue(response, MkString("trade_history"), MkArray(&VarArray{}))
	_ = tradeHistory
	market := OpCopy(MkUndefined())
	_ = market
	if IsTrue(OpNotEq2(symbol, MkUndefined())) {
...
func (this *Bitpanda) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetAccountTrades"), this.Extend(request, params))
	_ = response
	tradeHistory := this.SafeValue(response, MkString("trade_history"), MkArray(&VarArray{}))
	_ = tradeHistory
	return this.ParseTrades(tradeHistory, market, since, limit)
