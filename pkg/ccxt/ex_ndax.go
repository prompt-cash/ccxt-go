...
func (this *Ndax) SignIn(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"grant_type":MkString("client_credentials") }); _ = request;
  response := this.Call(MkString("publicGetAuthenticate"), this.Extend(request , params )); _ = response;
  sessionToken := this.SafeString(response , MkString("SessionToken") ); _ = sessionToken;
  if IsTrue(OpNotEq2(sessionToken , MkUndefined() )) {
    *(*this.At(MkString("options")) ).At(MkString("sessionToken") )= OpCopy(sessionToken );
    return response ;
  }
  pending2faToken := this.SafeString(response , MkString("Pending2FaToken") ); _ = pending2faToken;
  if IsTrue(OpNotEq2(pending2faToken , MkUndefined() )) {
    *(*this.At(MkString("options")) ).At(MkString("pending2faToken") )= OpCopy(pending2faToken );
    ...
    response := this.Call(MkString("publicGetAuthenticate2FA"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  omsId := this.SafeInteger(*this.At(MkString("options")) , MkString("omsId") , MkInteger(1) ); _ = omsId;
  request := MkMap(&VarMap{"omsId":omsId }); _ = request;
  response := this.Call(MkString("publicGetGetProducts"), this.Extend(request , params )); _ = response;
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    currency := *(response ).At(i ); _ = currency;
    id := this.SafeString(currency , MkString("ProductId") ); _ = id;
    name := this.SafeString(currency , MkString("ProductFullName") ); _ = name;
    type_ := this.SafeString(currency , MkString("ProductType") ); _ = type_;
    code := this.SafeCurrencyCode(this.SafeString(currency , MkString("Product") )); _ = code;
    precision := this.SafeNumber(currency , MkString("TickSize") ); _ = precision;
    isDisabled := this.SafeValue(currency , MkString("IsDisabled") ); _ = isDisabled;
    active := OpNot(isDisabled ); _ = active;
...
func (this *Ndax) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  omsId := this.SafeInteger(*this.At(MkString("options")) , MkString("omsId") , MkInteger(1) ); _ = omsId;
  request := MkMap(&VarMap{"omsId":omsId }); _ = request;
  response := this.Call(MkString("publicGetGetInstruments"), this.Extend(request , params )); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    market := *(response ).At(i ); _ = market;
    id := this.SafeString(market , MkString("InstrumentId") ); _ = id;
    baseId := this.SafeString(market , MkString("Product1") ); _ = baseId;
    quoteId := this.SafeString(market , MkString("Product2") ); _ = quoteId;
    base := this.SafeCurrencyCode(this.SafeString(market , MkString("Product1Symbol") )); _ = base;
    quote := this.SafeCurrencyCode(this.SafeString(market , MkString("Product2Symbol") )); _ = quote;
    ...
    sessionStatus := this.SafeString(market , MkString("SessionStatus") ); _ = sessionStatus;
    isDisable := this.SafeValue(market , MkString("IsDisable") ); _ = isDisable;
    sessionRunning := OpEq2(sessionStatus , MkString("Running") ); _ = sessionRunning;
    active := OpTrinary(OpAnd(sessionRunning , OpNot(isDisable )), MkBool(true) , MkBool(false) ); _ = active;
...
func (this *Ndax) ParseOrderBook(goArgs ...*Variant) *Variant{
  ...
  priceKey := GoGetArg(goArgs, 5, MkInteger(6) ); _ = priceKey;
  amountKey := GoGetArg(goArgs, 6, MkInteger(8) ); _ = amountKey;
  nonce := OpCopy(MkUndefined() ); _ = nonce;
  result := MkMap(&VarMap{
        "symbol":symbol ,
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , orderbook.Length )); OpIncr(& i ){
    level := *(orderbook ).At(i ); _ = level;
    if IsTrue(OpEq2(timestamp , MkUndefined() )) {
      timestamp = this.SafeInteger(level , MkInteger(2) );
    } else {
      newTimestamp := this.SafeInteger(level , MkInteger(2) ); _ = newTimestamp;
      timestamp = Math.Max(timestamp , newTimestamp );
    }
    if IsTrue(OpEq2(nonce , MkUndefined() )) {
      nonce = this.SafeInteger(level , MkInteger(0) );
    } else {
      newNonce := this.SafeInteger(level , MkInteger(0) ); _ = newNonce;
      nonce = Math.Max(nonce , newNonce );
    }
    bidask := this.ParseBidAsk(level , priceKey , amountKey ); _ = bidask;
    levelSide := this.SafeInteger(level , MkInteger(9) ); _ = levelSide;
    side := OpTrinary(levelSide , asksKey , bidsKey ); _ = side;
...
func (this *Ndax) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetGetL2Snapshot"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(ticker , MkString("InstrumentId") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  last := this.SafeNumber(ticker , MkString("LastTradedPx") ); _ = last;
  percentage := this.SafeNumber(ticker , MkString("Rolling24HrPxChangePercent") ); _ = percentage;
  change := this.SafeNumber(ticker , MkString("Rolling24HrPxChange") ); _ = change;
  open := this.SafeNumber(ticker , MkString("SessionOpen") ); _ = open;
  ...
  baseVolume := this.SafeNumber(ticker , MkString("Rolling24HrVolume") ); _ = baseVolume;
  quoteVolume := this.SafeNumber(ticker , MkString("Rolling24HrNotional") ); _ = quoteVolume;
...
func (this *Ndax) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetGetLevel1"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetGetTickerHistory"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(GoIsArray(trade )) {
    ...
    takerSide := this.SafeValue(trade , MkInteger(8) ); _ = takerSide;
    ...
    if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
      feeCurrencyId := this.SafeString(trade , MkString("FeeProductId") ); _ = feeCurrencyId;
...
func (this *Ndax) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetGetLastTrades"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) FetchAccounts(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetUserAccounts"), this.Extend(request , params )); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    accountId := this.SafeString(response , i ); _ = accountId;
    result.Push(MkMap(&VarMap{
            "id":accountId ,
            "type":MkUndefined() ,
...
func (this *Ndax) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  this.LoadAccounts();
  defaultAccountId := this.SafeInteger2(*this.At(MkString("options")) , MkString("accountId") , MkString("AccountId") , parseInt(*(*(*this.At(MkString("accounts")) ).At(MkInteger(0) )).At(MkString("id") ))); _ = defaultAccountId;
  accountId := this.SafeInteger2(params , MkString("accountId") , MkString("AccountId") , defaultAccountId ); _ = accountId;
  ...
  response := this.Call(MkString("privateGetGetAccountPositions"), this.Extend(request , params )); _ = response;
  result := MkMap(&VarMap{
        "info":response ,
        "timestamp":MkUndefined() ,
        "datetime":MkUndefined() ,
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    currencyId := this.SafeString(balance , MkString("ProductId") ); _ = currencyId;
...
func (this *Ndax) ParseLedgerEntry(goArgs ...*Variant) *Variant{
  ...
  referenceId := this.SafeString(item , MkString("ReferenceId") ); _ = referenceId;
  referenceAccount := this.SafeString(item , MkString("Counterparty") ); _ = referenceAccount;
  type_ := this.ParseLedgerEntryType(this.SafeString(item , MkString("ReferenceType") )); _ = type_;
  currencyId := this.SafeString(item , MkString("ProductId") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  credit := this.SafeNumber(item , MkString("CR") ); _ = credit;
  debit := this.SafeNumber(item , MkString("DR") ); _ = debit;
...
func (this *Ndax) FetchLedger(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetAccountTransactions"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  id := this.SafeString2(order , MkString("ReplacementOrderId") , MkString("OrderId") ); _ = id;
  timestamp := this.SafeInteger(order , MkString("ReceiveTime") ); _ = timestamp;
  lastTradeTimestamp := this.SafeInteger(order , MkString("LastUpdatedTime") ); _ = lastTradeTimestamp;
  marketId := this.SafeString(order , MkString("Instrument") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  side := this.SafeStringLower(order , MkString("Side") ); _ = side;
  type_ := this.SafeStringLower(order , MkString("OrderType") ); _ = type_;
  clientOrderId := this.SafeString2(order , MkString("ReplacementClOrdId") , MkString("ClientOrderId") ); _ = clientOrderId;
  price := this.SafeNumber(order , MkString("Price") , MkNumber(0.0) ); _ = price;
  price = OpTrinary(OpGt(price , MkNumber(0.0) ), price , MkUndefined() );
  amount := this.SafeNumber(order , MkString("OrigQuantity") ); _ = amount;
  filled := this.SafeNumber(order , MkString("QuantityExecuted") ); _ = filled;
  cost := this.SafeNumber(order , MkString("GrossValueExecuted") ); _ = cost;
  average := this.SafeNumber(order , MkString("AvgPrice") , MkNumber(0.0) ); _ = average;
  average = OpTrinary(OpGt(average , MkInteger(0) ), average , MkUndefined() );
  stopPrice := this.SafeNumber(order , MkString("StopPrice") , MkNumber(0.0) ); _ = stopPrice;
  stopPrice = OpTrinary(OpGt(stopPrice , MkNumber(0.0) ), stopPrice , MkUndefined() );
  timeInForce := OpCopy(MkUndefined() ); _ = timeInForce;
  status := this.ParseOrderStatus(this.SafeString(order , MkString("OrderState") )); _ = status;
  fee := OpCopy(MkUndefined() ); _ = fee;
...
func (this *Ndax) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  defaultAccountId := this.SafeInteger2(*this.At(MkString("options")) , MkString("accountId") , MkString("AccountId") , parseInt(*(*(*this.At(MkString("accounts")) ).At(MkInteger(0) )).At(MkString("id") ))); _ = defaultAccountId;
  accountId := this.SafeInteger2(params , MkString("accountId") , MkString("AccountId") , defaultAccountId ); _ = accountId;
  clientOrderId := this.SafeInteger2(params , MkString("ClientOrderId") , MkString("clientOrderId") ); _ = clientOrderId;
  params = this.Omit(params , MkArray(&VarArray{
          MkString("accountId") ,
          MkString("AccountId") ,
  ...
  market := this.Market(symbol ); _ = market;
  orderSide := OpTrinary(OpEq2(side , MkString("buy") ), MkInteger(0) , MkInteger(1) ); _ = orderSide;
  request := MkMap(&VarMap{
  ...
  response := this.Call(MkString("privatePostSendOrder"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) EditOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostCancelReplaceOrder"), this.Extend(request , params )); _ = response;
  return this.ParseOrder(response , market );
}
...
func (this *Ndax) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetTradesHistory"), this.Extend(request , params )); _ = response;
  return this.ParseTrades(response , market , since , limit );
}
...
func (this *Ndax) CancelAllOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostCancelAllOrders"), this.Extend(request , params )); _ = response;
  return response ;
}
...
func (this *Ndax) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"omsId":omsId }); _ = request;
  clientOrderId := this.SafeInteger2(params , MkString("clientOrderId") , MkString("ClOrderId") ); _ = clientOrderId;
  if IsTrue(OpNotEq2(clientOrderId , MkUndefined() )) {
...
func (this *Ndax) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetOpenOrders"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetOrdersHistory"), this.Extend(request , params )); _ = response;
  return this.ParseOrders(response , market , since , limit );
}
...
func (this *Ndax) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetOrderStatus"), this.Extend(request , params )); _ = response;
  return this.ParseOrder(response , market );
}
...
func (this *Ndax) FetchOrderTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostGetOrderHistoryByOrderId"), this.Extend(request , params )); _ = response;
  grouped := this.GroupBy(response , MkString("ChangeReason") ); _ = grouped;
  trades := this.SafeValue(grouped , MkString("Trade") , MkArray(&VarArray{})); _ = trades;
...
func (this *Ndax) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetDepositInfo"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) ParseDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  depositInfoString := this.SafeString(depositAddress , MkString("DepositInfo") ); _ = depositInfoString;
  depositInfo := JSON.Parse(depositInfoString ); _ = depositInfo;
  depositInfoLength := OpCopy(depositInfo.Length ); _ = depositInfoLength;
  lastString := this.SafeString(depositInfo , OpSub(depositInfoLength , MkInteger(1) )); _ = lastString;
  parts := lastString.Split(MkString("?memo=") ); _ = parts;
  address := this.SafeString(parts , MkInteger(0) ); _ = address;
  tag := this.SafeString(parts , MkInteger(1) ); _ = tag;
...
func (this *Ndax) CreateDepositAddress(goArgs ...*Variant) *Variant{
  code := GoGetArg(goArgs, 0, MkUndefined()); _ = code;
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"GenerateNewKey":MkBool(true) }); _ = request;
...
func (this *Ndax) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetDeposits"), this.Extend(request , params )); _ = response;
...
func (this *Ndax) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetGetWithdraws"), this.Extend(request , params )); _ = response;
  return this.ParseTransactions(response , currency , since , limit );
}
...
func (this *Ndax) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  id := this.SafeString(transaction , MkString("DepositId") ); _ = id;
  txid := OpCopy(MkUndefined() ); _ = txid;
  currencyId := this.SafeString(transaction , MkString("ProductId") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  timestamp := OpCopy(MkUndefined() ); _ = timestamp;
  type_ := OpCopy(MkUndefined() ); _ = type_;
  ...
  templateFormString := this.SafeString(transaction , MkString("TemplateForm") ); _ = templateFormString;
  address := OpCopy(MkUndefined() ); _ = address;
  updated := this.SafeInteger(transaction , MkString("LastUpdateTimeStamp") ); _ = updated;
  if IsTrue(OpNotEq2(templateFormString , MkUndefined() )) {
    templateForm := JSON.Parse(templateFormString ); _ = templateForm;
  ...
  status := this.ParseTransactionStatusByType(this.SafeString(transaction , MkString("TicketStatus") ), type_ ); _ = status;
  amount := this.SafeNumber(transaction , MkString("Amount") ); _ = amount;
  feeCost := this.SafeNumber(transaction , MkString("FeeAmount") ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    fee = MkMap(&VarMap{
        "currency":code ,
...
func (this *Ndax) Withdraw(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 4, MkMap(&VarMap{})); _ = params;
  sessionToken := this.SafeString(*this.At(MkString("options")) , MkString("sessionToken") ); _ = sessionToken;
  ...
  withdrawTemplateTypesResponse := this.Call(MkString("privateGetGetWithdrawTemplateTypes"), withdrawTemplateTypesRequest ); _ = withdrawTemplateTypesResponse;
  templateTypes := this.SafeValue(withdrawTemplateTypesResponse , MkString("TemplateTypes") , MkArray(&VarArray{})); _ = templateTypes;
  firstTemplateType := this.SafeValue(templateTypes , MkInteger(0) ); _ = firstTemplateType;
  ...
  templateName := this.SafeString(firstTemplateType , MkString("TemplateName") ); _ = templateName;
  ...
  withdrawTemplateResponse := this.Call(MkString("privateGetGetWithdrawTemplate"), withdrawTemplateRequest ); _ = withdrawTemplateResponse;
  template := this.SafeString(withdrawTemplateResponse , MkString("Template") ); _ = template;
  ...
  withdrawTemplate := JSON.Parse(template ); _ = withdrawTemplate;
  *(withdrawTemplate ).At(MkString("ExternalAddress") )= OpCopy(address );
  ...
  response := this.Call(MkString("privatePostCreateWithdrawTicket"), this.DeepExtend(withdrawRequest , params )); _ = response;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Ndax) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    if IsTrue(OpEq2(path , MkString("Authenticate") )) {
      auth := OpAdd(*this.At(MkString("login")) , OpAdd(MkString(":") , *this.At(MkString("password")) )); _ = auth;
      auth64 := this.StringToBase64(auth ); _ = auth64;
      headers = MkMap(&VarMap{"Authorization":OpAdd(MkString("Basic ") , this.Decode(auth64 ))});
    } else {
      if IsTrue(OpEq2(path , MkString("Authenticate2FA") )) {
        pending2faToken := this.SafeString(*this.At(MkString("options")) , MkString("pending2faToken") ); _ = pending2faToken;
        if IsTrue(OpNotEq2(pending2faToken , MkUndefined() )) {
...
func (this *Ndax) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  message := this.SafeString(response , MkString("errormsg") ); _ = message;
