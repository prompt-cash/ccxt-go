...
func (this *Binance) FetchTime(goArgs ...*Variant) *Variant{
  ...
  type_ := this.SafeString2(*this.At(MkString("options")) , MkString("fetchTime") , MkString("defaultType") , MkString("spot") ); _ = type_;
  method := MkString("publicGetTime") ; _ = method;
...
func (this *Binance) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  fetchCurrenciesEnabled := this.SafeValue(*this.At(MkString("options")) , MkString("fetchCurrencies") ); _ = fetchCurrenciesEnabled;
  ...
  apiBackup := this.SafeString(*this.At(MkString("urls")) , MkString("apiBackup") ); _ = apiBackup;
  if IsTrue(OpNotEq2(apiBackup , MkUndefined() )) {
    return MkUndefined() ;
  }
  response := this.Call(MkString("sapiGetCapitalConfigGetall"), params ); _ = response;
  result := MkMap(&VarMap{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    entry := *(response ).At(i ); _ = entry;
    id := this.SafeString(entry , MkString("coin") ); _ = id;
    ...
    networkList := this.SafeValue(entry , MkString("networkList") , MkArray(&VarArray{})); _ = networkList;
    fees := MkMap(&VarMap{}); _ = fees;
    fee := OpCopy(MkUndefined() ); _ = fee;
    for j := MkInteger(0) ; IsTrue(OpLw(j , networkList.Length )); OpIncr(& j ){
      networkItem := *(networkList ).At(j ); _ = networkItem;
      network := this.SafeString(networkItem , MkString("network") ); _ = network;
      withdrawFee := this.SafeNumber(networkItem , MkString("withdrawFee") ); _ = withdrawFee;
      depositEnable := this.SafeValue(networkItem , MkString("depositEnable") ); _ = depositEnable;
      withdrawEnable := this.SafeValue(networkItem , MkString("withdrawEnable") ); _ = withdrawEnable;
      ...
      *(fees ).At(network )= OpCopy(withdrawFee );
      isDefault := this.SafeValue(networkItem , MkString("isDefault") ); _ = isDefault;
      if IsTrue(OpOr(isDefault , OpEq2(fee , MkUndefined() ))) {
        fee = OpCopy(withdrawFee );
      }
    }
    trading := this.SafeValue(entry , MkString("trading") ); _ = trading;
    active := OpAnd(isWithdrawEnabled , OpAnd(isDepositEnabled , trading )); _ = active;
...
func (this *Binance) FetchMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchMarkets") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  ...
  method := MkString("publicGetExchangeInfo") ; _ = method;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
  ...
  markets := this.SafeValue(response , MkString("symbols") , MkArray(&VarArray{})); _ = markets;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    ...
    delivery := OpEq2(type_ , MkString("delivery") ); _ = delivery;
    id := this.SafeString(market , MkString("symbol") ); _ = id;
    lowercaseId := this.SafeStringLower(market , MkString("symbol") ); _ = lowercaseId;
    ...
    contractType := this.SafeString(market , MkString("contractType") ); _ = contractType;
    idSymbol := OpAnd(OpOr(future , delivery ), OpNotEq2(contractType , MkString("PERPETUAL") )); _ = idSymbol;
    symbol := OpCopy(MkUndefined() ); _ = symbol;
    expiry := OpCopy(MkUndefined() ); _ = expiry;
    if IsTrue(idSymbol ) {
      symbol = OpCopy(id );
    ...
    status := this.SafeString2(market , MkString("status") , MkString("contractStatus") ); _ = status;
    active := OpEq2(status , MkString("TRADING") ); _ = active;
    margin := this.SafeValue(market , MkString("isMarginTradingAllowed") , MkBool(false) ); _ = margin;
    contractSize := OpCopy(MkUndefined() ); _ = contractSize;
    fees := OpCopy(*this.At(MkString("fees")) ); _ = fees;
    ...
    maker := *(*(fees ).At(MkString("trading") )).At(MkString("maker") ); _ = maker;
    taker := *(*(fees ).At(MkString("trading") )).At(MkString("taker") ); _ = taker;
    entry := MkMap(&VarMap{
          "id":id ,
    ...
    if IsTrue(OpHasMember(MkString("PRICE_FILTER") , filtersByType )) {
      ...
      tickSize := this.SafeString(filter , MkString("tickSize") ); _ = tickSize;
    ...
    if IsTrue(OpHasMember(MkString("LOT_SIZE") , filtersByType )) {
      ...
      stepSize := this.SafeString(filter , MkString("stepSize") ); _ = stepSize;
...
func (this *Binance) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchBalance") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  method := MkString("privateGetAccount") ; _ = method;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
    options := this.SafeValue(*this.At(MkString("options")) , MkString("future") , MkMap(&VarMap{})); _ = options;
    fetchBalanceOptions := this.SafeValue(options , MkString("fetchBalance") , MkMap(&VarMap{})); _ = fetchBalanceOptions;
    method = this.SafeString(fetchBalanceOptions , MkString("method") , MkString("fapiPrivateV2GetAccount") );
  } else {
    if IsTrue(OpEq2(type_ , MkString("delivery") )) {
      options := this.SafeValue(*this.At(MkString("options")) , MkString("delivery") , MkMap(&VarMap{})); _ = options;
      fetchBalanceOptions := this.SafeValue(options , MkString("fetchBalance") , MkMap(&VarMap{})); _ = fetchBalanceOptions;
  ...
  if IsTrue(OpOr(OpEq2(type_ , MkString("spot") ), OpEq2(type_ , MkString("margin") ))) {
    ...
    balances := this.SafeValue2(response , MkString("balances") , MkString("userAssets") , MkArray(&VarArray{})); _ = balances;
    ...
    if IsTrue(OpEq2(type_ , MkString("savings") )) {
      positionAmountVos := this.SafeValue(response , MkString("positionAmountVos") ); _ = positionAmountVos;
      for i := MkInteger(0) ; IsTrue(OpLw(i , positionAmountVos.Length )); OpIncr(& i ){
        entry := *(positionAmountVos ).At(i ); _ = entry;
        currencyId := this.SafeString(entry , MkString("asset") ); _ = currencyId;
        code := this.SafeCurrencyCode(currencyId ); _ = code;
        account := this.Account(); _ = account;
        usedAndTotal := this.SafeString(entry , MkString("amount") ); _ = usedAndTotal;
      ...
      if IsTrue(OpEq2(type_ , MkString("pay") )) {
        for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
          ...
          *(account ).At(MkString("free") )= this.SafeString(entry , MkString("free") );
          frozen := this.SafeString(entry , MkString("freeze") ); _ = frozen;
          withdrawing := this.SafeString(entry , MkString("withdrawing") ); _ = withdrawing;
          locked := this.SafeString(entry , MkString("locked") ); _ = locked;
          *(account ).At(MkString("used") )= Precise.StringAdd(frozen , Precise.StringAdd(locked , withdrawing ));
          *(result ).At(code )= OpCopy(account );
        }
      } else {
        balances := OpCopy(response ); _ = balances;
...
func (this *Binance) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  method := MkString("publicGetDepth") ; _ = method;
  ...
  timestamp := this.SafeInteger(response , MkString("T") ); _ = timestamp;
...
func (this *Binance) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  isCoinm := OpHasMember(MkString("baseVolume") , ticker ); _ = isCoinm;
...
func (this *Binance) FetchStatus(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("sapiGetSystemStatus"), params ); _ = response;
...
func (this *Binance) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  method := MkString("publicGetTicker24hr") ; _ = method;
  if IsTrue(*(market ).At(MkString("linear") )) {
  ...
  if IsTrue(GoIsArray(response )) {
    firstTicker := this.SafeValue(response , MkInteger(0) , MkMap(&VarMap{})); _ = firstTicker;
...
func (this *Binance) FetchBidsAsks(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchBidsAsks") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  query := this.Omit(params , MkString("type") ); _ = query;
...
func (this *Binance) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchTickers") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  query := this.Omit(params , MkString("type") ); _ = query;
  defaultMethod := OpCopy(MkUndefined() ); _ = defaultMethod;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
  ...
  method := this.SafeString(*this.At(MkString("options")) , MkString("fetchTickersMethod") , defaultMethod ); _ = method;
  response := this.Call(method , query ); _ = response;
  return this.ParseTickers(response , symbols );
}
...
func (this *Binance) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  defaultLimit := MkInteger(500) ; _ = defaultLimit;
  maxLimit := MkInteger(1500) ; _ = maxLimit;
  ...
  if IsTrue(OpNotEq2(since , MkUndefined() )) {
    *(request ).At(MkString("startTime") )= OpCopy(since );
    if IsTrue(OpGt(since , MkInteger(0) )) {
      endTime := this.Sum(since , OpSub(OpMulti(limit , OpMulti(duration , MkInteger(1000) )), MkInteger(1) )); _ = endTime;
      now := this.Milliseconds(); _ = now;
      *(request ).At(MkString("endTime") )= Math.Min(now , endTime );
    }
  }
  method := MkString("publicGetKlines") ; _ = method;
  if IsTrue(*(market ).At(MkString("linear") )) {
...
func (this *Binance) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString2(trade , MkString("t") , MkString("a") ); _ = id;
...
func (this *Binance) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchTrades") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  query := this.Omit(params , MkString("type") ); _ = query;
  defaultMethod := OpCopy(MkUndefined() ); _ = defaultMethod;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
  ...
  method := this.SafeString(*this.At(MkString("options")) , MkString("fetchTradesMethod") , defaultMethod ); _ = method;
...
func (this *Binance) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  filledString := this.SafeString(order , MkString("executedQty") , MkString("0") ); _ = filledString;
  filled := this.ParseNumber(filledString ); _ = filled;
  filledFloat := parseFloat(filledString ); _ = filledFloat;
  timestamp := OpCopy(MkUndefined() ); _ = timestamp;
  ...
  averageString := this.SafeString(order , MkString("avgPrice") ); _ = averageString;
  average := this.ParseNumber(this.OmitZero(averageString )); _ = average;
  priceString := this.SafeString(order , MkString("price") ); _ = priceString;
  price := this.ParseNumber(this.OmitZero(priceString )); _ = price;
  amount := this.SafeNumber(order , MkString("origQty") ); _ = amount;
  cost := this.SafeNumber2(order , MkString("cummulativeQuoteQty") , MkString("cumQuote") ); _ = cost;
  ...
  postOnly := OpOr(OpEq2(type_ , MkString("limit_maker") ), OpEq2(timeInForce , MkString("GTX") )); _ = postOnly;
  if IsTrue(OpEq2(type_ , MkString("limit_maker") )) {
    type_ = MkString("limit") ;
  }
  stopPriceString := this.SafeString(order , MkString("stopPrice") ); _ = stopPriceString;
  stopPrice := this.ParseNumber(this.OmitZero(stopPriceString )); _ = stopPrice;
...
func (this *Binance) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("createOrder") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  orderType := this.SafeString(params , MkString("type") , defaultType ); _ = orderType;
  clientOrderId := this.SafeString2(params , MkString("newClientOrderId") , MkString("clientOrderId") ); _ = clientOrderId;
  params = this.Omit(params , MkArray(&VarArray{
          MkString("type") ,
          MkString("newClientOrderId") ,
          MkString("clientOrderId") ,
          }));
  method := MkString("privatePostOrder") ; _ = method;
  ...
  if IsTrue(*(market ).At(MkString("spot") )) {
    test := this.SafeValue(params , MkString("test") , MkBool(false) ); _ = test;
    if IsTrue(test ) {
  ...
  validOrderTypes := this.SafeValue(*(market ).At(MkString("info") ), MkString("orderTypes") ); _ = validOrderTypes;
  ...
  if IsTrue(OpEq2(clientOrderId , MkUndefined() )) {
    broker := this.SafeValue(*this.At(MkString("options")) , MkString("broker") ); _ = broker;
    if IsTrue(OpNotEq2(broker , MkUndefined() )) {
      brokerId := this.SafeString(broker , orderType ); _ = brokerId;
  ...
  timeInForceIsRequired := OpCopy(MkBool(false) ); _ = timeInForceIsRequired;
  priceIsRequired := OpCopy(MkBool(false) ); _ = priceIsRequired;
  stopPriceIsRequired := OpCopy(MkBool(false) ); _ = stopPriceIsRequired;
  quantityIsRequired := OpCopy(MkBool(false) ); _ = quantityIsRequired;
  if IsTrue(OpEq2(uppercaseType , MkString("MARKET") )) {
    quoteOrderQty := this.SafeValue(*this.At(MkString("options")) , MkString("quoteOrderQty") , MkBool(false) ); _ = quoteOrderQty;
    if IsTrue(quoteOrderQty ) {
      quoteOrderQty := this.SafeNumber(params , MkString("quoteOrderQty") ); _ = quoteOrderQty;
          ...
          if IsTrue(OpEq2(uppercaseType , MkString("LIMIT_MAKER") )) {
            ...
            if IsTrue(OpEq2(uppercaseType , MkString("STOP") )) {
              ...
              if IsTrue(OpOr(OpEq2(uppercaseType , MkString("STOP_MARKET") ), OpEq2(uppercaseType , MkString("TAKE_PROFIT_MARKET") ))) {
                closePosition := this.SafeValue(params , MkString("closePosition") ); _ = closePosition;
                ...
                if IsTrue(OpEq2(uppercaseType , MkString("TRAILING_STOP_MARKET") )) {
                  quantityIsRequired = OpCopy(MkBool(true) );
                  callbackRate := this.SafeNumber(params , MkString("callbackRate") ); _ = callbackRate;
...
func (this *Binance) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchOrder") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  method := MkString("privateGetOrder") ; _ = method;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
  ...
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  clientOrderId := this.SafeValue2(params , MkString("origClientOrderId") , MkString("clientOrderId") ); _ = clientOrderId;
...
func (this *Binance) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchOrders") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  method := MkString("privateGetAllOrders") ; _ = method;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
...
func (this *Binance) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    ...
    *(request ).At(MkString("symbol") )= *(market ).At(MkString("id") );
    defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchOpenOrders") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  ...
  method := MkString("privateGetOpenOrders") ; _ = method;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
...
func (this *Binance) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchOpenOrders") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  origClientOrderId := this.SafeValue2(params , MkString("origClientOrderId") , MkString("clientOrderId") ); _ = origClientOrderId;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  ...
  method := MkString("privateDeleteOrder") ; _ = method;
  if IsTrue(OpEq2(type_ , MkString("future") )) {
...
func (this *Binance) CancelAllOrders(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("cancelAllOrders") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  query := this.Omit(params , MkString("type") ); _ = query;
  method := MkString("privateDeleteOpenOrders") ; _ = method;
  if IsTrue(OpEq2(type_ , MkString("margin") )) {
...
func (this *Binance) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchMyTrades") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
...
func (this *Binance) FetchMyDustTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("sapiGetAssetDribblet"), this.Extend(request , params )); _ = response;
  results := this.SafeValue(response , MkString("userAssetDribblets") , MkArray(&VarArray{})); _ = results;
  rows := this.SafeInteger(response , MkString("total") , MkInteger(0) ); _ = rows;
  data := MkArray(&VarArray{}); _ = data;
  for i := MkInteger(0) ; IsTrue(OpLw(i , rows )); OpIncr(& i ){
    logs := this.SafeValue(*(results ).At(i ), MkString("userAssetDribbletDetails") , MkArray(&VarArray{})); _ = logs;
    for j := MkInteger(0) ; IsTrue(OpLw(j , logs.Length )); OpIncr(& j ){
      *(*(logs ).At(j )).At(MkString("isDustTrade") )= OpCopy(MkBool(true) );
      data.Push(*(logs ).At(j ));
    }
  }
  trades := this.ParseTrades(data , MkUndefined() , since , limit ); _ = trades;
...
func (this *Binance) ParseDustTrade(goArgs ...*Variant) *Variant{
  trade := GoGetArg(goArgs, 0, MkUndefined()); _ = trade;
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  orderId := this.SafeString(trade , MkString("transId") ); _ = orderId;
  timestamp := this.Parse8601(this.SafeString(trade , MkString("operateTime") )); _ = timestamp;
  currencyId := this.SafeString(trade , MkString("fromAsset") ); _ = currencyId;
  tradedCurrency := this.SafeCurrencyCode(currencyId ); _ = tradedCurrency;
  bnb := this.Currency(MkString("BNB") ); _ = bnb;
  earnedCurrency := *(bnb ).At(MkString("code") ); _ = earnedCurrency;
  applicantSymbol := OpAdd(earnedCurrency , OpAdd(MkString("/") , tradedCurrency )); _ = applicantSymbol;
  tradedCurrencyIsQuote := OpCopy(MkBool(false) ); _ = tradedCurrencyIsQuote;
  if IsTrue(OpHasMember(applicantSymbol , *this.At(MkString("markets")) )) {
    tradedCurrencyIsQuote = OpCopy(MkBool(true) );
  }
  feeCostString := this.SafeString(trade , MkString("serviceChargeAmount") ); _ = feeCostString;
  fee := MkMap(&VarMap{
...
func (this *Binance) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{}); _ = request;
  legalMoney := this.SafeValue(*this.At(MkString("options")) , MkString("legalMoney") , MkMap(&VarMap{})); _ = legalMoney;
  if IsTrue(OpHasMember(code , legalMoney )) {
    ...
    *(request ).At(MkString("transactionType") )= MkInteger(0) ;
    ...
    raw := this.Call(MkString("sapiGetFiatOrders"), this.Extend(request , params )); _ = raw;
...
func (this *Binance) ParseTransactionStatusByType(goArgs ...*Variant) *Variant{
  ...
  statuses := this.SafeValue(statusesByType , type_ , MkMap(&VarMap{})); _ = statuses;
  return this.SafeString(statuses , status , status );
}
...
func (this *Binance) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString2(transaction , MkString("id") , MkString("orderNo") ); _ = id;
  address := this.SafeString(transaction , MkString("address") ); _ = address;
  tag := this.SafeString(transaction , MkString("addressTag") ); _ = tag;
  ...
  currencyId := this.SafeString2(transaction , MkString("coin") , MkString("fiatCurrency") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  timestamp := OpCopy(MkUndefined() ); _ = timestamp;
  insertTime := this.SafeInteger2(transaction , MkString("insertTime") , MkString("createTime") ); _ = insertTime;
  applyTime := this.Parse8601(this.SafeString(transaction , MkString("applyTime") )); _ = applyTime;
  ...
  status := this.ParseTransactionStatusByType(this.SafeString(transaction , MkString("status") ), type_ ); _ = status;
  amount := this.SafeNumber(transaction , MkString("amount") ); _ = amount;
  feeCost := this.SafeNumber2(transaction , MkString("transactionFee") , MkString("totalFee") ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  ...
  updated := this.SafeInteger2(transaction , MkString("successTime") , MkString("updateTime") ); _ = updated;
  internal := this.SafeInteger(transaction , MkString("transferType") , MkBool(false) ); _ = internal;
...
func (this *Binance) ParseTransferStatus(goArgs ...*Variant) *Variant{
  status := GoGetArg(goArgs, 0, MkUndefined()); _ = status;
  statuses := MkMap(&VarMap{"CONFIRMED":MkString("ok") }); _ = statuses;
  return this.SafeString(statuses , status , status );
}

func (this *Binance) ParseTransfer(goArgs ...*Variant) *Variant{
  transfer := GoGetArg(goArgs, 0, MkUndefined()); _ = transfer;
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  id := this.SafeString(transfer , MkString("tranId") ); _ = id;
  currencyId := this.SafeString(transfer , MkString("asset") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  amount := this.SafeNumber(transfer , MkString("amount") ); _ = amount;
  type_ := this.SafeString(transfer , MkString("type") ); _ = type_;
  fromAccount := OpCopy(MkUndefined() ); _ = fromAccount;
  toAccount := OpCopy(MkUndefined() ); _ = toAccount;
  typesByAccount := this.SafeValue(*this.At(MkString("options")) , MkString("typesByAccount") , MkMap(&VarMap{})); _ = typesByAccount;
  if IsTrue(OpNotEq2(type_ , MkUndefined() )) {
    parts := type_.Split(MkString("_") ); _ = parts;
  ...
  timestamp := this.SafeInteger(transfer , MkString("timestamp") ); _ = timestamp;
  status := this.ParseTransferStatus(this.SafeString(transfer , MkString("status") )); _ = status;
  return MkMap(&VarMap{
        "info":transfer ,
        "id":id ,
        "timestamp":timestamp ,
...
func (this *Binance) ParseIncome(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  marketId := this.SafeString(income , MkString("symbol") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  amount := this.SafeNumber(income , MkString("income") ); _ = amount;
  currencyId := this.SafeString(income , MkString("asset") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId ); _ = code;
  id := this.SafeString(income , MkString("tranId") ); _ = id;
...
func (this *Binance) Transfer(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(type_ , MkUndefined() )) {
    ...
    fromId := this.SafeString(accountsByType , fromAccount , fromAccount ); _ = fromId;
    toId := this.SafeString(accountsByType , toAccount , toAccount ); _ = toId;
  ...
  response := this.Call(MkString("sapiPostAssetTransfer"), this.Extend(request , params )); _ = response;
  transfer := this.ParseTransfer(response , currency ); _ = transfer;
...
func (this *Binance) FetchTransfers(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  currency := this.Currency(code ); _ = currency;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchTransfers") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  fromAccount := this.SafeString(params , MkString("fromAccount") , defaultType ); _ = fromAccount;
  defaultTo := OpTrinary(OpEq2(fromAccount , MkString("future") ), MkString("spot") , MkString("future") ); _ = defaultTo;
  toAccount := this.SafeString(params , MkString("toAccount") , defaultTo ); _ = toAccount;
  type_ := this.SafeString(params , MkString("type") ); _ = type_;
  accountsByType := this.SafeValue(*this.At(MkString("options")) , MkString("accountsByType") , MkMap(&VarMap{})); _ = accountsByType;
  ...
  request := MkMap(&VarMap{"type":type_ }); _ = request;
  if IsTrue(OpNotEq2(since , MkUndefined() )) {
    *(request ).At(MkString("startTime") )= OpCopy(since );
  }
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
  ...
  response := this.Call(MkString("sapiGetAssetTransfer"), this.Extend(request , params )); _ = response;
  rows := this.SafeValue(response , MkString("rows") , MkArray(&VarArray{})); _ = rows;
...
func (this *Binance) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("sapiGetCapitalDepositAddress"), this.Extend(request , params )); _ = response;
  address := this.SafeString(response , MkString("address") ); _ = address;
  tag := this.SafeString(response , MkString("tag") ); _ = tag;
...
func (this *Binance) FetchFundingFees(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    entry := *(response ).At(i ); _ = entry;
    currencyId := this.SafeString(entry , MkString("coin") ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    networkList := this.SafeValue(entry , MkString("networkList") ); _ = networkList;
    *(withdrawFees ).At(code )= MkMap(&VarMap{});
    for j := MkInteger(0) ; IsTrue(OpLw(j , networkList.Length )); OpIncr(& j ){
      networkEntry := *(networkList ).At(j ); _ = networkEntry;
      networkId := this.SafeString(networkEntry , MkString("network") ); _ = networkId;
      networkCode := this.SafeCurrencyCode(networkId ); _ = networkCode;
      fee := this.SafeNumber(networkEntry , MkString("withdrawFee") ); _ = fee;
...
func (this *Binance) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("sapiPostCapitalWithdrawApply"), this.Extend(request , params )); _ = response;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Binance) ParseTradingFee(goArgs ...*Variant) *Variant{
  fee := GoGetArg(goArgs, 0, MkUndefined()); _ = fee;
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  marketId := this.SafeString(fee , MkString("symbol") ); _ = marketId;
...
func (this *Binance) FetchTradingFee(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"symbol":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("sapiGetAssetTradeFee"), this.Extend(request , params )); _ = response;
  first := this.SafeValue(response , MkInteger(0) , MkMap(&VarMap{})); _ = first;
...
func (this *Binance) FetchTradingFees(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  method := OpCopy(MkUndefined() ); _ = method;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchFundingRates") , MkString("defaultType") , MkString("future") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  query := this.Omit(params , MkString("type") ); _ = query;
  if IsTrue(OpOr(OpEq2(type_ , MkString("spot") ), OpEq2(type_ , MkString("margin") ))) {
  ...
  if IsTrue(OpOr(OpEq2(type_ , MkString("spot") ), OpEq2(type_ , MkString("margin") ))) {
    result := MkMap(&VarMap{}); _ = result;
    for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
      fee := this.ParseTradingFee(*(response ).At(i )); _ = fee;
      symbol := *(fee ).At(MkString("symbol") ); _ = symbol;
      *(result ).At(symbol )= OpCopy(fee );
    }
    return result ;
  } else {
    if IsTrue(OpEq2(type_ , MkString("future") )) {
      symbols := GoGetKeys(*this.At(MkString("markets")) ); _ = symbols;
      result := MkMap(&VarMap{}); _ = result;
      feeTier := this.SafeInteger(response , MkString("feeTier") ); _ = feeTier;
      feeTiers := *(*(*(*this.At(MkString("fees")) ).At(type_ )).At(MkString("trading") )).At(MkString("tiers") ); _ = feeTiers;
      maker := *(*(*(feeTiers ).At(MkString("maker") )).At(feeTier )).At(MkInteger(1) ); _ = maker;
      taker := *(*(*(feeTiers ).At(MkString("taker") )).At(feeTier )).At(MkInteger(1) ); _ = taker;
      for i := MkInteger(0) ; IsTrue(OpLw(i , symbols.Length )); OpIncr(& i ){
        symbol := *(symbols ).At(i ); _ = symbol;
...
func (this *Binance) FuturesTransfer(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("sapiPostFuturesTransfer"), this.Extend(request , params )); _ = response;
...
func (this *Binance) FetchFundingRates(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    entry := *(response ).At(i ); _ = entry;
    parsed := this.ParseFundingRate(entry ); _ = parsed;
    result.Push(parsed );
  }
...
func (this *Binance) ParseFundingRate(goArgs ...*Variant) *Variant{
  premiumIndex := GoGetArg(goArgs, 0, MkUndefined()); _ = premiumIndex;
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeInteger(premiumIndex , MkString("time") ); _ = timestamp;
  marketId := this.SafeString(premiumIndex , MkString("symbol") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  markPrice := this.SafeNumber(premiumIndex , MkString("markPrice") ); _ = markPrice;
  indexPrice := this.SafeNumber(premiumIndex , MkString("indexPrice") ); _ = indexPrice;
  interestRate := this.SafeNumber(premiumIndex , MkString("interestRate") ); _ = interestRate;
  fundingRate := this.SafeNumber(premiumIndex , MkString("lastFundingRate") ); _ = fundingRate;
  nextFundingTime := this.SafeInteger(premiumIndex , MkString("nextFundingTime") ); _ = nextFundingTime;
  return MkMap(&VarMap{
        "info":premiumIndex ,
        "symbol":symbol ,
        "markPrice":markPrice ,
...
func (this *Binance) ParseAccountPositions(goArgs ...*Variant) *Variant{
  account := GoGetArg(goArgs, 0, MkUndefined()); _ = account;
  positions := this.SafeValue(account , MkString("positions") ); _ = positions;
  assets := this.SafeValue(account , MkString("assets") ); _ = assets;
  balances := MkMap(&VarMap{}); _ = balances;
  for i := MkInteger(0) ; IsTrue(OpLw(i , assets.Length )); OpIncr(& i ){
    entry := *(assets ).At(i ); _ = entry;
    currencyId := this.SafeString(entry , MkString("asset") ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    crossWalletBalance := this.SafeString(entry , MkString("crossWalletBalance") ); _ = crossWalletBalance;
    crossUnPnl := this.SafeString(entry , MkString("crossUnPnl") ); _ = crossUnPnl;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , positions.Length )); OpIncr(& i ){
    ...
    marketId := this.SafeString(position , MkString("symbol") ); _ = marketId;
    market := this.SafeMarket(marketId ); _ = market;
    code := OpTrinary(OpEq2(*(*this.At(MkString("options")) ).At(MkString("defaultType") ), MkString("future") ), *(market ).At(MkString("quote") ), *(market ).At(MkString("base") )); _ = code;
...
func (this *Binance) ParsePosition(goArgs ...*Variant) *Variant{
  position := GoGetArg(goArgs, 0, MkUndefined()); _ = position;
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  marketId := this.SafeString(position , MkString("symbol") ); _ = marketId;
  market = this.SafeMarket(marketId , market );
  symbol := *(market ).At(MkString("symbol") ); _ = symbol;
  leverageString := this.SafeString(position , MkString("leverage") ); _ = leverageString;
  leverage := parseInt(leverageString ); _ = leverage;
  initialMarginString := this.SafeString(position , MkString("initialMargin") ); _ = initialMarginString;
  initialMargin := this.ParseNumber(initialMarginString ); _ = initialMargin;
  initialMarginPercentageString := Precise.StringDiv(MkString("1") , leverageString , MkInteger(8) ); _ = initialMarginPercentageString;
  rational := OpEq2(OpMod(MkInteger(1000) , leverage ), MkInteger(0) ); _ = rational;
  ...
  usdm := OpHasMember(MkString("notional") , position ); _ = usdm;
  maintenanceMarginString := this.SafeString(position , MkString("maintMargin") ); _ = maintenanceMarginString;
  maintenanceMargin := this.ParseNumber(maintenanceMarginString ); _ = maintenanceMargin;
  entryPriceString := this.SafeString(position , MkString("entryPrice") ); _ = entryPriceString;
  entryPrice := this.ParseNumber(entryPriceString ); _ = entryPrice;
  notionalString := this.SafeString2(position , MkString("notional") , MkString("notionalValue") ); _ = notionalString;
  notionalStringAbs := Precise.StringAbs(notionalString ); _ = notionalStringAbs;
  notionalFloat := parseFloat(notionalString ); _ = notionalFloat;
  notionalFloatAbs := parseFloat(notionalStringAbs ); _ = notionalFloatAbs;
  notional := this.ParseNumber(Precise.StringAbs(notionalString )); _ = notional;
  contractsString := this.SafeString(position , MkString("positionAmt") ); _ = contractsString;
  contractsStringAbs := Precise.StringAbs(contractsString ); _ = contractsStringAbs;
  if IsTrue(OpEq2(contractsString , MkUndefined() )) {
    entryNotional := Precise.StringMul(Precise.StringMul(leverageString , initialMarginString ), entryPriceString ); _ = entryNotional;
  ...
  contracts := this.ParseNumber(contractsStringAbs ); _ = contracts;
  leverageBrackets := this.SafeValue(*this.At(MkString("options")) , MkString("leverageBrackets") , MkMap(&VarMap{})); _ = leverageBrackets;
  leverageBracket := this.SafeValue(leverageBrackets , symbol , MkArray(&VarArray{})); _ = leverageBracket;
  maintenanceMarginPercentageString := OpCopy(MkUndefined() ); _ = maintenanceMarginPercentageString;
  for i := MkInteger(0) ; IsTrue(OpLw(i , leverageBracket.Length )); OpIncr(& i ){
    bracket := *(leverageBracket ).At(i ); _ = bracket;
    if IsTrue(OpLw(notionalFloatAbs , *(bracket ).At(MkInteger(0) ))) {
      break ;
    }
    maintenanceMarginPercentageString = *(bracket ).At(MkInteger(1) );
  }
  maintenanceMarginPercentage := this.ParseNumber(maintenanceMarginPercentageString ); _ = maintenanceMarginPercentage;
  unrealizedPnlString := this.SafeString(position , MkString("unrealizedProfit") ); _ = unrealizedPnlString;
  unrealizedPnl := this.ParseNumber(unrealizedPnlString ); _ = unrealizedPnl;
  timestamp := this.SafeInteger(position , MkString("updateTime") ); _ = timestamp;
  if IsTrue(OpEq2(timestamp , MkInteger(0) )) {
    timestamp = OpCopy(MkUndefined() );
  }
  isolated := this.SafeValue(position , MkString("isolated") ); _ = isolated;
  marginType := OpCopy(MkUndefined() ); _ = marginType;
  collateralString := OpCopy(MkUndefined() ); _ = collateralString;
  walletBalance := OpCopy(MkUndefined() ); _ = walletBalance;
  if IsTrue(isolated ) {
  ...
  collateral := this.ParseNumber(collateralString ); _ = collateral;
  marginRatio := OpCopy(MkUndefined() ); _ = marginRatio;
  side := OpCopy(MkUndefined() ); _ = side;
  percentage := OpCopy(MkUndefined() ); _ = percentage;
  liquidationPriceStringRaw := OpCopy(MkUndefined() ); _ = liquidationPriceStringRaw;
  liquidationPrice := OpCopy(MkUndefined() ); _ = liquidationPrice;
  if IsTrue(OpEq2(notionalFloat , MkNumber(0.0) )) {
    ...
    if IsTrue(usdm ) {
      onePlusMaintenanceMarginPercentageString := OpCopy(MkUndefined() ); _ = onePlusMaintenanceMarginPercentageString;
      entryPriceSignString := OpCopy(entryPriceString ); _ = entryPriceSignString;
      ...
      leftSide := Precise.StringDiv(walletBalance , Precise.StringMul(contractsStringAbs , onePlusMaintenanceMarginPercentageString )); _ = leftSide;
      rightSide := Precise.StringDiv(entryPriceSignString , onePlusMaintenanceMarginPercentageString ); _ = rightSide;
      ...
      size := Precise.StringMul(contractsStringAbs , *(market ).At(MkString("contractSize") )); _ = size;
      leftSide := Precise.StringMul(size , onePlusMaintenanceMarginPercentageString ); _ = leftSide;
      rightSide := Precise.StringSub(Precise.StringMul(Precise.StringDiv(MkString("1") , entryPriceSignString ), size ), walletBalance ); _ = rightSide;
      liquidationPriceStringRaw = Precise.StringDiv(leftSide , rightSide );
    }
    pricePrecision := *(*(market ).At(MkString("precision") )).At(MkString("price") ); _ = pricePrecision;
    pricePrecisionPlusOne := OpAdd(pricePrecision , MkInteger(1) ); _ = pricePrecisionPlusOne;
    pricePrecisionPlusOneString := pricePrecisionPlusOne.ToString(); _ = pricePrecisionPlusOneString;
    rounder := NewPrecise(OpAdd(MkString("5e-") , pricePrecisionPlusOneString )); _ = rounder;
    rounderString := rounder.ToString(); _ = rounderString;
    liquidationPriceRoundedString := Precise.StringAdd(rounderString , liquidationPriceStringRaw ); _ = liquidationPriceRoundedString;
    truncatedLiquidationPrice := Precise.StringDiv(liquidationPriceRoundedString , MkString("1") , pricePrecision ); _ = truncatedLiquidationPrice;
...
func (this *Binance) ParsePositionRisk(goArgs ...*Variant) *Variant{
  ...
  notional := this.ParseNumber(notionalStringAbs ); _ = notional;
  contractsAbs := Precise.StringAbs(this.SafeString(position , MkString("positionAmt") )); _ = contractsAbs;
  contracts := this.ParseNumber(contractsAbs ); _ = contracts;
  unrealizedPnlString := this.SafeString(position , MkString("unRealizedProfit") ); _ = unrealizedPnlString;
  unrealizedPnl := this.ParseNumber(unrealizedPnlString ); _ = unrealizedPnl;
  leverageString := this.SafeString(position , MkString("leverage") ); _ = leverageString;
  leverage := parseInt(leverageString ); _ = leverage;
  liquidationPrice := this.SafeNumber(position , MkString("liquidationPrice") ); _ = liquidationPrice;
  collateralString := this.SafeString(position , MkString("isolatedMargin") ); _ = collateralString;
  collateralFloat := parseFloat(collateralString ); _ = collateralFloat;
  collateral := this.ParseNumber(collateralString ); _ = collateral;
  markPriceString := this.SafeString(position , MkString("markPrice") ); _ = markPriceString;
  markPriceFloat := parseFloat(markPriceString ); _ = markPriceFloat;
  markPrice := OpCopy(MkUndefined() ); _ = markPrice;
  if IsTrue(OpNotEq2(markPriceFloat , MkNumber(0.0) )) {
    markPrice = this.ParseNumber(markPriceString );
  }
  entryPrice := this.SafeNumber(position , MkString("entryPrice") ); _ = entryPrice;
  timestamp := this.SafeInteger(position , MkString("updateTime") ); _ = timestamp;
  maintenanceMarginPercentage := this.ParseNumber(maintenanceMarginPercentageString ); _ = maintenanceMarginPercentage;
  maintenanceMarginString := Precise.StringMul(maintenanceMarginPercentageString , notionalStringAbs ); _ = maintenanceMarginString;
  maintenanceMargin := this.ParseNumber(maintenanceMarginString ); _ = maintenanceMargin;
  initialMarginPercentageString := Precise.StringDiv(MkString("1") , leverageString , MkInteger(8) ); _ = initialMarginPercentageString;
  rational := OpEq2(OpMod(MkInteger(1000) , leverage ), MkInteger(0) ); _ = rational;
  if IsTrue(OpNot(rational )) {
  ...
  initialMarginString := Precise.StringDiv(Precise.StringMul(notionalStringAbs , initialMarginPercentageString ), MkString("1") , MkInteger(8) ); _ = initialMarginString;
  initialMargin := this.ParseNumber(initialMarginString ); _ = initialMargin;
  marginRatio := OpCopy(MkUndefined() ); _ = marginRatio;
  side := OpCopy(MkUndefined() ); _ = side;
  percentage := OpCopy(MkUndefined() ); _ = percentage;
  ...
  marginType := this.SafeString(position , MkString("marginType") ); _ = marginType;
...
func (this *Binance) LoadLeverageBrackets(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  leverageBrackets := this.SafeValue(*this.At(MkString("options")) , MkString("leverageBrackets") ); _ = leverageBrackets;
  if IsTrue(OpOr(OpEq2(leverageBrackets , MkUndefined() ), reload )) {
    method := OpCopy(MkUndefined() ); _ = method;
    defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchPositions") , MkString("defaultType") , MkString("future") ); _ = defaultType;
    type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
    query := this.Omit(params , MkString("type") ); _ = query;
    if IsTrue(OpEq2(type_ , MkString("future") )) {
    ...
    *(*this.At(MkString("options")) ).At(MkString("leverageBrackets") )= MkMap(&VarMap{});
    for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
      entry := *(response ).At(i ); _ = entry;
      marketId := this.SafeString(entry , MkString("symbol") ); _ = marketId;
      symbol := this.SafeSymbol(marketId ); _ = symbol;
      brackets := this.SafeValue(entry , MkString("brackets") ); _ = brackets;
      result := MkArray(&VarArray{}); _ = result;
      for j := MkInteger(0) ; IsTrue(OpLw(j , brackets.Length )); OpIncr(& j ){
        bracket := *(brackets ).At(j ); _ = bracket;
        floorValue := this.SafeFloat2(bracket , MkString("notionalFloor") , MkString("qtyFloor") ); _ = floorValue;
        maintenanceMarginPercentage := this.SafeString(bracket , MkString("maintMarginRatio") ); _ = maintenanceMarginPercentage;
...
func (this *Binance) FetchPositions(goArgs ...*Variant) *Variant{
  ...
  account := this.Call(method , query ); _ = account;
  result := this.ParseAccountPositions(account ); _ = result;
...
func (this *Binance) FetchIsolatedPositions(goArgs ...*Variant) *Variant{
  ...
  market := OpCopy(MkUndefined() ); _ = market;
  method := OpCopy(MkUndefined() ); _ = method;
  defaultType := MkString("future") ; _ = defaultType;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(symbol );
    *(request ).At(MkString("symbol") )= *(market ).At(MkString("id") );
    if IsTrue(*(market ).At(MkString("linear") )) {
  ...
  if IsTrue(OpEq2(symbol , MkUndefined() )) {
    result := MkArray(&VarArray{}); _ = result;
    for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
      parsed := this.ParsePositionRisk(*(response ).At(i ), market ); _ = parsed;
...
func (this *Binance) FetchFundingHistory(goArgs ...*Variant) *Variant{
  ...
  method := OpCopy(MkUndefined() ); _ = method;
  defaultType := MkString("future") ; _ = defaultType;
  request := MkMap(&VarMap{"incomeType":MkString("FUNDING_FEE") }); _ = request;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(symbol );
    *(request ).At(MkString("symbol") )= *(market ).At(MkString("id") );
    if IsTrue(*(market ).At(MkString("linear") )) {
...
func (this *Binance) SetMarginMode(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  marginType := GoGetArg(goArgs, 1, MkUndefined()); _ = marginType;
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
...
func (this *Binance) Sign(goArgs ...*Variant) *Variant{
  ...
  userDataStream := OpOr(OpEq2(path , MkString("userDataStream") ), OpEq2(path , MkString("listenKey") )); _ = userDataStream;
  if IsTrue(userDataStream ) {
    ...
    if IsTrue(OpOr(OpEq2(api , MkString("private") ), OpOr(OpEq2(api , MkString("sapi") ), OpOr(OpAnd(OpEq2(api , MkString("wapi") ), OpNotEq2(path , MkString("systemStatus") )), OpOr(OpEq2(api , MkString("dapiPrivate") ), OpOr(OpEq2(api , MkString("dapiPrivateV2") ), OpOr(OpEq2(api , MkString("fapiPrivate") ), OpEq2(api , MkString("fapiPrivateV2") )))))))) {
      ...
      query := OpCopy(MkUndefined() ); _ = query;
      recvWindow := this.SafeInteger(*this.At(MkString("options")) , MkString("recvWindow") , MkInteger(5000) ); _ = recvWindow;
...
func (this *Binance) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNot(success )) {
    ...
    parsedMessage := OpCopy(MkUndefined() ); _ = parsedMessage;
  ...
  error := this.SafeString(response , MkString("code") ); _ = error;
...
func (this *Binance) CalculateRateLimiterCost(goArgs ...*Variant) *Variant{
  api := GoGetArg(goArgs, 0, MkUndefined()); _ = api;
  method := GoGetArg(goArgs, 1, MkUndefined()); _ = method;
  path := GoGetArg(goArgs, 2, MkUndefined()); _ = path;
  params := GoGetArg(goArgs, 3, MkUndefined()); _ = params;
  config := GoGetArg(goArgs, 4, MkMap(&VarMap{})); _ = config;
  context := GoGetArg(goArgs, 5, MkMap(&VarMap{})); _ = context;
  if IsTrue(OpAnd(OpHasMember(MkString("noSymbol") , config ), OpNot(OpHasMember(MkString("symbol") , params )))) {
    ...
    if IsTrue(OpAnd(OpHasMember(MkString("noPoolId") , config ), OpNot(OpHasMember(MkString("poolId") , params )))) {
      ...
      if IsTrue(OpAnd(OpHasMember(MkString("byLimit") , config ), OpHasMember(MkString("limit") , params ))) {
        limit := *(params ).At(MkString("limit") ); _ = limit;
        byLimit := *(config ).At(MkString("byLimit") ); _ = byLimit;
        for i := MkInteger(0) ; IsTrue(OpLw(i , byLimit.Length )); OpIncr(& i ){
          entry := *(byLimit ).At(i ); _ = entry;
...
func (this *Binance) Request(goArgs ...*Variant) *Variant{
  ...
  headers := GoGetArg(goArgs, 4, MkUndefined() ); _ = headers;
  body := GoGetArg(goArgs, 5, MkUndefined() ); _ = body;
  config := GoGetArg(goArgs, 6, MkMap(&VarMap{})); _ = config;
  context := GoGetArg(goArgs, 7, MkMap(&VarMap{})); _ = context;
  response := this.Fetch2(path , api , method , params , headers , body , config , context ); _ = response;
...
func (this *Binance) ModifyMarginHelper(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  amount := GoGetArg(goArgs, 1, MkUndefined()); _ = amount;
  addOrReduce := GoGetArg(goArgs, 2, MkUndefined()); _ = addOrReduce;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  defaultType := this.SafeString(*this.At(MkString("options")) , MkString("defaultType") , MkString("future") ); _ = defaultType;
  ...
  response := this.Call(method , this.Extend(request , params )); _ = response;
  rawType := this.SafeInteger(response , MkString("type") ); _ = rawType;
  resultType := OpTrinary(OpEq2(rawType , MkInteger(1) ), MkString("add") , MkString("reduce") ); _ = resultType;
  resultAmount := this.SafeNumber(response , MkString("amount") ); _ = resultAmount;
  errorCode := this.SafeString(response , MkString("code") ); _ = errorCode;
  status := OpTrinary(OpEq2(errorCode , MkString("200") ), MkString("ok") , MkString("failed") ); _ = status;
  return MkMap(&VarMap{
        "info":response ,
        "type":resultType ,
        "amount":resultAmount ,
