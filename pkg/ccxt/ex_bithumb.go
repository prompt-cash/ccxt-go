...
func (this *Bithumb) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  quoteCurrencies := this.SafeValue(*this.At(MkString("options")) , MkString("quoteCurrencies") , MkMap(&VarMap{})); _ = quoteCurrencies;
  quotes := GoGetKeys(quoteCurrencies ); _ = quotes;
  for i := MkInteger(0) ; IsTrue(OpLw(i , quotes.Length )); OpIncr(& i ){
    quote := *(quotes ).At(i ); _ = quote;
    extension := this.SafeValue(quoteCurrencies , quote , MkMap(&VarMap{})); _ = extension;
    method := OpAdd(MkString("publicGetTickerALL") , quote ); _ = method;
    response := this.Call(method , params ); _ = response;
    data := this.SafeValue(response , MkString("data") ); _ = data;
    currencyIds := GoGetKeys(data ); _ = currencyIds;
    for j := MkInteger(0) ; IsTrue(OpLw(j , currencyIds.Length )); OpIncr(& j ){
      currencyId := *(currencyIds ).At(j ); _ = currencyId;
      if IsTrue(OpEq2(currencyId , MkString("date") )) {
        continue ;
      }
      market := *(data ).At(currencyId ); _ = market;
      base := this.SafeCurrencyCode(currencyId ); _ = base;
      symbol := OpAdd(currencyId , OpAdd(MkString("/") , quote )); _ = symbol;
      active := OpCopy(MkBool(true) ); _ = active;
      if IsTrue(GoIsArray(market )) {
        numElements := OpCopy(market.Length ); _ = numElements;
...
func (this *Bithumb) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostInfoBalance"), this.Extend(request , params )); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , codes.Length )); OpIncr(& i ){
    ...
    lowerCurrencyId := this.SafeStringLower(currency , MkString("id") ); _ = lowerCurrencyId;
...
func (this *Bithumb) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"currency":OpAdd(*(market ).At(MkString("base") ), OpAdd(MkString("_") , *(market ).At(MkString("quote") )))}); _ = request;
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
    *(request ).At(MkString("count") )= OpCopy(limit );
  }
  response := this.Call(MkString("publicGetOrderbookCurrency"), this.Extend(request , params )); _ = response;
...
func (this *Bithumb) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(ticker , MkString("date") ); _ = timestamp;
  symbol := OpCopy(MkUndefined() ); _ = symbol;
  if IsTrue(OpNotEq2(market , MkUndefined() )) {
    symbol = *(market ).At(MkString("symbol") );
  }
  open := this.SafeNumber(ticker , MkString("opening_price") ); _ = open;
  close := this.SafeNumber(ticker , MkString("closing_price") ); _ = close;
  ...
  baseVolume := this.SafeNumber(ticker , MkString("units_traded_24H") ); _ = baseVolume;
  quoteVolume := this.SafeNumber(ticker , MkString("acc_trade_value_24H") ); _ = quoteVolume;
...
func (this *Bithumb) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  response := this.Call(MkString("publicGetTickerAll"), params ); _ = response;
  result := MkMap(&VarMap{}); _ = result;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  timestamp := this.SafeInteger(data , MkString("date") ); _ = timestamp;
  tickers := this.Omit(data , MkString("date") ); _ = tickers;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    ...
    isArray := GoIsArray(ticker ); _ = isArray;
...
func (this *Bithumb) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"currency":*(market ).At(MkString("base") )}); _ = request;
  response := this.Call(MkString("publicGetTickerCurrency"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
...
func (this *Bithumb) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetCandlestickCurrencyInterval"), this.Extend(request , params )); _ = response;
...
func (this *Bithumb) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  transactionDatetime := this.SafeString(trade , MkString("transaction_date") ); _ = transactionDatetime;
  if IsTrue(OpNotEq2(transactionDatetime , MkUndefined() )) {
    parts := transactionDatetime.Split(MkString(" ") ); _ = parts;
    numParts := OpCopy(parts.Length ); _ = numParts;
    if IsTrue(OpGt(numParts , MkInteger(1) )) {
      transactionDate := *(parts ).At(MkInteger(0) ); _ = transactionDate;
      transactionTime := *(parts ).At(MkInteger(1) ); _ = transactionTime;
  ...
  id := this.SafeString(trade , MkString("cont_no") ); _ = id;
  symbol := OpCopy(MkUndefined() ); _ = symbol;
  if IsTrue(OpNotEq2(market , MkUndefined() )) {
    symbol = *(market ).At(MkString("symbol") );
  }
  priceString := this.SafeString(trade , MkString("price") ); _ = priceString;
  amountString := this.SafeString2(trade , MkString("units_traded") , MkString("units") ); _ = amountString;
  price := this.ParseNumber(priceString ); _ = price;
  amount := this.ParseNumber(amountString ); _ = amount;
  cost := this.SafeNumber(trade , MkString("total") ); _ = cost;
  ...
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    ...
    feeCurrencyCode := this.CommonCurrencyCode(feeCurrencyId ); _ = feeCurrencyCode;
    fee = MkMap(&VarMap{
        "cost":feeCost ,
...
func (this *Bithumb) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTransactionHistoryCurrency"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Bithumb) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  method := MkString("privatePostTradePlace") ; _ = method;
...
func (this *Bithumb) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostInfoOrderDetail"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
...
func (this *Bithumb) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeIntegerProduct(order , MkString("order_date") , MkNumber(0.001) ); _ = timestamp;
  sideProperty := this.SafeValue2(order , MkString("type") , MkString("side") ); _ = sideProperty;
  side := OpTrinary(OpEq2(sideProperty , MkString("bid") ), MkString("buy") , MkString("sell") ); _ = side;
  status := this.ParseOrderStatus(this.SafeString(order , MkString("order_status") )); _ = status;
  price := this.SafeNumber2(order , MkString("order_price") , MkString("price") ); _ = price;
  type_ := MkString("limit") ; _ = type_;
  if IsTrue(OpEq2(price , MkInteger(0) )) {
    price = OpCopy(MkUndefined() );
    type_ = MkString("market") ;
  }
  amount := this.SafeNumber2(order , MkString("order_qty") , MkString("units") ); _ = amount;
  remaining := this.SafeNumber(order , MkString("units_remaining") ); _ = remaining;
  ...
  symbol := OpCopy(MkUndefined() ); _ = symbol;
  baseId := this.SafeString(order , MkString("order_currency") ); _ = baseId;
  quoteId := this.SafeString(order , MkString("payment_currency") ); _ = quoteId;
  ...
  rawTrades := this.SafeValue(order , MkString("contract") , MkArray(&VarArray{})); _ = rawTrades;
...
func (this *Bithumb) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostInfoOrders"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Bithumb) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  symbol := GoGetArg(goArgs, 1, MkUndefined() ); _ = symbol;
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  side_in_params := OpHasMember(MkString("side") , params ); _ = side_in_params;
  ...
  market := this.Market(symbol ); _ = market;
  side := OpTrinary(OpEq2(*(params ).At(MkString("side") ), MkString("buy") ), MkString("bid") , MkString("ask") ); _ = side;
...
func (this *Bithumb) CancelUnifiedOrder(goArgs ...*Variant) *Variant{
  order := GoGetArg(goArgs, 0, MkUndefined()); _ = order;
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"side":*(order ).At(MkString("side") )}); _ = request;
...
func (this *Bithumb) Withdraw(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpOr(OpEq2(currency , MkString("XRP") ), OpOr(OpEq2(currency , MkString("XMR") ), OpOr(OpEq2(currency , MkString("EOS") ), OpEq2(currency , MkString("STEEM") ))))) {
    destination := this.SafeString(params , MkString("destination") ); _ = destination;
  ...
  response := this.Call(MkString("privatePostTradeBtcWithdrawal"), this.Extend(request , params )); _ = response;
  return MkMap(&VarMap{
        "info":response ,
        "id":MkUndefined() ,
        });
...
func (this *Bithumb) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("public") )) {
    ...
    auth := OpAdd(endpoint , OpAdd(MkString("\000") , OpAdd(body , OpAdd(MkString("\000") , nonce )))); _ = auth;
    signature := this.Hmac(this.Encode(auth ), this.Encode(*this.At(MkString("secret")) ), MkString("sha512") ); _ = signature;
    signature64 := this.Decode(this.StringToBase64(signature )); _ = signature64;
