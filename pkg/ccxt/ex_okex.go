...
func (this *Okex) FetchStatus(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetSystemStatus"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    event := *(data ).At(i ); _ = event;
    state := this.SafeString(event , MkString("state") ); _ = state;
...
func (this *Okex) FetchTime(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetPublicTime"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  first := this.SafeValue(data , MkInteger(0) , MkMap(&VarMap{})); _ = first;
...
func (this *Okex) ParseMarket(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString(market , MkString("instId") ); _ = id;
  type_ := this.SafeStringLower(market , MkString("instType") ); _ = type_;
  spot := OpEq2(type_ , MkString("spot") ); _ = spot;
  futures := OpEq2(type_ , MkString("futures") ); _ = futures;
  swap := OpEq2(type_ , MkString("swap") ); _ = swap;
  option := OpEq2(type_ , MkString("option") ); _ = option;
  baseId := this.SafeString(market , MkString("baseCcy") ); _ = baseId;
  quoteId := this.SafeString(market , MkString("quoteCcy") ); _ = quoteId;
  settleCurrency := this.SafeString(market , MkString("settleCcy") ); _ = settleCurrency;
  underlying := this.SafeString(market , MkString("uly") ); _ = underlying;
  ...
  inverse := OpEq2(baseId , settleCurrency ); _ = inverse;
  linear := OpEq2(quoteId , settleCurrency ); _ = linear;
  ...
  tickSize := this.SafeString(market , MkString("tickSz") ); _ = tickSize;
  precision := MkMap(&VarMap{
        "amount":this.SafeNumber(market , MkString("lotSz") ),
        "price":this.ParseNumber(tickSize ),
        }); _ = precision;
  minAmountString := this.SafeString(market , MkString("minSz") ); _ = minAmountString;
  ...
  fees := this.SafeValue2(*this.At(MkString("fees")) , type_ , MkString("trading") , MkMap(&VarMap{})); _ = fees;
  contractSize := this.SafeString(market , MkString("ctVal") ); _ = contractSize;
...
func (this *Okex) FetchMarketsByType(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"instType":uppercaseType }); _ = request;
  if IsTrue(OpEq2(uppercaseType , MkString("OPTION") )) {
    defaultUnderlying := this.SafeValue(*this.At(MkString("options")) , MkString("defaultUnderlying") , MkString("BTC-USD") ); _ = defaultUnderlying;
    currencyId := this.SafeString2(params , MkString("uly") , MkString("marketId") , defaultUnderlying ); _ = currencyId;
  ...
  response := this.Call(MkString("publicGetPublicInstruments"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Okex) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("privateGetAssetCurrencies"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  result := MkMap(&VarMap{}); _ = result;
  dataByCurrencyId := this.GroupBy(data , MkString("ccy") ); _ = dataByCurrencyId;
  currencyIds := GoGetKeys(dataByCurrencyId ); _ = currencyIds;
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencyIds.Length )); OpIncr(& i ){
    currencyId := *(currencyIds ).At(i ); _ = currencyId;
    chains := *(dataByCurrencyId ).At(currencyId ); _ = chains;
    first := this.SafeValue(chains , MkInteger(0) ); _ = first;
    id := this.SafeString(first , MkString("ccy") ); _ = id;
    code := this.SafeCurrencyCode(id ); _ = code;
    precision := MkNumber(0.00000001) ; _ = precision;
    name := this.SafeString(first , MkString("name") ); _ = name;
    if IsTrue(OpAnd(OpNotEq2(name , MkUndefined() ), OpLw(name.Length , MkInteger(1) ))) {
      name = OpCopy(MkUndefined() );
    }
    canDeposit := this.SafeValue(first , MkString("canDep") ); _ = canDeposit;
    canWithdraw := this.SafeValue(first , MkString("canWd") ); _ = canWithdraw;
    canInternal := this.SafeValue(first , MkString("canInternal") ); _ = canInternal;
    active := OpTrinary(OpAnd(canDeposit , OpAnd(canWithdraw , canInternal )), MkBool(true) , MkBool(false) ); _ = active;
...
func (this *Okex) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"instId":*(market ).At(MkString("id") )}); _ = request;
  limit = OpTrinary(OpEq2(limit , MkUndefined() ), MkInteger(20) , limit );
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
    *(request ).At(MkString("sz") )= OpCopy(limit );
  }
  response := this.Call(MkString("publicGetMarketBooks"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  first := this.SafeValue(data , MkInteger(0) , MkMap(&VarMap{})); _ = first;
  timestamp := this.SafeInteger(first , MkString("ts") ); _ = timestamp;
...
func (this *Okex) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(ticker , MkString("ts") ); _ = timestamp;
  marketId := this.SafeString(ticker , MkString("instId") ); _ = marketId;
  ...
  open := this.SafeNumber(ticker , MkString("open24h") ); _ = open;
  quoteVolume := this.SafeNumber(ticker , MkString("volCcy24h") ); _ = quoteVolume;
  baseVolume := this.SafeNumber(ticker , MkString("vol24h") ); _ = baseVolume;
...
func (this *Okex) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"instId":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetMarketTicker"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  first := this.SafeValue(data , MkInteger(0) , MkMap(&VarMap{})); _ = first;
...
func (this *Okex) FetchTickersByType(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  uppercaseType := type_.ToUpperCase(); _ = uppercaseType;
  request := MkMap(&VarMap{"instType":type_.ToUpperCase()}); _ = request;
  if IsTrue(OpEq2(uppercaseType , MkString("OPTION") )) {
    defaultUnderlying := this.SafeValue(*this.At(MkString("options")) , MkString("defaultUnderlying") , MkString("BTC-USD") ); _ = defaultUnderlying;
    currencyId := this.SafeString2(params , MkString("uly") , MkString("marketId") , defaultUnderlying ); _ = currencyId;
    if IsTrue(OpEq2(currencyId , MkUndefined() )) {
  ...
  response := this.Call(MkString("publicGetMarketTickers"), this.Extend(request , params )); _ = response;
...
func (this *Okex) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(trade , MkString("instId") ); _ = marketId;
  market = this.SafeMarket(marketId , market , MkString("-") );
  symbol := *(market ).At(MkString("symbol") ); _ = symbol;
  timestamp := this.SafeInteger(trade , MkString("ts") ); _ = timestamp;
  priceString := this.SafeString2(trade , MkString("fillPx") , MkString("px") ); _ = priceString;
  amountString := this.SafeString2(trade , MkString("fillSz") , MkString("sz") ); _ = amountString;
  ...
  orderId := this.SafeString(trade , MkString("ordId") ); _ = orderId;
  feeCostString := this.SafeString(trade , MkString("fee") ); _ = feeCostString;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCostString , MkUndefined() )) {
    feeCostSigned := Precise.StringNeg(feeCostString ); _ = feeCostSigned;
    feeCurrencyId := this.SafeString(trade , MkString("feeCcy") ); _ = feeCurrencyId;
  ...
  takerOrMaker := this.SafeString(trade , MkString("execType") ); _ = takerOrMaker;
...
func (this *Okex) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketTrades"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Okex) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  method := OpAdd(MkString("publicGetMarket") , type_ ); _ = method;
...
func (this *Okex) ParseTradingBalance(goArgs ...*Variant) *Variant{
  ...
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  first := this.SafeValue(data , MkInteger(0) , MkMap(&VarMap{})); _ = first;
  timestamp := this.SafeInteger(first , MkString("uTime") ); _ = timestamp;
  details := this.SafeValue(first , MkString("details") , MkArray(&VarArray{})); _ = details;
  for i := MkInteger(0) ; IsTrue(OpLw(i , details.Length )); OpIncr(& i ){
    balance := *(details ).At(i ); _ = balance;
    currencyId := this.SafeString(balance , MkString("ccy") ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    account := this.Account(); _ = account;
    eq := this.SafeString(balance , MkString("eq") ); _ = eq;
    availEq := this.SafeString(balance , MkString("availEq") ); _ = availEq;
...
func (this *Okex) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  defaultType := this.SafeString(*this.At(MkString("options")) , MkString("defaultType") ); _ = defaultType;
  options := this.SafeValue(*this.At(MkString("options")) , MkString("fetchBalance") , MkMap(&VarMap{})); _ = options;
  type_ := this.SafeString(options , MkString("type") , defaultType ); _ = type_;
...
func (this *Okex) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(clientOrderId , MkUndefined() )) {
    brokerId := this.SafeString(*this.At(MkString("options")) , MkString("brokerId") ); _ = brokerId;
  ...
  if IsTrue(OpEq2(type_ , MkString("market") )) {
    if IsTrue(OpEq2(side , MkString("buy") )) {
      notional := this.SafeNumber(params , MkString("sz") ); _ = notional;
  ...
  response := this.Call(MkString("privatePostTradeOrder"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  first := this.SafeValue(data , MkInteger(0) ); _ = first;
  order := this.ParseOrder(first , market ); _ = order;
...
func (this *Okex) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostTradeCancelOrder"), this.Extend(request , query )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Okex) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  id := this.SafeString(order , MkString("ordId") ); _ = id;
  timestamp := this.SafeInteger(order , MkString("cTime") ); _ = timestamp;
  lastTradeTimestamp := this.SafeInteger(order , MkString("fillTime") ); _ = lastTradeTimestamp;
  side := this.SafeString(order , MkString("side") ); _ = side;
  type_ := this.SafeString(order , MkString("ordType") ); _ = type_;
  postOnly := OpCopy(MkUndefined() ); _ = postOnly;
  timeInForce := OpCopy(MkUndefined() ); _ = timeInForce;
  ...
  marketId := this.SafeString(order , MkString("instId") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("-") ); _ = symbol;
  filled := this.SafeNumber(order , MkString("accFillSz") ); _ = filled;
  price := this.SafeNumber2(order , MkString("px") , MkString("slOrdPx") ); _ = price;
  ...
  feeCostString := this.SafeString(order , MkString("fee") ); _ = feeCostString;
  ...
  if IsTrue(OpNotEq2(feeCostString , MkUndefined() )) {
    feeCostSigned := Precise.StringNeg(feeCostString ); _ = feeCostSigned;
    feeCurrencyId := this.SafeString(order , MkString("feeCcy") ); _ = feeCurrencyId;
    feeCurrencyCode := this.SafeCurrencyCode(feeCurrencyId ); _ = feeCurrencyCode;
    fee = MkMap(&VarMap{
        "cost":this.ParseNumber(feeCostSigned ),
        "currency":feeCurrencyCode ,
  ...
  stopPrice := this.SafeNumber(order , MkString("slTriggerPx") ); _ = stopPrice;
...
func (this *Okex) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetTradeOrder"), this.Extend(request , query )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  order := this.SafeValue(data , MkInteger(0) ); _ = order;
  return this.ParseOrder(order , market );
}
...
func (this *Okex) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetTradeOrdersPending"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Okex) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  defaultType := this.SafeString(*this.At(MkString("options")) , MkString("defaultType") ); _ = defaultType;
  options := this.SafeString(*this.At(MkString("options")) , MkString("fetchClosedOrders") , MkMap(&VarMap{})); _ = options;
  type_ := this.SafeString(options , MkString("type") , defaultType ); _ = type_;
  type_ = this.SafeString(params , MkString("type") , type_ );
  params = this.Omit(params , MkString("type") );
  request := MkMap(&VarMap{}); _ = request;
  ...
  *(request ).At(MkString("instType") )= type_.ToUpperCase();
  ...
  method := this.SafeString(options , MkString("method") , MkString("privateGetTradeOrdersHistory") ); _ = method;
  response := this.Call(method , this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  return this.ParseOrders(data , market , since , limit );
}
...
func (this *Okex) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"instType":MkString("SPOT") }); _ = request;
  market := OpCopy(MkUndefined() ); _ = market;
  this.LoadMarkets();
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(symbol );
  ...
  response := this.Call(MkString("privateGetTradeFillsHistory"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Okex) FetchOrderTrades(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 3, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 4, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"ordId":id }); _ = request;
...
func (this *Okex) FetchLedger(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  options := this.SafeValue(*this.At(MkString("options")) , MkString("fetchLedger") , MkMap(&VarMap{})); _ = options;
  method := this.SafeString(options , MkString("method") ); _ = method;
...
func (this *Okex) ParseLedgerEntry(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString(item , MkString("billId") ); _ = id;
  account := OpCopy(MkUndefined() ); _ = account;
  referenceId := this.SafeString(item , MkString("ordId") ); _ = referenceId;
  ...
  code := this.SafeCurrencyCode(this.SafeString(item , MkString("ccy") ), currency ); _ = code;
  amountString := this.SafeString(item , MkString("balChg") ); _ = amountString;
  amount := this.ParseNumber(amountString ); _ = amount;
  timestamp := this.SafeInteger(item , MkString("ts") ); _ = timestamp;
  feeCostString := this.SafeString(item , MkString("fee") ); _ = feeCostString;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCostString , MkUndefined() )) {
    fee = MkMap(&VarMap{
  ...
  afterString := this.SafeString(item , MkString("bal") ); _ = afterString;
  after := this.ParseNumber(afterString ); _ = after;
  status := MkString("ok") ; _ = status;
  marketId := this.SafeString(item , MkString("instId") ); _ = marketId;
...
func (this *Okex) ParseDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  address := this.SafeString(depositAddress , MkString("addr") ); _ = address;
  tag := this.SafeString2(depositAddress , MkString("tag") , MkString("pmtId") ); _ = tag;
  tag = this.SafeString(depositAddress , MkString("memo") , tag );
  currencyId := this.SafeString(depositAddress , MkString("ccy") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId ); _ = code;
...
func (this *Okex) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"ccy":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetAssetDepositAddress"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  addressesByCode := this.ParseDepositAddresses(data ); _ = addressesByCode;
...
func (this *Okex) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostAssetWithdrawal"), this.Extend(request , query )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  transaction := this.SafeValue(data , MkInteger(0) ); _ = transaction;
...
func (this *Okex) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetAssetDepositHistory"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Okex) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetAssetWithdrawalHistory"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  return this.ParseTransactions(data , currency , since , limit , params );
}
...
func (this *Okex) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  withdrawalId := this.SafeString(transaction , MkString("wdId") ); _ = withdrawalId;
  ...
  address := OpCopy(addressTo ); _ = address;
  tagTo := this.SafeString2(transaction , MkString("tag") , MkString("memo") ); _ = tagTo;
  ...
  currencyId := this.SafeString(transaction , MkString("ccy") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId ); _ = code;
  amount := this.SafeNumber(transaction , MkString("amt") ); _ = amount;
  ...
  timestamp := this.SafeInteger(transaction , MkString("ts") ); _ = timestamp;
...
func (this *Okex) FetchPosition(goArgs ...*Variant) *Variant{
  ...
  params = this.Omit(params , MkString("type") );
  response := this.Call(MkString("privateGetAccountPositions"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Okex) FetchPositions(goArgs ...*Variant) *Variant{
  ...
  params = this.Omit(params , MkString("type") );
  response := this.Call(MkString("privateGetAccountPositions"), this.Extend(request , params )); _ = response;
  positions := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = positions;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , positions.Length )); OpIncr(& i ){
    entry := *(positions ).At(i ); _ = entry;
    instrument := this.SafeString(entry , MkString("instType") ); _ = instrument;
...
func (this *Okex) ParsePosition(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  marketId := this.SafeString(position , MkString("instId") ); _ = marketId;
  market = this.SafeMarket(marketId , market );
  symbol := *(market ).At(MkString("symbol") ); _ = symbol;
  contractsString := this.SafeString(position , MkString("pos") ); _ = contractsString;
  contracts := OpCopy(MkUndefined() ); _ = contracts;
  if IsTrue(OpNotEq2(contractsString , MkUndefined() )) {
    contracts = parseInt(contractsString );
  }
  notionalString := this.SafeString(position , MkString("notionalUsd") ); _ = notionalString;
  notional := this.ParseNumber(notionalString ); _ = notional;
  marginType := this.SafeString(position , MkString("mgnMode") ); _ = marginType;
  initialMarginString := OpCopy(MkUndefined() ); _ = initialMarginString;
  entryPriceString := this.SafeString(position , MkString("avgPx") ); _ = entryPriceString;
  unrealizedPnlString := this.SafeString(position , MkString("upl") ); _ = unrealizedPnlString;
  ...
  maintenanceMarginString := this.SafeString(position , MkString("mmr") ); _ = maintenanceMarginString;
  maintenanceMargin := this.ParseNumber(maintenanceMarginString ); _ = maintenanceMargin;
  initialMarginPercentage := OpCopy(MkUndefined() ); _ = initialMarginPercentage;
  maintenanceMarginPercentage := OpCopy(MkUndefined() ); _ = maintenanceMarginPercentage;
  if IsTrue(*(market ).At(MkString("inverse") )) {
    notionalValue := Precise.StringDiv(Precise.StringMul(contractsString , *(market ).At(MkString("contractSize") )), entryPriceString ); _ = notionalValue;
  ...
  rounder := MkString("0.00005") ; _ = rounder;
  ...
  collateralString := Precise.StringAdd(initialMarginString , unrealizedPnlString ); _ = collateralString;
  liquidationPrice := this.SafeNumber(position , MkString("liqPx") ); _ = liquidationPrice;
  percentageString := this.SafeString(position , MkString("uplRatio") ); _ = percentageString;
  percentage := this.ParseNumber(Precise.StringMul(percentageString , MkString("100") )); _ = percentage;
  side := this.SafeString(position , MkString("posSide") ); _ = side;
  timestamp := this.SafeInteger(position , MkString("uTime") ); _ = timestamp;
  leverage := this.SafeInteger(position , MkString("lever") ); _ = leverage;
  marginRatio := this.ParseNumber(Precise.StringDiv(maintenanceMarginString , collateralString , MkInteger(4) )); _ = marginRatio;
  return MkMap(&VarMap{
        "info":position ,
        "symbol":symbol ,
        "notional":notional ,
...
func (this *Okex) Transfer(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostAssetTransfer"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  rawTransfer := this.SafeValue(data , MkInteger(0) , MkMap(&VarMap{})); _ = rawTransfer;
...
func (this *Okex) ParseTransfer(goArgs ...*Variant) *Variant{
  ...
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  id := this.SafeString(transfer , MkString("transId") ); _ = id;
  currencyId := this.SafeString(transfer , MkString("ccy") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  amount := this.SafeNumber(transfer , MkString("amt") ); _ = amount;
  fromAccountId := this.SafeString(transfer , MkString("from") ); _ = fromAccountId;
  toAccountId := this.SafeString(transfer , MkString("to") ); _ = toAccountId;
  typesByAccount := this.SafeValue(*this.At(MkString("options")) , MkString("typesByAccount") , MkMap(&VarMap{})); _ = typesByAccount;
  fromAccount := this.SafeString(typesByAccount , fromAccountId ); _ = fromAccount;
  toAccount := this.SafeString(typesByAccount , toAccountId ); _ = toAccount;
...
func (this *Okex) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(code , MkString("0") )) {
    ...
    data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
    for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
      error := *(data ).At(i ); _ = error;
      errorCode := this.SafeString(error , MkString("sCode") ); _ = errorCode;
      message := this.SafeString(error , MkString("sMsg") ); _ = message;
