...
func (this *Itbit) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = request
	orderbook := this.Call(MkString("publicGetMarketsSymbolOrderBook"), this.Extend(request, params))
	_ = orderbook
...
func (this *Itbit) FetchTicker(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"symbol": this.MarketId(symbol)})
	_ = request
	ticker := this.Call(MkString("publicGetMarketsSymbolTicker"), this.Extend(request, params))
	_ = ticker
	serverTimeUTC := this.SafeString(ticker, MkString("serverTimeUTC"))
	_ = serverTimeUTC
	...
	_ = timestamp
	vwap := this.SafeNumber(ticker, MkString("vwap24h"))
	_ = vwap
	baseVolume := this.SafeNumber(ticker, MkString("volume24h"))
	_ = baseVolume
...
func (this *Itbit) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = market
	id := this.SafeString2(trade, MkString("executionId"), MkString("matchNumber"))
	_ = id
	...
	_ = orderId
	feeCost := this.SafeNumber(trade, MkString("commissionPaid"))
	_ = feeCost
	feeCurrencyId := this.SafeString(trade, MkString("commissionCurrency"))
	_ = feeCurrencyId
	feeCurrency := this.SafeCurrencyCode(feeCurrencyId)
	_ = feeCurrency
	rebatesApplied := this.SafeNumber(trade, MkString("rebatesApplied"))
	_ = rebatesApplied
	if IsTrue(OpNotEq2(rebatesApplied, MkUndefined())) {
		rebatesApplied = OpNeg(rebatesApplied)
	}
	rebateCurrencyId := this.SafeString(trade, MkString("rebateCurrency"))
	_ = rebateCurrencyId
	rebateCurrency := this.SafeCurrencyCode(rebateCurrencyId)
	_ = rebateCurrency
	priceString := this.SafeString2(trade, MkString("price"), MkString("rate"))
	_ = priceString
	amountString := this.SafeString2(trade, MkString("currency1Amount"), MkString("amount"))
	_ = amountString
	...
	if IsTrue(OpNotEq2(marketId, MkUndefined())) {
		if IsTrue(OpHasMember(marketId, *this.At(MkString("markets_by_id")))) {
			...
			baseId := this.SafeString(trade, MkString("currency1"))
			_ = baseId
			quoteId := this.SafeString(trade, MkString("currency2"))
			_ = quoteId
...
func (this *Itbit) FetchTransactions(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	walletId := this.SafeString(params, MkString("walletId"))
	_ = walletId
	...
	response := this.Call(MkString("privateGetWalletsWalletIdFundingHistory"), this.Extend(request, params))
	_ = response
	items := *(response).At(MkString("fundingHistory"))
	_ = items
	...
	for i := MkInteger(0); IsTrue(OpLw(i, items.Length)); OpIncr(&i) {
		item := *(items).At(i)
		_ = item
		time := this.SafeString(item, MkString("time"))
		_ = time
		timestamp := this.Parse8601(time)
		_ = timestamp
		currency := this.SafeString(item, MkString("currency"))
		_ = currency
		destinationAddress := this.SafeString(item, MkString("destinationAddress"))
		_ = destinationAddress
		txnHash := this.SafeString(item, MkString("txnHash"))
		_ = txnHash
		transactionType := this.SafeStringLower(item, MkString("transactionType"))
		_ = transactionType
		transactionStatus := this.SafeString(item, MkString("status"))
		_ = transactionStatus
		status := this.ParseTransferStatus(transactionStatus)
		_ = status
...
func (this *Itbit) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetWalletsWalletIdTrades"), this.Extend(request, params))
	_ = response
	trades := this.SafeValue(response, MkString("tradingHistory"), MkArray(&VarArray{}))
	_ = trades
...
func (this *Itbit) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetMarketsSymbolTrades"), this.Extend(request, params))
	_ = response
	trades := this.SafeValue(response, MkString("recentTrades"), MkArray(&VarArray{}))
	_ = trades
	return this.ParseTrades(trades, market, since, limit)
}
...
func (this *Itbit) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.FetchWallets(params)
	_ = response
	balances := *(*(response).At(MkInteger(0))).At(MkString("balances"))
	_ = balances
...
func (this *Itbit) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"status": MkString("open")})
	_ = request
...
func (this *Itbit) FetchOrders(goArgs ...*Variant) *Variant {
	...
	walletIdInParams := OpHasMember(MkString("walletId"), params)
	_ = walletIdInParams
	...
	walletId := *(params).At(MkString("walletId"))
	_ = walletId
	request := MkMap(&VarMap{"walletId": walletId})
	_ = request
	response := this.Call(MkString("privateGetWalletsWalletIdOrders"), this.Extend(request, params))
	_ = response
...
func (this *Itbit) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = type_
	symbol := *(*(*this.At(MkString("markets_by_id"))).At(*(order).At(MkString("instrument")))).At(MkString("symbol"))
	_ = symbol
	timestamp := this.Parse8601(*(order).At(MkString("createdTime")))
	_ = timestamp
	amount := this.SafeNumber(order, MkString("amount"))
	_ = amount
	filled := this.SafeNumber(order, MkString("amountFilled"))
	_ = filled
	fee := OpCopy(MkUndefined())
	_ = fee
	price := this.SafeNumber(order, MkString("price"))
	_ = price
	average := this.SafeNumber(order, MkString("volumeWeightedAveragePrice"))
	_ = average
	clientOrderId := this.SafeString(order, MkString("clientOrderIdentifier"))
	_ = clientOrderId
	id := this.SafeString(order, MkString("id"))
	_ = id
	postOnlyString := this.SafeString(order, MkString("postOnly"))
	_ = postOnlyString
	postOnly := OpEq2(postOnlyString, MkString("True"))
	_ = postOnly
...
func (this *Itbit) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostWalletsWalletIdOrders"), this.Extend(request, params))
	_ = response
	return MkMap(&VarMap{
		"info": response,
...
func (this *Itbit) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetWalletsWalletIdOrdersId"), this.Extend(request, params))
	_ = response
...
func (this *Itbit) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("private"))) {
		...
		_ = nonce
		timestamp := OpCopy(nonce)
		_ = timestamp
		authBody := OpTrinary(OpEq2(method, MkString("POST")), body, MkString(""))
		_ = authBody
		...
		_ = auth
		message := OpAdd(nonce, (this.Json(auth)).Call(MkString("replace"), MkString("\\/"), MkString("/")))
		_ = message
		hash := this.Hash(this.Encode(message), MkString("sha256"), MkString("binary"))
		_ = hash
		binaryUrl := this.StringToBinary(this.Encode(url))
		_ = binaryUrl
		binhash := this.BinaryConcat(binaryUrl, hash)
		_ = binhash
		signature := this.Hmac(binhash, this.Encode(*this.At(MkString("secret"))), MkString("sha512"), MkString("base64"))
		_ = signature
