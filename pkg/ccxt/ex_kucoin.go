...
func (this *Kucoin) LoadTimeDifference(goArgs ...*Variant) *Variant{
  ...
  kucoinTime := this.SafeInteger(response , MkString("data") ); _ = kucoinTime;
...
func (this *Kucoin) FetchStatus(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetStatus"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  status := this.SafeValue(data , MkString("status") ); _ = status;
...
func (this *Kucoin) FetchMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetSymbols"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    ...
    active := this.SafeValue(market , MkString("enableTrading") ); _ = active;
    baseMaxSize := this.SafeNumber(market , MkString("baseMaxSize") ); _ = baseMaxSize;
    baseMinSizeString := this.SafeString(market , MkString("baseMinSize") ); _ = baseMinSizeString;
    quoteMaxSizeString := this.SafeString(market , MkString("quoteMaxSize") ); _ = quoteMaxSizeString;
    baseMinSize := this.ParseNumber(baseMinSizeString ); _ = baseMinSize;
    quoteMaxSize := this.ParseNumber(quoteMaxSizeString ); _ = quoteMaxSize;
    quoteMinSize := this.SafeNumber(market , MkString("quoteMinSize") ); _ = quoteMinSize;
...
func (this *Kucoin) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    entry := *(data ).At(i ); _ = entry;
    id := this.SafeString(entry , MkString("currency") ); _ = id;
    name := this.SafeString(entry , MkString("fullName") ); _ = name;
    code := this.SafeCurrencyCode(id ); _ = code;
    precision := this.SafeInteger(entry , MkString("precision") ); _ = precision;
    isWithdrawEnabled := this.SafeValue(entry , MkString("isWithdrawEnabled") , MkBool(false) ); _ = isWithdrawEnabled;
    isDepositEnabled := this.SafeValue(entry , MkString("isDepositEnabled") , MkBool(false) ); _ = isDepositEnabled;
    fee := this.SafeNumber(entry , MkString("withdrawalMinFee") ); _ = fee;
...
func (this *Kucoin) FetchAccounts(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    ...
    type_ := this.SafeString(account , MkString("type") ); _ = type_;
    result.Push(MkMap(&VarMap{
            "id":accountId ,
            "type":type_ ,
            "currency":code ,
...
func (this *Kucoin) FetchFundingFee(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetWithdrawalsQuotas"), this.Extend(request , params )); _ = response;
  data := *(response ).At(MkString("data") ); _ = data;
...
func (this *Kucoin) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  percentage := this.SafeNumber(ticker , MkString("changeRate") ); _ = percentage;
  if IsTrue(OpNotEq2(percentage , MkUndefined() )) {
    percentage = OpMulti(percentage , MkInteger(100) );
  }
  last := this.SafeNumber2(ticker , MkString("last") , MkString("lastTradedPrice") ); _ = last;
  marketId := this.SafeString(ticker , MkString("symbol") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("-") ); _ = symbol;
  baseVolume := this.SafeNumber(ticker , MkString("vol") ); _ = baseVolume;
  quoteVolume := this.SafeNumber(ticker , MkString("volValue") ); _ = quoteVolume;
  vwap := this.Vwap(baseVolume , quoteVolume ); _ = vwap;
  timestamp := this.SafeInteger2(ticker , MkString("time") , MkString("datetime") ); _ = timestamp;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Kucoin) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicGetMarketAllTickers"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  tickers := this.SafeValue(data , MkString("ticker") , MkArray(&VarArray{})); _ = tickers;
  result := MkMap(&VarMap{}); _ = result;
...
func (this *Kucoin) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketStats"), this.Extend(request , params )); _ = response;
...
func (this *Kucoin) FetchOHLCV(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  timeframe := GoGetArg(goArgs, 1, MkString("15m") ); _ = timeframe;
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  marketId := *(market ).At(MkString("id") ); _ = marketId;
  ...
  endAt := this.Milliseconds(); _ = endAt;
  ...
  *(request ).At(MkString("endAt") )= parseInt(Math.Floor(OpDiv(endAt , MkInteger(1000) )));
  response := this.Call(MkString("publicGetMarketCandles"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Kucoin) CreateDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostDepositAddresses"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  ...
  tag := this.SafeString(data , MkString("memo") ); _ = tag;
...
func (this *Kucoin) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetDepositAddresses"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  address := this.SafeString(data , MkString("address") ); _ = address;
  tag := this.SafeString(data , MkString("memo") ); _ = tag;
  if IsTrue(OpNotEq2(code , MkString("NIM") )) {
...
func (this *Kucoin) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  level := this.SafeInteger(params , MkString("level") , MkInteger(2) ); _ = level;
  request := MkMap(&VarMap{
        "symbol":marketId ,
        "level":level ,
        }); _ = request;
  method := MkString("privateGetMarketOrderbookLevelLevel") ; _ = method;
  ...
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  timestamp := this.SafeInteger(data , MkString("time") ); _ = timestamp;
  orderbook := this.ParseOrderBook(data , symbol , timestamp , MkString("bids") , MkString("asks") , OpSub(level , MkInteger(2) ), OpSub(level , MkInteger(1) )); _ = orderbook;
...
func (this *Kucoin) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  marketId := this.MarketId(symbol ); _ = marketId;
  clientOrderId := this.SafeString2(params , MkString("clientOid") , MkString("clientOrderId") , this.Uuid()); _ = clientOrderId;
  ...
  quoteAmount := this.SafeNumber2(params , MkString("cost") , MkString("funds") ); _ = quoteAmount;
  ...
  id := this.SafeString(data , MkString("orderId") ); _ = id;
  order := MkMap(&VarMap{
        "id":id ,
...
func (this *Kucoin) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  clientOrderId := this.SafeString2(params , MkString("clientOid") , MkString("clientOrderId") ); _ = clientOrderId;
  method := MkString("privateDeleteOrdersOrderId") ; _ = method;
...
func (this *Kucoin) FetchOrdersByStatus(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 4, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"status":status }); _ = request;
  market := OpCopy(MkUndefined() ); _ = market;
  if IsTrue(OpNotEq2(symbol , MkUndefined() )) {
    market = this.Market(symbol );
    *(request ).At(MkString("symbol") )= *(market ).At(MkString("id") );
  ...
  responseData := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = responseData;
  orders := this.SafeValue(responseData , MkString("items") , MkArray(&VarArray{})); _ = orders;
...
func (this *Kucoin) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  responseData := this.SafeValue(response , MkString("data") ); _ = responseData;
...
func (this *Kucoin) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(order , MkString("createdAt") ); _ = timestamp;
  datetime := this.Iso8601(timestamp ); _ = datetime;
  ...
  feeCurrencyId := this.SafeString(order , MkString("feeCurrency") ); _ = feeCurrencyId;
  ...
  filled := this.SafeNumber(order , MkString("dealSize") ); _ = filled;
  cost := this.SafeNumber(order , MkString("dealFunds") ); _ = cost;
  isActive := this.SafeValue(order , MkString("isActive") , MkBool(false) ); _ = isActive;
  cancelExist := this.SafeValue(order , MkString("cancelExist") , MkBool(false) ); _ = cancelExist;
  status := OpTrinary(isActive , MkString("open") , MkString("closed") ); _ = status;
  status = OpTrinary(cancelExist , MkString("canceled") , status );
  fee := MkMap(&VarMap{
        "currency":feeCurrency ,
        "cost":feeCost ,
        }); _ = fee;
  clientOrderId := this.SafeString(order , MkString("clientOid") ); _ = clientOrderId;
...
func (this *Kucoin) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  method := *(*this.At(MkString("options")) ).At(MkString("fetchMyTradesMethod") ); _ = method;
  parseResponseData := OpCopy(MkBool(false) ); _ = parseResponseData;
...
func (this *Kucoin) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketHistories"), this.Extend(request , params )); _ = response;
...
func (this *Kucoin) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  symbol := this.SafeSymbol(marketId , market , MkString("-") ); _ = symbol;
  id := this.SafeString2(trade , MkString("tradeId") , MkString("id") ); _ = id;
  ...
  priceString := this.SafeString2(trade , MkString("price") , MkString("dealPrice") ); _ = priceString;
  amountString := this.SafeString2(trade , MkString("size") , MkString("amount") ); _ = amountString;
  ...
  type_ := this.SafeString(trade , MkString("type") ); _ = type_;
  if IsTrue(OpEq2(type_ , MkString("match") )) {
    type_ = OpCopy(MkUndefined() );
  }
  cost := this.SafeNumber2(trade , MkString("funds") , MkString("dealValue") ); _ = cost;
...
func (this *Kucoin) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  txid := this.SafeString(transaction , MkString("walletTxId") ); _ = txid;
  if IsTrue(OpNotEq2(txid , MkUndefined() )) {
    txidParts := txid.Split(MkString("@") ); _ = txidParts;
    numTxidParts := OpCopy(txidParts.Length ); _ = numTxidParts;
  ...
  type_ := OpTrinary(OpEq2(txid , MkUndefined() ), MkString("withdrawal") , MkString("deposit") ); _ = type_;
  rawStatus := this.SafeString(transaction , MkString("status") ); _ = rawStatus;
  status := this.ParseTransactionStatus(rawStatus ); _ = status;
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(transaction , MkString("fee") ); _ = feeCost;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    rate := OpCopy(MkUndefined() ); _ = rate;
  ...
  timestamp := this.SafeInteger2(transaction , MkString("createdAt") , MkString("createAt") ); _ = timestamp;
  id := this.SafeString(transaction , MkString("id") ); _ = id;
  updated := this.SafeInteger(transaction , MkString("updatedAt") ); _ = updated;
  isV1 := OpNot(OpHasMember(MkString("createdAt") , transaction )); _ = isV1;
  if IsTrue(isV1 ) {
  ...
  comment := this.SafeString(transaction , MkString("remark") ); _ = comment;
  return MkMap(&VarMap{
        "id":id ,
        "info":transaction ,
        "timestamp":timestamp ,
...
func (this *Kucoin) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  method := MkString("privateGetDeposits") ; _ = method;
  if IsTrue(OpNotEq2(since , MkUndefined() )) {
  ...
  response := this.Call(method , this.Extend(request , params )); _ = response;
  responseData := *(*(response ).At(MkString("data") )).At(MkString("items") ); _ = responseData;
...
func (this *Kucoin) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  method := MkString("privateGetWithdrawals") ; _ = method;
  if IsTrue(OpNotEq2(since , MkUndefined() )) {
    if IsTrue(OpLw(since , MkInteger(1550448000000) )) {
      *(request ).At(MkString("startAt") )= parseInt(OpDiv(since , MkInteger(1000) ));
...
func (this *Kucoin) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("fetchBalance") , MkString("defaultType") , MkString("trade") ); _ = defaultType;
  requestedType := this.SafeString(params , MkString("type") , defaultType ); _ = requestedType;
  accountsByType := this.SafeValue(*this.At(MkString("options")) , MkString("accountsByType") ); _ = accountsByType;
  type_ := this.SafeString(accountsByType , requestedType ); _ = type_;
  ...
  if IsTrue(OpOr(OpEq2(type_ , MkString("contract") ), OpEq2(type_ , MkString("futures") ))) {
    response := this.Call(MkString("futuresPrivateGetAccountOverview"), params ); _ = response;
    result := MkMap(&VarMap{
          "info":response ,
    ...
    data := this.SafeValue(response , MkString("data") ); _ = data;
    currencyId := this.SafeString(data , MkString("currency") ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    ...
    *(result ).At(code )= OpCopy(account );
    ...
    for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
      ...
      balanceType := this.SafeString(balance , MkString("type") ); _ = balanceType;
...
func (this *Kucoin) Transfer(goArgs ...*Variant) *Variant{
  ...
  accountsById := this.SafeValue(*this.At(MkString("options")) , MkString("accountsByType") , MkMap(&VarMap{})); _ = accountsById;
  fromId := this.SafeString(accountsById , fromAccount ); _ = fromId;
  if IsTrue(OpEq2(fromId , MkUndefined() )) {
    keys := GoGetKeys(accountsById ); _ = keys;
    panic(NewExchangeError(OpAdd(*this.At(MkString("id")) , OpAdd(MkString(" fromAccount must be one of ") , keys.Join(MkString(", ") )))));
  }
  toId := this.SafeString(accountsById , toAccount ); _ = toId;
  ...
  if IsTrue(OpEq2(fromId , MkString("contract") )) {
    ...
    response := this.Call(MkString("futuresPrivatePostTransferOut"), this.Extend(request , params )); _ = response;
    data := this.SafeValue(response , MkString("data") ); _ = data;
    timestamp := this.SafeInteger(data , MkString("createdAt") ); _ = timestamp;
    id := this.SafeString(data , MkString("applyId") ); _ = id;
    currencyId := this.SafeString(data , MkString("currency") ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    amount := this.SafeNumber(data , MkString("amount") ); _ = amount;
    rawStatus := this.SafeString(data , MkString("status") ); _ = rawStatus;
    ...
    response := this.Call(MkString("privatePostAccountsInnerTransfer"), this.Extend(request , params )); _ = response;
    data := this.SafeValue(response , MkString("data") ); _ = data;
    id := this.SafeString(data , MkString("orderId") ); _ = id;
    return MkMap(&VarMap{
          "info":response ,
...
func (this *Kucoin) FetchLedger(goArgs ...*Variant) *Variant{
  ...
  this.LoadAccounts();
  currency := this.Currency(code ); _ = currency;
  accountId := this.SafeString(params , MkString("accountId") ); _ = accountId;
  ...
  response := this.Call(MkString("privateGetAccountsAccountIdLedgers"), this.Extend(request , params )); _ = response;
  items := *(*(response ).At(MkString("data") )).At(MkString("items") ); _ = items;
...
func (this *Kucoin) ParseLedgerEntry(goArgs ...*Variant) *Variant{
  ...
  direction := this.SafeString(item , MkString("direction") ); _ = direction;
  before := OpCopy(MkUndefined() ); _ = before;
  if IsTrue(OpAnd(OpNotEq2(after , MkUndefined() ), OpNotEq2(amount , MkUndefined() ))) {
    difference := OpTrinary(OpEq2(direction , MkString("out") ), amount , OpNeg(amount )); _ = difference;
    before = this.Sum(after , difference );
  }
  timestamp := this.SafeInteger(item , MkString("createdAt") ); _ = timestamp;
  type_ := this.ParseLedgerEntryType(this.SafeString(item , MkString("bizType") )); _ = type_;
  contextString := this.SafeString(item , MkString("context") ); _ = contextString;
  id := OpCopy(MkUndefined() ); _ = id;
  referenceId := OpCopy(MkUndefined() ); _ = referenceId;
  if IsTrue(this.IsJsonEncodedObject(contextString )) {
    context := this.ParseJson(contextString ); _ = context;
...
func (this *Kucoin) FetchPositions(goArgs ...*Variant) *Variant{
  symbols := GoGetArg(goArgs, 0, MkUndefined() ); _ = symbols;
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("futuresPrivateGetPositions"), params ); _ = response;
...
func (this *Kucoin) Sign(goArgs ...*Variant) *Variant{
  ...
  versions := this.SafeValue(*this.At(MkString("options")) , MkString("versions") , MkMap(&VarMap{})); _ = versions;
  apiVersions := this.SafeValue(versions , api , MkMap(&VarMap{})); _ = apiVersions;
  methodVersions := this.SafeValue(apiVersions , method , MkMap(&VarMap{})); _ = methodVersions;
  defaultVersion := this.SafeString(methodVersions , path , *(*this.At(MkString("options")) ).At(MkString("version") )); _ = defaultVersion;
  version := this.SafeString(params , MkString("version") , defaultVersion ); _ = version;
  params = this.Omit(params , MkString("version") );
  endpoint := OpAdd(MkString("/api/") , OpAdd(version , OpAdd(MkString("/") , this.ImplodeParams(path , params )))); _ = endpoint;
  query := this.Omit(params , this.ExtractParams(path )); _ = query;
  endpart := MkString("") ; _ = endpart;
  ...
  url := OpAdd(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(api ), endpoint ); _ = url;
  if IsTrue(OpOr(OpEq2(api , MkString("private") ), OpEq2(api , MkString("futuresPrivate") ))) {
    this.CheckRequiredCredentials();
    timestamp := (this.Nonce()).Call(MkString("toString") ); _ = timestamp;
    headers = this.Extend(MkMap(&VarMap{
            "KC-API-KEY-VERSION":MkString("2") ,
            "KC-API-KEY":*this.At(MkString("apiKey")) ,
            "KC-API-TIMESTAMP":timestamp ,
            }), headers );
    apiKeyVersion := this.SafeString(headers , MkString("KC-API-KEY-VERSION") ); _ = apiKeyVersion;
    if IsTrue(OpEq2(apiKeyVersion , MkString("2") )) {
      passphrase := this.Hmac(this.Encode(*this.At(MkString("password")) ), this.Encode(*this.At(MkString("secret")) ), MkString("sha256") , MkString("base64") ); _ = passphrase;
    ...
    payload := OpAdd(timestamp , OpAdd(method , OpAdd(endpoint , endpart ))); _ = payload;
    signature := this.Hmac(this.Encode(payload ), this.Encode(*this.At(MkString("secret")) ), MkString("sha256") , MkString("base64") ); _ = signature;
    *(headers ).At(MkString("KC-API-SIGN") )= OpCopy(signature );
    partner := this.SafeValue(*this.At(MkString("options")) , MkString("partner") , MkMap(&VarMap{})); _ = partner;
    partnerId := this.SafeString(partner , MkString("id") ); _ = partnerId;
    partnerSecret := this.SafeString(partner , MkString("secret") ); _ = partnerSecret;
    if IsTrue(OpAnd(OpNotEq2(partnerId , MkUndefined() ), OpNotEq2(partnerSecret , MkUndefined() ))) {
      partnerPayload := OpAdd(timestamp , OpAdd(partnerId , *this.At(MkString("apiKey")) )); _ = partnerPayload;
      partnerSignature := this.Hmac(this.Encode(partnerPayload ), this.Encode(partnerSecret ), MkString("sha256") , MkString("base64") ); _ = partnerSignature;
...
func (this *Kucoin) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  message := this.SafeString(response , MkString("msg") , MkString("") ); _ = message;
