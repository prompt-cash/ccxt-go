...
func (this *Buda) FetchCurrencyInfo(goArgs ...*Variant) *Variant{
  ...
  currencies := GoGetArg(goArgs, 1, MkUndefined() ); _ = currencies;
  if IsTrue(OpNot(currencies )) {
    response := this.Call(MkString("publicGetCurrencies")); _ = response;
    currencies = this.SafeValue(response , MkString("currencies") );
  }
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    currencyInfo := *(currencies ).At(i ); _ = currencyInfo;
...
func (this *Buda) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  marketsResponse := this.Call(MkString("publicGetMarkets"), params ); _ = marketsResponse;
  markets := this.SafeValue(marketsResponse , MkString("markets") ); _ = markets;
  currenciesResponse := this.Call(MkString("publicGetCurrencies")); _ = currenciesResponse;
  currencies := this.SafeValue(currenciesResponse , MkString("currencies") ); _ = currencies;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    ...
    baseInfo := this.FetchCurrencyInfo(baseId , currencies ); _ = baseInfo;
    quoteInfo := this.FetchCurrencyInfo(quoteId , currencies ); _ = quoteInfo;
    symbol := OpAdd(base , OpAdd(MkString("/") , quote )); _ = symbol;
    pricePrecisionString := this.SafeString(quoteInfo , MkString("input_decimals") ); _ = pricePrecisionString;
    ...
    minimumOrderAmount := this.SafeValue(market , MkString("minimum_order_amount") , MkArray(&VarArray{})); _ = minimumOrderAmount;
    limits := MkMap(&VarMap{
          "amount":MkMap(&VarMap{
...
func (this *Buda) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicGetCurrencies")); _ = response;
  currencies := *(response ).At(MkString("currencies") ); _ = currencies;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    ...
    precision := this.SafeNumber(currency , MkString("input_decimals") ); _ = precision;
    minimum := Math.Pow(MkInteger(10) , OpNeg(precision )); _ = minimum;
...
func (this *Buda) FetchFundingFees(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , codes.Length )); OpIncr(& i ){
    ...
    withdrawResponse := this.Call(MkString("publicGetCurrenciesCurrencyFeesWithdrawal"), request ); _ = withdrawResponse;
    depositResponse := this.Call(MkString("publicGetCurrenciesCurrencyFeesDeposit"), request ); _ = depositResponse;
...
func (this *Buda) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketsMarketTicker"), this.Extend(request , params )); _ = response;
  ticker := this.SafeValue(response , MkString("ticker") ); _ = ticker;
...
func (this *Buda) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  last := parseFloat(*(*(ticker ).At(MkString("last_price") )).At(MkInteger(0) )); _ = last;
  percentage := parseFloat(*(ticker ).At(MkString("price_variation_24h") )); _ = percentage;
  open := parseFloat(this.PriceToPrecision(symbol , OpDiv(last , OpAdd(percentage , MkInteger(1) )))); _ = open;
  change := OpSub(last , open ); _ = change;
...
func (this *Buda) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"market":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetMarketsMarketOrderBook"), this.Extend(request , params )); _ = response;
  orderbook := this.SafeValue(response , MkString("order_book") ); _ = orderbook;
...
func (this *Buda) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTvHistory"), this.Extend(request , params )); _ = response;
...
func (this *Buda) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , balances.Length )); OpIncr(& i ){
    ...
    currencyId := this.SafeString(balance , MkString("id") ); _ = currencyId;
...
func (this *Buda) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  order := this.SafeValue(response , MkString("order") ); _ = order;
...
func (this *Buda) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetMarketsMarketOrders"), this.Extend(request , params )); _ = response;
  orders := this.SafeValue(response , MkString("orders") ); _ = orders;
...
func (this *Buda) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"state":MkString("traded") }); _ = request;
  return this.FetchOrders(symbol , since , limit , this.Extend(request , params ));
}
...
func (this *Buda) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostMarketsMarketOrders"), this.Extend(request , params )); _ = response;
  order := this.SafeValue(response , MkString("order") ); _ = order;
  return this.ParseOrder(order );
}
...
func (this *Buda) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  type_ := this.SafeString(order , MkString("price_type") ); _ = type_;
  ...
  originalAmount := this.SafeValue(order , MkString("original_amount") , MkArray(&VarArray{})); _ = originalAmount;
  amount := this.SafeNumber(originalAmount , MkInteger(0) ); _ = amount;
  remainingAmount := this.SafeValue(order , MkString("amount") , MkArray(&VarArray{})); _ = remainingAmount;
  remaining := this.SafeNumber(remainingAmount , MkInteger(0) ); _ = remaining;
  tradedAmount := this.SafeValue(order , MkString("traded_amount") , MkArray(&VarArray{})); _ = tradedAmount;
  filled := this.SafeNumber(tradedAmount , MkInteger(0) ); _ = filled;
  totalExchanged := this.SafeValue(order , MkString("totalExchanged") , MkArray(&VarArray{})); _ = totalExchanged;
  cost := this.SafeNumber(totalExchanged , MkInteger(0) ); _ = cost;
  limitPrice := this.SafeValue(order , MkString("limit") , MkArray(&VarArray{})); _ = limitPrice;
  price := this.SafeNumber(limitPrice , MkInteger(0) ); _ = price;
  if IsTrue(OpEq2(price , MkUndefined() )) {
    if IsTrue(OpNotEq2(limitPrice , MkUndefined() )) {
      price = OpCopy(limitPrice );
    }
  }
  paidFee := this.SafeValue(order , MkString("paid_fee") , MkArray(&VarArray{})); _ = paidFee;
  feeCost := this.SafeNumber(paidFee , MkInteger(0) ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyId := this.SafeString(paidFee , MkInteger(1) ); _ = feeCurrencyId;
...
func (this *Buda) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetCurrenciesCurrencyReceiveAddresses"), this.Extend(request , params )); _ = response;
  receiveAddresses := this.SafeValue(response , MkString("receive_addresses") ); _ = receiveAddresses;
  addressPool := MkArray(&VarArray{}); _ = addressPool;
  for i := MkInteger(1) ; IsTrue(OpLw(i , receiveAddresses.Length )); OpIncr(& i ){
    receiveAddress := *(receiveAddresses ).At(i ); _ = receiveAddress;
    if IsTrue(*(receiveAddress ).At(MkString("ready") )) {
      address := *(receiveAddress ).At(MkString("address") ); _ = address;
      this.CheckAddress(address );
      addressPool.Push(address );
    }
  }
  addressPoolLength := OpCopy(addressPool.Length ); _ = addressPoolLength;
  ...
  address := *(addressPool ).At(MkInteger(0) ); _ = address;
  return MkMap(&VarMap{
        "currency":code ,
        "address":address ,
        "tag":MkUndefined() ,
...
func (this *Buda) CreateDepositAddress(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostCurrenciesCurrencyReceiveAddresses"), this.Extend(request , params )); _ = response;
  address := this.SafeString(*(response ).At(MkString("receive_address") ), MkString("address") ); _ = address;
  return MkMap(&VarMap{
        "currency":code ,
        "address":address ,
        "tag":MkUndefined() ,
...
func (this *Buda) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  amount := parseFloat(*(*(transaction ).At(MkString("amount") )).At(MkInteger(0) )); _ = amount;
  fee := parseFloat(*(*(transaction ).At(MkString("fee") )).At(MkInteger(0) )); _ = fee;
  feeCurrency := *(*(transaction ).At(MkString("fee") )).At(MkInteger(1) ); _ = feeCurrency;
  status := this.ParseTransactionStatus(this.SafeString(transaction , MkString("state") )); _ = status;
  type_ := OpTrinary(OpHasMember(MkString("deposit_data") , transaction ), MkString("deposit") , MkString("withdrawal") ); _ = type_;
  data := this.SafeValue(transaction , OpAdd(type_ , MkString("_data") ), MkMap(&VarMap{})); _ = data;
  address := this.SafeValue(data , MkString("target_address") ); _ = address;
  txid := this.SafeString(data , MkString("tx_hash") ); _ = txid;
  updated := this.Parse8601(this.SafeString(data , MkString("updated_at") )); _ = updated;
  return MkMap(&VarMap{
        "info":transaction ,
        "id":id ,
        "txid":txid ,
...
func (this *Buda) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetCurrenciesCurrencyDeposits"), this.Extend(request , params )); _ = response;
  deposits := this.SafeValue(response , MkString("deposits") ); _ = deposits;
...
func (this *Buda) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetCurrenciesCurrencyWithdrawals"), this.Extend(request , params )); _ = response;
  withdrawals := this.SafeValue(response , MkString("withdrawals") ); _ = withdrawals;
...
func (this *Buda) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostCurrenciesCurrencyWithdrawals"), this.Extend(request , params )); _ = response;
  withdrawal := this.SafeValue(response , MkString("withdrawal") ); _ = withdrawal;
...
func (this *Buda) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    if IsTrue(body ) {
      base64Body := this.StringToBase64(body ); _ = base64Body;
      components.Push(this.Decode(base64Body ));
    }
    components.Push(nonce );
    message := components.Join(MkString(" ") ); _ = message;
    signature := this.Hmac(this.Encode(message ), this.Encode(*this.At(MkString("secret")) ), MkString("sha384") ); _ = signature;
...
func (this *Buda) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpGtEq(code , MkInteger(400) )) {
    ...
    message := this.SafeString(response , MkString("message") , body ); _ = message;
