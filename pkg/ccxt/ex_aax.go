...
func (this *Aax) FetchStatus(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetAnnouncementMaintenance"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  timestamp := this.Milliseconds(); _ = timestamp;
  startTime := this.Parse8601(this.SafeString(data , MkString("startTime") )); _ = startTime;
  endTime := this.Parse8601(this.SafeString(data , MkString("endTime") )); _ = endTime;
  update := MkMap(&VarMap{"updated":this.SafeInteger(response , MkString("ts") , timestamp )}); _ = update;
  if IsTrue(OpNotEq2(endTime , MkUndefined() )) {
    startTimeIsOk := OpTrinary(OpEq2(startTime , MkUndefined() ), MkBool(true) , OpLw(timestamp , startTime )); _ = startTimeIsOk;
    isOk := OpOr(OpGt(timestamp , endTime ), startTimeIsOk ); _ = isOk;
...
func (this *Aax) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    ...
    active := OpEq2(status , MkString("enable") ); _ = active;
    ...
    quanto := OpCopy(MkUndefined() ); _ = quanto;
    spot := OpEq2(type_ , MkString("spot") ); _ = spot;
    futures := OpEq2(type_ , MkString("futures") ); _ = futures;
    settleType := this.SafeStringLower(market , MkString("settleType") ); _ = settleType;
...
func (this *Aax) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    ...
    id := this.SafeString(currency , MkString("chain") ); _ = id;
    name := this.SafeString(currency , MkString("displayName") ); _ = name;
    code := this.SafeCurrencyCode(id ); _ = code;
    precision := this.SafeNumber(currency , MkString("withdrawPrecision") ); _ = precision;
    enableWithdraw := this.SafeValue(currency , MkString("enableWithdraw") ); _ = enableWithdraw;
    enableDeposit := this.SafeValue(currency , MkString("enableDeposit") ); _ = enableDeposit;
    fee := this.SafeNumber(currency , MkString("withdrawFee") ); _ = fee;
    visible := this.SafeValue(currency , MkString("visible") ); _ = visible;
    active := OpAnd(enableWithdraw , OpAnd(enableDeposit , visible )); _ = active;
    network := this.SafeString(currency , MkString("network") ); _ = network;
...
func (this *Aax) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(ticker , MkString("t") ); _ = timestamp;
  marketId := this.SafeString(ticker , MkString("s") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market ); _ = symbol;
  last := this.SafeNumber(ticker , MkString("c") ); _ = last;
  open := this.SafeNumber(ticker , MkString("o") ); _ = open;
  ...
  quoteVolume := this.SafeNumber(ticker , MkString("v") ); _ = quoteVolume;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Aax) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  response := this.Call(MkString("publicGetMarketTickers"), params ); _ = response;
  tickers := this.SafeValue(response , MkString("tickers") , MkArray(&VarArray{})); _ = tickers;
  result := MkArray(&VarArray{}); _ = result;
  timestamp := this.SafeInteger(response , MkString("t") ); _ = timestamp;
  for i := MkInteger(0) ; IsTrue(OpLw(i , tickers.Length )); OpIncr(& i ){
    ticker := this.ParseTicker(this.Extend(*(tickers ).At(i ), MkMap(&VarMap{"t":timestamp }))); _ = ticker;
    result.Push(ticker );
  }
...
func (this *Aax) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeInteger(trade , MkString("t") ); _ = timestamp;
  if IsTrue(OpEq2(timestamp , MkUndefined() )) {
    timestamp = this.Parse8601(this.SafeString(trade , MkString("createTime") ));
  }
  id := this.SafeString2(trade , MkString("tid") , MkString("tradeID") ); _ = id;
  ...
  priceString := this.SafeString2(trade , MkString("p") , MkString("filledPrice") ); _ = priceString;
  amountString := this.SafeString2(trade , MkString("q") , MkString("filledQty") ); _ = amountString;
  orderId := this.SafeString(trade , MkString("orderID") ); _ = orderId;
  isTaker := this.SafeValue(trade , MkString("taker") ); _ = isTaker;
  ...
  orderType := this.ParseOrderType(this.SafeString(trade , MkString("orderType") )); _ = orderType;
...
func (this *Aax) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketTrades"), request ); _ = response;
...
func (this *Aax) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketHistoryCandles"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
...
func (this *Aax) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  types := this.SafeValue(*this.At(MkString("options")) , MkString("types") , MkMap(&VarMap{})); _ = types;
  purseType := this.SafeString(types , type_ , type_ ); _ = purseType;
  request := MkMap(&VarMap{"purseType":purseType }); _ = request;
  params = this.Omit(params , MkString("type") );
  response := this.Call(MkString("privateGetAccountBalances"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , data.Length )); OpIncr(& i ){
    ...
    balanceType := this.SafeString(balance , MkString("purseType") ); _ = balanceType;
...
func (this *Aax) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  timeInForce := this.SafeString(params , MkString("timeInForce") ); _ = timeInForce;
...
func (this *Aax) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"orderID":id }); _ = request;
  method := OpCopy(MkUndefined() ); _ = method;
  defaultType := this.SafeString2(*this.At(MkString("options")) , MkString("cancelOrder") , MkString("defaultType") , MkString("spot") ); _ = defaultType;
  type_ := this.SafeString(params , MkString("type") , defaultType ); _ = type_;
  params = this.Omit(params , MkString("type") );
...
func (this *Aax) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  *(params ).At(MkString("type") )= this.SafeString(params , MkString("type") , defaultType );
  ...
  orders := this.FetchOrders(symbol , MkUndefined() , MkUndefined() , this.Extend(request , params )); _ = orders;
...
func (this *Aax) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(method , this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  orders := this.SafeValue(data , MkString("list") , MkArray(&VarArray{})); _ = orders;
...
func (this *Aax) FetchClosedOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"orderStatus":MkString("2") }); _ = request;
...
func (this *Aax) FetchCanceledOrders(goArgs ...*Variant) *Variant{
  ...
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"orderStatus":MkString("3") }); _ = request;
  return this.FetchOrders(symbol , since , limit , this.Extend(request , params ));
}
...
func (this *Aax) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeValue(order , MkString("createTime") ); _ = timestamp;
  if IsTrue(OpEq2(OpType(timestamp ), MkString("string") )) {
    timestamp = this.Parse8601(timestamp );
  }
  status := this.ParseOrderStatus(this.SafeString(order , MkString("orderStatus") )); _ = status;
  ...
  postOnly := OpEq2(execInst , MkString("Post-Only") ); _ = postOnly;
  ...
  lastTradeTimestamp := this.SafeValue(order , MkString("transactTime") ); _ = lastTradeTimestamp;
...
func (this *Aax) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(method , this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  trades := this.SafeValue(data , MkString("list") , MkArray(&VarArray{})); _ = trades;
  return this.ParseTrades(trades , market , since , limit );
}
...
func (this *Aax) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetAccountDepositAddress"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
...
func (this *Aax) ParseDepositAddress(goArgs ...*Variant) *Variant{
  ...
  tag := this.SafeString(depositAddress , MkString("tag") ); _ = tag;
  ...
  if IsTrue(OpEq2(api , MkString("v1") )) {
    ...
    if IsTrue(OpEq2(api , MkString("public") )) {
      ...
      if IsTrue(OpEq2(api , MkString("private") )) {
        ...
        auth := OpAdd(nonce , OpAdd(MkString(":") , method )); _ = auth;
