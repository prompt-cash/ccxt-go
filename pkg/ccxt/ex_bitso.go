...
func (this *Bitso) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetAvailableBooks"), params ); _ = response;
  markets := this.SafeValue(response , MkString("payload") ); _ = markets;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    market := *(markets ).At(i ); _ = market;
    id := this.SafeString(market , MkString("book") ); _ = id;
    ...
    defaultPricePrecision := this.SafeNumber(*(*this.At(MkString("options")) ).At(MkString("precision") ), quote , *(*this.At(MkString("options")) ).At(MkString("defaultPrecision") )); _ = defaultPricePrecision;
    pricePrecision := this.SafeNumber(market , MkString("tick_size") , defaultPricePrecision ); _ = pricePrecision;
    precision := MkMap(&VarMap{
    ...
    fees := this.SafeValue(market , MkString("fees") , MkMap(&VarMap{})); _ = fees;
    flatRate := this.SafeValue(fees , MkString("flat_rate") , MkMap(&VarMap{})); _ = flatRate;
    makerString := this.SafeString(flatRate , MkString("maker") ); _ = makerString;
    takerString := this.SafeString(flatRate , MkString("taker") ); _ = takerString;
    ...
    feeTiers := this.SafeValue(fees , MkString("structure") , MkArray(&VarArray{})); _ = feeTiers;
    fee := MkMap(&VarMap{
          "taker":taker ,
          "maker":maker ,
    ...
    for j := MkInteger(0) ; IsTrue(OpLw(j , feeTiers.Length )); OpIncr(& j ){
      tier := *(feeTiers ).At(j ); _ = tier;
      volume := this.SafeNumber(tier , MkString("volume") ); _ = volume;
      takerFee := this.SafeNumber(tier , MkString("taker") ); _ = takerFee;
      makerFee := this.SafeNumber(tier , MkString("maker") ); _ = makerFee;
...
func (this *Bitso) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  payload := this.SafeValue(response , MkString("payload") , MkMap(&VarMap{})); _ = payload;
  balances := this.SafeValue(payload , MkString("balances") ); _ = balances;
  result := MkMap(&VarMap{
        "info":response ,
...
func (this *Bitso) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  request := MkMap(&VarMap{"book":this.MarketId(symbol )}); _ = request;
  response := this.Call(MkString("publicGetOrderBook"), this.Extend(request , params )); _ = response;
  orderbook := this.SafeValue(response , MkString("payload") ); _ = orderbook;
  timestamp := this.Parse8601(this.SafeString(orderbook , MkString("updated_at") )); _ = timestamp;
...
func (this *Bitso) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  ticker := this.SafeValue(response , MkString("payload") ); _ = ticker;
  timestamp := this.Parse8601(this.SafeString(ticker , MkString("created_at") )); _ = timestamp;
...
func (this *Bitso) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString(trade , MkString("book") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("_") ); _ = symbol;
  side := this.SafeString2(trade , MkString("side") , MkString("maker_side") ); _ = side;
  amount := this.SafeNumber2(trade , MkString("amount") , MkString("major") ); _ = amount;
  if IsTrue(OpNotEq2(amount , MkUndefined() )) {
    amount = Math.Abs(amount );
  }
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(trade , MkString("fees_amount") ); _ = feeCost;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyId := this.SafeString(trade , MkString("fees_currency") ); _ = feeCurrencyId;
  ...
  cost := this.SafeNumber(trade , MkString("minor") ); _ = cost;
  if IsTrue(OpNotEq2(cost , MkUndefined() )) {
    cost = Math.Abs(cost );
  }
  price := this.SafeNumber(trade , MkString("price") ); _ = price;
  orderId := this.SafeString(trade , MkString("oid") ); _ = orderId;
...
func (this *Bitso) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"book":*(market ).At(MkString("id") )}); _ = request;
...
func (this *Bitso) FetchMyTrades(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined() ); _ = symbol;
  since := GoGetArg(goArgs, 1, MkUndefined() ); _ = since;
  limit := GoGetArg(goArgs, 2, MkInteger(25) ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  markerInParams := OpHasMember(MkString("marker") , params ); _ = markerInParams;
...
func (this *Bitso) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString(*(response ).At(MkString("payload") ), MkString("oid") ); _ = id;
  return MkMap(&VarMap{
        "info":response ,
        "id":id ,
        });
...
func (this *Bitso) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"oid":id }); _ = request;
...
func (this *Bitso) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  id := this.SafeString(order , MkString("oid") ); _ = id;
  ...
  marketId := this.SafeString(order , MkString("book") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("_") ); _ = symbol;
  ...
  remaining := this.SafeNumber(order , MkString("unfilled_amount") ); _ = remaining;
  clientOrderId := this.SafeString(order , MkString("client_id") ); _ = clientOrderId;
...
func (this *Bitso) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  orders := this.ParseOrders(*(response ).At(MkString("payload") ), market , since , limit ); _ = orders;
  return orders ;
}
...
func (this *Bitso) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetOrdersOid"), MkMap(&VarMap{"oid":id })); _ = response;
  payload := this.SafeValue(response , MkString("payload") ); _ = payload;
  if IsTrue(GoIsArray(payload )) {
    numOrders := *(*(response ).At(MkString("payload") )).At(MkString("length") ); _ = numOrders;
...
func (this *Bitso) FetchOrderTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"oid":id }); _ = request;
  response := this.Call(MkString("privateGetOrderTradesOid"), this.Extend(request , params )); _ = response;
...
func (this *Bitso) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"fund_currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateGetFundingDestination"), this.Extend(request , params )); _ = response;
  address := this.SafeString(*(response ).At(MkString("payload") ), MkString("account_identifier") ); _ = address;
  tag := OpCopy(MkUndefined() ); _ = tag;
  if IsTrue(OpGtEq(address.IndexOf(MkString("?dt=") ), MkInteger(0) )) {
    parts := address.Split(MkString("?dt=") ); _ = parts;
...
func (this *Bitso) Withdraw(goArgs ...*Variant) *Variant{
  ...
  method := OpTrinary(OpHasMember(code , methods ), *(methods ).At(code ), MkUndefined() ); _ = method;
  ...
  classMethod := OpAdd(MkString("privatePost") , OpAdd(method , MkString("Withdrawal") )); _ = classMethod;
  response := this.Call(classMethod , this.Extend(request , params )); _ = response;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Bitso) Sign(goArgs ...*Variant) *Variant{
  ...
  endpoint := OpAdd(MkString("/") , OpAdd(*this.At(MkString("version")) , OpAdd(MkString("/") , this.ImplodeParams(path , params )))); _ = endpoint;
  ...
  url := OpAdd(*(*this.At(MkString("urls")) ).At(MkString("api") ), endpoint ); _ = url;
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    request := (MkArray(&VarArray{
            ...
            method ,
            endpoint ,
            })).Call(MkString("join") , MkString("") ); _ = request;
    ...
    signature := this.Hmac(this.Encode(request ), this.Encode(*this.At(MkString("secret")) )); _ = signature;
    auth := OpAdd(*this.At(MkString("apiKey")) , OpAdd(MkString(":") , OpAdd(nonce , OpAdd(MkString(":") , signature )))); _ = auth;
...
func (this *Bitso) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("success") , response )) {
    ...
    if IsTrue(OpNot(success )) {
      ...
      code := this.SafeString(error , MkString("code") ); _ = code;
