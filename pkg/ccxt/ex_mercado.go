...
func (this *Mercado) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = result
	amountLimits := this.SafeValue(*this.At(MkString("options")), MkString("limits"), MkMap(&VarMap{}))
	_ = amountLimits
...
func (this *Mercado) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"coin": *(market).At(MkString("base"))})
	_ = request
	response := this.Call(MkString("publicGetCoinOrderbook"), this.Extend(request, params))
	_ = response
...
func (this *Mercado) FetchTicker(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"coin": *(market).At(MkString("base"))})
	_ = request
	response := this.Call(MkString("publicGetCoinTicker"), this.Extend(request, params))
	_ = response
...
func (this *Mercado) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = market
	timestamp := this.SafeTimestamp2(trade, MkString("date"), MkString("executed_timestamp"))
	_ = timestamp
	...
	id := this.SafeString2(trade, MkString("tid"), MkString("operation_id"))
	_ = id
	...
	_ = priceString
	amountString := this.SafeString2(trade, MkString("amount"), MkString("quantity"))
	_ = amountString
	...
	_ = cost
	feeCost := this.SafeNumber(trade, MkString("fee_rate"))
	_ = feeCost
...
func (this *Mercado) FetchTrades(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	method := MkString("publicGetCoinTrades")
	_ = method
	request := MkMap(&VarMap{"coin": *(market).At(MkString("base"))})
	_ = request
	...
	to := this.SafeInteger(params, MkString("to"))
	_ = to
...
func (this *Mercado) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privatePostGetAccountInfo"), params)
	_ = response
	data := this.SafeValue(response, MkString("response_data"), MkMap(&VarMap{}))
	_ = data
	balances := this.SafeValue(data, MkString("balance"), MkMap(&VarMap{}))
	_ = balances
...
func (this *Mercado) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"coin_pair": this.MarketId(symbol)})
	_ = request
	method := OpAdd(this.Capitalize(side), MkString("Order"))
	_ = method
...
func (this *Mercado) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = response
	responseData := this.SafeValue(response, MkString("response_data"), MkMap(&VarMap{}))
	_ = responseData
	order := this.SafeValue(responseData, MkString("order"), MkMap(&VarMap{}))
	_ = order
...
func (this *Mercado) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = id
	order_type := this.SafeString(order, MkString("order_type"))
	_ = order_type
	...
	_ = status
	marketId := this.SafeString(order, MkString("coin_pair"))
	_ = marketId
	market = this.SafeMarket(marketId, market)
	timestamp := this.SafeTimestamp(order, MkString("created_timestamp"))
	_ = timestamp
	fee := MkMap(&VarMap{
	...
	_ = price
	average := this.SafeNumber(order, MkString("executed_price_avg"))
	_ = average
	amount := this.SafeNumber(order, MkString("quantity"))
	_ = amount
	filled := this.SafeNumber(order, MkString("executed_quantity"))
	_ = filled
	lastTradeTimestamp := this.SafeTimestamp(order, MkString("updated_timestamp"))
	_ = lastTradeTimestamp
	rawTrades := this.SafeValue(order, MkString("operations"), MkArray(&VarArray{}))
	_ = rawTrades
...
func (this *Mercado) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostGetOrder"), this.Extend(request, params))
	_ = response
	responseData := this.SafeValue(response, MkString("response_data"), MkMap(&VarMap{}))
	_ = responseData
	order := this.SafeValue(responseData, MkString("order"))
	_ = order
	return this.ParseOrder(order, market)
}
...
func (this *Mercado) Withdraw(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(code, MkString("BRL"))) {
		account_ref := OpHasMember(MkString("account_ref"), params)
		_ = account_ref
		...
		if IsTrue(OpNotEq2(code, MkString("LTC"))) {
			tx_fee := OpHasMember(MkString("tx_fee"), params)
			_ = tx_fee
...
func (this *Mercado) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("v4PublicGetCoinCandle"), this.Extend(request, params))
	_ = response
...
func (this *Mercado) FetchOrders(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	request := MkMap(&VarMap{"coin_pair": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("privatePostListOrders"), this.Extend(request, params))
	_ = response
	responseData := this.SafeValue(response, MkString("response_data"), MkMap(&VarMap{}))
	_ = responseData
	orders := this.SafeValue(responseData, MkString("orders"), MkArray(&VarArray{}))
	_ = orders
...
func (this *Mercado) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	responseData := this.SafeValue(response, MkString("response_data"), MkMap(&VarMap{}))
	_ = responseData
	ordersRaw := this.SafeValue(responseData, MkString("orders"), MkArray(&VarArray{}))
	_ = ordersRaw
	orders := this.ParseOrders(ordersRaw, market, since, limit)
	_ = orders
...
func (this *Mercado) OrdersToTrades(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, orders.Length)); OpIncr(&i) {
		trades := this.SafeValue(*(orders).At(i), MkString("trades"), MkArray(&VarArray{}))
		_ = trades
...
func (this *Mercado) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpOr(OpEq2(api, MkString("public")), OpEq2(api, MkString("v4Public")))) {
		...
		auth := OpAdd(MkString("/tapi/"), OpAdd(*this.At(MkString("version")), OpAdd(MkString("/"), OpAdd(MkString("?"), body))))
		_ = auth
...
func (this *Mercado) HandleErrors(goArgs ...*Variant) *Variant {
	...
	errorMessage := this.SafeValue(response, MkString("error_message"))
	_ = errorMessage
