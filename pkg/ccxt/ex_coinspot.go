...
func (this *Coinspot) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  method := this.SafeString(*this.At(MkString("options")) , MkString("fetchBalance") , MkString("private_post_my_balances") ); _ = method;
  response := this.Call(method , params ); _ = response;
  result := MkMap(&VarMap{"info":response }); _ = result;
  balances := this.SafeValue2(response , MkString("balance") , MkString("balances") ); _ = balances;
  if IsTrue(GoIsArray(balances )) {
    for i := MkInteger(0) ; IsTrue(OpLw(i , balances.Length )); OpIncr(& i ){
      currencies := *(balances ).At(i ); _ = currencies;
      currencyIds := GoGetKeys(currencies ); _ = currencyIds;
      for j := MkInteger(0) ; IsTrue(OpLw(j , currencyIds.Length )); OpIncr(& j ){
        currencyId := *(currencyIds ).At(j ); _ = currencyId;
        balance := *(currencies ).At(currencyId ); _ = balance;
...
func (this *Coinspot) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"cointype":*(market ).At(MkString("id") )}); _ = request;
  orderbook := this.Call(MkString("privatePostOrders"), this.Extend(request , params )); _ = orderbook;
...
func (this *Coinspot) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  response := this.Call(MkString("publicGetLatest"), params ); _ = response;
  id := this.MarketId(symbol ); _ = id;
  id = id.ToLowerCase();
  ticker := *(*(response ).At(MkString("prices") )).At(id ); _ = ticker;
...
func (this *Coinspot) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"cointype":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privatePostOrdersHistory"), this.Extend(request , params )); _ = response;
  trades := this.SafeValue(response , MkString("orders") , MkArray(&VarArray{})); _ = trades;
...
func (this *Coinspot) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(trade , MkString("solddate") ); _ = timestamp;
...
func (this *Coinspot) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  method := OpAdd(MkString("privatePostMy") , this.Capitalize(side )); _ = method;
...
func (this *Coinspot) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  method := OpAdd(MkString("privatePostMy") , OpAdd(this.Capitalize(side ), MkString("Cancel") )); _ = method;
