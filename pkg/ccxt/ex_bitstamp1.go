...
func (this *Bitstamp1) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	this.LoadMarkets()
	orderbook := this.Call(MkString("publicGetOrderBook"), params)
	_ = orderbook
...
func (this *Bitstamp1) FetchTicker(goArgs ...*Variant) *Variant {
	...
	this.LoadMarkets()
	ticker := this.Call(MkString("publicGetTicker"), params)
	_ = ticker
...
func (this *Bitstamp1) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = market
	timestamp := this.SafeTimestamp2(trade, MkString("date"), MkString("datetime"))
	_ = timestamp
	side := OpTrinary(OpEq2(*(trade).At(MkString("type")), MkInteger(0)), MkString("buy"), MkString("sell"))
	_ = side
...
func (this *Bitstamp1) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"time": MkString("minute")})
	_ = request
...
func (this *Bitstamp1) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	_ = pair
	request := MkMap(&VarMap{"id": pair})
	_ = request
	response := this.Call(MkString("privatePostOpenOrdersId"), this.Extend(request, params))
	_ = response
	return this.ParseTrades(response, market, since, limit)
}
...
func (this *Bitstamp1) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		_ = auth
		signature := this.Encode(this.Hmac(this.Encode(auth), this.Encode(*this.At(MkString("secret")))))
		_ = signature
