...
func (this *Bitmart) FetchTime(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicSystemGetTime"), params ); _ = response;
...
func (this *Bitmart) FetchStatus(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicSystemGetService"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  services := this.SafeValue(data , MkString("service") , MkArray(&VarArray{})); _ = services;
  servicesByType := this.IndexBy(services , MkString("service_type") ); _ = servicesByType;
  if IsTrue(OpOr(OpEq2(type_ , MkString("swap") ), OpEq2(type_ , MkString("future") ))) {
    type_ = MkString("contract") ;
  }
  service := this.SafeValue(servicesByType , type_ ); _ = service;
  status := OpCopy(MkUndefined() ); _ = status;
  eta := OpCopy(MkUndefined() ); _ = eta;
  if IsTrue(OpNotEq2(service , MkUndefined() )) {
    statusCode := this.SafeInteger(service , MkString("status") ); _ = statusCode;
...
func (this *Bitmart) FetchSpotMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicSpotGetSymbolsDetails"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  symbols := this.SafeValue(data , MkString("symbols") , MkArray(&VarArray{})); _ = symbols;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , symbols.Length )); OpIncr(& i ){
    market := *(symbols ).At(i ); _ = market;
    id := this.SafeString(market , MkString("symbol") ); _ = id;
    numericId := this.SafeInteger(market , MkString("symbol_id") ); _ = numericId;
    ...
    pricePrecision := this.SafeInteger(market , MkString("price_max_precision") ); _ = pricePrecision;
    ...
    minBuyCost := this.SafeNumber(market , MkString("min_buy_amount") ); _ = minBuyCost;
    minSellCost := this.SafeNumber(market , MkString("min_sell_amount") ); _ = minSellCost;
    minCost := Math.Max(minBuyCost , minSellCost ); _ = minCost;
    limits := MkMap(&VarMap{
          "amount":MkMap(&VarMap{
...
func (this *Bitmart) FetchContractMarkets(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicContractGetContracts"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  contracts := this.SafeValue(data , MkString("contracts") , MkArray(&VarArray{})); _ = contracts;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , contracts.Length )); OpIncr(& i ){
    market := *(contracts ).At(i ); _ = market;
    contract := this.SafeValue(market , MkString("contract") , MkMap(&VarMap{})); _ = contract;
    id := this.SafeString(contract , MkString("contract_id") ); _ = id;
    numericId := this.SafeInteger(contract , MkString("contract_id") ); _ = numericId;
    baseId := this.SafeString(contract , MkString("base_coin") ); _ = baseId;
    quoteId := this.SafeString(contract , MkString("quote_coin") ); _ = quoteId;
    base := this.SafeCurrencyCode(baseId ); _ = base;
    quote := this.SafeCurrencyCode(quoteId ); _ = quote;
    symbol := this.SafeString(contract , MkString("name") ); _ = symbol;
    amountPrecision := this.SafeNumber(contract , MkString("vol_unit") ); _ = amountPrecision;
    pricePrecision := this.SafeNumber(contract , MkString("price_unit") ); _ = pricePrecision;
    precision := MkMap(&VarMap{
    ...
    contractType := this.SafeValue(contract , MkString("contract_type") ); _ = contractType;
    future := OpCopy(MkBool(false) ); _ = future;
    swap := OpCopy(MkBool(false) ); _ = swap;
    type_ := MkString("contract") ; _ = type_;
    ...
    feeConfig := this.SafeValue(market , MkString("fee_config") , MkMap(&VarMap{})); _ = feeConfig;
    maker := this.SafeNumber(feeConfig , MkString("maker_fee") ); _ = maker;
    taker := this.SafeNumber(feeConfig , MkString("taker_fee") ); _ = taker;
    result.Push(MkMap(&VarMap{
            "id":id ,
            "numericId":numericId ,
            "symbol":symbol ,
...
func (this *Bitmart) FetchFundingFee(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateAccountGetWithdrawCharge"), this.Extend(request , params )); _ = response;
...
func (this *Bitmart) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeTimestamp(ticker , MkString("timestamp") , this.Milliseconds()); _ = timestamp;
  marketId := this.SafeString2(ticker , MkString("symbol") , MkString("contract_id") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("_") ); _ = symbol;
  last := this.SafeNumber2(ticker , MkString("close_24h") , MkString("last_price") ); _ = last;
  percentage := this.SafeNumber(ticker , MkString("fluctuation") , MkString("rise_fall_rate") ); _ = percentage;
  if IsTrue(OpNotEq2(percentage , MkUndefined() )) {
    OpMultiAssign(& percentage , MkInteger(100) );
  }
  baseVolume := this.SafeNumber2(ticker , MkString("base_volume_24h") , MkString("base_coin_volume") ); _ = baseVolume;
  quoteVolume := this.SafeNumber2(ticker , MkString("quote_volume_24h") , MkString("quote_coin_volume") ); _ = quoteVolume;
  vwap := this.Vwap(baseVolume , quoteVolume ); _ = vwap;
  open := this.SafeNumber2(ticker , MkString("open_24h") , MkString("open") ); _ = open;
  ...
  price := this.SafeValue(ticker , MkString("depth_price") , ticker ); _ = price;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
...
func (this *Bitmart) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  tickers := this.SafeValue(data , MkString("tickers") , MkArray(&VarArray{})); _ = tickers;
  tickersById := this.IndexBy(tickers , MkString("symbol") ); _ = tickersById;
  ticker := this.SafeValue(tickersById , *(market ).At(MkString("id") )); _ = ticker;
...
func (this *Bitmart) FetchCurrencies(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  response := this.Call(MkString("publicAccountGetCurrencies"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencies.Length )); OpIncr(& i ){
    ...
    withdrawEnabled := this.SafeValue(currency , MkString("withdraw_enabled") ); _ = withdrawEnabled;
    depositEnabled := this.SafeValue(currency , MkString("deposit_enabled") ); _ = depositEnabled;
    active := OpAnd(withdrawEnabled , depositEnabled ); _ = active;
...
func (this *Bitmart) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  id := this.SafeString2(trade , MkString("trade_id") , MkString("detail_id") ); _ = id;
  timestamp := this.SafeInteger2(trade , MkString("order_time") , MkString("create_time") ); _ = timestamp;
  ...
  way := this.SafeInteger(trade , MkString("way") ); _ = way;
  side := this.SafeStringLower2(trade , MkString("type") , MkString("side") ); _ = side;
  ...
  execType := this.SafeString(trade , MkString("exec_type") ); _ = execType;
  if IsTrue(OpNotEq2(execType , MkUndefined() )) {
    takerOrMaker = OpTrinary(OpEq2(execType , MkString("M") ), MkString("maker") , MkString("taker") );
  }
  price := this.SafeNumber2(trade , MkString("price") , MkString("deal_price") ); _ = price;
  price = this.SafeNumber(trade , MkString("price_avg") , price );
  amount := this.SafeNumber2(trade , MkString("amount") , MkString("deal_vol") ); _ = amount;
  amount = this.SafeNumber(trade , MkString("size") , amount );
  cost := this.SafeNumber2(trade , MkString("count") , MkString("notional") ); _ = cost;
  ...
  orderId := this.SafeInteger(trade , MkString("order_id") ); _ = orderId;
  marketId := this.SafeString2(trade , MkString("contract_id") , MkString("symbol") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("_") ); _ = symbol;
  feeCost := this.SafeNumber(trade , MkString("fees") ); _ = feeCost;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyId := this.SafeString(trade , MkString("fee_coin_name") ); _ = feeCurrencyId;
...
func (this *Bitmart) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(GoIsArray(data )) {
    ...
    klines := this.SafeValue(data , MkString("klines") , MkArray(&VarArray{})); _ = klines;
...
func (this *Bitmart) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(method , params ); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  wallet := this.SafeValue2(data , MkString("wallet") , MkString("accounts") , MkArray(&VarArray{})); _ = wallet;
  result := MkMap(&VarMap{"info":response }); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , wallet.Length )); OpIncr(& i ){
    balance := *(wallet ).At(i ); _ = balance;
    currencyId := this.SafeString2(balance , MkString("id") , MkString("currency") ); _ = currencyId;
...
func (this *Bitmart) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  marketId := this.SafeString2(order , MkString("symbol") , MkString("contract_id") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("_") ); _ = symbol;
  status := OpCopy(MkUndefined() ); _ = status;
  ...
  average := this.SafeNumber2(order , MkString("price_avg") , MkString("done_avg_price") ); _ = average;
  amount := this.SafeNumber2(order , MkString("size") , MkString("vol") ); _ = amount;
  filled := this.SafeNumber2(order , MkString("filled_size") , MkString("done_vol") ); _ = filled;
  side := this.SafeString(order , MkString("side") ); _ = side;
  side = this.SafeString(order , MkString("way") , side );
  category := this.SafeInteger(order , MkString("category") ); _ = category;
...
func (this *Bitmart) ParseOrderStatusByType(goArgs ...*Variant) *Variant{
  ...
  status := GoGetArg(goArgs, 1, MkUndefined()); _ = status;
...
func (this *Bitmart) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  succeeded := this.SafeValue(data , MkString("succeed") ); _ = succeeded;
  if IsTrue(OpNotEq2(succeeded , MkUndefined() )) {
    ...
    result := this.SafeValue(data , MkString("result") ); _ = result;
  ...
  order := this.ParseOrder(id , market ); _ = order;
...
func (this *Bitmart) CancelAllOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateSpotPostCancelOrders"), this.Extend(request , params )); _ = response;
  return response ;
}
...
func (this *Bitmart) CancelOrders(goArgs ...*Variant) *Variant{
  ...
  request := MkMap(&VarMap{"orders":orders }); _ = request;
  response := this.Call(MkString("privateContractPostCancelOrders"), this.Extend(request , params )); _ = response;
  return response ;
}
...
func (this *Bitmart) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpHasMember(MkString("orders") , data )) {
    orders := this.SafeValue(data , MkString("orders") , MkArray(&VarArray{})); _ = orders;
    firstOrder := this.SafeValue(orders , MkInteger(0) ); _ = firstOrder;
...
func (this *Bitmart) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("privateAccountGetDepositAddress"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  address := this.SafeString(data , MkString("address") ); _ = address;
  tag := this.SafeString(data , MkString("address_memo") ); _ = tag;
...
func (this *Bitmart) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateAccountPostWithdrawApply"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  transaction := this.ParseTransaction(data , currency ); _ = transaction;
...
func (this *Bitmart) FetchTransactionsByType(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateAccountGetDepositWithdrawHistory"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  records := this.SafeValue(data , MkString("records") , MkArray(&VarArray{})); _ = records;
...
func (this *Bitmart) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  id := OpCopy(MkUndefined() ); _ = id;
  withdrawId := this.SafeString(transaction , MkString("withdraw_id") ); _ = withdrawId;
  depositId := this.SafeString(transaction , MkString("deposit_id") ); _ = depositId;
  type_ := OpCopy(MkUndefined() ); _ = type_;
  ...
  amount := this.SafeNumber(transaction , MkString("arrival_amount") ); _ = amount;
  timestamp := this.SafeInteger(transaction , MkString("apply_time") ); _ = timestamp;
...
func (this *Bitmart) Sign(goArgs ...*Variant) *Variant{
  ...
  baseUrl := this.ImplodeHostname(*(*this.At(MkString("urls")) ).At(MkString("api") )); _ = baseUrl;
  access := this.SafeString(api , MkInteger(0) ); _ = access;
  type_ := this.SafeString(api , MkInteger(1) ); _ = type_;
  url := OpAdd(baseUrl , OpAdd(MkString("/") , type_ )); _ = url;
  ...
  if IsTrue(OpEq2(type_ , MkString("system") )) {
    ...
    if IsTrue(OpEq2(access , MkString("public") )) {
      ...
      if IsTrue(OpEq2(access , MkString("private") )) {
        ...
        auth := OpAdd(timestamp , OpAdd(MkString("#") , OpAdd(*this.At(MkString("uid")) , OpAdd(MkString("#") , queryString )))); _ = auth;
