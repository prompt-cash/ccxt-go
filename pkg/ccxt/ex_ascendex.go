...
func (this *Ascendex) GetAccount(goArgs ...*Variant) *Variant {
	...
	_ = params
	account := this.SafeValue(params, MkString("account"), *(*this.At(MkString("options"))).At(MkString("account")))
	_ = account
...
func (this *Ascendex) FetchCurrencies(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	assets := this.Call(MkString("publicGetAssets"), params)
	_ = assets
	margin := this.Call(MkString("publicGetMarginAssets"), params)
	_ = margin
	cash := this.Call(MkString("publicGetCashAssets"), params)
	_ = cash
	assetsData := this.SafeValue(assets, MkString("data"), MkArray(&VarArray{}))
	_ = assetsData
	marginData := this.SafeValue(margin, MkString("data"), MkArray(&VarArray{}))
	_ = marginData
	cashData := this.SafeValue(cash, MkString("data"), MkArray(&VarArray{}))
	_ = cashData
	assetsById := this.IndexBy(assetsData, MkString("assetCode"))
	_ = assetsById
	marginById := this.IndexBy(marginData, MkString("assetCode"))
	_ = marginById
	cashById := this.IndexBy(cashData, MkString("assetCode"))
	_ = cashById
	dataById := this.DeepExtend(assetsById, marginById, cashById)
	_ = dataById
	ids := GoGetKeys(dataById)
	_ = ids
	result := MkMap(&VarMap{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		id := *(ids).At(i)
		_ = id
		currency := *(dataById).At(id)
		_ = currency
		code := this.SafeCurrencyCode(id)
		_ = code
		precision := this.SafeString2(currency, MkString("precisionScale"), MkString("nativeScale"))
		_ = precision
		minAmount := this.ParsePrecision(precision)
		_ = minAmount
		fee := this.SafeNumber2(currency, MkString("withdrawFee"), MkString("withdrawalFee"))
		_ = fee
		status := this.SafeString2(currency, MkString("status"), MkString("statusCode"))
		_ = status
		active := OpEq2(status, MkString("Normal"))
		_ = active
		margin := OpHasMember(MkString("borrowAssetCode"), currency)
		_ = margin
...
func (this *Ascendex) FetchMarkets(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	products := this.Call(MkString("publicGetProducts"), params)
	_ = products
	cash := this.Call(MkString("publicGetCashProducts"), params)
	_ = cash
	futures := this.Call(MkString("publicGetFuturesContracts"), params)
	_ = futures
	productsData := this.SafeValue(products, MkString("data"), MkArray(&VarArray{}))
	_ = productsData
	productsById := this.IndexBy(productsData, MkString("symbol"))
	_ = productsById
	cashData := this.SafeValue(cash, MkString("data"), MkArray(&VarArray{}))
	_ = cashData
	futuresData := this.SafeValue(futures, MkString("data"), MkArray(&VarArray{}))
	_ = futuresData
	cashAndFuturesData := this.ArrayConcat(cashData, futuresData)
	_ = cashAndFuturesData
	cashAndFuturesById := this.IndexBy(cashAndFuturesData, MkString("symbol"))
	_ = cashAndFuturesById
	dataById := this.DeepExtend(productsById, cashAndFuturesById)
	_ = dataById
	ids := GoGetKeys(dataById)
	_ = ids
	...
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		...
		active := OpEq2(status, MkString("Normal"))
		_ = active
		type_ := OpTrinary(OpHasMember(MkString("useLot"), market), MkString("spot"), MkString("future"))
		_ = type_
		spot := OpEq2(type_, MkString("spot"))
		_ = spot
		future := OpEq2(type_, MkString("future"))
		_ = future
		margin := this.SafeValue(market, MkString("marginTradable"), MkBool(false))
		_ = margin
		symbol := OpCopy(id)
		_ = symbol
		...
		fee := this.SafeNumber(market, MkString("commissionReserveRate"))
		_ = fee
...
func (this *Ascendex) FetchAccounts(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	accountGroup := this.SafeString(*this.At(MkString("options")), MkString("account-group"))
	_ = accountGroup
...
func (this *Ascendex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	this.LoadMarkets()
	this.LoadAccounts()
	defaultAccountCategory := this.SafeString(*this.At(MkString("options")), MkString("account-category"), MkString("cash"))
	_ = defaultAccountCategory
	options := this.SafeValue(*this.At(MkString("options")), MkString("fetchBalance"), MkMap(&VarMap{}))
	_ = options
	accountCategory := this.SafeString(options, MkString("account-category"), defaultAccountCategory)
	_ = accountCategory
	...
	account := this.SafeValue(*this.At(MkString("accounts")), MkInteger(0), MkMap(&VarMap{}))
	_ = account
	accountGroup := this.SafeString(account, MkString("id"))
	_ = accountGroup
	request := MkMap(&VarMap{"account-group": accountGroup})
	_ = request
	method := MkString("accountCategoryGetBalance")
	_ = method
	...
	_ = result
	balances := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = balances
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		balance := *(balances).At(i)
		_ = balance
		code := this.SafeCurrencyCode(this.SafeString(balance, MkString("asset")))
		_ = code
...
func (this *Ascendex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	orderbook := this.SafeValue(data, MkString("data"), MkMap(&VarMap{}))
	_ = orderbook
	timestamp := this.SafeInteger(orderbook, MkString("ts"))
	_ = timestamp
	result := this.ParseOrderBook(orderbook, symbol, timestamp)
	_ = result
...
func (this *Ascendex) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = close
	bid := this.SafeValue(ticker, MkString("bid"), MkArray(&VarArray{}))
	_ = bid
	ask := this.SafeValue(ticker, MkString("ask"), MkArray(&VarArray{}))
	_ = ask
...
func (this *Ascendex) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	_ = options
	defaultLimit := this.SafeInteger(options, MkString("limit"), MkInteger(500))
	_ = defaultLimit
	...
	response := this.Call(MkString("publicGetBarhist"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
...
func (this *Ascendex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	priceString := this.SafeString2(trade, MkString("price"), MkString("p"))
	_ = priceString
	amountString := this.SafeString(trade, MkString("q"))
	_ = amountString
	...
	_ = cost
	buyerIsMaker := this.SafeValue(trade, MkString("bm"), MkBool(false))
	_ = buyerIsMaker
	makerOrTaker := OpTrinary(buyerIsMaker, MkString("maker"), MkString("taker"))
	_ = makerOrTaker
	side := OpTrinary(buyerIsMaker, MkString("buy"), MkString("sell"))
	_ = side
	symbol := OpCopy(MkUndefined())
	_ = symbol
	if IsTrue(OpAnd(OpEq2(symbol, MkUndefined()), OpNotEq2(market, MkUndefined()))) {
...
func (this *Ascendex) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = response
	records := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = records
	trades := this.SafeValue(records, MkString("data"), MkArray(&VarArray{}))
	_ = trades
...
func (this *Ascendex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = symbol
	timestamp := this.SafeInteger2(order, MkString("timestamp"), MkString("sendingTime"))
	_ = timestamp
	lastTradeTimestamp := this.SafeInteger(order, MkString("lastExecTime"))
	...
	_ = average
	filled := this.SafeNumber2(order, MkString("cumFilledQty"), MkString("cumQty"))
	_ = filled
	id := this.SafeString(order, MkString("orderId"))
	_ = id
	clientOrderId := this.SafeString(order, MkString("id"))
	_ = clientOrderId
	...
	_ = side
	feeCost := this.SafeNumber(order, MkString("cumFee"))
	_ = feeCost
	fee := OpCopy(MkUndefined())
	_ = fee
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCurrencyId := this.SafeString(order, MkString("feeAsset"))
		_ = feeCurrencyId
...
func (this *Ascendex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	defaultAccountCategory := this.SafeString(*this.At(MkString("options")), MkString("account-category"), MkString("cash"))
	_ = defaultAccountCategory
	options := this.SafeValue(*this.At(MkString("options")), MkString("createOrder"), MkMap(&VarMap{}))
	_ = options
	accountCategory := this.SafeString(options, MkString("account-category"), defaultAccountCategory)
	_ = accountCategory
	accountCategory = this.SafeString(params, MkString("account-category"), accountCategory)
	...
	account := this.SafeValue(*this.At(MkString("accounts")), MkInteger(0), MkMap(&VarMap{}))
	_ = account
	accountGroup := this.SafeValue(account, MkString("id"))
	_ = accountGroup
	clientOrderId := this.SafeString2(params, MkString("clientOrderId"), MkString("id"))
	_ = clientOrderId
	request := MkMap(&VarMap{
	...
	response := this.Call(MkString("accountCategoryPostOrder"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	info := this.SafeValue(data, MkString("info"), MkMap(&VarMap{}))
	_ = info
...
func (this *Ascendex) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("accountCategoryGetOrderStatus"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
...
func (this *Ascendex) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("accountCategoryGetOrderOpen"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
	if IsTrue(OpEq2(accountCategory, MkString("futures"))) {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, data.Length)); OpIncr(&i) {
		order := this.ParseOrder(*(data).At(i), market)
		_ = order
		orders.Push(order)
	}
...
func (this *Ascendex) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	defaultAccountCategory := this.SafeString(*this.At(MkString("options")), MkString("account-category"))
	_ = defaultAccountCategory
	...
	method := this.SafeValue(options, MkString("method"), MkString("accountGroupGetOrderHist"))
	_ = method
...
func (this *Ascendex) CancelOrder(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("accountCategoryDeleteOrder"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	info := this.SafeValue(data, MkString("info"), MkMap(&VarMap{}))
...
func (this *Ascendex) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("accountCategoryDeleteOrderAll"), this.Extend(request, params))
	_ = response
	return response
}
...
func (this *Ascendex) ParseDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = address
	tagId := this.SafeString(depositAddress, MkString("tagId"))
	_ = tagId
	tag := this.SafeString(depositAddress, tagId)
	_ = tag
...
func (this *Ascendex) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = currency
	chainName := this.SafeString(params, MkString("chainName"))
	_ = chainName
	params = this.Omit(params, MkString("chainName"))
	request := MkMap(&VarMap{"asset": *(currency).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("privateGetWalletDepositAddress"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	addresses := this.SafeValue(data, MkString("address"), MkArray(&VarArray{}))
	_ = addresses
	numAddresses := OpCopy(addresses.Length)
	_ = numAddresses
	address := OpCopy(MkUndefined())
	_ = address
	if IsTrue(OpGt(numAddresses, MkInteger(1))) {
		addressesByChainName := this.IndexBy(addresses, MkString("chainName"))
		_ = addressesByChainName
		if IsTrue(OpEq2(chainName, MkUndefined())) {
			chainNames := GoGetKeys(addressesByChainName)
			_ = chainNames
			chains := chainNames.Join(MkString(", "))
			_ = chains
	...
	result := this.ParseDepositAddress(address, currency)
	_ = result
...
func (this *Ascendex) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"txType": MkString("deposit")})
	_ = request
...
func (this *Ascendex) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"txType": MkString("withdrawal")})
	_ = request
	return this.FetchTransactions(code, since, limit, this.Extend(request, params))
}
...
func (this *Ascendex) FetchTransactions(goArgs ...*Variant) *Variant {
	...
	data := this.SafeValue(response, MkString("data"), MkMap(&VarMap{}))
	_ = data
	transactions := this.SafeValue(data, MkString("data"), MkArray(&VarArray{}))
	_ = transactions
...
func (this *Ascendex) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	currency := GoGetArg(goArgs, 1, MkUndefined())
	_ = currency
	id := this.SafeString(transaction, MkString("requestId"))
	_ = id
	amount := this.SafeNumber(transaction, MkString("amount"))
	_ = amount
	destAddress := this.SafeValue(transaction, MkString("destAddress"), MkMap(&VarMap{}))
	_ = destAddress
	address := this.SafeString(destAddress, MkString("address"))
	_ = address
	tag := this.SafeString(destAddress, MkString("destTag"))
	_ = tag
	txid := this.SafeString(transaction, MkString("networkTransactionId"))
	_ = txid
	type_ := this.SafeString(transaction, MkString("transactionType"))
	_ = type_
...
func (this *Ascendex) Sign(goArgs ...*Variant) *Variant {
	...
	query := OpCopy(params)
	_ = query
	accountCategory := OpEq2(api, MkString("accountCategory"))
	_ = accountCategory
	...
	request := this.ImplodeParams(path, query)
	_ = request
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		_ = timestamp
		payload := OpAdd(timestamp, OpAdd(MkString("+"), request))
		_ = payload
		hmac := this.Hmac(this.Encode(payload), this.Encode(*this.At(MkString("secret"))), MkString("sha256"), MkString("base64"))
		_ = hmac
...
func (this *Ascendex) HandleErrors(goArgs ...*Variant) *Variant {
	...
	_ = message
	error := OpAnd(OpNotEq2(code, MkUndefined()), OpNotEq2(code, MkString("0")))
	_ = error
