...
func (this *Okex3) FetchTime(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("generalGetTime"), params)
	_ = response
...
func (this *Okex3) ParseMarket(goArgs ...*Variant) *Variant {
	...
	_ = market
	id := this.SafeString(market, MkString("instrument_id"))
	_ = id
	...
	if IsTrue(OpNotEq2(contractVal, MkUndefined())) {
		if IsTrue(OpHasMember(MkString("option_type"), market)) {
			...
			underlying := this.SafeString(market, MkString("underlying"))
			_ = underlying
			...
			futuresAlias := this.SafeString(market, MkString("alias"))
			_ = futuresAlias
	...
	_ = symbol
	lotSize := this.SafeNumber2(market, MkString("lot_size"), MkString("trade_increment"))
	_ = lotSize
	minPrice := this.SafeString(market, MkString("tick_size"))
	_ = minPrice
	...
	_ = precision
	minAmountString := this.SafeString2(market, MkString("min_size"), MkString("base_min_size"))
	_ = minAmountString
...
func (this *Okex3) FetchMarketsByType(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(type_, MkString("option"))) {
		underlying := this.Call(MkString("optionGetUnderlying"), params)
		_ = underlying
		result := MkArray(&VarArray{})
		_ = result
		for i := MkInteger(0); IsTrue(OpLw(i, underlying.Length)); OpIncr(&i) {
			response := this.Call(MkString("optionGetInstrumentsUnderlying"), MkMap(&VarMap{"underlying": *(underlying).At(i)}))
			_ = response
		...
		if IsTrue(OpOr(OpEq2(type_, MkString("spot")), OpOr(OpEq2(type_, MkString("futures")), OpEq2(type_, MkString("swap"))))) {
			method := OpAdd(type_, MkString("GetInstruments"))
			_ = method
...
func (this *Okex3) FetchCurrencies(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("accountGetCurrencies"), params)
	_ = response
	result := MkMap(&VarMap{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = name
		canDeposit := this.SafeInteger(currency, MkString("can_deposit"))
		_ = canDeposit
		canWithdraw := this.SafeInteger(currency, MkString("can_withdraw"))
		_ = canWithdraw
		active := OpTrinary(OpAnd(canDeposit, canWithdraw), MkBool(true), MkBool(false))
		_ = active
...
func (this *Okex3) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = market
	method := OpAdd(*(market).At(MkString("type")), MkString("GetInstrumentsInstrumentId"))
	_ = method
	OpTrinary(OpAddAssign(&method, OpEq2(*(market).At(MkString("type")), MkString("swap"))), MkString("Depth"), MkString("Book"))
	request := MkMap(&VarMap{"instrument_id": *(market).At(MkString("id"))})
	_ = request
	...
	_ = response
	timestamp := this.Parse8601(this.SafeString2(response, MkString("timestamp"), MkString("time")))
	_ = timestamp
...
func (this *Okex3) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = symbol
	marketId := this.SafeString(ticker, MkString("instrument_id"))
	_ = marketId
...
func (this *Okex3) FetchTicker(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	method := OpAdd(*(market).At(MkString("type")), MkString("GetInstrumentsInstrumentIdTicker"))
	_ = method
	request := MkMap(&VarMap{"instrument_id": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(method, this.Extend(request, params))
...
func (this *Okex3) FetchTickersByType(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	method := OpAdd(type_, MkString("GetInstrumentsTicker"))
	_ = method
	response := this.Call(method, params)
	_ = response
	result := MkMap(&VarMap{})
...
func (this *Okex3) ParseTrade(goArgs ...*Variant) *Variant {
	...
	symbol := OpCopy(MkUndefined())
	_ = symbol
	marketId := this.SafeString(trade, MkString("instrument_id"))
	_ = marketId
	...
	timestamp := this.Parse8601(this.SafeString2(trade, MkString("timestamp"), MkString("created_at")))
	_ = timestamp
	priceString := this.SafeString(trade, MkString("price"))
	_ = priceString
	amountString := this.SafeString2(trade, MkString("size"), MkString("qty"))
	_ = amountString
...
func (this *Okex3) FetchTrades(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	method := OpAdd(*(market).At(MkString("type")), MkString("GetInstrumentsInstrumentIdTrades"))
	_ = method
...
func (this *Okex3) ParseOHLCV(goArgs ...*Variant) *Variant {
	...
	if IsTrue(GoIsArray(ohlcv)) {
		numElements := OpCopy(ohlcv.Length)
		_ = numElements
		volumeIndex := OpTrinary(OpGt(numElements, MkInteger(6)), MkInteger(6), MkInteger(5))
		_ = volumeIndex
		timestamp := this.SafeValue(ohlcv, MkInteger(0))
		_ = timestamp
...
func (this *Okex3) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	method := OpAdd(*(market).At(MkString("type")), OpAdd(MkString("GetInstrumentsInstrumentId"), type_))
	_ = method
...
func (this *Okex3) ParseMarginBalance(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		balance := *(response).At(i)
		_ = balance
		marketId := this.SafeString(balance, MkString("instrument_id"))
		_ = marketId
		...
		for k := MkInteger(0); IsTrue(OpLw(k, keys.Length)); OpIncr(&k) {
			key := *(keys).At(k)
			_ = key
			marketBalance := *(balance).At(key)
			_ = marketBalance
			if IsTrue(OpGtEq(key.IndexOf(MkString(":")), MkInteger(0))) {
				parts := key.Split(MkString(":"))
				_ = parts
				currencyId := *(parts).At(MkInteger(1))
				_ = currencyId
				code := this.SafeCurrencyCode(currencyId)
				_ = code
				account := this.Account()
...
func (this *Okex3) ParseFuturesBalance(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		...
		code := this.SafeCurrencyCode(id)
		_ = code
		balance := this.SafeValue(info, id, MkMap(&VarMap{}))
		_ = balance
		account := this.Account()
		_ = account
		totalAvailBalance := this.SafeString(balance, MkString("total_avail_balance"))
		_ = totalAvailBalance
		if IsTrue(OpEq2(this.SafeString(balance, MkString("margin_mode")), MkString("fixed"))) {
			contracts := this.SafeValue(balance, MkString("contracts"), MkArray(&VarArray{}))
			_ = contracts
			free := OpCopy(totalAvailBalance)
			_ = free
			for i := MkInteger(0); IsTrue(OpLw(i, contracts.Length)); OpIncr(&i) {
				contract := *(contracts).At(i)
				_ = contract
				fixedBalance := this.SafeString(contract, MkString("fixed_balance"))
				_ = fixedBalance
				realizedPnl := this.SafeString(contract, MkString("realized_pnl"))
				_ = realizedPnl
				marginFrozen := this.SafeString(contract, MkString("margin_frozen"))
				_ = marginFrozen
				marginForUnfilled := this.SafeString(contract, MkString("margin_for_unfilled"))
				_ = marginForUnfilled
				margin := Precise.StringSub(Precise.StringSub(Precise.StringAdd(fixedBalance, realizedPnl), marginFrozen), marginForUnfilled)
				_ = margin
				free = Precise.StringAdd(free, margin)
			}
			*(account).At(MkString("free")) = OpCopy(free)
		} else {
			realizedPnl := this.SafeString(balance, MkString("realized_pnl"))
			_ = realizedPnl
			unrealizedPnl := this.SafeString(balance, MkString("unrealized_pnl"))
			_ = unrealizedPnl
			marginFrozen := this.SafeString(balance, MkString("margin_frozen"))
			_ = marginFrozen
			marginForUnfilled := this.SafeString(balance, MkString("margin_for_unfilled"))
			_ = marginForUnfilled
			positive := Precise.StringAdd(Precise.StringAdd(totalAvailBalance, realizedPnl), unrealizedPnl)
			_ = positive
...
func (this *Okex3) ParseSwapBalance(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	info := this.SafeValue(response, MkString("info"), MkArray(&VarArray{}))
	_ = info
	for i := MkInteger(0); IsTrue(OpLw(i, info.Length)); OpIncr(&i) {
		...
		balanceTimestamp := this.Parse8601(this.SafeString(balance, MkString("timestamp")))
		_ = balanceTimestamp
...
func (this *Okex3) FetchBalance(goArgs ...*Variant) *Variant {
	...
	this.LoadMarkets()
	suffix := OpTrinary(OpEq2(type_, MkString("account")), MkString("Wallet"), MkString("Accounts"))
	_ = suffix
	method := OpAdd(type_, OpAdd(MkString("Get"), suffix))
	_ = method
...
func (this *Okex3) CreateOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"instrument_id": *(market).At(MkString("id"))})
	_ = request
	clientOrderId := this.SafeString2(params, MkString("client_oid"), MkString("clientOrderId"))
	_ = clientOrderId
	...
	if IsTrue(OpOr(*(market).At(MkString("futures")), *(market).At(MkString("swap")))) {
		size := OpTrinary(*(market).At(MkString("futures")), this.NumberToString(amount), this.AmountToPrecision(symbol, amount))
		_ = size
		request = this.Extend(request, MkMap(&VarMap{
			"type": type_,
			"size": size,
		}))
		orderType := this.SafeString(params, MkString("order_type"))
		_ = orderType
		isMarketOrder := OpOr(OpEq2(type_, MkString("market")), OpEq2(orderType, MkString("4")))
		_ = isMarketOrder
		...
		marginTrading := this.SafeString(params, MkString("margin_trading"), MkString("1"))
		_ = marginTrading
		request = this.Extend(request, MkMap(&VarMap{
			"side":           side,
			"type":           type_,
...
func (this *Okex3) CancelOrder(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpOr(*(market).At(MkString("futures")), *(market).At(MkString("swap")))) {
		...
		defaultType := this.SafeString2(*this.At(MkString("options")), MkString("cancelOrder"), MkString("defaultType"), *(market).At(MkString("type")))
		_ = defaultType
		type_ = this.SafeString(params, MkString("type"), defaultType)
	}
	if IsTrue(OpEq2(type_, MkUndefined())) {
	...
	method := OpAdd(type_, MkString("PostCancelOrder"))
	_ = method
	request := MkMap(&VarMap{"instrument_id": *(market).At(MkString("id"))})
	_ = request
	if IsTrue(OpOr(*(market).At(MkString("futures")), *(market).At(MkString("swap")))) {
	...
	_ = response
	result := OpTrinary(OpHasMember(MkString("result"), response), response, this.SafeValue(response, *(market).At(MkString("id")), MkMap(&VarMap{})))
	_ = result
...
func (this *Okex3) ParseOrder(goArgs ...*Variant) *Variant {
	...
	symbol := OpCopy(MkUndefined())
	_ = symbol
	marketId := this.SafeString(order, MkString("instrument_id"))
	_ = marketId
	if IsTrue(OpHasMember(marketId, *this.At(MkString("markets_by_id")))) {
		market = *(*this.At(MkString("markets_by_id"))).At(marketId)
		symbol = *(market).At(MkString("symbol"))
	...
	_ = amount
	filled := this.SafeNumber2(order, MkString("filled_size"), MkString("filled_qty"))
	_ = filled
	...
	cost := this.SafeNumber2(order, MkString("filled_notional"), MkString("funds"))
	_ = cost
	...
	stopPrice := this.SafeNumber(order, MkString("trigger_price"))
	_ = stopPrice
	return MkMap(&VarMap{
		"info":               order,
		"id":                 id,
...
func (this *Okex3) FetchOrder(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	defaultType := this.SafeString2(*this.At(MkString("options")), MkString("fetchOrder"), MkString("defaultType"), *(market).At(MkString("type")))
	_ = defaultType
	type_ := this.SafeString(params, MkString("type"), defaultType)
	_ = type_
	if IsTrue(OpEq2(type_, MkUndefined())) {
	...
	instrumentId := OpTrinary(OpOr(*(market).At(MkString("futures")), *(market).At(MkString("swap"))), MkString("InstrumentId"), MkString(""))
	_ = instrumentId
	method := OpAdd(type_, OpAdd(MkString("GetOrders"), instrumentId))
	_ = method
	request := MkMap(&VarMap{"instrument_id": *(market).At(MkString("id"))})
	_ = request
...
func (this *Okex3) FetchOrdersByState(goArgs ...*Variant) *Variant {
	...
	_ = request
	method := OpAdd(type_, MkString("GetOrders"))
	_ = method
	if IsTrue(OpOr(*(market).At(MkString("futures")), *(market).At(MkString("swap")))) {
		OpAddAssign(&method, MkString("InstrumentId"))
	}
	...
	if IsTrue(OpOr(*(market).At(MkString("swap")), *(market).At(MkString("futures")))) {
		...
		responseLength := OpCopy(response.Length)
		_ = responseLength
		if IsTrue(OpLw(responseLength, MkInteger(1))) {
			return MkArray(&VarArray{})
		}
		if IsTrue(OpGt(responseLength, MkInteger(1))) {
			before := this.SafeValue(*(response).At(MkInteger(1)), MkString("before"))
			_ = before
...
func (this *Okex3) ParseDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = address
	tag := this.SafeString2(depositAddress, MkString("tag"), MkString("payment_id"))
	_ = tag
...
func (this *Okex3) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = response
	addressesByCode := this.ParseDepositAddresses(response)
	_ = addressesByCode
...
func (this *Okex3) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("accountPostWithdrawal"), this.Extend(request, query))
	_ = response
	return MkMap(&VarMap{
		"info": response,
...
func (this *Okex3) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	method := MkString("accountGetDepositHistory")
	_ = method
...
func (this *Okex3) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	method := MkString("accountGetWithdrawalHistory")
	_ = method
	currency := OpCopy(MkUndefined())
	_ = currency
	if IsTrue(OpNotEq2(code, MkUndefined())) {
...
func (this *Okex3) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = address
	withdrawalId := this.SafeString(transaction, MkString("withdrawal_id"))
	_ = withdrawalId
	addressFrom := this.SafeString(transaction, MkString("from"))
	_ = addressFrom
	addressTo := this.SafeString(transaction, MkString("to"))
	_ = addressTo
	tagTo := this.SafeString(transaction, MkString("tag"))
	_ = tagTo
	...
	_ = txid
	timestamp := this.Parse8601(this.SafeString(transaction, MkString("timestamp")))
	_ = timestamp
	...
	if IsTrue(OpEq2(type_, MkString("deposit"))) {
		...
		if IsTrue(OpNotEq2(currencyId, MkUndefined())) {
			feeWithCurrencyId := this.SafeString(transaction, MkString("fee"))
			_ = feeWithCurrencyId
			if IsTrue(OpNotEq2(feeWithCurrencyId, MkUndefined())) {
				lowercaseCurrencyId := currencyId.ToLowerCase()
				_ = lowercaseCurrencyId
				feeWithoutCurrencyId := feeWithCurrencyId.Replace(lowercaseCurrencyId, MkString(""))
				_ = feeWithoutCurrencyId
...
func (this *Okex3) ParseMyTrade(goArgs ...*Variant) *Variant {
	pair := GoGetArg(goArgs, 0, MkUndefined())
	_ = pair
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	userTrade := this.SafeValue(pair, MkInteger(1))
	_ = userTrade
	otherTrade := this.SafeValue(pair, MkInteger(0))
	_ = otherTrade
	firstMarketId := this.SafeString(otherTrade, MkString("instrument_id"))
	_ = firstMarketId
	secondMarketId := this.SafeString(userTrade, MkString("instrument_id"))
	_ = secondMarketId
	...
	_ = cost
	receivedCurrencyId := this.SafeString(userTrade, MkString("currency"))
	_ = receivedCurrencyId
	feeCurrencyId := OpCopy(MkUndefined())
	_ = feeCurrencyId
	...
	id := this.SafeString(userTrade, MkString("trade_id"))
	_ = id
	price := this.SafeNumber(userTrade, MkString("price"))
	_ = price
	feeCostFirst := this.SafeNumber(otherTrade, MkString("fee"))
	_ = feeCostFirst
	feeCostSecond := this.SafeNumber(userTrade, MkString("fee"))
	_ = feeCostSecond
	feeCurrencyCodeFirst := this.SafeCurrencyCode(this.SafeString(otherTrade, MkString("currency")))
	_ = feeCurrencyCodeFirst
	feeCurrencyCodeSecond := this.SafeCurrencyCode(this.SafeString(userTrade, MkString("currency")))
	_ = feeCurrencyCodeSecond
	fee := OpCopy(MkUndefined())
	_ = fee
	fees := OpCopy(MkUndefined())
	_ = fees
	...
	timestamp := this.Parse8601(this.SafeString2(userTrade, MkString("timestamp"), MkString("created_at")))
	_ = timestamp
	takerOrMaker := this.SafeString2(userTrade, MkString("exec_type"), MkString("liquidity"))
	_ = takerOrMaker
	if IsTrue(OpEq2(takerOrMaker, MkString("M"))) {
		takerOrMaker = MkString("maker")
	} else {
	...
	orderId := this.SafeString(userTrade, MkString("order_id"))
	_ = orderId
	result := MkMap(&VarMap{
		"info":         pair,
		"timestamp":    timestamp,
...
func (this *Okex3) ParseMyTrades(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 4, MkMap(&VarMap{}))
	_ = params
	grouped := this.GroupBy(trades, MkString("trade_id"))
	_ = grouped
	tradeIds := GoGetKeys(grouped)
	_ = tradeIds
	result := MkArray(&VarArray{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, tradeIds.Length)); OpIncr(&i) {
		tradeId := *(tradeIds).At(i)
		_ = tradeId
		pair := *(grouped).At(tradeId)
		_ = pair
		numTradesInPair := OpCopy(pair.Length)
		_ = numTradesInPair
		if IsTrue(OpEq2(numTradesInPair, MkInteger(2))) {
			trade := this.ParseMyTrade(pair)
			_ = trade
			result.Push(trade)
		}
	}
...
func (this *Okex3) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"instrument_id": *(market).At(MkString("id"))})
	_ = request
	defaultType := this.SafeString2(*this.At(MkString("options")), MkString("fetchMyTrades"), MkString("defaultType"))
	_ = defaultType
	type_ := this.SafeString(params, MkString("type"), defaultType)
	_ = type_
	query := this.Omit(params, MkString("type"))
	_ = query
	method := OpAdd(type_, MkString("GetFills"))
	_ = method
	response := this.Call(method, this.Extend(request, query))
	_ = response
...
func (this *Okex3) FetchPosition(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpOr(OpEq2(type_, MkString("futures")), OpEq2(type_, MkString("swap")))) {
		...
		if IsTrue(OpEq2(type_, MkString("option"))) {
			underlying := this.SafeString(params, MkString("underlying"))
			_ = underlying
...
func (this *Okex3) FetchPositions(goArgs ...*Variant) *Variant {
	...
	method := OpCopy(MkUndefined())
	_ = method
	defaultType := this.SafeString2(*this.At(MkString("options")), MkString("fetchPositions"), MkString("defaultType"))
	_ = defaultType
	type_ := this.SafeString(params, MkString("type"), defaultType)
	_ = type_
	if IsTrue(OpOr(OpEq2(type_, MkString("futures")), OpEq2(type_, MkString("swap")))) {
...
func (this *Okex3) FetchLedger(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	defaultType := this.SafeString2(*this.At(MkString("options")), MkString("fetchLedger"), MkString("defaultType"))
	_ = defaultType
	type_ := this.SafeString(params, MkString("type"), defaultType)
	_ = type_
	query := this.Omit(params, MkString("type"))
	_ = query
	suffix := OpTrinary(OpEq2(type_, MkString("account")), MkString(""), MkString("Accounts"))
	_ = suffix
	argument := MkString("")
	_ = argument
	request := MkMap(&VarMap{})
	...
	if IsTrue(OpEq2(type_, MkString("spot"))) {
		...
		*(request).At(MkString("currency")) = *(currency).At(MkString("id"))
		...
		if IsTrue(OpEq2(type_, MkString("futures"))) {
			...
			_ = marketInfo
			settlementCurrencyId := this.SafeString(marketInfo, MkString("settlement_currency"))
			_ = settlementCurrencyId
			settlementCurrencyCode := this.SafeCurrencyCode(settlementCurrencyId)
			_ = settlementCurrencyCode
			currency = this.Currency(settlementCurrencyCode)
			underlyingId := this.SafeString(marketInfo, MkString("underlying"))
			_ = underlyingId
	...
	method := OpAdd(type_, OpAdd(MkString("Get"), OpAdd(suffix, OpAdd(argument, MkString("Ledger")))))
	_ = method
	response := this.Call(method, this.Extend(request, query))
	_ = response
	responseLength := OpCopy(response.Length)
	...
	isArray := GoIsArray(*(response).At(MkInteger(0)))
	_ = isArray
	isMargin := OpEq2(type_, MkString("margin"))
	_ = isMargin
	entries := OpTrinary(OpAnd(isMargin, isArray), *(response).At(MkInteger(0)), response)
	_ = entries
	if IsTrue(OpEq2(type_, MkString("swap"))) {
		ledgerEntries := this.ParseLedger(entries)
		_ = ledgerEntries
...
func (this *Okex3) ParseLedgerEntry(goArgs ...*Variant) *Variant {
	...
	currency := GoGetArg(goArgs, 1, MkUndefined())
	_ = currency
	id := this.SafeString(item, MkString("ledger_id"))
	_ = id
	account := OpCopy(MkUndefined())
	_ = account
	details := this.SafeValue(item, MkString("details"), MkMap(&VarMap{}))
	_ = details
	referenceId := this.SafeString(details, MkString("order_id"))
	_ = referenceId
	referenceAccount := OpCopy(MkUndefined())
	_ = referenceAccount
	type_ := this.ParseLedgerEntryType(this.SafeString(item, MkString("type")))
	_ = type_
	code := this.SafeCurrencyCode(this.SafeString(item, MkString("currency")), currency)
	_ = code
	amount := this.SafeNumber(item, MkString("amount"))
	_ = amount
	timestamp := this.Parse8601(this.SafeString(item, MkString("timestamp")))
	_ = timestamp
	fee := MkMap(&VarMap{
	...
	status := MkString("ok")
	_ = status
	marketId := this.SafeString(item, MkString("instrument_id"))
	_ = marketId
	symbol := OpCopy(MkUndefined())
	_ = symbol
	if IsTrue(OpHasMember(marketId, *this.At(MkString("markets_by_id")))) {
...
func (this *Okex3) Sign(goArgs ...*Variant) *Variant {
	...
	_ = isArray
	request := OpAdd(MkString("/api/"), OpAdd(api, OpAdd(MkString("/"), OpAdd(*this.At(MkString("version")), MkString("/")))))
	_ = request
	OpTrinary(OpAddAssign(&request, isArray), path, this.ImplodeParams(path, params))
	query := OpTrinary(isArray, params, this.Omit(params, this.ExtractParams(path)))
	_ = query
	url := OpAdd(this.ImplodeHostname(*(*(*this.At(MkString("urls"))).At(MkString("api"))).At(MkString("rest"))), request)
	_ = url
	type_ := this.GetPathAuthenticationType(path)
	_ = type_
...
func (this *Okex3) GetPathAuthenticationType(goArgs ...*Variant) *Variant {
	...
	auth := this.SafeValue(*this.At(MkString("options")), MkString("auth"), MkMap(&VarMap{}))
	_ = auth
...
func (this *Okex3) HandleErrors(goArgs ...*Variant) *Variant {
	...
	_ = message
	errorCode := this.SafeString2(response, MkString("code"), MkString("error_code"))
	_ = errorCode
	nonEmptyMessage := OpAnd(OpNotEq2(message, MkUndefined()), OpNotEq2(message, MkString("")))
	_ = nonEmptyMessage
	nonZeroErrorCode := OpAnd(OpNotEq2(errorCode, MkUndefined()), OpNotEq2(errorCode, MkString("0")))
	_ = nonZeroErrorCode
