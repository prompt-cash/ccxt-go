...
func (this *Btcalpha) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = id
		baseId := this.SafeString(market, MkString("currency1"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("currency2"))
		_ = quoteId
		...
		_ = symbol
		pricePrecision := this.SafeString(market, MkString("price_precision"))
		_ = pricePrecision
		...
		_ = precision
		amountLimit := this.SafeString(market, MkString("minimum_order_size"))
		_ = amountLimit
...
func (this *Btcalpha) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"pair_name": this.MarketId(symbol)})
	_ = request
	if IsTrue(limit) {
		*(request).At(MkString("limit_sell")) = OpCopy(limit)
		*(request).At(MkString("limit_buy")) = OpCopy(limit)
	}
	response := this.Call(MkString("publicGetOrderbookPairName"), this.Extend(request, params))
	_ = response
...
func (this *Btcalpha) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = id
	side := this.SafeString2(trade, MkString("my_side"), MkString("side"))
	_ = side
	orderId := this.SafeString(trade, MkString("o_id"))
	_ = orderId
	return MkMap(&VarMap{
		"id":           id,
		"info":         trade,
...
func (this *Btcalpha) FetchTrades(goArgs ...*Variant) *Variant {
	...
	trades := this.Call(MkString("publicGetExchanges"), this.Extend(request, params))
	_ = trades
...
func (this *Btcalpha) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetChartsPairTypeChart"), this.Extend(request, params))
	_ = response
...
func (this *Btcalpha) ParseOrder(goArgs ...*Variant) *Variant {
	...
	timestamp := this.SafeTimestamp(order, MkString("date"))
	_ = timestamp
	...
	_ = status
	id := this.SafeString2(order, MkString("oid"), MkString("id"))
	_ = id
	trades := this.SafeValue(order, MkString("trades"), MkArray(&VarArray{}))
	_ = trades
	trades = this.ParseTrades(trades, market)
	side := this.SafeString2(order, MkString("my_side"), MkString("type"))
	_ = side
...
func (this *Btcalpha) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	order := this.Call(MkString("privateGetOrderId"), this.Extend(request, params))
	_ = order
...
func (this *Btcalpha) FetchOrders(goArgs ...*Variant) *Variant {
	...
	orders := this.Call(MkString("privateGetOrdersOwn"), this.Extend(request, params))
	_ = orders
...
func (this *Btcalpha) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"status": MkString("1")})
	_ = request
...
func (this *Btcalpha) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"status": MkString("3")})
	_ = request
	return this.FetchOrders(symbol, since, limit, this.Extend(request, params))
}
...
func (this *Btcalpha) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	trades := this.Call(MkString("privateGetExchangesOwn"), this.Extend(request, params))
	_ = trades
...
func (this *Btcalpha) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	query := this.Urlencode(this.Keysort(this.Omit(params, this.ExtractParams(path))))
	_ = query
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		payload := OpCopy(*this.At(MkString("apiKey")))
		_ = payload
