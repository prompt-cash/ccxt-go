...
func (this *Upbit) FetchCurrencyById(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"currency":id }); _ = request;
  response := this.Call(MkString("privateGetWithdrawsChance"), this.Extend(request , params )); _ = response;
  memberInfo := this.SafeValue(response , MkString("member_level") , MkMap(&VarMap{})); _ = memberInfo;
  currencyInfo := this.SafeValue(response , MkString("currency") , MkMap(&VarMap{})); _ = currencyInfo;
  withdrawLimits := this.SafeValue(response , MkString("withdraw_limit") , MkMap(&VarMap{})); _ = withdrawLimits;
  canWithdraw := this.SafeValue(withdrawLimits , MkString("can_withdraw") ); _ = canWithdraw;
  walletState := this.SafeString(currencyInfo , MkString("wallet_state") ); _ = walletState;
  walletLocked := this.SafeValue(memberInfo , MkString("wallet_locked") ); _ = walletLocked;
  locked := this.SafeValue(memberInfo , MkString("locked") ); _ = locked;
  ...
  maxOnetimeWithdrawal := this.SafeNumber(withdrawLimits , MkString("onetime") ); _ = maxOnetimeWithdrawal;
  maxDailyWithdrawal := this.SafeNumber(withdrawLimits , MkString("daily") , maxOnetimeWithdrawal ); _ = maxDailyWithdrawal;
  remainingDailyWithdrawal := this.SafeNumber(withdrawLimits , MkString("remaining_daily") , maxDailyWithdrawal ); _ = remainingDailyWithdrawal;
  maxWithdrawLimit := OpCopy(MkUndefined() ); _ = maxWithdrawLimit;
  ...
  currencyId := this.SafeString(currencyInfo , MkString("code") ); _ = currencyId;
...
func (this *Upbit) FetchMarketById(goArgs ...*Variant) *Variant{
  id := GoGetArg(goArgs, 0, MkUndefined()); _ = id;
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  request := MkMap(&VarMap{"market":id }); _ = request;
  response := this.Call(MkString("privateGetOrdersChance"), this.Extend(request , params )); _ = response;
  marketInfo := this.SafeValue(response , MkString("market") ); _ = marketInfo;
  bid := this.SafeValue(marketInfo , MkString("bid") ); _ = bid;
  ask := this.SafeValue(marketInfo , MkString("ask") ); _ = ask;
  marketId := this.SafeString(marketInfo , MkString("id") ); _ = marketId;
  baseId := this.SafeString(ask , MkString("currency") ); _ = baseId;
  quoteId := this.SafeString(bid , MkString("currency") ); _ = quoteId;
  ...
  state := this.SafeString(marketInfo , MkString("state") ); _ = state;
  active := OpEq2(state , MkString("active") ); _ = active;
  bidFee := this.SafeNumber(response , MkString("bid_fee") ); _ = bidFee;
  askFee := this.SafeNumber(response , MkString("ask_fee") ); _ = askFee;
  fee := Math.Max(bidFee , askFee ); _ = fee;
  return MkMap(&VarMap{
        "info":response ,
        "id":marketId ,
        "symbol":symbol ,
...
func (this *Upbit) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketAll"), params ); _ = response;
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    active := OpCopy(MkBool(true) ); _ = active;
    makerFee := this.SafeNumber(*(*this.At(MkString("options")) ).At(MkString("tradingFeesByQuoteCurrency") ), quote , *(*(*this.At(MkString("fees")) ).At(MkString("trading") )).At(MkString("maker") )); _ = makerFee;
    takerFee := this.SafeNumber(*(*this.At(MkString("options")) ).At(MkString("tradingFeesByQuoteCurrency") ), quote , *(*(*this.At(MkString("fees")) ).At(MkString("trading") )).At(MkString("taker") )); _ = takerFee;
...
func (this *Upbit) FetchOrderBooks(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    ...
    timestamp := this.SafeInteger(orderbook , MkString("timestamp") ); _ = timestamp;
...
func (this *Upbit) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(ticker , MkString("trade_timestamp") ); _ = timestamp;
  marketId := this.SafeString2(ticker , MkString("market") , MkString("code") ); _ = marketId;
  symbol := this.SafeSymbol(marketId , market , MkString("-") ); _ = symbol;
  previous := this.SafeNumber(ticker , MkString("prev_closing_price") ); _ = previous;
  last := this.SafeNumber(ticker , MkString("trade_price") ); _ = last;
  change := this.SafeNumber(ticker , MkString("signed_change_price") ); _ = change;
  percentage := this.SafeNumber(ticker , MkString("signed_change_rate") ); _ = percentage;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "timestamp":timestamp ,
        "datetime":this.Iso8601(timestamp ),
...
func (this *Upbit) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  for t := MkInteger(0) ; IsTrue(OpLw(t , response.Length )); OpIncr(& t ){
    ticker := this.ParseTicker(*(response ).At(t )); _ = ticker;
...
func (this *Upbit) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  id := this.SafeString2(trade , MkString("sequential_id") , MkString("uuid") ); _ = id;
  orderId := OpCopy(MkUndefined() ); _ = orderId;
  timestamp := this.SafeInteger(trade , MkString("timestamp") ); _ = timestamp;
  ...
  askOrBid := this.SafeStringLower2(trade , MkString("ask_bid") , MkString("side") ); _ = askOrBid;
  ...
  priceString := this.SafeString2(trade , MkString("trade_price") , MkString("price") ); _ = priceString;
  amountString := this.SafeString2(trade , MkString("trade_volume") , MkString("volume") ); _ = amountString;
  ...
  marketId := this.SafeString2(trade , MkString("market") , MkString("code") ); _ = marketId;
  ...
  feeCost := this.SafeString(trade , OpAdd(askOrBid , MkString("_fee") )); _ = feeCost;
...
func (this *Upbit) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTradesTicks"), this.Extend(request , params )); _ = response;
...
func (this *Upbit) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  timeframePeriod := this.ParseTimeframe(timeframe ); _ = timeframePeriod;
  timeframeValue := *(*this.At(MkString("timeframes")) ).At(timeframe ); _ = timeframeValue;
  if IsTrue(OpEq2(limit , MkUndefined() )) {
    limit = MkInteger(200) ;
  }
  request := MkMap(&VarMap{
  ...
  method := MkString("publicGetCandlesTimeframe") ; _ = method;
  if IsTrue(OpEq2(timeframeValue , MkString("minutes") )) {
    numMinutes := Math.Round(OpDiv(timeframePeriod , MkInteger(60) )); _ = numMinutes;
...
func (this *Upbit) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  orderSide := OpCopy(MkUndefined() ); _ = orderSide;
  if IsTrue(OpEq2(side , MkString("buy") )) {
  ...
  clientOrderId := this.SafeString2(params , MkString("clientOrderId") , MkString("identifier") ); _ = clientOrderId;
...
func (this *Upbit) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 2, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"uuid":id }); _ = request;
...
func (this *Upbit) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetWithdraws"), this.Extend(request , params )); _ = response;
  return this.ParseTransactions(response , currency , since , limit );
}
...
func (this *Upbit) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString(transaction , MkString("uuid") ); _ = id;
  ...
  updated := this.Parse8601(this.SafeString(transaction , MkString("done_at") )); _ = updated;
  timestamp := this.Parse8601(this.SafeString(transaction , MkString("created_at") , updated )); _ = timestamp;
  ...
  code := this.SafeCurrencyCode(currencyId ); _ = code;
  status := this.ParseTransactionStatus(this.SafeStringLower(transaction , MkString("state") )); _ = status;
...
func (this *Upbit) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  id := this.SafeString(order , MkString("uuid") ); _ = id;
  ...
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(order , MkString("paid_fee") ); _ = feeCost;
  ...
  if IsTrue(OpGt(numTrades , MkInteger(0) )) {
    ...
    getFeesFromTrades := OpCopy(MkBool(false) ); _ = getFeesFromTrades;
    ...
    for i := MkInteger(0) ; IsTrue(OpLw(i , numTrades )); OpIncr(& i ){
      ...
      if IsTrue(getFeesFromTrades ) {
        tradeFee := this.SafeValue(*(trades ).At(i ), MkString("fee") , MkMap(&VarMap{})); _ = tradeFee;
        tradeFeeCost := this.SafeNumber(tradeFee , MkString("cost") ); _ = tradeFeeCost;
...
func (this *Upbit) FetchOrdersByState(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 4, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  request := MkMap(&VarMap{"state":state }); _ = request;
...
func (this *Upbit) FetchDepositAddresses(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetDepositsCoinAddresses"), params ); _ = response;
...
func (this *Upbit) ParseDepositAddress(goArgs ...*Variant) *Variant{
  ...
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  address := this.SafeString(depositAddress , MkString("deposit_address") ); _ = address;
  tag := this.SafeString(depositAddress , MkString("secondary_address") ); _ = tag;
...
func (this *Upbit) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  currency := this.Currency(code ); _ = currency;
  response := this.Call(MkString("privateGetDepositsCoinAddress"), this.Extend(MkMap(&VarMap{"currency":*(currency ).At(MkString("id") )}), params )); _ = response;
...
func (this *Upbit) CreateDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostDepositsGenerateCoinAddress"), this.Extend(request , params )); _ = response;
...
func (this *Upbit) Withdraw(goArgs ...*Variant) *Variant{
  ...
  method := MkString("privatePostWithdraws") ; _ = method;
...
func (this *Upbit) Sign(goArgs ...*Variant) *Variant{
  ...
  url := this.ImplodeParams(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(api ), MkMap(&VarMap{"hostname":*this.At(MkString("hostname")) })); _ = url;
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    if IsTrue(*(GoGetKeys(query )).At(MkString("length") )) {
      auth := this.Urlencode(query ); _ = auth;
      hash := this.Hash(this.Encode(auth ), MkString("sha512") ); _ = hash;
...
func (this *Upbit) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(error , MkUndefined() )) {
    ...
    name := this.SafeString(error , MkString("name") ); _ = name;
