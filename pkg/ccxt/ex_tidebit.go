...
func (this *Tidebit) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpHasMember(MkString("success"), response)) {
		if IsTrue(*(response).At(MkString("success"))) {
			...
			_ = address
			tag := this.SafeString(response, MkString("addressTag"))
			_ = tag
			return MkMap(&VarMap{
				"currency": code,
...
func (this *Tidebit) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = id
		symbol := this.SafeString(market, MkString("name"))
		_ = symbol
		baseId := MkUndefined()
		_ = baseId
		quoteId := MkUndefined()
...
func (this *Tidebit) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	tickers := this.Call(MkString("publicGetTickers"), params)
	_ = tickers
...
func (this *Tidebit) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetK"), this.Extend(request, params))
	_ = response
...
func (this *Tidebit) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	result := this.Call(MkString("privatePostOrderDelete"), this.Extend(request, params))
	_ = result
...
func (this *Tidebit) Withdraw(goArgs ...*Variant) *Variant {
	...
	result := this.Call(MkString("privatePostWithdrawsApply"), this.Extend(request, params))
	_ = result
	return MkMap(&VarMap{
		"info": result,
		"id":   MkUndefined(),
...
func (this *Tidebit) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	request := OpAdd(MkString("/"), OpAdd(MkString("api/"), OpAdd(*this.At(MkString("version")), OpAdd(MkString("/"), OpAdd(this.ImplodeParams(path, params), MkString(".json"))))))
	_ = request
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		_ = query
		payload := OpAdd(method, OpAdd(MkString("|"), OpAdd(request, OpAdd(MkString("|"), query))))
		_ = payload
		signature := this.Hmac(this.Encode(payload), this.Encode(*this.At(MkString("secret"))))
		_ = signature
		suffix := OpAdd(query, OpAdd(MkString("&signature="), signature))
		_ = suffix
