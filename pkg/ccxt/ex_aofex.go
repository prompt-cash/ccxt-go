...
func (this *Aofex) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	markets := this.Call(MkString("publicGetMarketSymbols"), params)
	_ = markets
	precisions := this.Call(MkString("publicGetMarketPrecision"))
	_ = precisions
	...
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		...
		_ = numericId
		precision := this.SafeValue(precisions, id, MkMap(&VarMap{}))
		_ = precision
		makerFeeString := this.SafeString(market, MkString("maker_fee"))
		_ = makerFeeString
		takerFeeString := this.SafeString(market, MkString("taker_fee"))
		_ = takerFeeString
		makerFee := this.ParseNumber(Precise.StringDiv(makerFeeString, MkString("1000")))
		_ = makerFee
		takerFee := this.ParseNumber(Precise.StringDiv(takerFeeString, MkString("1000")))
		_ = takerFee
...
func (this *Aofex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = options
	showAll := this.SafeValue(options, MkString("show_all"), MkString("0"))
	_ = showAll
	request := MkMap(&VarMap{"show_all": showAll})
	_ = request
	response := this.Call(MkString("privateGetWalletList"), this.Extend(request, params))
	_ = response
	result := MkMap(&VarMap{
		"info":      response,
...
func (this *Aofex) FetchTradingFee(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetEntrustRate"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
	return MkMap(&VarMap{
...
func (this *Aofex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
	timestamp := this.SafeInteger(result, MkString("ts"))
	_ = timestamp
...
func (this *Aofex) ParseTicker(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.SafeTimestamp(ticker, MkString("id"))
	_ = timestamp
...
func (this *Aofex) FetchTickers(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetMarket24kline"), this.Extend(request, params))
	_ = response
	...
	for i := MkInteger(0); IsTrue(OpLw(i, tickers.Length)); OpIncr(&i) {
		...
		_ = marketId
		market := this.SafeMarket(marketId, MkUndefined(), MkString("-"))
		_ = market
		symbol := *(market).At(MkString("symbol"))
		_ = symbol
		data := this.SafeValue(*(tickers).At(i), MkString("data"), MkMap(&VarMap{}))
		_ = data
...
func (this *Aofex) FetchTicker(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"symbol": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetMarketDetail"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
...
func (this *Aofex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	_ = id
	ctime := this.Parse8601(this.SafeString(trade, MkString("ctime")))
	_ = ctime
	timestamp := OpSub(this.SafeTimestamp(trade, MkString("ts"), ctime), MkInteger(28800000))
	_ = timestamp
	symbol := OpCopy(MkUndefined())
	_ = symbol
	...
	_ = priceString
	amountString := this.SafeString2(trade, MkString("amount"), MkString("number"))
	_ = amountString
	price := this.ParseNumber(priceString)
	_ = price
	amount := this.ParseNumber(amountString)
	_ = amount
	cost := this.SafeNumber(trade, MkString("total_price"))
	_ = cost
...
func (this *Aofex) FetchTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"symbol": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetMarketTrade"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
	data := this.SafeValue(result, MkString("data"), MkArray(&VarArray{}))
...
func (this *Aofex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	id := this.SafeString(order, MkString("order_sn"))
	_ = id
	orderStatus := this.SafeString(order, MkString("status"))
	_ = orderStatus
	status := this.ParseOrderStatus(orderStatus)
	_ = status
	marketId := this.SafeString(order, MkString("symbol"))
	_ = marketId
	market = this.SafeMarket(marketId, market, MkString("-"))
	timestamp := this.Parse8601(this.SafeString(order, MkString("ctime")))
	_ = timestamp
	...
	_ = orderType
	type_ := OpTrinary(OpEq2(orderType, MkString("2")), MkString("limit"), MkString("market"))
	_ = type_
	...
	_ = average
	number := this.SafeNumber(order, MkString("number"))
	_ = number
	totalPrice := this.SafeNumber(order, MkString("total_price"))
	_ = totalPrice
...
func (this *Aofex) FetchClosedOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"order_sn": id})
	_ = request
	response := this.Call(MkString("privateGetEntrustDetail"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
	trades := this.SafeValue(result, MkString("trades"), MkArray(&VarArray{}))
	_ = trades
	order := this.SafeValue(result, MkString("entrust"), MkMap(&VarMap{}))
	_ = order
...
func (this *Aofex) FetchOrderTrades(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 4, MkMap(&VarMap{}))
	_ = params
	response := this.FetchClosedOrder(id, symbol, params)
	_ = response
...
func (this *Aofex) FetchOrdersWithMethod(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"direct": MkString("prev")})
	_ = request
...
func (this *Aofex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	orderType := OpAdd(side, OpAdd(MkString("-"), type_))
	_ = orderType
	request := MkMap(&VarMap{
		"symbol": *(market).At(MkString("id")),
		"type":   orderType,
	...
	response := this.Call(MkString("privatePostEntrustAdd"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
...
func (this *Aofex) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"order_ids": id})
	_ = request
	response := this.Call(MkString("privatePostEntrustCancel"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
	success := this.SafeValue(result, MkString("success"), MkArray(&VarArray{}))
	_ = success
...
func (this *Aofex) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	url := OpAdd(this.ImplodeHostname(*(*(*this.At(MkString("urls"))).At(MkString("api"))).At(api)), OpAdd(MkString("/"), path))
	_ = url
	keys := GoGetKeys(params)
	_ = keys
	keysLength := OpCopy(keys.Length)
	_ = keysLength
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		uuid := this.Uuid()
		_ = uuid
		randomString := uuid.Slice(MkInteger(0), MkInteger(5))
		_ = randomString
		nonceString := OpAdd(nonce, OpAdd(MkString("_"), randomString))
		_ = nonceString
		auth := MkMap(&VarMap{})
		_ = auth
		...
		*(auth).At(nonceString) = OpCopy(nonceString)
		...
		signature := this.Hash(this.Encode(stringToSign), MkString("sha1"))
		_ = signature
...
func (this *Aofex) HandleErrors(goArgs ...*Variant) *Variant {
	...
	error := this.SafeString(response, MkString("errno"))
	_ = error
	if IsTrue(OpAnd(OpNotEq2(error, MkUndefined()), OpNotEq2(error, MkString("0")))) {
		message := this.SafeString(response, MkString("errmsg"))
		_ = message
