...
func (this *Coinex) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetMarketInfo"), params ); _ = response;
  markets := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = markets;
  result := MkArray(&VarArray{}); _ = result;
  keys := GoGetKeys(markets ); _ = keys;
  for i := MkInteger(0) ; IsTrue(OpLw(i , keys.Length )); OpIncr(& i ){
    key := *(keys ).At(i ); _ = key;
    market := *(markets ).At(key ); _ = market;
    id := this.SafeString(market , MkString("name") ); _ = id;
    tradingName := this.SafeString(market , MkString("trading_name") ); _ = tradingName;
    baseId := OpCopy(tradingName ); _ = baseId;
    quoteId := this.SafeString(market , MkString("pricing_name") ); _ = quoteId;
...
func (this *Coinex) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("publicGetMarketTickerAll"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  timestamp := this.SafeInteger(data , MkString("date") ); _ = timestamp;
  tickers := this.SafeValue(data , MkString("ticker") ); _ = tickers;
  marketIds := GoGetKeys(tickers ); _ = marketIds;
...
func (this *Coinex) FetchOrderBook(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  limit := GoGetArg(goArgs, 1, MkInteger(20) ); _ = limit;
...
func (this *Coinex) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeTimestamp(trade , MkString("create_time") ); _ = timestamp;
  ...
  cost := this.SafeNumber(trade , MkString("deal_money") ); _ = cost;
  ...
  if IsTrue(OpNotEq2(feeCost , MkUndefined() )) {
    feeCurrencyId := this.SafeString(trade , MkString("fee_asset") ); _ = feeCurrencyId;
...
func (this *Coinex) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"market":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetMarketDeals"), this.Extend(request , params )); _ = response;
...
func (this *Coinex) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetBalanceInfo"), params ); _ = response;
  result := MkMap(&VarMap{"info":response }); _ = result;
  balances := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = balances;
...
func (this *Coinex) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  cost := this.SafeNumber(order , MkString("deal_money") ); _ = cost;
  ...
  feeCurrencyId := this.SafeString(order , MkString("fee_asset") ); _ = feeCurrencyId;
...
func (this *Coinex) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  method := OpAdd(MkString("privatePostOrder") , this.Capitalize(type_ )); _ = method;
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{
        "market":*(market ).At(MkString("id") ),
        "type":side ,
...
func (this *Coinex) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateDeleteOrderPending"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  return this.ParseOrder(data , market );
}
...
func (this *Coinex) FetchOrdersByStatus(goArgs ...*Variant) *Variant{
  ...
  method := OpAdd(MkString("privateGetOrder") , this.Capitalize(status )); _ = method;
  response := this.Call(method , this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  orders := this.SafeValue(data , MkString("data") , MkArray(&VarArray{})); _ = orders;
...
func (this *Coinex) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrderUserDeals"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  trades := this.SafeValue(data , MkString("data") , MkArray(&VarArray{})); _ = trades;
...
func (this *Coinex) Withdraw(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostBalanceCoinWithdraw"), this.Extend(request , params )); _ = response;
  transaction := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = transaction;
...
func (this *Coinex) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  id := this.SafeString2(transaction , MkString("coin_withdraw_id") , MkString("coin_deposit_id") ); _ = id;
  address := this.SafeString(transaction , MkString("coin_address") ); _ = address;
  tag := this.SafeString(transaction , MkString("remark") ); _ = tag;
  if IsTrue(OpNotEq2(tag , MkUndefined() )) {
    if IsTrue(OpLw(tag.Length , MkInteger(1) )) {
      tag = OpCopy(MkUndefined() );
    }
  }
  txid := this.SafeValue(transaction , MkString("tx_id") ); _ = txid;
  if IsTrue(OpNotEq2(txid , MkUndefined() )) {
    if IsTrue(OpLw(txid.Length , MkInteger(1) )) {
      txid = OpCopy(MkUndefined() );
    }
  }
  currencyId := this.SafeString(transaction , MkString("coin_type") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  timestamp := this.SafeTimestamp(transaction , MkString("create_time") ); _ = timestamp;
  type_ := OpTrinary(OpHasMember(MkString("coin_withdraw_id") , transaction ), MkString("withdraw") , MkString("deposit") ); _ = type_;
  ...
  feeCost := this.SafeNumber(transaction , MkString("tx_fee") ); _ = feeCost;
...
func (this *Coinex) FetchWithdrawals(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  currency := this.Currency(code ); _ = currency;
  request := MkMap(&VarMap{"coin_type":*(currency ).At(MkString("id") )}); _ = request;
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
    *(request ).At(MkString("Limit") )= OpCopy(limit );
  }
  response := this.Call(MkString("privateGetBalanceCoinWithdraw"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
...
func (this *Coinex) FetchDeposits(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetBalanceCoinDeposit"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  if IsTrue(OpNot(GoIsArray(data ))) {
    data = this.SafeValue(data , MkString("data") , MkArray(&VarArray{}));
  }
...
func (this *Coinex) Sign(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpOr(OpEq2(api , MkString("public") ), OpEq2(api , MkString("perpetualPublic") ))) {
    ...
    urlencoded := this.Rawencode(query ); _ = urlencoded;
    signature := this.Hash(this.Encode(OpAdd(urlencoded , OpAdd(MkString("&secret_key=") , *this.At(MkString("secret")) )))); _ = signature;
...
func (this *Coinex) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpOr(OpNotEq2(code , MkString("0") ), OpOr(OpEq2(data , MkUndefined() ), OpAnd(OpNotEq2(message , MkString("Success") ), OpAnd(OpNotEq2(message , MkString("Ok") ), OpNot(data )))))) {
    ...
    ErrorClass := this.SafeValue(responseCodes , code , ExchangeError ); _ = ErrorClass;
