...
func (this *Bittrex) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = market
		baseId := this.SafeString(market, MkString("baseCurrencySymbol"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("quoteCurrencySymbol"))
		_ = quoteId
		...
		_ = symbol
		pricePrecision := this.SafeInteger(market, MkString("precision"), MkInteger(8))
		_ = pricePrecision
...
func (this *Bittrex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	balances := this.Call(MkString("privateGetBalances"), params)
	_ = balances
	result := MkMap(&VarMap{"info": balances})
	_ = result
	indexed := this.IndexBy(balances, MkString("currencySymbol"))
	_ = indexed
	currencyIds := GoGetKeys(indexed)
	_ = currencyIds
...
func (this *Bittrex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"marketSymbol": this.MarketId(symbol)})
	_ = request
	...
	response := this.Call(MkString("publicGetMarketsMarketSymbolOrderbook"), this.Extend(request, params))
	_ = response
	sequence := this.SafeInteger(*this.At(MkString("last_response_headers")), MkString("Sequence"))
	_ = sequence
	orderbook := this.ParseOrderBook(response, symbol, MkUndefined(), MkString("bid"), MkString("ask"), MkString("rate"), MkString("quantity"))
	_ = orderbook
...
func (this *Bittrex) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = fee
		isActive := this.SafeString(currency, MkString("status"))
		_ = isActive
...
func (this *Bittrex) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	timestamp := this.Parse8601(this.SafeString(ticker, MkString("updatedAt")))
	_ = timestamp
	...
	_ = percentage
	last := this.SafeNumber(ticker, MkString("lastTradeRate"))
	_ = last
	return MkMap(&VarMap{
		"symbol":        symbol,
		"timestamp":     timestamp,
...
func (this *Bittrex) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = options
	defaultMethod := this.SafeString(options, MkString("method"), MkString("publicGetMarketsTickers"))
	_ = defaultMethod
...
func (this *Bittrex) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"marketSymbol": *(market).At(MkString("id"))})
	_ = request
	options := this.SafeValue(*this.At(MkString("options")), MkString("fetchTicker"), MkMap(&VarMap{}))
	_ = options
	defaultMethod := this.SafeString(options, MkString("method"), MkString("publicGetMarketsMarketSymbolTicker"))
	_ = defaultMethod
	method := this.SafeString(params, MkString("method"), defaultMethod)
	_ = method
	params = this.Omit(params, MkString("method"))
...
func (this *Bittrex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.Parse8601(this.SafeString(trade, MkString("executedAt")))
	_ = timestamp
	id := this.SafeString(trade, MkString("id"))
	_ = id
	order := this.SafeString(trade, MkString("orderId"))
	_ = order
	marketId := this.SafeString(trade, MkString("marketSymbol"))
	_ = marketId
	...
	_ = takerOrMaker
	isTaker := this.SafeValue(trade, MkString("isTaker"))
	_ = isTaker
	...
	side := this.SafeStringLower(trade, MkString("takerSide"))
	_ = side
	return MkMap(&VarMap{
		"info":         trade,
		"timestamp":    timestamp,
...
func (this *Bittrex) FetchTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"marketSymbol": this.MarketId(symbol)})
	_ = request
	response := this.Call(MkString("publicGetMarketsMarketSymbolTrades"), this.Extend(request, params))
	_ = response
...
func (this *Bittrex) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	reverseId := OpAdd(*(market).At(MkString("baseId")), OpAdd(MkString("-"), *(market).At(MkString("quoteId"))))
	_ = reverseId
	...
	_ = request
	method := MkString("publicGetMarketsMarketSymbolCandlesCandleIntervalRecent")
	_ = method
	if IsTrue(OpNotEq2(since, MkUndefined())) {
		now := this.Milliseconds()
		_ = now
		difference := Math.Abs(OpSub(now, since))
		_ = difference
		sinceDate := this.Ymd(since)
		_ = sinceDate
		parts := sinceDate.Split(MkString("-"))
		_ = parts
		sinceYear := this.SafeInteger(parts, MkInteger(0))
		_ = sinceYear
		sinceMonth := this.SafeInteger(parts, MkInteger(1))
		_ = sinceMonth
		sinceDay := this.SafeInteger(parts, MkInteger(2))
		_ = sinceDay
...
func (this *Bittrex) FetchOrderTrades(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetOrdersOrderIdExecutions"), this.Extend(request, params))
	_ = response
	market := OpCopy(MkUndefined())
	_ = market
	if IsTrue(OpNotEq2(symbol, MkUndefined())) {
...
func (this *Bittrex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	isCeilingLimit := OpEq2(uppercaseType, MkString("CEILING_LIMIT"))
	_ = isCeilingLimit
	isCeilingMarket := OpEq2(uppercaseType, MkString("CEILING_MARKET"))
	_ = isCeilingMarket
	isCeilingOrder := OpOr(isCeilingLimit, isCeilingMarket)
	_ = isCeilingOrder
...
func (this *Bittrex) CancelOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"orderId": id})
	_ = request
	response := this.Call(MkString("privateDeleteOrdersOrderId"), this.Extend(request, params))
	_ = response
...
func (this *Bittrex) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateDeleteOrdersOpen"), this.Extend(request, params))
	_ = response
	orders := MkArray(&VarArray{})
	_ = orders
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		result := this.SafeValue(*(response).At(i), MkString("result"), MkMap(&VarMap{}))
		_ = result
		orders.Push(result)
	}
...
func (this *Bittrex) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetDepositsClosed"), this.Extend(request, params))
	_ = response
...
func (this *Bittrex) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetWithdrawalsClosed"), this.Extend(request, params))
	_ = response
...
func (this *Bittrex) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = amount
	address := this.SafeString(transaction, MkString("cryptoAddress"))
	_ = address
	txid := this.SafeString(transaction, MkString("txId"))
	_ = txid
	updated := this.Parse8601(this.SafeString(transaction, MkString("updatedAt")))
	_ = updated
	opened := this.Parse8601(this.SafeString(transaction, MkString("createdAt")))
	_ = opened
	timestamp := OpTrinary(opened, opened, updated)
	_ = timestamp
	type_ := OpTrinary(OpEq2(opened, MkUndefined()), MkString("deposit"), MkString("withdrawal"))
	_ = type_
	currencyId := this.SafeString(transaction, MkString("currencySymbol"))
	_ = currencyId
	...
	if IsTrue(OpEq2(type_, MkString("deposit"))) {
		...
		responseStatus := this.SafeString(transaction, MkString("status"))
		_ = responseStatus
	...
	feeCost := this.SafeNumber(transaction, MkString("txCost"))
	_ = feeCost
...
func (this *Bittrex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	marketSymbol := this.SafeString(order, MkString("marketSymbol"))
	_ = marketSymbol
	...
	_ = feeCurrency
	direction := this.SafeStringLower(order, MkString("direction"))
	_ = direction
	createdAt := this.SafeString(order, MkString("createdAt"))
	_ = createdAt
	updatedAt := this.SafeString(order, MkString("updatedAt"))
	_ = updatedAt
	closedAt := this.SafeString(order, MkString("closedAt"))
	_ = closedAt
	...
	_ = type_
	quantity := this.SafeNumber(order, MkString("quantity"))
	_ = quantity
	limit := this.SafeNumber(order, MkString("limit"))
	_ = limit
	fillQuantity := this.SafeNumber(order, MkString("fillQuantity"))
	_ = fillQuantity
	commission := this.SafeNumber(order, MkString("commission"))
	_ = commission
	proceeds := this.SafeNumber(order, MkString("proceeds"))
	_ = proceeds
	status := this.SafeStringLower(order, MkString("status"))
	_ = status
				...
				if e := recover().(*Variant); e != nil {
					ret_ = func(this *Bittrex) *Variant {
						...
						if IsTrue(*this.At(MkString("last_json_response"))) {
							message := this.SafeString(*this.At(MkString("last_json_response")), MkString("message"))
							_ = message
...
func (this *Bittrex) OrderToTrade(goArgs ...*Variant) *Variant {
	order := GoGetArg(goArgs, 0, MkUndefined())
	_ = order
	timestamp := this.SafeInteger2(order, MkString("lastTradeTimestamp"), MkString("timestamp"))
	_ = timestamp
	return MkMap(&VarMap{
		"id":           this.SafeString(order, MkString("id")),
...
func (this *Bittrex) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetOrdersClosed"), this.Extend(request, params))
	_ = response
	orders := this.ParseOrders(response, market)
	_ = orders
	trades := this.OrdersToTrades(orders)
	_ = trades
...
func (this *Bittrex) CreateDepositAddress(goArgs ...*Variant) *Variant {
	...
	currency := this.Currency(code)
	_ = currency
	request := MkMap(&VarMap{"currencySymbol": *(currency).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("privatePostAddresses"), this.Extend(request, params))
	_ = response
	address := this.SafeString(response, MkString("cryptoAddress"))
	_ = address
	message := this.SafeString(response, MkString("status"))
	_ = message
	...
	tag := this.SafeString(response, MkString("cryptoAddressTag"))
	_ = tag
...
func (this *Bittrex) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"currencySymbol": *(currency).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("privateGetAddressesCurrencySymbol"), this.Extend(request, params))
	_ = response
	address := this.SafeString(response, MkString("cryptoAddress"))
	_ = address
	message := this.SafeString(response, MkString("status"))
...
func (this *Bittrex) Sign(goArgs ...*Variant) *Variant {
	...
	_ = path
	api := GoGetArg(goArgs, 1, MkString("v3"))
	_ = api
	...
	_ = body
	url := OpAdd(this.ImplodeParams(*(*(*this.At(MkString("urls"))).At(MkString("api"))).At(api), MkMap(&VarMap{"hostname": *this.At(MkString("hostname"))})), MkString("/"))
	_ = url
	if IsTrue(OpEq2(api, MkString("private"))) {
		...
		contentHash := this.Hash(this.Encode(hashString), MkString("sha512"), MkString("hex"))
		_ = contentHash
		timestamp := (this.Milliseconds()).Call(MkString("toString"))
		_ = timestamp
		auth := OpAdd(timestamp, OpAdd(url, OpAdd(method, contentHash)))
		_ = auth
		subaccountId := this.SafeValue(*this.At(MkString("options")), MkString("subaccountId"))
		_ = subaccountId
	...
	if IsTrue(OpEq2(*(body).At(MkInteger(0)), MkString("{"))) {
		...
		if IsTrue(OpNot(success)) {
			...
			if IsTrue(OpEq2(message, MkString("INVALID_ORDER"))) {
				cancel := MkString("cancel")
				_ = cancel
				indexOfCancel := url.IndexOf(cancel)
				_ = indexOfCancel
				if IsTrue(OpGtEq(indexOfCancel, MkInteger(0))) {
					urlParts := url.Split(MkString("?"))
					_ = urlParts
					numParts := OpCopy(urlParts.Length)
					_ = numParts
					if IsTrue(OpGt(numParts, MkInteger(1))) {
						query := *(urlParts).At(MkInteger(1))
						_ = query
						params := query.Split(MkString("&"))
						_ = params
						numParams := OpCopy(params.Length)
						_ = numParams
