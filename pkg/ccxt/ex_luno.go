...
func (this *Luno) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("exchangeGetMarkets"), params ); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  markets := this.SafeValue(response , MkString("markets") , MkArray(&VarArray{})); _ = markets;
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    market := *(markets ).At(i ); _ = market;
    id := this.SafeString(market , MkString("market_id") ); _ = id;
    baseId := this.SafeString(market , MkString("base_currency") ); _ = baseId;
    quoteId := this.SafeString(market , MkString("counter_currency") ); _ = quoteId;
    ...
    status := this.SafeString(market , MkString("trading_status") ); _ = status;
...
func (this *Luno) FetchAccounts(goArgs ...*Variant) *Variant{
  ...
  wallets := this.SafeValue(response , MkString("balance") , MkArray(&VarArray{})); _ = wallets;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , wallets.Length )); OpIncr(& i ){
    account := *(wallets ).At(i ); _ = account;
    accountId := this.SafeString(account , MkString("account_id") ); _ = accountId;
...
func (this *Luno) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , wallets.Length )); OpIncr(& i ){
    wallet := *(wallets ).At(i ); _ = wallet;
    currencyId := this.SafeString(wallet , MkString("asset") ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    reserved := this.SafeString(wallet , MkString("reserved") ); _ = reserved;
    unconfirmed := this.SafeString(wallet , MkString("unconfirmed") ); _ = unconfirmed;
    balance := this.SafeString(wallet , MkString("balance") ); _ = balance;
    reservedUnconfirmed := Precise.StringAdd(reserved , unconfirmed ); _ = reservedUnconfirmed;
    balanceUnconfirmed := Precise.StringAdd(balance , unconfirmed ); _ = balanceUnconfirmed;
...
func (this *Luno) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  method := MkString("publicGetOrderbook") ; _ = method;
...
func (this *Luno) ParseOrderStatus(goArgs ...*Variant) *Variant{
  ...
  statuses := MkMap(&VarMap{"PENDING":MkString("open") }); _ = statuses;
...
func (this *Luno) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  amount := this.SafeNumber(order , MkString("limit_volume") ); _ = amount;
  quoteFee := this.SafeNumber(order , MkString("fee_counter") ); _ = quoteFee;
  baseFee := this.SafeNumber(order , MkString("fee_base") ); _ = baseFee;
  filled := this.SafeNumber(order , MkString("base") ); _ = filled;
  cost := this.SafeNumber(order , MkString("counter") ); _ = cost;
  fee := MkMap(&VarMap{"currency":MkUndefined() }); _ = fee;
  if IsTrue(quoteFee ) {
...
func (this *Luno) FetchOrdersByState(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetListorders"), this.Extend(request , params )); _ = response;
...
func (this *Luno) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  tickers := this.IndexBy(*(response ).At(MkString("tickers") ), MkString("pair") ); _ = tickers;
...
func (this *Luno) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  feeBase := this.SafeNumber(trade , MkString("fee_base") ); _ = feeBase;
  feeCounter := this.SafeNumber(trade , MkString("fee_counter") ); _ = feeCounter;
...
func (this *Luno) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetListtrades"), this.Extend(request , params )); _ = response;
  trades := this.SafeValue(response , MkString("trades") , MkArray(&VarArray{})); _ = trades;
  return this.ParseTrades(trades , market , since , limit );
}
...
func (this *Luno) FetchTradingFees(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetFeeInfo"), params ); _ = response;
  return MkMap(&VarMap{
        "info":response ,
...
func (this *Luno) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  method := MkString("privatePost") ; _ = method;
  request := MkMap(&VarMap{"pair":this.MarketId(symbol )}); _ = request;
...
func (this *Luno) FetchLedgerByEntries(goArgs ...*Variant) *Variant{
  ...
  entry := GoGetArg(goArgs, 1, OpNeg(MkInteger(1) )); _ = entry;
  limit := GoGetArg(goArgs, 2, MkInteger(1) ); _ = limit;
  params := GoGetArg(goArgs, 3, MkMap(&VarMap{})); _ = params;
  since := OpCopy(MkUndefined() ); _ = since;
...
func (this *Luno) FetchLedger(goArgs ...*Variant) *Variant{
  ...
  min_row := this.SafeValue(params , MkString("min_row") ); _ = min_row;
  max_row := this.SafeValue(params , MkString("max_row") ); _ = max_row;
  ...
  response := this.Call(MkString("privateGetAccountsIdTransactions"), this.Extend(params , request )); _ = response;
  entries := this.SafeValue(response , MkString("transactions") , MkArray(&VarArray{})); _ = entries;
  return this.ParseLedger(entries , currency , since , limit );
}

func (this *Luno) ParseLedgerComment(goArgs ...*Variant) *Variant{
  comment := GoGetArg(goArgs, 0, MkUndefined()); _ = comment;
  words := comment.Split(MkString(" ") ); _ = words;
  types := MkMap(&VarMap{
  ...
  firstWord := this.SafeString(words , MkInteger(0) ); _ = firstWord;
  thirdWord := this.SafeString(words , MkInteger(2) ); _ = thirdWord;
  fourthWord := this.SafeString(words , MkInteger(3) ); _ = fourthWord;
  type_ := this.SafeString(types , firstWord , MkUndefined() ); _ = type_;
...
func (this *Luno) ParseLedgerEntry(goArgs ...*Variant) *Variant{
  entry := GoGetArg(goArgs, 0, MkUndefined()); _ = entry;
  currency := GoGetArg(goArgs, 1, MkUndefined() ); _ = currency;
  id := this.SafeString(entry , MkString("row_index") ); _ = id;
  account_id := this.SafeString(entry , MkString("account_id") ); _ = account_id;
  timestamp := this.SafeValue(entry , MkString("timestamp") ); _ = timestamp;
  currencyId := this.SafeString(entry , MkString("currency") ); _ = currencyId;
  code := this.SafeCurrencyCode(currencyId , currency ); _ = code;
  available_delta := this.SafeNumber(entry , MkString("available_delta") ); _ = available_delta;
  balance_delta := this.SafeNumber(entry , MkString("balance_delta") ); _ = balance_delta;
  after := this.SafeNumber(entry , MkString("balance") ); _ = after;
  comment := this.SafeString(entry , MkString("description") ); _ = comment;
  before := OpCopy(after ); _ = before;
  amount := MkNumber(0.0) ; _ = amount;
  result := this.ParseLedgerComment(comment ); _ = result;
  type_ := *(result ).At(MkString("type") ); _ = type_;
  referenceId := *(result ).At(MkString("referenceId") ); _ = referenceId;
