...
func (this *Cex) FetchCurrenciesFromCache(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpOr(OpEq2(timestamp, MkUndefined()), OpGt(OpSub(now, timestamp), expires))) {
		response := this.Call(MkString("publicGetCurrencyProfile"), params)
		_ = response
...
func (this *Cex) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	*(*this.At(MkString("options"))).At(MkString("currencies")) = MkMap(&VarMap{
	...
	_ = data
	currencies := this.SafeValue(data, MkString("symbols"), MkArray(&VarArray{}))
	_ = currencies
	result := MkMap(&VarMap{})
	_ = result
...
func (this *Cex) FetchMarkets(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	currenciesResponse := this.FetchCurrenciesFromCache(params)
	_ = currenciesResponse
	currenciesData := this.SafeValue(currenciesResponse, MkString("data"), MkMap(&VarMap{}))
	_ = currenciesData
	currencies := this.SafeValue(currenciesData, MkString("symbols"), MkArray(&VarArray{}))
	_ = currencies
	currenciesById := this.IndexBy(currencies, MkString("code"))
	_ = currenciesById
	pairs := this.SafeValue(currenciesData, MkString("pairs"), MkArray(&VarArray{}))
	_ = pairs
	response := this.Call(MkString("publicGetCurrencyLimits"), params)
	_ = response
	result := MkArray(&VarArray{})
	_ = result
	markets := this.SafeValue(*(response).At(MkString("data")), MkString("pairs"))
	_ = markets
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		market := *(markets).At(i)
		_ = market
		baseId := this.SafeString(market, MkString("symbol1"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("symbol2"))
		_ = quoteId
		...
		_ = symbol
		baseCurrency := this.SafeValue(currenciesById, baseId, MkMap(&VarMap{}))
		_ = baseCurrency
		quoteCurrency := this.SafeValue(currenciesById, quoteId, MkMap(&VarMap{}))
		_ = quoteCurrency
		pricePrecision := this.SafeInteger(quoteCurrency, MkString("precision"), MkInteger(8))
		_ = pricePrecision
		...
		baseCcyPrecision := this.SafeInteger(baseCurrency, MkString("precision"), MkInteger(8))
		_ = baseCcyPrecision
		baseCcyScale := this.SafeInteger(baseCurrency, MkString("scale"), MkInteger(0))
		_ = baseCcyScale
		amountPrecision := OpSub(baseCcyPrecision, baseCcyScale)
		_ = amountPrecision
...
func (this *Cex) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	_ = request
	{
		ret__ := func(this *Cex) (ret_ *Variant) {
			...
			// try block:
			response := this.Call(MkString("publicGetOhlcvHdYyyymmddPair"), this.Extend(request, params))
			_ = response
			key := OpAdd(MkString("data"), *(*this.At(MkString("timeframes"))).At(timeframe))
			_ = key
			data := this.SafeString(response, key)
			_ = data
			ohlcvs := JSON.Parse(data)
			_ = ohlcvs
...
func (this *Cex) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = timestamp
	volume := this.SafeNumber(ticker, MkString("volume"))
	_ = volume
...
func (this *Cex) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = currencies
	request := MkMap(&VarMap{"currencies": currencies.Join(MkString("/"))})
	_ = request
	response := this.Call(MkString("publicGetTickersCurrencies"), this.Extend(request, params))
	_ = response
	tickers := *(response).At(MkString("data"))
	_ = tickers
	result := MkMap(&VarMap{})
	_ = result
	for t := MkInteger(0); IsTrue(OpLw(t, tickers.Length)); OpIncr(&t) {
		ticker := *(tickers).At(t)
		_ = ticker
		symbol := (*(ticker).At(MkString("pair"))).Call(MkString("replace"), MkString(":"), MkString("/"))
		_ = symbol
...
func (this *Cex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostPlaceOrderPair"), this.Extend(request, params))
	_ = response
	placedAmount := this.SafeNumber(response, MkString("amount"))
	_ = placedAmount
	remaining := this.SafeNumber(response, MkString("pending"))
	_ = remaining
	timestamp := this.SafeValue(response, MkString("time"))
	_ = timestamp
	complete := this.SafeValue(response, MkString("complete"))
	_ = complete
	status := OpTrinary(complete, MkString("closed"), MkString("open"))
	_ = status
...
func (this *Cex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.SafeValue(order, MkString("time"))
	_ = timestamp
	...
	if IsTrue(OpEq2(market, MkUndefined())) {
		baseId := this.SafeString(order, MkString("symbol1"))
		_ = baseId
		quoteId := this.SafeString(order, MkString("symbol2"))
		_ = quoteId
		base := this.SafeCurrencyCode(baseId)
		_ = base
		quote := this.SafeCurrencyCode(quoteId)
	...
	remaining := this.SafeNumber2(order, MkString("pending"), MkString("remains"))
	_ = remaining
	...
	if IsTrue(OpNotEq2(market, MkUndefined())) {
		symbol = *(market).At(MkString("symbol"))
		taCost := this.SafeNumber(order, OpAdd(MkString("ta:"), *(market).At(MkString("quote"))))
		_ = taCost
		ttaCost := this.SafeNumber(order, OpAdd(MkString("tta:"), *(market).At(MkString("quote"))))
		_ = ttaCost
		cost = this.Sum(taCost, ttaCost)
		baseFee := OpAdd(MkString("fa:"), *(market).At(MkString("base")))
		_ = baseFee
		baseTakerFee := OpAdd(MkString("tfa:"), *(market).At(MkString("base")))
		_ = baseTakerFee
		quoteFee := OpAdd(MkString("fa:"), *(market).At(MkString("quote")))
		_ = quoteFee
		quoteTakerFee := OpAdd(MkString("tfa:"), *(market).At(MkString("quote")))
		_ = quoteTakerFee
		feeRate := this.SafeNumber(order, MkString("tradingFeeMaker"))
		_ = feeRate
		...
		if IsTrue(OpOr(OpHasMember(baseFee, order), OpHasMember(baseTakerFee, order))) {
			baseFeeCost := this.SafeNumber2(order, baseFee, baseTakerFee)
			_ = baseFeeCost
			fee = MkMap(&VarMap{
			...
			if IsTrue(OpOr(OpHasMember(quoteFee, order), OpHasMember(quoteTakerFee, order))) {
				quoteFeeCost := this.SafeNumber2(order, quoteFee, quoteTakerFee)
				_ = quoteFeeCost
				fee = MkMap(&VarMap{
	...
	if IsTrue(OpHasMember(MkString("vtx"), order)) {
		...
		for i := MkInteger(0); IsTrue(OpLw(i, *(*(order).At(MkString("vtx"))).At(MkString("length")))); OpIncr(&i) {
			item := *(*(order).At(MkString("vtx"))).At(i)
			_ = item
			tradeSide := this.SafeString(item, MkString("type"))
			_ = tradeSide
			if IsTrue(OpEq2(tradeSide, MkString("cancel"))) {
				continue
			}
			tradePrice := this.SafeNumber(item, MkString("price"))
			_ = tradePrice
			...
			tradeTimestamp := this.Parse8601(this.SafeString(item, MkString("time")))
			_ = tradeTimestamp
			tradeAmount := this.SafeNumber(item, MkString("amount"))
			_ = tradeAmount
			feeCost := this.SafeNumber(item, MkString("fee_amount"))
			_ = feeCost
			absTradeAmount := OpTrinary(OpLw(tradeAmount, MkInteger(0)), OpNeg(tradeAmount), tradeAmount)
			_ = absTradeAmount
			tradeCost := OpCopy(MkUndefined())
			_ = tradeCost
...
func (this *Cex) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	method := MkString("privatePostOpenOrders")
	_ = method
	...
	orders := this.Call(method, this.Extend(request, params))
	_ = orders
...
func (this *Cex) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	method := MkString("privatePostArchivedOrdersPair")
	_ = method
...
func (this *Cex) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"id": id.ToString()})
	_ = request
	response := this.Call(MkString("privatePostGetOrderTx"), this.Extend(request, params))
	_ = response
...
func (this *Cex) FetchOrders(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostArchivedOrdersPair"), this.Extend(request, params))
	_ = response
	results := MkArray(&VarArray{})
	_ = results
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = side
		baseAmount := this.SafeNumber(order, OpAdd(MkString("a:"), OpAdd(baseId, MkString(":cds"))))
		_ = baseAmount
		quoteAmount := this.SafeNumber(order, OpAdd(MkString("a:"), OpAdd(quoteId, MkString(":cds"))))
		_ = quoteAmount
		fee := this.SafeNumber(order, OpAdd(MkString("f:"), OpAdd(quoteId, MkString(":cds"))))
		_ = fee
		amount := this.SafeNumber(order, MkString("amount"))
		_ = amount
		price := this.SafeNumber(order, MkString("price"))
		_ = price
		remaining := this.SafeNumber(order, MkString("remains"))
		_ = remaining
		filled := OpSub(amount, remaining)
		_ = filled
		orderAmount := OpCopy(MkUndefined())
		_ = orderAmount
		cost := OpCopy(MkUndefined())
		_ = cost
		...
		if IsTrue(OpNot(price)) {
			...
			ta := this.SafeNumber(order, OpAdd(MkString("ta:"), quoteId), MkInteger(0))
			_ = ta
			tta := this.SafeNumber(order, OpAdd(MkString("tta:"), quoteId), MkInteger(0))
			_ = tta
			fa := this.SafeNumber(order, OpAdd(MkString("fa:"), quoteId), MkInteger(0))
			_ = fa
			tfa := this.SafeNumber(order, OpAdd(MkString("tfa:"), quoteId), MkInteger(0))
			_ = tfa
		...
		time := this.SafeString(order, MkString("time"))
		_ = time
		lastTxTime := this.SafeString(order, MkString("lastTxTime"))
		_ = lastTxTime
...
func (this *Cex) EditOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostCancelReplaceOrderPair"), this.Extend(request, params))
	_ = response
...
func (this *Cex) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePostGetAddress"), this.Extend(request, params))
	_ = response
	address := this.SafeString(response, MkString("data"))
	_ = address
