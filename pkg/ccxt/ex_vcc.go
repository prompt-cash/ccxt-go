...
func (this *Vcc) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  markets := this.SafeValue(data , MkString("symbols") ); _ = markets;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , markets.Length )); OpIncr(& i ){
    market := this.SafeValue(markets , i ); _ = market;
    symbol := this.SafeString(market , MkString("symbol") ); _ = symbol;
    id := symbol.Replace(MkString("/") , MkString("_") ); _ = id;
    baseId := this.SafeString(market , MkString("coin") ); _ = baseId;
    quoteId := this.SafeString(market , MkString("currency") ); _ = quoteId;
    ...
    precision := this.SafeValue(market , MkString("precision") , MkMap(&VarMap{})); _ = precision;
    limits := this.SafeValue(market , MkString("limits") , MkMap(&VarMap{})); _ = limits;
    amountLimits := this.SafeValue(limits , MkString("amount") , MkMap(&VarMap{})); _ = amountLimits;
    priceLimits := this.SafeValue(limits , MkString("price") , MkMap(&VarMap{})); _ = priceLimits;
    costLimits := this.SafeValue(limits , MkString("cost") , MkMap(&VarMap{})); _ = costLimits;
    entry := MkMap(&VarMap{
          "info":market ,
          "id":id ,
          "symbol":symbol ,
...
func (this *Vcc) FetchCurrencies(goArgs ...*Variant) *Variant{
  ...
  data := this.SafeValue(response , MkString("data") ); _ = data;
  ids := GoGetKeys(data ); _ = ids;
  for i := MkInteger(0) ; IsTrue(OpLw(i , ids.Length )); OpIncr(& i ){
    id := this.SafeStringLower(ids , i ); _ = id;
    currency := this.SafeValue(data , *(ids ).At(i )); _ = currency;
    code := this.SafeCurrencyCode(id ); _ = code;
    canDeposit := this.SafeValue(currency , MkString("can_deposit") ); _ = canDeposit;
    canWithdraw := this.SafeValue(currency , MkString("can_withdraw") ); _ = canWithdraw;
...
func (this *Vcc) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , currencyIds.Length )); OpIncr(& i ){
    ...
    balance := this.SafeValue(data , currencyId ); _ = balance;
...
func (this *Vcc) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetChartBars"), this.Extend(request , params )); _ = response;
...
func (this *Vcc) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetOrderbookMarketPair"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  timestamp := this.SafeValue(data , MkString("timestamp") ); _ = timestamp;
...
func (this *Vcc) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  open := this.SafeNumber(ticker , MkString("open_price") ); _ = open;
...
func (this *Vcc) FetchTickers(goArgs ...*Variant) *Variant{
  ...
  for i := MkInteger(0) ; IsTrue(OpLw(i , marketIds.Length )); OpIncr(& i ){
    ...
    market := this.SafeMarket(marketId , MkUndefined() , MkString("_") ); _ = market;
...
func (this *Vcc) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeInteger2(trade , MkString("trade_timestamp") , MkString("created_at") ); _ = timestamp;
  baseId := this.SafeStringUpper(trade , MkString("coin") ); _ = baseId;
  quoteId := this.SafeStringUpper(trade , MkString("currency") ); _ = quoteId;
  ...
  amountString := this.SafeString2(trade , MkString("base_volume") , MkString("quantity") ); _ = amountString;
  price := this.ParseNumber(priceString ); _ = price;
  amount := this.ParseNumber(amountString ); _ = amount;
  cost := this.SafeNumber2(trade , MkString("quote_volume") , MkString("amount") ); _ = cost;
  if IsTrue(OpEq2(cost , MkUndefined() )) {
    cost = this.ParseNumber(Precise.StringMul(priceString , amountString ));
  }
  side := this.SafeString2(trade , MkString("type") , MkString("trade_type") ); _ = side;
...
func (this *Vcc) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"market_pair":*(market ).At(MkString("id") )}); _ = request;
  if IsTrue(OpNotEq2(limit , MkUndefined() )) {
    *(request ).At(MkString("count") )= Math.Min(MkInteger(1000) , limit );
  }
  response := this.Call(MkString("publicGetTradesMarketPair"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
...
func (this *Vcc) ParseTransaction(goArgs ...*Variant) *Variant{
  ...
  address := this.SafeString(transaction , MkString("blockchain_address") ); _ = address;
  ...
  type_ := OpTrinary(OpGt(amount , MkInteger(0) ), MkString("deposit") , MkString("withdrawal") ); _ = type_;
  return MkMap(&VarMap{
        "info":transaction ,
        "id":id ,
        "txid":txid ,
...
func (this *Vcc) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpEq2(type_ , MkString("ceiling_market") )) {
    ceiling := this.SafeValue(params , MkString("ceiling") ); _ = ceiling;
  ...
  stopPrice := this.SafeValue2(params , MkString("stop_price") , MkString("stopPrice") ); _ = stopPrice;
...
func (this *Vcc) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePutOrdersOrderIdCancel"), this.Extend(request , params )); _ = response;
...
func (this *Vcc) CancelAllOrders(goArgs ...*Variant) *Variant{
  ...
  method := OpTrinary(OpEq2(type_ , MkUndefined() ), MkString("privatePutOrdersCancelAll") , MkString("privatePutOrdersCancelByType") ); _ = method;
  request := MkMap(&VarMap{}); _ = request;
...
func (this *Vcc) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  created := this.SafeValue(order , MkString("created_at") ); _ = created;
  updated := this.SafeValue(order , MkString("updated_at") ); _ = updated;
  baseId := this.SafeStringUpper(order , MkString("coin") ); _ = baseId;
  quoteId := this.SafeStringUpper(order , MkString("currency") ); _ = quoteId;
...
func (this *Vcc) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetOrdersTrades"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  data = this.SafeValue(data , MkString("data") , MkArray(&VarArray{}));
  return this.ParseTrades(data , market , since , limit );
}
...
func (this *Vcc) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  data := this.SafeValue(response , MkString("data") ); _ = data;
  status := this.SafeString(data , MkString("status") ); _ = status;
  ...
  address := this.SafeString(data , MkString("blockchain_address") ); _ = address;
  this.CheckAddress(address );
  tag := this.SafeString(data , MkString("blockchain_tag") ); _ = tag;
...
func (this *Vcc) Sign(goArgs ...*Variant) *Variant{
  ...
  url := OpAdd(*this.At(MkString("version")) , OpAdd(MkString("/") , this.ImplodeParams(path , params ))); _ = url;
  ...
  if IsTrue(OpEq2(api , MkString("private") )) {
    ...
    auth := OpAdd(method , OpAdd(MkString(" ") , url )); _ = auth;
