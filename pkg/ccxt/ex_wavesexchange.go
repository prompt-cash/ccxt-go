...
func (this *Wavesexchange) GetQuotes(goArgs ...*Variant) *Variant{
  quotes := this.SafeValue(*this.At(MkString("options")) , MkString("quotes") ); _ = quotes;
  if IsTrue(quotes ) {
    return quotes ;
  } else {
    response := this.Call(MkString("matcherGetMatcherSettings")); _ = response;
    quotes = MkMap(&VarMap{});
    priceAssets := this.SafeValue(response , MkString("priceAssets") ); _ = priceAssets;
...
func (this *Wavesexchange) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("marketGetTickers")); _ = response;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , response.Length )); OpIncr(& i ){
    entry := *(response ).At(i ); _ = entry;
    baseId := this.SafeString(entry , MkString("amountAssetID") ); _ = baseId;
    quoteId := this.SafeString(entry , MkString("priceAssetID") ); _ = quoteId;
    ...
    symbol := OpAdd(this.SafeCurrencyCode(base ), OpAdd(MkString("/") , this.SafeCurrencyCode(quote ))); _ = symbol;
...
func (this *Wavesexchange) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("matcherGetMatcherOrderbookAmountAssetPriceAsset"), request ); _ = response;
  timestamp := this.SafeInteger(response , MkString("timestamp") ); _ = timestamp;
  bids := this.ParseOrderBookSide(this.SafeValue(response , MkString("bids") ), market , limit ); _ = bids;
  asks := this.ParseOrderBookSide(this.SafeValue(response , MkString("asks") ), market , limit ); _ = asks;
  return MkMap(&VarMap{
        "symbol":symbol ,
        "bids":bids ,
        "asks":asks ,
...
func (this *Wavesexchange) ParseOrderBookSide(goArgs ...*Variant) *Variant{
  bookSide := GoGetArg(goArgs, 0, MkUndefined()); _ = bookSide;
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  limit := GoGetArg(goArgs, 2, MkUndefined() ); _ = limit;
  precision := *(market ).At(MkString("precision") ); _ = precision;
  wavesPrecision := this.SafeInteger(*this.At(MkString("options")) , MkString("wavesPrecision") , MkInteger(8) ); _ = wavesPrecision;
  amountPrecision := Math.Pow(MkInteger(10) , *(precision ).At(MkString("amount") )); _ = amountPrecision;
  difference := OpSub(*(precision ).At(MkString("amount") ), *(precision ).At(MkString("price") )); _ = difference;
  pricePrecision := Math.Pow(MkInteger(10) , OpSub(wavesPrecision , difference )); _ = pricePrecision;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , bookSide.Length )); OpIncr(& i ){
    entry := *(bookSide ).At(i ); _ = entry;
    price := OpDiv(this.SafeInteger(entry , MkString("price") , MkInteger(0) ), pricePrecision ); _ = price;
    amount := OpDiv(this.SafeInteger(entry , MkString("amount") , MkInteger(0) ), amountPrecision ); _ = amount;
...
func (this *Wavesexchange) CheckRequiredKeys(goArgs ...*Variant) *Variant{
  ...
  apiKeyBytes := OpCopy(MkUndefined() ); _ = apiKeyBytes;
  secretKeyBytes := OpCopy(MkUndefined() ); _ = secretKeyBytes;
  {
  ...
  hexApiKeyBytes := this.BinaryToBase16(apiKeyBytes ); _ = hexApiKeyBytes;
  hexSecretKeyBytes := this.BinaryToBase16(secretKeyBytes ); _ = hexSecretKeyBytes;
...
func (this *Wavesexchange) Sign(goArgs ...*Variant) *Variant{
  ...
  isCancelOrder := OpEq2(path , MkString("matcher/orders/{wavesAddress}/cancel") ); _ = isCancelOrder;
  path = this.ImplodeParams(path , params );
  url := OpAdd(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(api ), OpAdd(MkString("/") , path )); _ = url;
  queryString := this.Urlencode(query ); _ = queryString;
  if IsTrue(OpOr(OpEq2(api , MkString("private") ), OpEq2(api , MkString("forward") ))) {
    ...
    accessToken := this.SafeString(*this.At(MkString("options")) , MkString("accessToken") ); _ = accessToken;
...
func (this *Wavesexchange) GetAccessToken(goArgs ...*Variant) *Variant{
  if IsTrue(OpNot(this.SafeString(*this.At(MkString("options")) , MkString("accessToken") ))) {
    prefix := MkString("ffffff01") ; _ = prefix;
    expiresDelta := OpMulti(MkInteger(60) , OpMulti(MkInteger(60) , OpMulti(MkInteger(24) , MkInteger(7) ))); _ = expiresDelta;
    seconds := this.Sum(this.Seconds(), expiresDelta ); _ = seconds;
    seconds = seconds.ToString();
    clientId := MkString("waves.exchange") ; _ = clientId;
    message := OpAdd(MkString("W:") , OpAdd(clientId , OpAdd(MkString(":") , seconds ))); _ = message;
    messageHex := this.BinaryToBase16(this.StringToBinary(this.Encode(message ))); _ = messageHex;
    payload := OpAdd(prefix , messageHex ); _ = payload;
    hexKey := this.BinaryToBase16(this.Base58ToBinary(*this.At(MkString("secret")) )); _ = hexKey;
    signature := this.Eddsa(payload , hexKey , MkString("ed25519") ); _ = signature;
    ...
    response := this.Call(MkString("privatePostOauth2Token"), request ); _ = response;
...
func (this *Wavesexchange) ParseTicker(goArgs ...*Variant) *Variant{
  ...
  baseId := this.SafeString(ticker , MkString("amountAsset") ); _ = baseId;
  quoteId := this.SafeString(ticker , MkString("priceAsset") ); _ = quoteId;
  symbol := OpCopy(MkUndefined() ); _ = symbol;
  if IsTrue(OpAnd(OpNotEq2(baseId , MkUndefined() ), OpNotEq2(quoteId , MkUndefined() ))) {
    marketId := OpAdd(baseId , OpAdd(MkString("/") , quoteId )); _ = marketId;
  ...
  data := this.SafeValue(ticker , MkString("data") , MkMap(&VarMap{})); _ = data;
  last := this.SafeNumber(data , MkString("lastPrice") ); _ = last;
  low := this.SafeNumber(data , MkString("low") ); _ = low;
  high := this.SafeNumber(data , MkString("high") ); _ = high;
  vwap := this.SafeNumber(data , MkString("weightedAveragePrice") ); _ = vwap;
  baseVolume := this.SafeNumber(data , MkString("volume") ); _ = baseVolume;
  quoteVolume := this.SafeNumber(data , MkString("quoteVolume") ); _ = quoteVolume;
  open := this.SafeNumber(data , MkString("firstPrice") ); _ = open;
...
func (this *Wavesexchange) FetchTicker(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"pairs":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetPairs"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  ticker := this.SafeValue(data , MkInteger(0) , MkMap(&VarMap{})); _ = ticker;
...
func (this *Wavesexchange) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(since , MkUndefined() )) {
    *(request ).At(MkString("timeStart") )= since.ToString();
    ...
    allowedCandles := this.SafeInteger(*this.At(MkString("options")) , MkString("allowedCandles") , MkInteger(1440) ); _ = allowedCandles;
    timeframeUnix := OpMulti(this.ParseTimeframe(timeframe ), MkInteger(1000) ); _ = timeframeUnix;
    currentTime := OpMulti(Math.Floor(OpDiv(this.Milliseconds(), timeframeUnix )), timeframeUnix ); _ = currentTime;
    delta := OpMulti(OpSub(allowedCandles , MkInteger(1) ), timeframeUnix ); _ = delta;
    timeStart := OpSub(currentTime , delta ); _ = timeStart;
    *(request ).At(MkString("timeStart") )= timeStart.ToString();
  }
  response := this.Call(MkString("publicGetCandlesBaseIdQuoteId"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkArray(&VarArray{})); _ = data;
  result := this.ParseOHLCVs(data , market , timeframe , since , limit ); _ = result;
  lastClose := OpCopy(MkUndefined() ); _ = lastClose;
  length := OpCopy(result.Length ); _ = length;
  for i := MkInteger(0) ; IsTrue(OpLw(i , result.Length )); OpIncr(& i ){
    j := OpSub(length , OpSub(i , MkInteger(1) )); _ = j;
    entry := *(result ).At(j ); _ = entry;
    open := *(entry ).At(MkInteger(1) ); _ = open;
...
func (this *Wavesexchange) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  params := GoGetArg(goArgs, 1, MkMap(&VarMap{})); _ = params;
  this.GetAccessToken();
  supportedCurrencies := this.Call(MkString("privateGetDepositCurrencies")); _ = supportedCurrencies;
  currencies := MkMap(&VarMap{}); _ = currencies;
  items := this.SafeValue(supportedCurrencies , MkString("items") , MkArray(&VarArray{})); _ = items;
  for i := MkInteger(0) ; IsTrue(OpLw(i , items.Length )); OpIncr(& i ){
    entry := *(items ).At(i ); _ = entry;
    currencyCode := this.SafeString(entry , MkString("id") ); _ = currencyCode;
    *(currencies ).At(currencyCode )= OpCopy(MkBool(true) );
  }
  if IsTrue(OpNot(OpHasMember(code , currencies ))) {
    codes := GoGetKeys(currencies ); _ = codes;
  ...
  request := this.Extend(MkMap(&VarMap{"code":code }), params ); _ = request;
  response := this.Call(MkString("privateGetDepositAddressesCode"), request ); _ = response;
  addresses := this.SafeValue(response , MkString("deposit_addresses") ); _ = addresses;
  address := this.SafeString(addresses , MkInteger(0) ); _ = address;
  return MkMap(&VarMap{
        "address":address ,
        "code":code ,
        "tag":MkUndefined() ,
...
func (this *Wavesexchange) GetMatcherPublicKey(goArgs ...*Variant) *Variant{
  matcherPublicKey := this.SafeString(*this.At(MkString("options")) , MkString("matcherPublicKey") ); _ = matcherPublicKey;
  if IsTrue(matcherPublicKey ) {
    return matcherPublicKey ;
  } else {
    response := this.Call(MkString("matcherGetMatcher")); _ = response;
...
func (this *Wavesexchange) GetAssetBytes(goArgs ...*Variant) *Variant{
  currencyId := GoGetArg(goArgs, 0, MkUndefined()); _ = currencyId;
...
func (this *Wavesexchange) PriceToPrecision(goArgs ...*Variant) *Variant{
  symbol := GoGetArg(goArgs, 0, MkUndefined()); _ = symbol;
  price := GoGetArg(goArgs, 1, MkUndefined()); _ = price;
  market := *(*this.At(MkString("markets")) ).At(symbol ); _ = market;
  wavesPrecision := this.SafeInteger(*this.At(MkString("options")) , MkString("wavesPrecision") , MkInteger(8) ); _ = wavesPrecision;
  difference := OpSub(*(*(market ).At(MkString("precision") )).At(MkString("amount") ), *(*(market ).At(MkString("precision") )).At(MkString("price") )); _ = difference;
...
func (this *Wavesexchange) FromPrecision(goArgs ...*Variant) *Variant{
  ...
  precise := NewPrecise(amount ); _ = precise;
...
func (this *Wavesexchange) ToPrecision(goArgs ...*Variant) *Variant{
  ...
  precise := NewPrecise(amountString ); _ = precise;
...
func (this *Wavesexchange) CurrencyFromPrecision(goArgs ...*Variant) *Variant{
  currency := GoGetArg(goArgs, 0, MkUndefined()); _ = currency;
  amount := GoGetArg(goArgs, 1, MkUndefined()); _ = amount;
  scale := *(*(*this.At(MkString("currencies")) ).At(currency )).At(MkString("precision") ); _ = scale;
...
func (this *Wavesexchange) PriceFromPrecision(goArgs ...*Variant) *Variant{
  ...
  market := *(*this.At(MkString("markets")) ).At(symbol ); _ = market;
  wavesPrecision := this.SafeInteger(*this.At(MkString("options")) , MkString("wavesPrecision") , MkInteger(8) ); _ = wavesPrecision;
  scale := OpSub(wavesPrecision , OpAdd(*(*(market ).At(MkString("precision") )).At(MkString("amount") ), *(*(market ).At(MkString("precision") )).At(MkString("price") ))); _ = scale;
...
func (this *Wavesexchange) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  market := this.Market(symbol ); _ = market;
  matcherPublicKey := this.GetMatcherPublicKey(); _ = matcherPublicKey;
  amountAsset := this.GetAssetId(*(market ).At(MkString("baseId") )); _ = amountAsset;
  priceAsset := this.GetAssetId(*(market ).At(MkString("quoteId") )); _ = priceAsset;
  amount = this.AmountToPrecision(symbol , amount );
  price = this.PriceToPrecision(symbol , price );
  orderType := OpTrinary(OpEq2(side , MkString("buy") ), MkInteger(0) , MkInteger(1) ); _ = orderType;
  timestamp := this.Milliseconds(); _ = timestamp;
  defaultExpiryDelta := this.SafeInteger(*this.At(MkString("options")) , MkString("createOrderDefaultExpiry") , MkInteger(2419200000) ); _ = defaultExpiryDelta;
  expiration := this.Sum(timestamp , defaultExpiryDelta ); _ = expiration;
  settings := this.Call(MkString("matcherGetMatcherSettings")); _ = settings;
  orderFee := this.SafeValue(settings , MkString("orderFee") ); _ = orderFee;
  dynamic := this.SafeValue(orderFee , MkString("dynamic") ); _ = dynamic;
  baseMatcherFee := this.SafeString(dynamic , MkString("baseFee") ); _ = baseMatcherFee;
  wavesMatcherFee := this.CurrencyFromPrecision(MkString("WAVES") , baseMatcherFee ); _ = wavesMatcherFee;
  rates := this.SafeValue(dynamic , MkString("rates") ); _ = rates;
  priceAssets := GoGetKeys(rates ); _ = priceAssets;
  matcherFeeAssetId := OpCopy(MkUndefined() ); _ = matcherFeeAssetId;
  matcherFee := OpCopy(MkUndefined() ); _ = matcherFee;
  if IsTrue(OpHasMember(MkString("feeAssetId") , params )) {
    ...
    if IsTrue(OpHasMember(MkString("feeAssetId") , *this.At(MkString("options")) )) {
      ...
      balances := this.FetchBalance(); _ = balances;
      if IsTrue(OpGt(*(*(balances ).At(MkString("WAVES") )).At(MkString("free") ), wavesMatcherFee )) {
        ...
        for i := MkInteger(0) ; IsTrue(OpLw(i , priceAssets.Length )); OpIncr(& i ){
          assetId := *(priceAssets ).At(i ); _ = assetId;
          code := this.SafeCurrencyCode(assetId ); _ = code;
          balance := this.SafeValue(this.SafeValue(balances , code , MkMap(&VarMap{})), MkString("free") ); _ = balance;
          assetFee := Precise.StringMul(*(rates ).At(assetId ), wavesMatcherFee ); _ = assetFee;
  ...
  if IsTrue(OpEq2(matcherFee , MkUndefined() )) {
    wavesPrecision := this.SafeInteger(*this.At(MkString("options")) , MkString("wavesPrecision") , MkInteger(8) ); _ = wavesPrecision;
    rate := this.SafeString(rates , matcherFeeAssetId ); _ = rate;
    code := this.SafeCurrencyCode(matcherFeeAssetId ); _ = code;
    currency := this.Currency(code ); _ = currency;
    newPrecison := OpSub(wavesPrecision , *(currency ).At(MkString("precision") )); _ = newPrecison;
  ...
  signature := this.Eddsa(this.BinaryToBase16(binary ), this.BinaryToBase16(this.Base58ToBinary(*this.At(MkString("secret")) )), MkString("ed25519") ); _ = signature;
  ...
  response := this.Call(MkString("matcherPostMatcherOrderbook"), body ); _ = response;
  value := this.SafeValue(response , MkString("message") ); _ = value;
...
func (this *Wavesexchange) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  this.CheckRequiredKeys();
  this.GetAccessToken();
  wavesAddress := this.GetWavesAddress(); _ = wavesAddress;
  response := this.Call(MkString("forwardPostMatcherOrdersWavesAddressCancel"), MkMap(&VarMap{
            "wavesAddress":wavesAddress ,
            "orderId":id ,
            })); _ = response;
  message := this.SafeValue(response , MkString("message") ); _ = message;
  firstMessage := this.SafeValue(message , MkInteger(0) ); _ = firstMessage;
  firstOrder := this.SafeValue(firstMessage , MkInteger(0) ); _ = firstOrder;
  returnedId := this.SafeString(firstOrder , MkString("orderId") ); _ = returnedId;
  return MkMap(&VarMap{
        "info":response ,
        "id":returnedId ,
...
func (this *Wavesexchange) FetchOrders(goArgs ...*Variant) *Variant{
  ...
  binary := this.BinaryConcatArray(byteArray ); _ = binary;
  hexSecret := this.BinaryToBase16(this.Base58ToBinary(*this.At(MkString("secret")) )); _ = hexSecret;
  signature := this.Eddsa(this.BinaryToBase16(binary ), hexSecret , MkString("ed25519") ); _ = signature;
  request := MkMap(&VarMap{
  ...
  response := this.Call(MkString("matcherGetMatcherOrderbookBaseIdQuoteIdPublicKeyPublicKey"), this.Extend(request , params )); _ = response;
...
func (this *Wavesexchange) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  address := this.GetWavesAddress(); _ = address;
  request := MkMap(&VarMap{
        "address":address ,
        "activeOnly":MkBool(true) ,
        }); _ = request;
  response := this.Call(MkString("forwardGetMatcherOrdersAddress"), request ); _ = response;
  return this.ParseOrders(response , market , since , limit );
}
...
func (this *Wavesexchange) GetSymbolFromAssetPair(goArgs ...*Variant) *Variant{
  assetPair := GoGetArg(goArgs, 0, MkUndefined()); _ = assetPair;
  baseId := this.SafeString(assetPair , MkString("amountAsset") , MkString("WAVES") ); _ = baseId;
  quoteId := this.SafeString(assetPair , MkString("priceAsset") , MkString("WAVES") ); _ = quoteId;
...
func (this *Wavesexchange) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  side := this.SafeString2(order , MkString("type") , MkString("orderType") ); _ = side;
  ...
  filledString := this.SafeString(order , MkString("filled") ); _ = filledString;
  priceString := this.SafeString(order , MkString("price") ); _ = priceString;
  amountString := this.SafeString(order , MkString("amount") ); _ = amountString;
  assetPair := this.SafeValue(order , MkString("assetPair") ); _ = assetPair;
  symbol := OpCopy(MkUndefined() ); _ = symbol;
  ...
  amountCurrency := this.SafeCurrencyCode(this.SafeString(assetPair , MkString("amountAsset") , MkString("WAVES") )); _ = amountCurrency;
  price := this.ParseNumber(this.PriceFromPrecision(symbol , priceString )); _ = price;
  amount := this.ParseNumber(this.CurrencyFromPrecision(amountCurrency , amountString )); _ = amount;
  filled := this.ParseNumber(this.CurrencyFromPrecision(amountCurrency , filledString )); _ = filled;
  average := this.ParseNumber(this.PriceFromPrecision(symbol , this.SafeString(order , MkString("avgWeighedPrice") ))); _ = average;
  status := this.ParseOrderStatus(this.SafeString(order , MkString("status") )); _ = status;
  fee := OpCopy(MkUndefined() ); _ = fee;
  if IsTrue(OpHasMember(MkString("type") , order )) {
    currency := this.SafeCurrencyCode(this.SafeString(order , MkString("feeAsset") )); _ = currency;
    fee = MkMap(&VarMap{
        "currency":currency ,
        "fee":this.ParseNumber(this.CurrencyFromPrecision(currency , this.SafeString(order , MkString("filledFee") ))),
        });
  } else {
    currency := this.SafeCurrencyCode(this.SafeString(order , MkString("matcherFeeAssetId") , MkString("WAVES") )); _ = currency;
    fee = MkMap(&VarMap{
        "currency":currency ,
...
func (this *Wavesexchange) GetWavesAddress(goArgs ...*Variant) *Variant{
  cachedAddreess := this.SafeString(*this.At(MkString("options")) , MkString("wavesAddress") ); _ = cachedAddreess;
  if IsTrue(OpEq2(cachedAddreess , MkUndefined() )) {
    request := MkMap(&VarMap{"publicKey":*this.At(MkString("apiKey")) }); _ = request;
    response := this.Call(MkString("nodeGetAddressesPublicKeyPublicKey"), request ); _ = response;
...
func (this *Wavesexchange) FetchBalance(goArgs ...*Variant) *Variant{
  ...
  this.LoadMarkets();
  wavesAddress := this.GetWavesAddress(); _ = wavesAddress;
  request := MkMap(&VarMap{"address":wavesAddress }); _ = request;
  totalBalance := this.Call(MkString("nodeGetAssetsBalanceAddress"), request ); _ = totalBalance;
  balances := this.SafeValue(totalBalance , MkString("balances") ); _ = balances;
  result := MkMap(&VarMap{}); _ = result;
  timestamp := OpCopy(MkUndefined() ); _ = timestamp;
  for i := MkInteger(0) ; IsTrue(OpLw(i , balances.Length )); OpIncr(& i ){
    entry := *(balances ).At(i ); _ = entry;
    entryTimestamp := this.SafeInteger(entry , MkString("timestamp") ); _ = entryTimestamp;
    timestamp = OpTrinary(OpEq2(timestamp , MkUndefined() ), entryTimestamp , Math.Max(timestamp , entryTimestamp ));
    issueTransaction := this.SafeValue(entry , MkString("issueTransaction") ); _ = issueTransaction;
    decimals := this.SafeInteger(issueTransaction , MkString("decimals") ); _ = decimals;
    currencyId := this.SafeString(entry , MkString("assetId") ); _ = currencyId;
    balance := this.SafeString(entry , MkString("balance") ); _ = balance;
  ...
  currentTimestamp := this.Milliseconds(); _ = currentTimestamp;
  byteArray := MkArray(&VarArray{
        this.Base58ToBinary(*this.At(MkString("apiKey")) ),
  ...
  reservedBalance := this.Call(MkString("matcherGetMatcherBalanceReservedPublicKey"), matcherRequest ); _ = reservedBalance;
  reservedKeys := GoGetKeys(reservedBalance ); _ = reservedKeys;
  for i := MkInteger(0) ; IsTrue(OpLw(i , reservedKeys.Length )); OpIncr(& i ){
    currencyId := *(reservedKeys ).At(i ); _ = currencyId;
    code := this.SafeCurrencyCode(currencyId ); _ = code;
    if IsTrue(OpNot(OpHasMember(code , result ))) {
      *(result ).At(code )= this.Account();
    }
    amount := this.SafeString(reservedBalance , currencyId ); _ = amount;
    *(*(result ).At(code )).At(MkString("used") )= this.CurrencyFromPrecision(code , amount );
  }
  wavesRequest := MkMap(&VarMap{"address":wavesAddress }); _ = wavesRequest;
  wavesTotal := this.Call(MkString("nodeGetAddressesBalanceAddress"), wavesRequest ); _ = wavesTotal;
  ...
  *(*(result ).At(MkString("WAVES") )).At(MkString("total") )= this.CurrencyFromPrecision(MkString("WAVES") , this.SafeString(wavesTotal , MkString("balance") ));
  codes := GoGetKeys(result ); _ = codes;
...
func (this *Wavesexchange) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetTransactionsExchange"), request ); _ = response;
...
func (this *Wavesexchange) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  data := this.SafeValue(trade , MkString("data") ); _ = data;
  datetime := this.SafeString(data , MkString("timestamp") ); _ = datetime;
  timestamp := this.Parse8601(datetime ); _ = timestamp;
  id := this.SafeString(data , MkString("id") ); _ = id;
  priceString := this.SafeString(data , MkString("price") ); _ = priceString;
  amountString := this.SafeString(data , MkString("amount") ); _ = amountString;
  ...
  order1 := this.SafeValue(data , MkString("order1") ); _ = order1;
  order2 := this.SafeValue(data , MkString("order2") ); _ = order2;
  order := OpCopy(MkUndefined() ); _ = order;
  ...
  side := this.SafeString(order , MkString("orderType") ); _ = side;
...
func (this *Wavesexchange) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  Exception := this.SafeValue(*this.At(MkString("exceptions")) , errorCode ); _ = Exception;
...
func (this *Wavesexchange) Withdraw(goArgs ...*Variant) *Variant{
  ...
  if IsTrue(OpNotEq2(code , MkString("WAVES") )) {
    supportedCurrencies := this.Call(MkString("privateGetWithdrawCurrencies")); _ = supportedCurrencies;
    currencies := MkMap(&VarMap{}); _ = currencies;
    items := this.SafeValue(supportedCurrencies , MkString("items") , MkArray(&VarArray{})); _ = items;
    for i := MkInteger(0) ; IsTrue(OpLw(i , items.Length )); OpIncr(& i ){
      entry := *(items ).At(i ); _ = entry;
  ...
  set := MkMap(&VarMap{}); _ = set;
  for i := MkInteger(0) ; IsTrue(OpLw(i , hexChars.Length )); OpIncr(& i ){
    key := *(hexChars ).At(i ); _ = key;
    *(set ).At(key )= OpCopy(MkBool(true) );
  }
  isErc20 := OpCopy(MkBool(true) ); _ = isErc20;
  noPrefix := this.Remove0xPrefix(address ); _ = noPrefix;
  lower := noPrefix.ToLowerCase(); _ = lower;
  for i := MkInteger(0) ; IsTrue(OpLw(i , lower.Length )); OpIncr(& i ){
    character := *(lower ).At(i ); _ = character;
  ...
  this.GetAccessToken();
  proxyAddress := OpCopy(MkUndefined() ); _ = proxyAddress;
  if IsTrue(OpAnd(OpEq2(code , MkString("WAVES") ), OpNot(isErc20 ))) {
    ...
    withdrawAddress := this.Call(MkString("privateGetWithdrawAddressesCurrencyAddress"), withdrawAddressRequest ); _ = withdrawAddress;
    currency := this.SafeValue(withdrawAddress , MkString("currency") ); _ = currency;
    allowedAmount := this.SafeValue(currency , MkString("allowed_amount") ); _ = allowedAmount;
    minimum := this.SafeNumber(allowedAmount , MkString("min") ); _ = minimum;
    ...
    proxyAddresses := this.SafeValue(withdrawAddress , MkString("proxy_addresses") , MkArray(&VarArray{})); _ = proxyAddresses;
    proxyAddress = this.SafeString(proxyAddresses , MkInteger(0) );
  }
  fee := this.SafeInteger(*this.At(MkString("options")) , MkString("withdrawFeeWAVES") , MkInteger(100000) ); _ = fee;
  feeAssetId := MkString("WAVES") ; _ = feeAssetId;
  type_ := MkInteger(4) ; _ = type_;
  version := MkInteger(2) ; _ = version;
  amountInteger := this.CurrencyToPrecision(code , amount ); _ = amountInteger;
  currency := this.Currency(code ); _ = currency;
  timestamp := this.Milliseconds(); _ = timestamp;
  byteArray := MkArray(&VarArray{
