...
func (this *Bitbank) FetchMarkets(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("marketsGetSpotPairs"), params ); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  pairs := this.SafeValue(data , MkString("pairs") , MkArray(&VarArray{})); _ = pairs;
  result := MkArray(&VarArray{}); _ = result;
  for i := MkInteger(0) ; IsTrue(OpLw(i , pairs.Length )); OpIncr(& i ){
    entry := *(pairs ).At(i ); _ = entry;
    id := this.SafeString(entry , MkString("name") ); _ = id;
    baseId := this.SafeString(entry , MkString("base_asset") ); _ = baseId;
    quoteId := this.SafeString(entry , MkString("quote_asset") ); _ = quoteId;
    ...
    maker := this.SafeNumber(entry , MkString("maker_fee_rate_quote") ); _ = maker;
    taker := this.SafeNumber(entry , MkString("taker_fee_rate_quote") ); _ = taker;
    pricePrecisionString := this.SafeString(entry , MkString("price_digits") ); _ = pricePrecisionString;
    ...
    active := this.SafeValue(entry , MkString("is_enabled") ); _ = active;
    minAmountString := this.SafeString(entry , MkString("unit_amount") ); _ = minAmountString;
    minCost := Precise.StringMul(minAmountString , priceLimit ); _ = minCost;
    limits := MkMap(&VarMap{
          "amount":MkMap(&VarMap{
...
func (this *Bitbank) FetchOrderBook(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetPairDepth"), this.Extend(request , params )); _ = response;
...
func (this *Bitbank) ParseTrade(goArgs ...*Variant) *Variant{
  ...
  market := GoGetArg(goArgs, 1, MkUndefined() ); _ = market;
  timestamp := this.SafeInteger(trade , MkString("executed_at") ); _ = timestamp;
  symbol := OpCopy(MkUndefined() ); _ = symbol;
  ...
  id := this.SafeString2(trade , MkString("transaction_id") , MkString("trade_id") ); _ = id;
  takerOrMaker := this.SafeString(trade , MkString("maker_taker") ); _ = takerOrMaker;
  fee := OpCopy(MkUndefined() ); _ = fee;
  feeCost := this.SafeNumber(trade , MkString("fee_amount_quote") ); _ = feeCost;
...
func (this *Bitbank) FetchTrades(goArgs ...*Variant) *Variant{
  ...
  market := this.Market(symbol ); _ = market;
  request := MkMap(&VarMap{"pair":*(market ).At(MkString("id") )}); _ = request;
  response := this.Call(MkString("publicGetPairTransactions"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  trades := this.SafeValue(data , MkString("transactions") , MkArray(&VarArray{})); _ = trades;
...
func (this *Bitbank) FetchOHLCV(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("publicGetPairCandlestickCandletypeYyyymmdd"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  candlestick := this.SafeValue(data , MkString("candlestick") , MkArray(&VarArray{})); _ = candlestick;
  first := this.SafeValue(candlestick , MkInteger(0) , MkMap(&VarMap{})); _ = first;
  ohlcv := this.SafeValue(first , MkString("ohlcv") , MkArray(&VarArray{})); _ = ohlcv;
...
func (this *Bitbank) FetchBalance(goArgs ...*Variant) *Variant{
  params := GoGetArg(goArgs, 0, MkMap(&VarMap{})); _ = params;
  this.LoadMarkets();
  response := this.Call(MkString("privateGetUserAssets"), params ); _ = response;
  result := MkMap(&VarMap{
        "info":response ,
  ...
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  assets := this.SafeValue(data , MkString("assets") , MkArray(&VarArray{})); _ = assets;
  for i := MkInteger(0) ; IsTrue(OpLw(i , assets.Length )); OpIncr(& i ){
    balance := *(assets ).At(i ); _ = balance;
...
func (this *Bitbank) ParseOrder(goArgs ...*Variant) *Variant{
  ...
  timestamp := this.SafeInteger(order , MkString("ordered_at") ); _ = timestamp;
...
func (this *Bitbank) CreateOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostUserSpotOrder"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
...
func (this *Bitbank) CancelOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privatePostUserSpotCancelOrder"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  return data ;
}
...
func (this *Bitbank) FetchOrder(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetUserSpotOrder"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  return this.ParseOrder(data , market );
}
...
func (this *Bitbank) FetchOpenOrders(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetUserSpotActiveOrders"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
...
func (this *Bitbank) FetchMyTrades(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetUserSpotTradeHistory"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
...
func (this *Bitbank) FetchDepositAddress(goArgs ...*Variant) *Variant{
  ...
  response := this.Call(MkString("privateGetUserWithdrawalAccount"), this.Extend(request , params )); _ = response;
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  accounts := this.SafeValue(data , MkString("accounts") , MkArray(&VarArray{})); _ = accounts;
  firstAccount := this.SafeValue(accounts , MkInteger(0) , MkMap(&VarMap{})); _ = firstAccount;
  address := this.SafeString(firstAccount , MkString("address") ); _ = address;
  return MkMap(&VarMap{
        "currency":currency ,
        "address":address ,
        "tag":MkUndefined() ,
...
func (this *Bitbank) Withdraw(goArgs ...*Variant) *Variant{
  ...
  data := this.SafeValue(response , MkString("data") , MkMap(&VarMap{})); _ = data;
  txid := this.SafeString(data , MkString("txid") ); _ = txid;
  return MkMap(&VarMap{
        "info":response ,
        "id":txid ,
        });
...
func (this *Bitbank) Sign(goArgs ...*Variant) *Variant{
  ...
  url := OpAdd(this.ImplodeHostname(*(*(*this.At(MkString("urls")) ).At(MkString("api") )).At(api )), MkString("/") ); _ = url;
...
func (this *Bitbank) HandleErrors(goArgs ...*Variant) *Variant{
  ...
  success := this.SafeInteger(response , MkString("success") ); _ = success;
  data := this.SafeValue(response , MkString("data") ); _ = data;
  if IsTrue(OpOr(OpNot(success ), OpNot(data ))) {
    ...
    errorClasses := OpCopy(*this.At(MkString("exceptions")) ); _ = errorClasses;
    code := this.SafeString(data , MkString("code") ); _ = code;
    message := this.SafeString(errorMessages , code , MkString("Error") ); _ = message;
    ErrorClass := this.SafeValue(errorClasses , code ); _ = ErrorClass;
