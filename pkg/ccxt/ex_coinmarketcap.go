...
func (this *Coinmarketcap) CurrencyCode(goArgs ...*Variant) *Variant {
	base := GoGetArg(goArgs, 0, MkUndefined())
	_ = base
	name := GoGetArg(goArgs, 1, MkUndefined())
	_ = name
...
func (this *Coinmarketcap) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	request := MkMap(&VarMap{"limit": MkInteger(0)})
	_ = request
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = market
		currencies := OpCopy(*this.At(MkString("currencyCodes")))
		_ = currencies
		for j := MkInteger(0); IsTrue(OpLw(j, currencies.Length)); OpIncr(&j) {
			quote := *(currencies).At(j)
			_ = quote
			quoteId := quote.ToLowerCase()
			_ = quoteId
			baseId := *(market).At(MkString("id"))
			_ = baseId
			base := this.CurrencyCode(*(market).At(MkString("symbol")), *(market).At(MkString("name")))
			_ = base
...
func (this *Coinmarketcap) FetchGlobal(goArgs ...*Variant) *Variant {
	currency := GoGetArg(goArgs, 0, MkString("USD"))
	_ = currency
	this.LoadMarkets()
...
func (this *Coinmarketcap) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	timestamp := this.SafeTimestamp(ticker, MkString("last_updated"))
	_ = timestamp
	if IsTrue(OpEq2(timestamp, MkUndefined())) {
		timestamp = this.Milliseconds()
	}
	change := this.SafeNumber(ticker, MkString("percent_change_24h"))
	_ = change
	last := OpCopy(MkUndefined())
	_ = last
	symbol := OpCopy(MkUndefined())
	_ = symbol
	volume := OpCopy(MkUndefined())
	_ = volume
	if IsTrue(OpNotEq2(market, MkUndefined())) {
		symbol = *(market).At(MkString("symbol"))
		priceKey := OpAdd(MkString("price_"), *(market).At(MkString("quoteId")))
		_ = priceKey
		last = this.SafeNumber(ticker, priceKey)
		volumeKey := OpAdd(MkString("24h_volume_"), *(market).At(MkString("quoteId")))
		_ = volumeKey
...
func (this *Coinmarketcap) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"limit": MkInteger(10000)})
	_ = request
	if IsTrue(currency) {
		*(request).At(MkString("convert")) = OpCopy(currency)
	}
	...
	for t := MkInteger(0); IsTrue(OpLw(t, response.Length)); OpIncr(&t) {
		ticker := *(response).At(t)
		_ = ticker
		currencyId := currency.ToLowerCase()
		_ = currencyId
		id := OpAdd(*(ticker).At(MkString("id")), OpAdd(MkString("/"), currencyId))
		_ = id
		symbol := OpCopy(id)
		_ = symbol
...
func (this *Coinmarketcap) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetTickerId"), this.Extend(request, params))
	_ = response
	ticker := *(response).At(MkInteger(0))
	_ = ticker
...
func (this *Coinmarketcap) HandleErrors(goArgs ...*Variant) *Variant {
	...
	error := this.SafeValue(response, MkString("error"), MkBool(false))
	_ = error
	if IsTrue(error) {
