...
func (this *Hollaex) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("publicGetConstants"), params)
	_ = response
	pairs := this.SafeValue(response, MkString("pairs"), MkMap(&VarMap{}))
	_ = pairs
	keys := GoGetKeys(pairs)
	_ = keys
	...
	for i := MkInteger(0); IsTrue(OpLw(i, keys.Length)); OpIncr(&i) {
		...
		_ = id
		baseId := this.SafeString(market, MkString("pair_base"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("pair_2"))
		_ = quoteId
...
func (this *Hollaex) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetConstants"), params)
	_ = response
	coins := this.SafeValue(response, MkString("coins"), MkMap(&VarMap{}))
	_ = coins
	keys := GoGetKeys(coins)
	_ = keys
	result := MkMap(&VarMap{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, keys.Length)); OpIncr(&i) {
		...
		_ = fee
		precision := this.SafeNumber(currency, MkString("increment_unit"))
		_ = precision
		withdrawalLimits := this.SafeValue(currency, MkString("withdrawal_limits"), MkArray(&VarArray{}))
		_ = withdrawalLimits
...
func (this *Hollaex) FetchOrderBooks(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("publicGetOrderbooks"), params)
	_ = response
	result := MkMap(&VarMap{})
	_ = result
	...
	for i := MkInteger(0); IsTrue(OpLw(i, marketIds.Length)); OpIncr(&i) {
		...
		_ = marketId
		orderbook := *(response).At(marketId)
		_ = orderbook
		symbol := this.SafeSymbol(marketId, MkUndefined(), MkString("-"))
		_ = symbol
		timestamp := this.Parse8601(this.SafeString(orderbook, MkString("timestamp")))
		_ = timestamp
...
func (this *Hollaex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetOrderbooks"), this.Extend(request, params))
	_ = response
	orderbook := this.SafeValue(response, marketId)
	_ = orderbook
	timestamp := this.Parse8601(this.SafeString(orderbook, MkString("timestamp")))
	_ = timestamp
...
func (this *Hollaex) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("publicGetTickers"), this.Extend(params))
	_ = response
...
func (this *Hollaex) ParseTickers(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, keys.Length)); OpIncr(&i) {
		key := *(keys).At(i)
		_ = key
		ticker := *(response).At(key)
		_ = ticker
		marketId := this.SafeString(ticker, MkString("symbol"), key)
		_ = marketId
...
func (this *Hollaex) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = symbol
	timestamp := this.Parse8601(this.SafeString2(ticker, MkString("time"), MkString("timestamp")))
	_ = timestamp
...
func (this *Hollaex) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = response
	trades := this.SafeValue(response, *(market).At(MkString("id")), MkArray(&VarArray{}))
	_ = trades
...
func (this *Hollaex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	symbol := *(market).At(MkString("symbol"))
	_ = symbol
	datetime := this.SafeString(trade, MkString("timestamp"))
	_ = datetime
	...
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		...
		_ = quote
		feeCurrencyCode := OpTrinary(OpNotEq2(market, MkUndefined()), *(market).At(MkString("quote")), quote)
		_ = feeCurrencyCode
		fee = MkMap(&VarMap{
			"cost":     feeCost,
...
func (this *Hollaex) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(since, MkUndefined())) {
		if IsTrue(OpEq2(limit, MkUndefined())) {
			...
			_ = end
			start := OpSub(end, OpMulti(duration, limit))
			_ = start
	...
	response := this.Call(MkString("publicGetChart"), this.Extend(request, params))
	_ = response
...
func (this *Hollaex) ParseOHLCV(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timeframe := GoGetArg(goArgs, 2, MkString("1h"))
	_ = timeframe
...
func (this *Hollaex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetUserBalance"), params)
	_ = response
	timestamp := this.Parse8601(this.SafeString(response, MkString("updated_at")))
	_ = timestamp
	result := MkMap(&VarMap{
		"info":      response,
		"timestamp": timestamp,
	...
	_ = result
	currencyIds := GoGetKeys(*this.At(MkString("currencies_by_id")))
	_ = currencyIds
...
func (this *Hollaex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	stopPrice := this.SafeFloat2(params, MkString("stopPrice"), MkString("stop"))
	_ = stopPrice
...
func (this *Hollaex) ParseDepositAddress(goArgs ...*Variant) *Variant {
	...
	network := this.SafeString(depositAddress, MkString("network"))
	_ = network
	return MkMap(&VarMap{
...
func (this *Hollaex) FetchDepositAddresses(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	network := this.SafeString(params, MkString("network"))
	_ = network
	params = this.Omit(params, MkString("network"))
	response := this.Call(MkString("privateGetUser"), params)
	_ = response
	wallet := this.SafeValue(response, MkString("wallet"), MkArray(&VarArray{}))
	_ = wallet
	addresses := OpTrinary(OpEq2(network, MkUndefined()), wallet, this.FilterBy(wallet, MkString("network"), network))
	_ = addresses
...
func (this *Hollaex) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetUserDeposits"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
...
func (this *Hollaex) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetUserWithdrawals"), this.Extend(request, params))
	_ = response
	data := this.SafeValue(response, MkString("data"), MkArray(&VarArray{}))
	_ = data
	return this.ParseTransactions(data, currency, since, limit)
...
func (this *Hollaex) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	code := this.SafeCurrencyCode(currencyId)
	_ = code
	status := this.SafeValue(transaction, MkString("status"))
	_ = status
	dismissed := this.SafeValue(transaction, MkString("dismissed"))
	_ = dismissed
	rejected := this.SafeValue(transaction, MkString("rejected"))
	_ = rejected
	if IsTrue(status) {
		status = MkString("ok")
	} else {
...
func (this *Hollaex) Withdraw(goArgs ...*Variant) *Variant {
	...
	_ = request
	otp := this.SafeString(params, MkString("otp_code"))
	_ = otp
...
func (this *Hollaex) Sign(goArgs ...*Variant) *Variant {
	...
	url := OpAdd(*(*this.At(MkString("urls"))).At(MkString("api")), path)
	_ = url
	if IsTrue(OpEq2(api, MkString("private"))) {
		this.CheckRequiredCredentials()
		defaultExpires := this.SafeInteger2(*this.At(MkString("options")), MkString("api-expires"), MkString("expires"), parseInt(OpDiv(*this.At(MkString("timeout")), MkInteger(1000))))
		_ = defaultExpires
		expires := this.Sum(this.Seconds(), defaultExpires)
		_ = expires
		expiresString := expires.ToString()
		_ = expiresString
		auth := OpAdd(method, OpAdd(path, expiresString))
		_ = auth
